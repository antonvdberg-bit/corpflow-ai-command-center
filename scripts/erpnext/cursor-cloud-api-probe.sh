#!/usr/bin/env bash
# CorpFlowAI — read-only ERPNext API access probe for Cursor Cloud (#899 / #893).
#
...
# Non-secret injection inventory for #899 evidence (names + presence only).
...
HTTP_AUTH_STATUS=""
OVERALL="FAIL"
MASTER_ADMIN_PRESENT="absent"

print_header() {
  MASTER_ADMIN_PRESENT="$(require_secret_present MASTER_ADMIN_KEY)"
  ...
  log "MASTER_ADMIN_KEY: ${MASTER_ADMIN_PRESENT} (must be absent for ordinary Cursor Cloud execution per #899)"
...
fail_closed() {
  ...
  log "MASTER_ADMIN_KEY: ${MASTER_ADMIN_PRESENT}"
...
if [[ "$OVERALL" == "PASS" ]]; then
  if [[ "$MASTER_ADMIN_PRESENT" == "present" ]]; then
    log "security_correction_#899: INCOMPLETE — MASTER_ADMIN_KEY still injected into this ordinary Cursor Cloud run (UI-only removal required)"
  else
    log "security_correction_#899: PASS — MASTER_ADMIN_KEY absent from this ordinary Cursor Cloud run"
  fi
  log "exact_blocker: NONE (ERPNext direct API)"
  if ((${#DENIED[@]} == 0)); then
