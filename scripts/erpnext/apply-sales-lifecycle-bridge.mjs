#!/usr/bin/env node
/**
 * Apply / prove the WP2 ERPNext sales lifecycle bridge (#1018).
 *
 ...
 * Synthetic Lead / Opportunity / Customer only. Runs the same event twice.
 *
 * Usage:
 *   node scripts/erpnext/apply-sales-lifecycle-bridge.mjs --dry-run
 *   node scripts/erpnext/apply-sales-lifecycle-bridge.mjs
 */
...
const ARTIFACT_DIR = path.join(ROOT, 'artifacts', 'erpnext', 'sales-lifecycle-bridge-1018');
const FIXTURE_REL = 'fixtures/erpnext-sales-lifecycle-bridge/synthetic-prospect.json';
...
function printHeader(dryRun) {
  const cfg = loadSalesLifecycleBridgeConfig(ROOT);
  log('ERPNext WP2 sales lifecycle bridge apply (#1018)');
...
function summarizeResult(label, result) {
  return {
    label,
    ok: result.ok === true,
    lifecycle_class: result.lifecycle_class || null,
    lead_action: result.lead_action || result.action || null,
    opportunity_action: result.opportunity_action || null,
    customer_action: result.customer_action || null,
    ...
    duplicate_lead_count: result.duplicate_lead_count ?? null,
    duplicate_opportunity_count: result.duplicate_opportunity_count ?? null,
    duplicate_customer_count: result.duplicate_customer_count ?? null,
...
if (dryRun) {
  ...
  log(`planned_event: ${fixture.lead_id} → Lead then Opportunity then WP1 Customer search-before-create`);
  log('planned_replay: same event a second time; expected UPDATE on Lead, Opportunity, and Customer; duplicate counts 1/1/1');
  log(`ERPNext WP2 sales lifecycle bridge: DRY-RUN (${CANONICAL_VERDICT} pending live run)`);
  process.exit(0);
}
...
if (missing.length) {
  log(`ERPNext WP2 sales lifecycle bridge NOT READY — missing injected secrets: ${missing.join(' ')}`);
...
try {
  ...
  log('ERPNext WP2 sales lifecycle bridge NOT READY — Frappe client could not be constructed from named secrets');
  process.exit(1);
}
...
if (!auth.ok || auth.user !== 'integrations@corpflowai.com') {
  log('ERPNext WP2 sales lifecycle bridge NOT READY — authentication failed');
  writeFileSync(
...
const proof = await proveSalesLifecycleIdempotency(event, {
  client,
  referenceStore,
  repoRoot: ROOT,
});
...
const evidence = {
  schema: 'corpflow.erpnext.sales_lifecycle_bridge_apply.v1',
  issue: 1018,
  ...
  duplicate_lead_count: proof.second?.duplicate_lead_count ?? proof.first?.duplicate_lead_count ?? null,
  duplicate_opportunity_count: proof.second?.duplicate_opportunity_count ?? null,
  duplicate_customer_count: proof.second?.duplicate_customer_count ?? null,
...
log(`first_run: lead=${evidence.first.lead_action} opportunity=${evidence.first.opportunity_action} customer=${evidence.first.customer_action} ok=${evidence.first.ok}`);
log(`second_run: lead=${evidence.second.lead_action} opportunity=${evidence.second.opportunity_action} customer=${evidence.second.customer_action} ok=${evidence.second.ok}`);
log(`created_on_replay: ${evidence.created_on_replay}`);
log(`duplicate_lead_count: ${evidence.duplicate_lead_count}`);
log(`duplicate_opportunity_count: ${evidence.duplicate_opportunity_count}`);
log(`duplicate_customer_count: ${evidence.duplicate_customer_count}`);
log(`erpnext_lead: ${evidence.second.erpnext_lead || evidence.first.erpnext_lead || 'none'}`);
log(`erpnext_opportunity: ${evidence.second.erpnext_opportunity || 'none'}`);
...
if (!proof.ok) {
  log(`ERPNext WP2 sales lifecycle bridge NOT READY — ${proof.second?.error || proof.first?.error || 'idempotency proof failed'}`);
  process.exit(1);
}
