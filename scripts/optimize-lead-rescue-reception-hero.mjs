 ...
 * Mauritius property reception hero used as the full-bleed photo+glass
 * background on `/lead-rescue/property-mauritius`.
 *
 * Mirrors the Product A hero pipeline (sharp: jpeg mozjpeg, webp, avif) and the
 * naming the PublicMarketingPhotoGlassShell expects:
 *   - <base>.{avif,webp,jpg}        (desktop)
 *   - <base>-768.{avif,webp,jpg}    (mobile)
 *
 * Drop the source JPEG/PNG at:
 *   public/assets/visuals/lead-rescue-property-reception-hero-source.jpg
 * then run:
 *   node scripts/optimize-lead-rescue-reception-hero.mjs
 *
 ...
 * replace with a >=2400px master before the page goes to `published`.
 */
...
const src = path.join(dir, 'lead-rescue-property-reception-hero-source.jpg');
const baseName = 'lead-rescue-property-reception-hero-v1';
