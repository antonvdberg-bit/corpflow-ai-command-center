#!/usr/bin/env node
/**
 * Set `tenant_personas.billing_exempt = false` for tenant **luxe-maurice**.
 * Lux is a paying client — approve-build token gates and client estimate lines apply.
 *
 * Prerequisites: production (or target) Postgres reachable via env.
 *
 * PowerShell:
 *   $env:POSTGRES_URL = "postgresql://..."
 *   node scripts/clear-luxe-maurice-billing-exempt.mjs
 *
 * Optional:
 *   node scripts/clear-luxe-maurice-billing-exempt.mjs --dry-run
 */
      ...
      JSON.stringify(
        {
          ...
          would_set_billing_exempt: false,
        },
        null,
        2,
      ),
