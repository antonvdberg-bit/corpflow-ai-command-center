 ...
 *   http://127.0.0.1:4790/app/core          (simulated Core session)
 *   http://127.0.0.1:4790/app/tenant        (simulated Tenant session)
 *   http://127.0.0.1:4790/app/core?proof=1  (proof harness)
 */
...
/**
 * Attach simulated session for local evidence when proof is not requested.
 * @param {any} fakeReq
 * @param {'core'|'tenant'} environment
 * @param {URL} u
 */
function attachLocalAuth(fakeReq, environment, u) {
  const proof =
    u.searchParams.get('proof') === '1' ||
    String(fakeReq.headers?.['x-corpflow-app-proof'] || '').trim() === '1';
  if (proof) return;

  process.env.NODE_ENV = 'test';
  if (environment === 'core') {
    fakeReq.__testSessionPayload = {
      typ: 'admin',
      username: 'slice2-core-operator',
      user_id: 'syn_user_slice2_core',
    };
    fakeReq.__testMembershipTenantIds = [];
  } else {
    fakeReq.__testSessionPayload = {
      typ: 'tenant',
      username: 'slice2-tenant-member',
      user_id: 'syn_user_slice2_tenant',
      tenant_id: REFERENCE_TENANT_ID,
    };
    fakeReq.__testMembershipTenantIds = [REFERENCE_TENANT_ID];
  }
}
...
<body>
  ...
  <script type="module">
    ...
    const isProof = params.get('proof') === '1';
    ...
    const detailId = encodeURIComponent(shell.canonical_request_id || listRows[0]?.request_id || '');
    const detail = await fetch('/api/app/request?' + qs + '&id=' + detailId).then(r => r.json());
    ...
    const listHtml = listRows.length
      ? listRows.map(r => \`
      ...
      : '<p class="cf-app-lead" data-testid="app-empty">No authorised request records match this view.</p>';
    ...
    if (environment === 'tenant') {
      body = \`
        ...
        <section class="cf-app-panel" data-testid="tenant-requests-progress">
          <h1 class="cf-app-h1">\${escapeHtml(req.title||'Request')}</h1>
      ...
      body = \`
        <section class="cf-app-panel" data-testid="core-request-list">
          ...
          <p class="cf-app-lead">Global queue · internal evidence · production-shaped adapters</p>
          <div class="cf-app-grid">\${listHtml}</div>
        </section>
    ...
    document.getElementById('root').innerHTML = \`
      <style>\${css}</style>
      <div class="cf-app-root" data-scope="\${environment}" data-environment="\${environment}" style="\${styleVars}" data-testid="slice2-auth-root">
        <header class="cf-app-chrome" data-testid="app-chrome">
          <div class="cf-app-brand">CorpFlowAI</div>
          <div class="cf-app-meta">
            <span class="cf-app-chip" data-tone="accent" data-testid="chip-scope">Environment · <strong>\${envLabel}</strong></span>
            <span class="cf-app-chip" data-testid="chip-tenant">Tenant · <strong>\${environment==='tenant'?'CorpFlowAI':'—'}</strong></span>
            <span class="cf-app-chip" data-testid="chip-role">Role · <strong>\${escapeHtml(shell.actor?.role||'')}</strong></span>
            <span class="cf-app-chip" data-testid="chip-user">User · <strong>\${escapeHtml(shell.actor?.username||'')}</strong></span>
            \${isProof ? '<span class="cf-app-chip" data-testid="chip-proof">Proof mode</span>' : '<span class="cf-app-chip" data-testid="chip-auth">Auth · session</span>'}
          </div>
        </header>
        <main class="cf-app-main">
          ...
          <p class="cf-app-muted">
            ...
            <a class="cf-app-btn" href="/app/core?proof=1">Core proof</a>
            <a class="cf-app-btn" href="/app/tenant?proof=1">Tenant proof</a>
          </p>
          \${body}
        </main>
      </div>\`;
...
const server = http.createServer(async (req, nodeRes) => {
  ...
  // Probe helpers for evidence JSON (local only)
  if (path === '/api/app/_probe/actors') {
    return res.status(200).json({
      ok: true,
      session_core: actorFromSessionPayload({
        typ: 'admin',
        username: 'slice2-core-operator',
        user_id: 'syn_user_slice2_core',
      }),
      session_tenant: actorFromSessionPayload({
        typ: 'tenant',
        username: 'slice2-tenant-member',
        user_id: 'syn_user_slice2_tenant',
        tenant_id: REFERENCE_TENANT_ID,
      }),
      proof_core: buildProofCoreActor(),
      proof_tenant: buildProofTenantActor(),
    });
  }
...
server.listen(PORT, '127.0.0.1', () => {
  console.log(`Slice 2 auth server http://127.0.0.1:${PORT}/app/core`);
  console.log(`                    http://127.0.0.1:${PORT}/app/tenant`);
  console.log(`                    http://127.0.0.1:${PORT}/app/core?proof=1 (harness)`);
});
