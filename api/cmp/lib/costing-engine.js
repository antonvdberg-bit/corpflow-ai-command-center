 ...
 * When is_demo is true, UI may show displayed_client_usd = full × (1 - DEMO_DISCOUNT_RATE).
 */
 ...
 * If is_demo:
 *   displayed_client_usd = round( full_market_value_usd × (1 - 0.2) )
 * Else:
 *   displayed_client_usd = full_market_value_usd
 *
...
export function computeMarketValueCost(input) {
  ...
  const displayed_client_usd = is_demo
    ? roundUsd(full_market_value_usd * (1 - DEMO_DISCOUNT_RATE))
  ...
  return {
    ...
    breakdown: {
      ...
      demo_rule: is_demo
        ? `displayed_client_usd = round(full_market_value_usd * ${1 - DEMO_DISCOUNT_RATE})`
        : 'displayed_client_usd = full_market_value_usd',
    },
  };
}
