    ...
    async function redirectTenantSession() {
      ...
      const timer =
        controller && typeof setTimeout === 'function'
          ...
          : null;
      try {
        const res = await fetch('/api/app/shell?env=tenant', {
        ...
        // Stay on the chooser when the shell probe fails or times out.
      } finally {
  ...
  return (
    <div className="cf-app-root" data-environment="chooser" style={themeStyleVars(CORE_THEME)}>
      ...
      <main className="cf-app-main">
        {checkingTenant ? (
          <section className="cf-app-panel" data-testid="chooser-tenant-check">
            <h1 className="cf-app-h1">Checking workspace session</h1>
            <p className="cf-app-lead">
              If you already have a Tenant Workspace session, you will continue there. Staff without
              a tenant session stay on this chooser.
            </p>
          </section>
        ) : (
