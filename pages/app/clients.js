...
export default function AppClientsPage() {
  ...
  const [clients, setClients] = useState(/** @type {Array<Record<string, unknown>>} */ ([]));
  ...
  const selectedId = router.isReady
    ? String(Array.isArray(router.query.id) ? router.query.id[0] : router.query.id || '')
    : '';
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const listRes = await fetch(`/api/app/clients?${apiBase}`, { credentials: 'same-origin' });
      ...
      if (!listRes.ok || !listJson.ok) {
        setError(String(listJson.error || `clients_${listRes.status}`));
        setClients([]);
        return;
      }
      setClients(Array.isArray(listJson.clients) ? listJson.clients : []);
      ...
      <AppShell environment="core" role="—">
        <section className="cf-app-panel" data-testid="app-auth-required">
          ...
          <p className="cf-app-lead">
            Clients is staff-only. Use the existing Core / admin session.
          </p>
          <div className="cf-app-actions">
            <a
              className="cf-app-btn"
              data-primary="true"
              href={`/login?next=${encodeURIComponent('/app/clients')}`}
            >
          ...
          <p className="cf-app-muted" style={{ marginTop: 16 }} data-testid="proof-harness-hint">
            Deterministic test harness only: <a href="/app/clients?proof=1">Clients proof</a>
          </p>
        </section>
      </AppShell>
    );
    ...
    return (
      <AppShell environment="core" role="—">
        <section className="cf-app-panel" data-testid="app-core-denied">
          ...
          <p className="cf-app-lead">
            A Tenant session cannot open the Clients summary. Sign in with Core credentials.
          </p>
          ...
          <div className="cf-app-actions">
            ...
            <a className="cf-app-btn" href={`/login?next=${encodeURIComponent('/app/clients')}`}>
              Operating Workspace sign in
            </a>
          </div>
        </section>
  ...
  return (
    ...
    >
      ...
      />
      <p className="cf-app-muted" style={{ marginTop: -8, marginBottom: 16 }} data-testid="clients-meta">
        Operating Workspace · Clients · staff only · no Tenant leakage · existing records only
        {dataSource ? (
          <>
