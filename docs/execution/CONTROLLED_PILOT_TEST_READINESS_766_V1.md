...
**Prerequisite refresh:** [#874](https://github.com/antonvdberg-bit/corpflow-ai-command-center/issues/874) (repo-only; current `main`)  
...
| Commercial approval rail | `docs/revenue/COMMERCIAL_APPROVAL_RAIL_V1.md` · #714 | Gate + handoff helpers exist on `main` (merged #737). **#714 remains the durable commercial-rail controller.** Closed PR #870 must **not** be revived; further commercial-rail completion stays under #714. |
| LR / WR onboarding + delivery | #715 / #716 system-proof CLIs | `acceptance_ready` with no sends / no real DNS |

### #874 prerequisite refresh (2026-08-10)

Repo-only refresh of the #771 package against current `main` (operating model `2026-08-09-v1`, environment `local` for this validation).

| Check | Result |
| ----- | ------ |
| `node scripts/gtm-integrated-scenarios-711.mjs` | PASS — Scenarios A/B `acceptance_ready`; external sends empty |
| `node --test node-tests/gtm-integrated-scenarios-711.test.mjs` | PASS |
| `node --test node-tests/commercial-approval-rail.test.mjs` | PASS (existing #714/#737 unit gate on `main`) |
| `node scripts/controlled-pilot-rehearsal-766.mjs` | PASS packaging freshness; controlled-client verdict still **NOT READY** |
| Public GET smoke (read-only) | `corpflowai.com/contact`, `/lead-rescue`, `/offers/premium-landing-page-rescue` → **200** |
| Packet C evidence slots | Still empty — operator-owned |
| Packet B live access ticks | Still pending Anton-held credentials |

**D766 reconciliation (still accurate):**

- **D766-001** — still the **exact release blocker**. No ERPNext sandbox synthetic commercial-control evidence has been recorded in `packet-c-evidence-slots.json`.
- **D766-002** — still an **important non-blocker** for packaging; still blocks first client-login rehearsal until Packet B live ticks are completed with Anton-held test passwords (keep passwords out of GitHub).

**Commercial dependency note:** Keep working #714 as the commercial-rail controller. Do not recreate or revive PR #870. Incomplete #714 follow-ups do not replace D766-001 as the controlled-pilot exact blocker while integrated FA handoff simulation still PASSes.

Stale open PR #771 (branch `cursor/dispatcher-issue-766-5754`) predates current `main`; this #874 refresh is the current-main package vehicle. Operator may close #771 as superseded after this refresh lands.
...
## Operator checklist — remaining sandbox / access steps (Anton-held)

Cursor must **not** access ERPNext sandbox, production DB passwords, or live login credentials. Remaining steps:

### Packet B (test passwords — private)

1. Confirm `cursor-test-admin@corpflowai.com` and `cursor-test-tenant@corpflowai.com` exist (operator machine: `npm run provision:cursor-test-users` / `npm run verify:cursor-test-users -- --db-shape` with `POSTGRES_URL`).
2. Complete B1–B7 live ticks in `artifacts/controlled-pilot-766/packet-b-access-checklist.json` (set `pass` true/false + private notes). Keep passwords out of GitHub / screenshots.
3. Optional live auth probe only on an operator machine with env passwords: `npm run verify:cursor-test-users -- --live`.

### Packet C (ERPNext sandbox — private)

1. On **ERPNext sandbox only**, execute the chain in `docs/operations/ERPNEXT_LEAD_RESCUE_PAID_PILOT_REHEARSAL_V1.md` and Packet C above.
2. Fill `artifacts/controlled-pilot-766/packet-c-evidence-slots.json` with synthetic/redacted IDs only (or leave screenshots operator-private and record private refs).
3. Answer: is standard ERPNext sufficient for the first controlled pilot? YES/NO + one sentence.
4. Re-run `node scripts/controlled-pilot-rehearsal-766.mjs` after slots are filled (update defect register if D766-001 clears).

### Explicit non-actions for agents

- No ERPNext writes, no client sends, no payment, no DNS/deploy, no #870 revival, no merge from this packet alone.
...
- `artifacts/controlled-pilot-766/prerequisite-refresh-874.json`
