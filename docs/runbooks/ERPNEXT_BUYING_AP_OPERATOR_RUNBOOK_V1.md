...
Factory identity `integrations@corpflowai.com` can **read** Supplier but got **HTTP 403** on create (2026-08-26). Current-main GET 2026-08-28 (#1213 generation 2 on `be671871`) did not re-POST; the synthetic Supplier is still absent, so this grant is still outstanding. That is a permission gap, not accountant CoA approval. Closed PR #1217 must not resume.
