...
**Source:** [#1098](https://github.com/antonvdberg-bit/corpflow-ai-command-center/issues/1098)  
...
4. Search existing Purchase Invoices for the same Supplier + `bill_no`. If found, stop. Duplicate capture is the usual error.
...
Factory identity `integrations@corpflowai.com` can **read** Supplier but got **HTTP 403** on create (2026-08-26).
...
Then operators (or a re-run of `node scripts/erpnext/apply-buying-ap-readiness.mjs`) can create the synthetic test Supplier. Real suppliers still need Anton approval.
