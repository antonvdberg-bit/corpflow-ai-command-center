...
| PR-B | **Cold-Sprint-V1-Copy** ✅ shipped 2026-06-03 (`JE-2026-06-03-5`) — partial | small page-copy | P-1 shipped (Mauritius-trust line); hero subheadline channels-first rewrite shipped (broader than P-1..P-12 list); nav CTA aligned to doctrine. **Deferred:** P-3 (re-style "See how it works" as tertiary text-link), P-6 (refund-link inline), P-11 (OG/Twitter description Mauritius hook), P-12 (hero badge wrap CSS) — each remains HELD pending its own DECISION | Requires Anton DECISION; marketing-quality-gate; per-line doctrine review; full live-page production smoke after merge per `.cursor/rules/delivery-reality.mdc` | Low risk; ~7 lines edited (smaller than original ~30–50 estimate due to scope tightening) |
| PR-C | **Cold-Sprint-V1-Tracking** ✅ shipped 2026-06-03 (`JE-2026-06-03-3` PR #291 + `JE-2026-06-03-4` PR #293 SSG fix) | small instrumentation | P-2 / § 7.3 — 4 Plausible custom events shipped (`lr_primary_cta_click` with `location:nav` and `location:hero`, `lr_secondary_cta_click`, `lr_intake_submit_attempt`, `lr_intake_submit_success`); SSG runtime host-gated loader added in PR #293 to fix injection gap on apex `/lead-rescue`; segment prop deferred to PR-F | Requires Anton DECISION; node-tests pass; Plausible dashboard verification post-merge | Low risk; ~20 lines added |
