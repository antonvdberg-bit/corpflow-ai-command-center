...
| **Active client_production deployment candidates** | **1** | Only one `client_production` candidate at a time. CorpFlowAI-hosted `corpflow_test` publish after merge does not consume this slot. |
