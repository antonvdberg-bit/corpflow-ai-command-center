...
## 2. Surface map — the 13 active monitors (Monitor # 13 active 2026-06-16; sub-probe 8 / n8n explicitly deferred)
...
> **Monitor # 13 active 2026-06-16 (`JE-2026-06-16-2`):** Monitor # 13 (Uptime Kuma probe set on `corpflow-exec-01-u69678`) is **installed and probing** as of 2026-06-16. Pinned image `louislam/uptime-kuma:1.23.13`; container `uptime-kuma` `Up 39 minutes (healthy)` at install observation; loopback bind `127.0.0.1:3001->3001/tcp` confirmed by `docker ps` PORTS column; on-box `curl -I http://127.0.0.1:3001` returned `HTTP/1.1 302 Found Location: /dashboard`; off-box `curl -I --connect-timeout 8 http://5.78.213.185:3001` from the operator's laptop returned `curl: (28) Connection timed out after 8016 milliseconds` (the canonical K2 PASS signal). **K1=PASS / K2=PASS / K3=PASS-WITH-DEFERRAL / K4=PASS / K5=PASS-BY-CONSTRUCTION** per the runbook § 11 verification block. **Sub-probe 8 (n8n origin) explicitly DEFERRED** — operator (Anton) was not 100% sure of the canonical n8n health URL family and chose deferral over guessing (correct call per runbook § 8.2.8 + § 12.2). Sub-probes 1–7 (the seven CorpFlow public floor URLs that match `MONITORING_ARCHITECTURE.md` § 5) are configured and Up at 60 s intervals with retries=2; alerts route to the operator's Telegram chat via Kuma's own BotFather bot, **independent of the in-repo `TELEGRAM_BOT_TOKEN`** and **independent of n8n** (§ 9.2 SMTP and § 9.3 n8n forwarding both not-configured for v1 — n8n is therefore not in any alert path, which makes K5 PASS-by-construction stronger than the runbook K5 floor requires). **Blind spot # 7 (no third-location uptime monitoring)** is now **closed for the seven CorpFlow public floor URLs** (see § 6 row 7 update). Authorization basis chain: `docs/decisions/20260615-uptime-kuma-on-exec01.md` (ADR ACCEPTED) + `docs/execution/UPTIME_KUMA_ON_EXEC01_AUTHORIZATION_PACKET.md` (`JE-2026-06-15-1`) + `docs/runbooks/UPTIME_KUMA_ON_EXEC01_INSTALL_RUNBOOK_V1.md` (`JE-2026-06-16-1`) + this install execution (`JE-2026-06-16-2`). Evidence captured at `artifacts/self-hosted-ops-stack-v1/2026-06-15-phase-1a-live-verification.md` § 7.4 (no secrets, no tokens, no chat ids).
...
| 13 | **Uptime Kuma probe set** (`corpflow-exec-01-u69678`) | single Docker container on the box, image **`louislam/uptime-kuma:1.23.13`** (pinned), container `uptime-kuma`, `127.0.0.1:3001->3001/tcp` loopback only, persistent volume `~/uptime-kuma-data/` (mode 700). **Active 2026-06-16** (`JE-2026-06-16-2`). | internal Kuma scheduler — 60 s heartbeat / 2 retries / 60 s retry-interval / 15 s timeout per monitor; operator UI access via SSH local-port-forward only (`ssh -L 3001:localhost:3001 anton@5.78.213.185`) | **7 of 8 sub-probes Up:** seven CorpFlow public floor URLs (`https://core.corpflowai.com/api/factory/health`, `https://core.corpflowai.com/api/factory/production-pulse/runtime`, `https://corpflowai.com/`, `https://corpflowai.com/lead-rescue`, `https://aileadrescue.corpflowai.com/`, `https://lux.corpflowai.com/`, `https://lux.corpflowai.com/change`). **Sub-probe 8 (n8n origin) explicitly DEFERRED** at install time (2026-06-16) — operator was not 100% sure of the canonical n8n health URL family and chose deferral over guessing (correct per runbook § 8.2.8 + § 12.2). Will be added as a single-monitor edit inside Kuma's UI in a future small follow-up once Anton confirms the URL family — does not require a new PR. | does not probe state-mutating routes; does not probe factory-master / `/api/admin/*` / `/api/automation/ingest`; does not hold any CorpFlow secret; does not write to Postgres; does not run anywhere outside `corpflow-exec-01`. **Authorization is for Uptime Kuma alone — does not authorize Chatwoot, Open WebUI, Dify, Coolify, Langfuse, AgentSpan, OpenJarvis, generic chatbot, generic agent framework, n8n migration, public port exposure, or any second container.** | **Kuma's own Telegram bot (separate BotFather token from in-repo `TELEGRAM_BOT_TOKEN`; credentials held in operator's Infisical vault)** is the **primary** alert channel and the only configured notifier in v1. The optional SMTP backup (§ 9.2) and n8n forwarding (§ 9.3) are both **NOT-CONFIGURED** in v1 — n8n is therefore not in any alert path, which makes the K5 invariant (alert path independent of n8n) PASS-by-construction stronger than the runbook K5 floor requires. Failure-domain isolation by design — must work even if n8n, in-repo Telegram bot, or Vercel is down. | `docs/decisions/20260615-uptime-kuma-on-exec01.md` (ADR — authorization), `docs/execution/UPTIME_KUMA_ON_EXEC01_AUTHORIZATION_PACKET.md` (`JE-2026-06-15-1`), `docs/runbooks/UPTIME_KUMA_ON_EXEC01_INSTALL_RUNBOOK_V1.md` (`JE-2026-06-16-1`), `docs/operations/SELF_HOSTED_OPS_STACK_V1.md` § 3 (Phase 1 doctrine), `docs/operations/SERVER_AGENT_ACCESS_AND_EXECUTION_BOUNDARY_V1.md` § 5.5 (named carve-out), `artifacts/self-hosted-ops-stack-v1/2026-06-15-phase-1a-live-verification.md` § 7.4 (install evidence — `JE-2026-06-16-2`), this doc § 11.1 (state row). |
...
| #13 Uptime Kuma probe set | ✅ Kuma's own bot (separate BotFather token from in-repo `TELEGRAM_BOT_TOKEN`; credentials in operator's Infisical vault — failure-domain isolation by design) | **NOT-CONFIGURED** in v1 (n8n forwarding § 9.3 deliberately not enabled — Kuma's primary path is direct to `api.telegram.org`, not via n8n; this is stronger than the runbook K5 floor requires) | **active 2026-06-16** (`JE-2026-06-16-2`) — installed at L3 by Anton; 7 of 8 sub-probes Up; sub-probe 8 / n8n explicitly deferred (operator did not have the URL family confirmed at install time); test alert delivered via Kuma's own bot (K4 PASS) |
...
7. **~~No third-location uptime.~~** **CLOSED 2026-06-16 for the seven CorpFlow public floor URLs** (`JE-2026-06-16-2`). Monitor # 13 (Uptime Kuma on `corpflow-exec-01-u69678`) probes the seven floor URLs from § 5 every 60 s, with alerts via Kuma's own Telegram bot independent of GitHub Actions, Vercel, n8n, and the in-repo `TELEGRAM_BOT_TOKEN`. **Residual gap:** sub-probe 8 (n8n origin) was deferred at install time and will be added inside Kuma's UI when Anton confirms the URL family — this does not block the closure for the seven floor URLs. The named complementary future packet `exec01-uptime-from-third-location` (managed third-party SaaS uptime probe — BetterStack / Cronitor / similar) remains in § 11.2 as a *second* third-location signal; it is no longer the only mitigation, only an additional one to be re-evaluated after # 13 has 30 production days of clean signal.
...
| #13 Uptime Kuma probe set (`corpflow-exec-01-u69678`) | ✅ active (with sub-probe 8 / n8n deferred) | **Installed at L3 by Anton 2026-06-16** (`JE-2026-06-16-2`) per `docs/runbooks/UPTIME_KUMA_ON_EXEC01_INSTALL_RUNBOOK_V1.md`. Pinned image `louislam/uptime-kuma:1.23.13`; container `uptime-kuma` `Up (healthy)`; loopback `127.0.0.1:3001->3001/tcp`; 7 of 8 sub-probes Up (the seven CorpFlow public floor URLs); sub-probe 8 (n8n origin) explicitly deferred — operator was not 100% sure of the canonical URL family and chose deferral over guessing (correct per runbook § 8.2.8 + § 12.2). K1=PASS / K2=PASS / K3=PASS-WITH-DEFERRAL / K4=PASS / K5=PASS-BY-CONSTRUCTION (n8n is not in any alert path — § 9.2 SMTP and § 9.3 n8n forwarding both not-configured for v1). Authorization basis chain: ADR `docs/decisions/20260615-uptime-kuma-on-exec01.md` + packet `docs/execution/UPTIME_KUMA_ON_EXEC01_AUTHORIZATION_PACKET.md` (`JE-2026-06-15-1`) + runbook `docs/runbooks/UPTIME_KUMA_ON_EXEC01_INSTALL_RUNBOOK_V1.md` (`JE-2026-06-16-1`) + this install execution (`JE-2026-06-16-2`). Single named carve-out from `docs/operations/SERVER_AGENT_ACCESS_AND_EXECUTION_BOUNDARY_V1.md` § 5.5. Evidence at `artifacts/self-hosted-ops-stack-v1/2026-06-15-phase-1a-live-verification.md` § 7.4. |
...
| `kuma-monitor-8-n8n-add` | **Deferred sub-probe (single-monitor add inside Kuma UI; no PR needed).** Add the eighth Kuma monitor against the canonical n8n health URL once Anton confirms the URL family. Configured directly in Kuma's UI through the SSH tunnel; configuration is recorded inside the Kuma SQLite DB (`~/uptime-kuma-data/kuma.db`), not in repo. **Out of scope:** any change to the n8n host itself, n8n migration, or pointing the monitor at the production webhook ingest URL (`<n8n-host>/webhook/automation-forward`) — that would create real `automation_events` rows on every probe and is forbidden. | Anton confirms the canonical n8n health URL (e.g. `<n8n-host>/healthz`) family. |
