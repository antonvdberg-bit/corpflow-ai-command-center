...
- `docs/operations/BESZEL_SERVER_UTILISATION_PILOT.md` — **docs/scaffold only** Beszel utilisation pilot for `corpflow-exec-01-u69678` (#727); not installed / not live; does not replace #13 or #14.
- `docs/operations/BACKUP_HEALTH_MONITOR.md` — Monitor #14 (restic → R2 backup correctness).
...
## 2. Surface map — the 13 active monitors + Monitor #14 (backup-health authored 2026-07-27; L3 timer pending)

> **Monitor #14 (backup health) — repo-authored 2026-07-27; not yet runtime-active on `corpflow-exec-01`.** Script + user-systemd templates live in-repo (`scripts/ops/backup-health-check.sh`, `scripts/ops/systemd/corpflowai-ops-backup-health.{service,timer}`). Canonical operator doc: `docs/operations/BACKUP_HEALTH_MONITOR.md`. Until Anton enables the timer on the box, treat #14 as **authored / install-pending**, not live.
...
> **Monitor # 13 active 2026-06-16 — all 8 sub-probes Up (`JE-2026-06-16-3`, supersedes `JE-2026-06-16-2`'s "with-n8n-deferred" caveat):** Monitor # 13 (Uptime Kuma probe set on `corpflow-exec-01-u69678`) is **installed and probing** as of 2026-06-16. Pinned image `louislam/uptime-kuma:1.23.13`; container `uptime-kuma` `Up (healthy)`; loopback bind `127.0.0.1:3001->3001/tcp` confirmed by `docker ps` PORTS column; on-box `curl -I http://127.0.0.1:3001` returned `HTTP/1.1 302 Found Location: /dashboard`; off-box `curl -I --connect-timeout 8 http://5.78.213.185:3001` from the operator's laptop returned `curl: (28) Connection timed out after 8016 milliseconds` (the canonical K2 PASS signal). **K1=PASS / K2=PASS / K3=PASS / K4=PASS / K5=PASS-BY-CONSTRUCTION** per the runbook § 11 verification block (K3 fully PASS now that all 8 sub-probes are Up — the earlier K3=PASS-WITH-DEFERRAL state is closed). **All 8 sub-probes Up:** sub-probes 1–7 (the seven CorpFlow public floor URLs that match `MONITORING_ARCHITECTURE.md` § 5) plus sub-probe 8 (n8n origin `/healthz` — the canonical n8n v1.x anonymous health endpoint, GET-only, returns `{"status":"ok"}` 200, no auth, no state mutation; URL family operator-confirmed by Anton on 2026-06-16; URL itself recorded only inside Kuma's SQLite DB at `~/uptime-kuma-data/kuma.db`, not in this repo). All eight at 60 s intervals with retries=2; alerts route to the operator's Telegram chat via Kuma's own BotFather bot, **independent of the in-repo `TELEGRAM_BOT_TOKEN`** and **independent of n8n** (§ 9.2 SMTP and § 9.3 n8n forwarding both not-configured for v1 — n8n is therefore not in any alert path, which makes K5 PASS-by-construction stronger than the runbook K5 floor requires; sub-probe 8 monitors n8n's *availability* but does not route through n8n for alerting). The Telegram alert path was test-confirmed for sub-probe 8 specifically on 2026-06-16. **Blind spot # 7 (no third-location uptime monitoring)** is now **CLOSED end-to-end** (see § 6 row 7 update). Authorization basis chain: `docs/decisions/20260615-uptime-kuma-on-exec01.md` (ADR ACCEPTED) + `docs/execution/UPTIME_KUMA_ON_EXEC01_AUTHORIZATION_PACKET.md` (`JE-2026-06-15-1`) + `docs/runbooks/UPTIME_KUMA_ON_EXEC01_INSTALL_RUNBOOK_V1.md` (`JE-2026-06-16-1`) + install execution (`JE-2026-06-16-2`, PR #373 merged 2026-06-16T07:01:15Z at `a9157216`) + sub-probe 8 add (`JE-2026-06-16-3`). Evidence captured at `artifacts/self-hosted-ops-stack-v1/2026-06-15-phase-1a-live-verification.md` § 7.4 (no secrets, no tokens, no chat ids — including the actual n8n hostname per § 12.2 of the runbook).
...
| 13 | **Uptime Kuma probe set** (`corpflow-exec-01-u69678`) | single Docker container on the box, image **`louislam/uptime-kuma:1.23.13`** (pinned), container `uptime-kuma`, `127.0.0.1:3001->3001/tcp` loopback only, persistent volume `~/uptime-kuma-data/` (mode 700). **Active 2026-06-16; all 8 sub-probes Up** (`JE-2026-06-16-3`, supersedes `JE-2026-06-16-2`'s with-n8n-deferred caveat). | internal Kuma scheduler — 60 s heartbeat / 2 retries / 60 s retry-interval / 15 s timeout per monitor; operator UI access via SSH local-port-forward only (`ssh -L 3001:localhost:3001 anton@5.78.213.185`) | **All 8 sub-probes Up:** seven CorpFlow public floor URLs (`https://core.corpflowai.com/api/factory/health`, `https://core.corpflowai.com/api/factory/production-pulse/runtime`, `https://corpflowai.com/`, `https://corpflowai.com/lead-rescue`, `https://aileadrescue.corpflowai.com/`, `https://lux.corpflowai.com/`, `https://lux.corpflowai.com/change`) + sub-probe 8 (n8n origin `/healthz` — the canonical n8n v1.x anonymous health endpoint; GET-only, returns `{"status":"ok"}` 200, no auth, no state mutation; URL family operator-confirmed by Anton 2026-06-16; URL itself recorded only inside Kuma's SQLite DB at `~/uptime-kuma-data/kuma.db`, never in this repo per runbook § 12.2). | does not probe state-mutating routes; does not probe factory-master / `/api/admin/*` / `/api/automation/ingest`; does not probe `<n8n>/webhook/*` (state-mutating, would create real `automation_events` rows on every probe — explicitly forbidden per runbook § 8.5 anti-pattern guard); does not hold any CorpFlow secret; does not write to Postgres; does not run anywhere outside `corpflow-exec-01`. **Authorization is for Uptime Kuma alone — does not authorize Chatwoot, Open WebUI, Dify, Coolify, Langfuse, AgentSpan, OpenJarvis, generic chatbot, generic agent framework, n8n migration, public port exposure, or any second container.** | **Kuma's own Telegram bot (separate BotFather token from in-repo `TELEGRAM_BOT_TOKEN`; credentials held in operator's Infisical vault)** is the **primary** alert channel and the only configured notifier in v1, attached to all 8 sub-probes (test-confirmed for sub-probes 1–7 on 2026-06-16 at install + sub-probe 8 on 2026-06-16 at sub-probe-add). The optional SMTP backup (§ 9.2) and n8n forwarding (§ 9.3) are both **NOT-CONFIGURED** in v1 — n8n is therefore not in any alert path, which makes the K5 invariant (alert path independent of n8n) PASS-by-construction stronger than the runbook K5 floor requires; sub-probe 8 monitors n8n's *availability* but its alert routes around n8n entirely. Failure-domain isolation by design — must work even if n8n, in-repo Telegram bot, or Vercel is down. | `docs/decisions/20260615-uptime-kuma-on-exec01.md` (ADR — authorization), `docs/execution/UPTIME_KUMA_ON_EXEC01_AUTHORIZATION_PACKET.md` (`JE-2026-06-15-1`), `docs/runbooks/UPTIME_KUMA_ON_EXEC01_INSTALL_RUNBOOK_V1.md` (`JE-2026-06-16-1`), install evidence `artifacts/self-hosted-ops-stack-v1/2026-06-15-phase-1a-live-verification.md` § 7.4 (`JE-2026-06-16-2`, PR #373 merged 2026-06-16T07:01:15Z at `a9157216`), sub-probe 8 add (`JE-2026-06-16-3` — this PR), `docs/operations/SELF_HOSTED_OPS_STACK_V1.md` § 3 (Phase 1 doctrine), `docs/operations/SERVER_AGENT_ACCESS_AND_EXECUTION_BOUNDARY_V1.md` § 5.5 (named carve-out), this doc § 11.1 (state row). |
| 14 | **Backup health (restic → R2)** | `corpflow-exec-01-u69678` user-systemd (`corpflowai-ops-backup-health.timer` / `.service`); script `~/.local/bin/corpflowai-ops-backup-health-check.sh` from `scripts/ops/backup-health-check.sh`. **Repo-authored 2026-07-27; L3 enable pending Anton.** | daily **07:15 UTC** (+ ≤120s jitter) once timer enabled | Independent read-only check that the existing restic ops repo is reachable, has ≥1 recent snapshot (age ≤36h), snapshot count is plausible (≥2), and `restic stats` total_size is within a sane band. Fail-closed if health cannot be determined. | Does **not** run backup/prune; does **not** touch production Postgres / `POSTGRES_URL`; does **not** prove path-level restore of every volume; does **not** replace Kuma (#13); does **not** send success spam. | Telegram **failure-only** via `TELEGRAM_BOT_TOKEN` + `TELEGRAM_ALERT_CHAT_ID` (same names as § 4.1); silent on success; anti-spam `kind=backup_health` × UTC hour. | `docs/operations/BACKUP_HEALTH_MONITOR.md` + `docs/operations/SELF_HOSTED_OPS_R2_RESTIC.md` + `docs/operations/SERVER_SAFETY_BASELINE_AND_CHATWOOT_DECISION_V1.md` §3 |
...
- **restic heartbeat + retention** (user-systemd on `corpflow-exec-01`, operational since 2026-06-26 per `SELF_HOSTED_OPS_R2_RESTIC.md`) — these are the **backup jobs**, not the health monitor. Monitor #14 is the independent checker.
...
| 07:15 | every day | #14 | Backup health (restic → R2) — **intended** once Anton enables `corpflowai-ops-backup-health.timer` on `corpflow-exec-01` |
...
| #13 Uptime Kuma probe set | ✅ Kuma's own bot (separate BotFather token from in-repo `TELEGRAM_BOT_TOKEN`; credentials in operator's Infisical vault — failure-domain isolation by design) | **NOT-CONFIGURED** in v1 (n8n forwarding § 9.3 deliberately not enabled — Kuma's primary path is direct to `api.telegram.org`, not via n8n; this stays true even though sub-probe 8 monitors n8n itself, because the *alert path* never touches n8n) | **active 2026-06-16; all 8 sub-probes Up** (`JE-2026-06-16-3`) — install evidence in PR #373 (`JE-2026-06-16-2`), sub-probe 8 (n8n `/healthz`) added by operator inside Kuma's UI on 2026-06-16 once URL family was confirmed; test alert delivered via Kuma's own bot for sub-probe 8 specifically (K4 PASS extended to all 8) |
| #14 Backup health (restic → R2) | ✅ direct (script curl → Telegram API) once L3 timer enabled; failure-only | — | **authored in-repo 2026-07-27; runtime pending Anton install** (`docs/operations/BACKUP_HEALTH_MONITOR.md` §7). Uses in-repo `TELEGRAM_BOT_TOKEN` + `TELEGRAM_ALERT_CHAT_ID` names (not Kuma's bot). |
...
7. **~~No third-location uptime.~~** **CLOSED end-to-end 2026-06-16** (`JE-2026-06-16-2` for the seven CorpFlow public floor URLs + `JE-2026-06-16-3` for sub-probe 8 / n8n `/healthz`). Monitor # 13 (Uptime Kuma on `corpflow-exec-01-u69678`) probes all eight surfaces — the seven floor URLs from § 5 plus the n8n origin `/healthz` — every 60 s, with alerts via Kuma's own Telegram bot independent of GitHub Actions, Vercel, n8n, and the in-repo `TELEGRAM_BOT_TOKEN`. (Sub-probe 8 monitors n8n itself, but the alert routing never traverses n8n — see § 4 row 13.) The named complementary future packet `exec01-uptime-from-third-location` (managed third-party SaaS uptime probe — BetterStack / Cronitor / similar) remains in § 11.2 as a *second* third-location signal; it is no longer needed as a primary mitigation, only as additional belt-and-suspenders to be re-evaluated after # 13 has 30 production days of clean signal (≈ 2026-07-16).
8. **`corpflow-exec-01` host heartbeat is still thin.** The box now runs scheduled user-systemd work (restic heartbeat + retention since 2026-06-26; Monitor #14 backup-health timer pending Anton enable). There is still **no** general “is the VM up?” probe independent of those jobs. Kuma (#13) runs *on* the box, so a full host/power failure can silence both Kuma and restic alerts until something external notices. Future packet `exec01-uptime-from-third-location` remains the belt-and-suspenders path.
9. **~~Ops backup failure was journal-only.~~** **Repo mitigation authored 2026-07-27** as Monitor #14 (`BACKUP_HEALTH_MONITOR.md`). **Runtime still pending** until Anton enables `corpflowai-ops-backup-health.timer`. Until then, restic failures remain visible only via `journalctl` on the box.
10. **Guest utilisation / container metrics are not visible without SSH (or a future Beszel install).** Uptime Kuma answers reachability; Monitor #14 answers backup correctness; supplier consoles may show external CPU/network but generally not guest RAM or Docker stats. Docs/scaffold for a private Beszel pilot: `docs/operations/BESZEL_SERVER_UTILISATION_PILOT.md` (packet `beszel-utilisation-pilot-on-exec01`, #727) — **not live** until Anton runs a separate authorized L3 install. Backup health will still not live inside Beszel; see future packet `ops-health-backup-indicator`.
...
- `scripts/ops/backup-health-check.sh` — #14 backup-health monitor (L3; Telegram failure-only)
- `docs/operations/BACKUP_HEALTH_MONITOR.md` — #14 canonical operator doc + install packet
- `docs/operations/SELF_HOSTED_OPS_R2_RESTIC.md` — restic → R2 backup mechanism that #14 verifies
...
| #13 Uptime Kuma probe set (`corpflow-exec-01-u69678`) | ✅ active (all 8 sub-probes Up) | **Installed at L3 by Anton 2026-06-16** (`JE-2026-06-16-2`, PR #373 merged at `a9157216`) per `docs/runbooks/UPTIME_KUMA_ON_EXEC01_INSTALL_RUNBOOK_V1.md`; **sub-probe 8 (n8n `/healthz`) added 2026-06-16** (`JE-2026-06-16-3`, this PR) once Anton confirmed the URL family. Pinned image `louislam/uptime-kuma:1.23.13`; container `uptime-kuma` `Up (healthy)`; loopback `127.0.0.1:3001->3001/tcp`; **all 8 sub-probes Up** — the seven CorpFlow public floor URLs + sub-probe 8 (n8n origin `/healthz`, GET-only, returns `{"status":"ok"}` 200, no auth, no state mutation; URL recorded only inside Kuma's SQLite DB at `~/uptime-kuma-data/kuma.db`, never in this repo per runbook § 12.2). K1=PASS / K2=PASS / K3=PASS / K4=PASS / K5=PASS-BY-CONSTRUCTION (n8n is not in any alert path — § 9.2 SMTP and § 9.3 n8n forwarding both not-configured for v1; sub-probe 8 monitors n8n's *availability* but its alert routes around n8n entirely; Telegram path test-confirmed for sub-probe 8 specifically on 2026-06-16). Authorization basis chain: ADR `docs/decisions/20260615-uptime-kuma-on-exec01.md` + packet `docs/execution/UPTIME_KUMA_ON_EXEC01_AUTHORIZATION_PACKET.md` (`JE-2026-06-15-1`) + runbook `docs/runbooks/UPTIME_KUMA_ON_EXEC01_INSTALL_RUNBOOK_V1.md` (`JE-2026-06-16-1`) + install execution (`JE-2026-06-16-2`) + sub-probe 8 add (`JE-2026-06-16-3`). Single named carve-out from `docs/operations/SERVER_AGENT_ACCESS_AND_EXECUTION_BOUNDARY_V1.md` § 5.5 — sub-probe 8 add operates entirely *inside* the existing carve-out (no new permissions, no new container, no port change, no new auth). Evidence at `artifacts/self-hosted-ops-stack-v1/2026-06-15-phase-1a-live-verification.md` § 7.4. |
| #14 Backup health (restic → R2) | ⚠️ authored / L3 pending | Script + systemd templates + `docs/operations/BACKUP_HEALTH_MONITOR.md` landed in-repo 2026-07-27. **Not runtime-active** until Anton runs the §7 install packet on `corpflow-exec-01` (enable `corpflowai-ops-backup-health.timer`). Closes the named gap from `SERVER_SAFETY_BASELINE_AND_CHATWOOT_DECISION_V1.md` §3 at the repo layer only. |
...
| ~~`Server-Backup-Health-Check-And-Alert-1`~~ | **REPO-LANDED 2026-07-27** (script + docs + Monitor #14 row). Runtime L3 timer enable remains Anton-gated — see `docs/operations/BACKUP_HEALTH_MONITOR.md` §7. Row kept struck for history; do not re-author the script. | Anton install on `corpflow-exec-01` + Telegram names in restic/telegram env. |
...
| ~~`kuma-on-exec01-install`~~ | **LANDED 2026-06-16** via `JE-2026-06-16-2`. Anton ran `docs/runbooks/UPTIME_KUMA_ON_EXEC01_INSTALL_RUNBOOK_V1.md` end-to-end at L3; K1–K5 PASS (with K3 PASS-WITH-DEFERRAL for sub-probe 8 / n8n and K5 PASS-BY-CONSTRUCTION). Monitor # 13 is now in § 11.1 as ✅ active. Row kept here (struck through) for the historical record only — no further action; do **not** re-open. | n/a |
| ~~`kuma-monitor-8-n8n-add`~~ | **LANDED 2026-06-16** via `JE-2026-06-16-3` (this PR). Anton added sub-probe 8 against the canonical n8n `/healthz` endpoint inside Kuma's UI through the SSH tunnel (no new PR was needed for the *configuration*; this PR is docs-sync only). Sub-probe Up at 60 s; Telegram alert path test-confirmed; all probe constraints satisfied (GET-only `/healthz`, returns `{"status":"ok"}` 200, no auth, no state mutation; webhook routes explicitly avoided per runbook § 8.5 anti-pattern guard); URL recorded only inside Kuma's SQLite DB at `~/uptime-kuma-data/kuma.db`, never in this repo per runbook § 12.2. Row kept here (struck through) for the historical record only — no further action; do **not** re-open. | n/a |
| `exec01-uptime-from-third-location` | Add a *managed third-party* SaaS uptime probe (BetterStack / Cronitor / similar) as a **second** third-location signal, complementary to (not a replacement for) Monitor # 13 now that # 13 is active. | Vendor selection + DPA. Re-evaluate priority after # 13 has 30 production days of clean signal (i.e. ≈ 2026-07-16). |
| `beszel-utilisation-pilot-on-exec01` | **Docs/scaffold authored** in `docs/operations/BESZEL_SERVER_UTILISATION_PILOT.md` + `ops/beszel/` (#727). Loopback Beszel hub+agent for CPU/RAM/disk/network/container utilisation via SSH tunnel. **Not installed / not live.** Does **not** replace Uptime Kuma (#13) or Backup Monitor #14. L3 install remains Anton-gated and requires a separate Beszel carve-out authorization. | Anton approval + ADR/carve-out + §6 verification in the Beszel runbook; 48h observation before any Beszel alert fan-out. |
| `ops-health-backup-indicator` | Internal Ops Health card/page showing **sanitized** Backup Monitor #14 status (green/amber/red/unknown, latest run, snapshot age/count, last log line, timer waiting). **Not** Beszel-native; do not query R2 from a public dashboard. Tracked as [#735](https://github.com/antonvdberg-bit/corpflow-ai-command-center/issues/735). | Monitor #14 timer live + sanitized status artifact design; follow-up from #727. |
| `openhands-on-exec01-install` | **Docs-only, PENDING — nothing installed.** Full Phase 1 documentation set authored 2026-08-04 ([#743](https://github.com/antonvdberg-bit/corpflow-ai-command-center/issues/743), parent [#661](https://github.com/antonvdberg-bit/corpflow-ai-command-center/issues/661)): `docs/operations/OPENHANDS_ARCHITECTURE.md`, `OPENHANDS_INSTALL_RUNBOOK.md`, `OPENHANDS_SECURITY_MODEL.md`, `OPENHANDS_OPERATING_RUNBOOK.md`, `OPENHANDS_MODEL_AND_COST_POLICY.md`, `OPENHANDS_ROLLBACK_AND_UNINSTALL.md`; `docs/execution/OPENHANDS_WORK_PACKET_TEMPLATE.md`, `OPENHANDS_SYNTHETIC_VALIDATION_PLAN.md`, `OPENHANDS_ON_EXEC01_AUTHORIZATION_PACKET.md`; ADR `docs/decisions/20260804-openhands-on-exec01.md`. Reviewed-but-inactive package at `ops/openhands/`, `config/openhands/`, `scripts/ops/openhands/`. **Not** a live monitor and **not** added to § 11.1 — there is nothing to probe. Would add a control-plane container with a Docker-socket mount (larger blast radius than Kuma's #13 carve-out — see the security model § 3). **Capacity contradiction to resolve live before any install decision**, not by this row: historical § 11.3 figure `2 vCPU / 2 GB RAM / 38 GB disk` (stale, pre-resize) vs. post-resize authoritative `4 vCPU / ~7.5 GiB RAM / 150 GB disk` per `SERVER_AGENT_ACCESS_AND_EXECUTION_BOUNDARY_V1.md` § 5.1 vs. Anton's informal Beszel-style observation of ~6 CPU / ~25.7 GiB RAM / ~17–18 GiB headroom (not live-verified). `scripts/ops/openhands/inspect-host-capacity.sh` is the required live tie-breaker at install time. | Anton merge of the ADR + authorization packet (the § 5.5 carve-out) **and** a further, separate, explicit install go-ahead. Second named exception alongside Kuma — does not generalize Docker usage on the box. |
