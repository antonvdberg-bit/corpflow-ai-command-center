...
> **Monitor # 13 authorization note (2026-06-15):** Monitor # 13 (Uptime Kuma probe set on `corpflow-exec-01-u69678`) is **authorized** by `docs/decisions/20260615-uptime-kuma-on-exec01.md` + `docs/execution/UPTIME_KUMA_ON_EXEC01_AUTHORIZATION_PACKET.md` (`JE-2026-06-15-1`) but the install runbook is a **separate follow-up packet** (`UPTIME_KUMA_ON_EXEC01_INSTALL_RUNBOOK_V1`) — not yet authored, not yet executed. The row below documents the authorized shape; until the follow-up packet is merged and the operator runs the install at L3 per the § 5.4 pattern in `docs/operations/SERVER_AGENT_ACCESS_AND_EXECUTION_BOUNDARY_V1.md`, # 13 is **not probing** and blind spot # 7 (no third-location uptime) is **not** yet closed. Cross-reference: § 11.1 row state = `🟡 authorized, not installed`; § 11.2 row `kuma-on-exec01-install` is the follow-up.
...
| #13 Uptime Kuma probe set | ✅ Kuma's own bot (separate from the in-repo bot — failure-domain isolation by design) | secondary only — never on the critical-outage primary path | authorized 2026-06-15 (`JE-2026-06-15-1`); install runbook is a follow-up packet (`kuma-on-exec01-install` in § 11.2); not yet probing |
...
| #13 Uptime Kuma probe set (`corpflow-exec-01-u69678`) | 🟡 authorized, not installed | Authorized 2026-06-15 by `docs/decisions/20260615-uptime-kuma-on-exec01.md` + `docs/execution/UPTIME_KUMA_ON_EXEC01_AUTHORIZATION_PACKET.md` (`JE-2026-06-15-1`). Install runbook is a follow-up packet (`kuma-on-exec01-install` in § 11.2). Until the install runbook merges and the operator runs it at L3, # 13 is **not** probing and blind spot # 7 is **not** closed. Single named carve-out from `docs/operations/SERVER_AGENT_ACCESS_AND_EXECUTION_BOUNDARY_V1.md` § 5.5. |
...
| `kuma-on-exec01-install` | Install runbook for Monitor # 13 (Uptime Kuma) on `corpflow-exec-01-u69678` per `docs/decisions/20260615-uptime-kuma-on-exec01.md`. Cursor authors at L1 (operator-paste runbook); Anton runs at L3 per `SERVER_AGENT_ACCESS_AND_EXECUTION_BOUNDARY_V1.md` § 5.4 pattern; live verification per the install-runbook template in `docs/execution/UPTIME_KUMA_ON_EXEC01_AUTHORIZATION_PACKET.md` § 9 (K1–K5). | Authorization packet (`JE-2026-06-15-1`) merged. Anton's password manager has a fresh Kuma admin password generated. Operator has created a separate Telegram bot for Kuma (independent of the in-repo `TELEGRAM_BOT_TOKEN`). |
