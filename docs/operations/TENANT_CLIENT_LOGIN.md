...
- The Change Console stores `**client_view.automation.client_site_preview_url**` on the ticket: the usual preview URL plus a signed `**cf_preview=**` query parameter. For `***.vercel.app**` hosts, if the deployment URL is the **root** (`/` only), the stored link is rewritten to `**/lux-landing-static**` so clients avoid Vercel **404** on `/` while `/api/*` still works.
...
| 10 | Service-factory readiness overlay (#988): `**docs/operations/CIPC_SERVICE_FACTORY_READINESS_V1.md**` maps existing `cmp_tickets` / `console_json` / email-intake onto one template for Annual Returns, Director Changes, and BO. No schema. Not a public launch. |
| 11 | Response automation overlay (#987): `**docs/operations/CIPC_RESPONSE_AUTOMATION_V1.md**` stores classification, drafts, approval/send/follow-up state, and reply links on existing ticket/lead JSON. Operator queue is `/change`. Send stays blocked. |
| 12 | Internal pricing decision model (#989): `**docs/operations/CIPC_PRICING_MODEL_V1.md**` — operator panel on `/change` only. Not public pricing. Not a client quotation. Statutory CIPC fees are pass-through, never revenue. |
...
| 4    | Provision access: `node scripts/provision-tenant-test-access.mjs --tenant=luxe-maurice --pin` and/or `--username=... --gen-password` (requires `POSTGRES_URL`). For dedicated Cursor test users (admin + Lux, #696): `docs/runbooks/CURSOR_TEST_USERS_PROVISIONING.md` / `npm run provision:cursor-test-users -- --gen-password`. |
