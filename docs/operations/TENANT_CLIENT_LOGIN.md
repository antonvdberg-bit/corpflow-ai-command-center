...
**Commercial slice (corpflow_test):** Dedicated `CipcDeskLanding` on `/` (SME + professional-partner routes, provisional catalogue, email-first enquiry). Operator queue on `/change`. Client progress strip on `/client/change-decisions` when `cipc_desk_status` is present. Magic links prefer the standing CIPC host over apex. Serah checklist: `docs/operations/CIPC_DESK_SERAH_VALIDATION_PACKET.md`. Research (brand/competitors/routes): `docs/product/CIPC_DESK_RESEARCH_PACKET_V1.md` — **NO IMPLEMENTATION AUTHORIZED** for domains, payments, or live messaging.
