...
> Cross-reference: this section (and § 4b below) is the canonical wording cited by **`docs/operations/MONITORING_ARCHITECTURE.md`** § 4 ("Health surfaces vs deeper checks"). When you hit a "green health, broken DB" symptom, read both this section and § 4 of the monitoring doc together.
...
- `docs/operations/MONITORING_ARCHITECTURE.md` — canonical component map for every monitor in the stack. § 3 row 5 names `diagnose-postgres-env.yml` (this doc's § 4b/§ 5b playbook); § 4 is built on top of this doc's § 3 wording.
