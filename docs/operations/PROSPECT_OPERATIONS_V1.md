...
**Status:** Slice 1 shipped. Slice 2 shared detail/action layer shipped (`/app/prospects/[id]`, #994). Slice 3 shared Prospect Workbench shipped (`/app/workbench`, #996). Slice 3 Pipeline Kanban shipped (`/app/pipeline`, #997). Canonical Action Queue shipped (`/app/queue`, #995). **No schema. No env. No deploy. No external send.**
...
- Action Queue: **shipped in Operating Workspace (#995)** at `/app/queue` + `GET /api/app/queue`. Default filter is needs-action. Rows open `#994` `/app/prospects/[id]`. Safe edits reuse `PATCH /api/app/prospect`. Temporary Rapid Delivery desk remains at `/admin/rapid-delivery`.
...
**#772 first visible route:** `/app/prospects` (Operating Workspace, staff-only) reuses this view-model. **#772 Today / My Work:** `/app/today` applies `matchesMyWorkTodayFilter` to the same list. **#994 shared detail:** `/app/prospects/[id]`. **#996 Prospect Workbench:** `/app/workbench` (Lead Rescue, Website Rescue, and general enquiries; product grid at `/admin/lead-rescue` reduced to a temporary desk). **#997 Pipeline:** `/app/pipeline`. **#995 Action Queue:** `/app/queue` (cross-product; Rapid Delivery desk at `/admin/rapid-delivery` remains temporary). Product desks at `/admin/lead-rescue` and `/admin/rapid-delivery` remain temporary sources until later slices. See `docs/architecture/OPERATING_TENANT_WORKSPACE_CONSOLIDATION_V1.md`.
