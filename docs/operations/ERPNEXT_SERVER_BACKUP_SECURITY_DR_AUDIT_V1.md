...
| **S8** | `MASTER_ADMIN_KEY` not in ordinary Cursor Cloud | **NOT PRESENT** (control failed) | This run: **PRESENT**. `#899` / `JE-2026-08-12-1` still **INCOMPLETE**. Factory master credential is injected into ordinary ERPNext workers. **Anton UI delete** of the secret **name** from Cursor Dashboard Cloud Agents Secrets. Values never printed. |
...
| `MASTER_ADMIN_KEY` still in Cursor Cloud (S8) | **P0** | Cursor Dashboard → Cloud Agents → Secrets → **delete name** `MASTER_ADMIN_KEY` from general scope. Keep the three ERPNext API names. Fresh run must show **ABSENT**. | **Yes** (UI-only). Already authorized by #899; **not** a new ceremony. |
