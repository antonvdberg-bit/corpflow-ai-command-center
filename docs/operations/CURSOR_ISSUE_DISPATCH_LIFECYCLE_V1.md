...
**Created:** 2026-07-28.
**Updated:** 2026-08-27 (#1116 follow-up — current-generation `work_request_id` for Cloud Agents).
...
- Operator-review, merge-ready / implementation-complete, closed, and terminal-failed transitions **release the execution slot immediately** and strip active execution labels **and `dispatch:cursor-ready`** in the same lifecycle step. A further implementation attempt requires an explicit `CURSOR REQUEUE` generation boundary before restoring ready. An authorised operator/controller `CURSOR REQUEUE` heading (or durable `corpflow.cursor_requeue.v1` marker) creates the next generation with its own claim / `work_request_id`; historical Generation N evidence must not suppress Generation N+1, and replay of the same instruction is idempotent. Handoff then backfills the freed slot.
...
| **Primary — explicit CURSOR REQUEUE** | `issue_comment` created whose body **starts with** `CURSOR REQUEUE` from an authorised operator/controller (human actors only) → **Handoff**. Creates or reuses the next durable generation; ordinary ready relabels without this instruction remain non-executable after a completed generation |
