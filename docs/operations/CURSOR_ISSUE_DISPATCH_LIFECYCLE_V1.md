...
**Created:** 2026-07-28.
**Updated:** 2026-08-23 (#1032 live-activation prep; #1041 scheduled reconcile inherit `event_name`).
...
| **Temporal wrap (#1032)** | `lib/server/factory-temporal-phase1.js` may request the same Handoff after GitHub eligibility/WIP checks. Cursor Cloud must not `workflow_dispatch` Handoff itself (loop risk). Keep #1023 / #1041 until a live Temporal timer on `corpflow-exec-01` is proven. Live start is operator-paste L3 only: `docs/runbooks/TEMPORAL_FACTORY_PHASE1_LIVE_ACTIVATION.md` |
