...
| **Telegram** | Optional exception-only notification mirror (future) | **No — must not become source of truth** |
...
| **3** | Optional Telegram exception-only notification mirror | Link + first line only; not source of truth |
| **4** | Optional labels: `bridge:needs-anton`, `bridge:blocked`, `bridge:ready-to-merge` | Advisory labels in GitHub UI |

**Phase 0 does not** create workflows, secrets, chat bots, or runtime changes.
...
- Retired chat integrations must not be reintroduced into CorpFlow ops.
...
3. **Telegram:** optional exception-only notification mirror only (Phase 3+).
