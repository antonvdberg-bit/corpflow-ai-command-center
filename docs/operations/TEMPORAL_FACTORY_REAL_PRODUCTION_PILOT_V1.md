...
|----------|---------------|--------------------|
| Execution WIP | **1** verified current-generation Cursor run maximum (#1249). `CORPFLOW_TEMPORAL_PILOT=active` cannot add capacity. | Never requests Handoff while the single Cursor implementation lane is occupied. |
