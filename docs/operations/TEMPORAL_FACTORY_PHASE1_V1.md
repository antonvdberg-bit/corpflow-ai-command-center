...
**Status:** Repo packet + live-activation preparation. Live L3 worker not started.  
**Issue:** [#1032](https://github.com/antonvdberg-bit/corpflow-ai-command-center/issues/1032)  
**Owner:** Cursor (repo audit, implementation, synthetic proof, PR); Anton (L3 worker / credential mint).  
...
`NOT READY — HOST_MISMATCH — Cursor Cloud has no SSH to corpflow-exec-01; live Temporal worker is operator-paste L3 only`

Anton authorized live activation on 2026-08-22. This packet still proves the **control-plane design and synthetic loop** (A–J) in GitHub/CI and now records the exact L3 start path. It does **not** start Temporal on `corpflow-exec-01` from Cursor Cloud.

Exact remaining protected action (authorized, not performed by this worker):
...
Operator-paste: `docs/runbooks/TEMPORAL_FACTORY_PHASE1_LIVE_ACTIVATION.md`
...
| Temporal Phase 1 supervisor | **Introduce; live-activation prepared** | Live worker remains operator-paste L3 |
...
- Fail-closed: `ops/temporal/phase1-worker.mjs` dry-run only; `--live` exits `2` from Cursor Cloud / CI and must not `workflow_dispatch` Handoff (agent-loop risk).
- Anton authorized L3 activation on #1032. Smallest credential: a **box-local least-privilege GitHub token** that can `workflow_dispatch` Handoff only, stored at `~/.corpflow-temporal-handoff.token` on `corpflow-exec-01`. Do not add a Vercel / `.env.template` name. Do not reuse `CMP_GITHUB_TOKEN`, `GITHUB_TOKEN`, production Postgres, or Cursor webhook secrets.
...
Until that L3 proof exists, GitHub event-driven Handoff + #1023 / #1041 remain the production loop. This packet is the reversible wrapper, not a cutover.
