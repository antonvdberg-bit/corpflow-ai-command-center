...
This is the v1 of the *wiring contract* — future revisions can add additional channels (email via n8n, SMS) by extending the contract, not by re-shipping each workflow. Retired chat integrations must not be re-added as alert channels.
...
- **Scope:**
  ...
  - **Out:** Adding new monitors (those follow `MONITORING_ARCHITECTURE.md` § 9). Adding new alert channels (email, SMS) — future v2. Adding monitoring of `corpflow-exec-01` itself — separate future packet `exec01-cron-pulse`. Changing the bot, the chat id, or the API token rotation policy — operator-owned.
- **Constraints:**
