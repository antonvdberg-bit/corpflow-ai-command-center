...
**Cursor agent ID:** `bc-0674dfaa-97cb-44f0-87db-d18e8adb8d4c`  
**Cursor agent URL:** https://cursor.com/agents/bc-0674dfaa-97cb-44f0-87db-d18e8adb8d4c  
**Factory handoff run:** `32315503793`  
...
That verdict means every remaining WP7 control is classified. It does **not** mean ERPNext is irreplaceable, and it does **not** mean backups, restore, Neon PITR, or Monitor #14 are live-proven.

| # | Control | Mark |
|---|---------|------|
| 1 | Current deployed ERPNext / Frappe versions vs support + patch baseline | **PROVEN** |
| 2 | Whether an update is required now (not applied) | **PROVEN** — yes, update is required |
...
factory health HTTP 500 (Vercel HTML /500)
production-pulse HTTP 500 (Vercel HTML /500)
...
Public GitHub (no secrets):

| Fact | Evidence |
|------|----------|
| Latest ERPNext release | `v16.32.3` published 2026-08-18 ([frappe/erpnext releases](https://github.com/frappe/erpnext/releases/latest)) |
| Latest Frappe Framework release | `v16.31.0` published 2026-08-11 ([frappe/frappe releases](https://github.com/frappe/frappe/releases/latest)) |
| v16 support window | End of 2029 (planned) ([Frappe supported versions](https://frappe.io/support-versions)) |
| Critical SSTI | [GHSA-qq49-v74j-hjh7](https://github.com/frappe/erpnext/security/advisories/GHSA-qq49-v74j-hjh7) / CVE-2026-72911 — affected **erpnext < 16.29.0**, patched **16.29.0** |
| Later medium advisories still ahead of 16.26.2 | [GHSA-48wc-j2qh-rcgw](https://github.com/frappe/erpnext/security/advisories/GHSA-48wc-j2qh-rcgw) and [GHSA-m5xh-ghvj-xjrv](https://github.com/frappe/erpnext/security/advisories/GHSA-m5xh-ghvj-xjrv) patched **16.31.0** |
| Factory control loop (Monitor #1) | Last completed **success** 2026-08-19T06:38:33Z, run [32224241484](https://github.com/antonvdberg-bit/corpflow-ai-command-center/actions/runs/32224241484) |
| Frappe Cloud backup product docs | Daily backups; each backup is database + public files + private files + site config; offsite S3 on $25+/mo plans ([docs.frappe.io/cloud/sites/backups](https://docs.frappe.io/cloud/sites/backups)) |
| Neon Instant restore product docs | History window by plan (Free ≤6h, Launch ≤7d, Scale ≤30d); restore from root branch ([neon.com/docs/introduction/history-window](https://neon.com/docs/introduction/history-window)) |
...
|------|-------------------|------------------|--------------------------|
| ERPNext | **16.26.2** | v16 supported through end-2029 | Floor from #959 C5: **≥ 16.29.0**. Current release: **16.32.3**. Advisories still open vs 16.26.2 include **16.29.0** (critical SSTI) and **16.31.0** (later mediums). |
| Frappe Framework | **16.25.0** | Same v16 major line | Current release: **16.31.0**. |
| Extra app | `email_delivery_service` 0.0.1 | n/a | Unchanged vs #956. |
...
Evidence date: **2026-08-20** GET `frappe.utils.change_log.get_versions` HTTP 200.

### 2. Patch / update posture — **PROVEN** (update required; not applied)

An update **is required now**.

- Deployed **erpnext 16.26.2** is **below** the #959 C5 floor of **16.29.0**.
- CVE-2026-72911 (critical SSTI) is patched only from **16.29.0**.
- Two 2026-08-13 advisories are patched only from **16.31.0**.
- Current upstream **16.32.3** (2026-08-18) is the then-current patched 16.x.

This packet **did not** apply packages, trigger a Frappe Cloud upgrade, or mutate hosting.

Exact later action (not performed): Anton upgrades the vendor-hosted site via the Frappe Cloud dashboard to a patched v16 (**≥ 16.31.0**, preferably current **16.32.3**), then a later read-only probe records the new version strings.
...
This identity still cannot read backup jobs:
...
- `docs/operations/POSTGRES_PROVIDER.md` now records the **product** Instant restore model and that **this project’s** history window is still unread
...
| Monitor | Current evidence (2026-08-20) |
|---------|-------------------------------|
| #1 Factory control loop | GitHub Actions **success** 2026-08-19T06:38:33Z ([run 32224241484](https://github.com/antonvdberg-bit/corpflow-ai-command-center/actions/runs/32224241484)). Daily 06:00 UTC. Active. |
| #2 / #3 factory health + production-pulse | This worker GET **HTTP 500** HTML (`x-matched-path: /500`) at 2026-08-20T00:01Z. That is current runtime evidence of the monitored surfaces. Lux `/` and `/change` and apex `/` were **HTTP 200**. |
...
Factory health HTTP 500 is recorded here as WP7 evidence. Fixing that 500 is **out of this packet’s scope** (not an ERPNext backup/restore control).
...
1. Upgrade vendor ERPNext/Frappe to patched v16 (**≥ 16.31.0**, preferably **16.32.3**) in the Frappe Cloud dashboard.
2. Read the Frappe Cloud **Backups** tab (names only) as in control 3.
3. Restore a current vendor backup into a **disposable** site as in control 4.
4. Read Neon **Instant restore** history window as in control 5.
5. (Optional, ops not ERPNext) Enable Monitor #14 timer per `BACKUP_HEALTH_MONITOR.md` §7.
...
- **2026-08-20** — Initial WP7 closure for #1010. Verdict: **WP7 SECURITY/PATCH/RECOVERY CLOSURE READY FOR REVIEW**.
