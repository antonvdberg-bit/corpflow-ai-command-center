...
**Status:** Decision-ready operating model (docs / mapping / GET-only read-back). **No live bank or accounting truth.**  
**Source issue:** [#1139](https://github.com/antonvdberg-bit/corpflow-ai-command-center/issues/1139)  
...
This verdict means CorpFlowAI now has a **minimum standard-ERPNext bank/reconciliation operating model**, a **manual/import-first process**, **synthetic/offline proof** that matching is idempotent, and a **named accountant input list**. It does **not** mean books can be posted. It does **not** mean a live bank is connected.
...
| What must Anton do because of this packet? | **Merge this PR when satisfied.** No bank, payment, or accountant ceremony is created here. |
...
GET-only probe as `integrations@corpflowai.com` on 2026-08-27. Host URL is not recorded. `MASTER_ADMIN_KEY` unused.
