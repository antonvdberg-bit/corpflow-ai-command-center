...
> Optional second branch on the IF node: if your Gmail node fails, write to a small Telegram/Sheets node so a human can follow up. **Do not** include the reset code in that alert beyond the operator-only channel.
