...
- [ ] **Step 3 — restic backup and restore discipline (fully eligible 2026-06-16; not initiated by `JE-2026-06-16-3`).** Step 2 gate is now **fully** satisfied — Step 2 = COMPLETE end-to-end (all 8 sub-probes Up, Monitor # 13 active, Kuma data dir `~/uptime-kuma-data/` is the operationally-relevant volume that Step 3 would back up). Step 1 (n8n automation-forward end-to-end live verification) remains PARTIAL pending operator evidence at `artifacts/self-hosted-ops-stack-v1/2026-06-15-phase-1a-live-verification.md` § 7.1 / § 7.2 / § 7.3 — Step 3 author-time can choose to gate on Step 1 = COMPLETE (strict reading) or to author Step 3 with a "Step 1 still PARTIAL — restore drill includes n8n config dir; verification of that config dir's correctness is downstream of Step 1" caveat (pragmatic reading). Neither the install closure PR (`JE-2026-06-16-2`) nor the sub-probe 8 add docs-sync PR (`JE-2026-06-16-3`) initiates Step 3 — Anton explicitly directed *"Do not proceed to restic"* until separate authorization. Step 3 is **not** authored in this baseline; the gate and constraints are in `docs/operations/SELF_HOSTED_OPS_STACK_V1.md` § 4. Restore drills must target a **disposable directory or disposable server path** — never production volumes, never the production Postgres.
