...
- **Technical Lead Phase A (observer):** daily cron + Postgres `technical_lead_audits` (see `docs/VERCEL_DEPLOYMENT.md`, `npm run technical-lead:run`). **Align:** `POSTGRES_URL` GitHub secret if you want CI `prisma migrate deploy`; cron Bearer = `CORPFLOW_CRON_SECRET` / `CRON_SECRET`; `VERCEL_*` for preview evidence.
