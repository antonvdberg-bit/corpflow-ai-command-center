...
- `/app/delivery` is the staff-only Delivery summary (#1005). `GET /api/app/delivery` is Core only. Tenant sessions receive 403. It projects existing Lead Rescue, Website Rescue, and Change/request records. `/change` remains the tenant service/change surface.
