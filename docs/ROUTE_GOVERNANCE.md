...
- `/app/queue` is the staff-only canonical Prospect Action Queue (#995). It reuses the shared prospect list, `#721` `matchesActionQueueFilter`, and opens `#994` shared detail. `GET /api/app/queue` is Core only. Tenant sessions receive 403. `/admin/rapid-delivery` remains a temporary product desk.
