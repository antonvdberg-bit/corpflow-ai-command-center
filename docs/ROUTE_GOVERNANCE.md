...
- `/app/clients` is the staff-only Clients summary (#999). `GET /api/app/clients` is Core only. Tenant sessions receive 403. Company Master remains the evidence/asset editor.
...
- `/app/delivery` is the staff-only Delivery summary (#1005 / #1119 / #1170). `GET /api/app/delivery` is Core only. Tenant sessions receive 403. It projects existing Lead Rescue, Website Rescue, and Change/request records, plus an existing ERPNext Project/Issue reference when the `corpflow.delivery.erpnext.v1` pointer already exists. `/change` remains the tenant service/change surface. Tenant Workspace does not show ERPNext project oversight.
- Tenant Workspace (#1073 / #1006 / #1120): nav is **Requests & Progress** plus **Service & change** (`/change?from=tenant-workspace`). Tenant chrome does not show **Choose workspace**, a staff chooser, a proof-harness hint, or an internal data-source label. `/change` remains the canonical tenant service/change surface. Placeholder Home / My Work / Documents / Reports / Support nav is retired. Navigation does not create a ticket. `/app/commercial` and `/app/delivery` are staff-only (not tenant routes).
