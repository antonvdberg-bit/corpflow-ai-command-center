...
- `/app/delivery` is the staff-only Delivery summary (#1005 / #1119 / #1178). `GET /api/app/delivery` is Core only. Tenant sessions receive 403. It projects existing Lead Rescue, Website Rescue, and Change/request records. Row evidence opens Client, Prospect, Commercial, and `/change` where those paths are already recorded. Proof-harness hops keep `proof=1` on `/app/*` routes only. `/change` remains the tenant service/change surface.
