...
| Options considered | Merge stale #1141; redesign banking; land the proven helper/docs/evidence onto current `main` and close #1141 |
| Evidence reviewed | #1139 GET-only hosted-test as `integrations@corpflowai.com`; #1055 template CoA / 0 Bank Account rows; #551/#714 rail; #1166 invoice-does-not-approve gate; Phase C synthetic recon arithmetic; accountant pack §8 |
| Decision | Land [`ERPNEXT_BANK_RECONCILIATION_READINESS_V1.md`](../../erpnext/ERPNEXT_BANK_RECONCILIATION_READINESS_V1.md) on current `main`. Manual/import-first. Bank-feed **NOT REQUIRED**. Payment evidence never authorizes Payment Entry. Invoice existence never grants Proceed Approved. Synthetic offline proof only. Close #1141 without merge. |
...
| Supersedes / superseded-by | Current-main replacement for stale PR #1141 / #1139. Does **not** supersede Version 2, #918 `payment_evidence` blocked bridge, #1055, or #1166. |
