...
| 6 | `Phase 5 — Business process configuration` | 5 | Open | #920 Lead → Opportunity → Customer → draft Quotation → Project → Task → Timesheet → Issue. #1097 operational proof on the same Project/Issue (owner, Task next-action, Issue close/reopen). Timesheet **DEFER**; Workflow / Notification / SLA HTTP 403; no external send. Prestige pack #919 not sent. #1139/#1220 maps bank/reconciliation (manual/import-first) without posting. | **PARTIAL** |
...
| 8 | `Phase 7 — Data migration and cutover` | 7 | Open / preparation only | #1245 opening/cutover pack: [`../../erpnext/ERPNEXT_OPENING_BALANCE_CUTOVER_V1.md`](../../erpnext/ERPNEXT_OPENING_BALANCE_CUTOVER_V1.md). Template + Method A vs B unchosen. No posting. Accountant still chooses date, method, totals, funding class. | **REQUIRES DECISION** |
...
| [#1056](https://github.com/antonvdberg-bit/corpflow-ai-command-center/issues/1056) / [#1166](https://github.com/antonvdberg-bit/corpflow-ai-command-center/issues/1166) | CF1018 draft MUR Quotation `SAL-QTN-2026-00005` + #714 Proceed Approved fail-closed mapping. #1166 lands the proven slice onto current `main` after #1162 (no second design; retire stale PR #1128). | Post Sales Invoice; Payment Entry; tax/CoA mutation |
...
| [#1097](https://github.com/antonvdberg-bit/corpflow-ai-command-center/issues/1097) / [#1134](https://github.com/antonvdberg-bit/corpflow-ai-command-center/issues/1134) | Onboarding E operational proof on reused `PROJ-0001` / `ISS-2026-00001` (owner, next-action, Issue close/reopen). #1134 lands that proof on current `main`; do not merge stale PR #1102. | A second project/helpdesk app; Timesheet submit; SLA/Assignment Rule config; a second live apply |
...
| [#1245](https://github.com/antonvdberg-bit/corpflow-ai-command-center/issues/1245) | Opening-balance / cutover preparation: placeholder template, Method A vs B unchosen, GET-only inspect (0 JE / 0 GL / Accounting Onboarding 0/6). Canonical: `docs/erpnext/ERPNEXT_OPENING_BALANCE_CUTOVER_V1.md`. | Post Opening Entry; Opening Invoice; Data Import; CoA mutation; choose the accountant method |
...
| [#1206](https://github.com/antonvdberg-bit/corpflow-ai-command-center/issues/1206) | Customer/Contact/Address GET-only quotation/delivery acceptance on reused CF1018 (`Lee Synthetic`, Billing Address, `SAL-QTN-2026-00005` party), re-landed on current `main` after closed PR #1211. Search-before-create duplicate count 1. Commercial Workspace projects the recorded Customer pointer. Preserves #1162 / #1168 / #1228 / #1230. | ERPNext write; Company Master name-join; live Postgres lead PATCH; real Prestige Customer; revive closed PR #1211 |
