...
| R-ERP-06 | Source-of-truth divergence (CorpFlowAI vs ERPNext) | Anton / Cursor | Reconcile, do not duplicate; bridge contract mapping-only | Currently low volume; monitor | `docs/erpnext/ERPNEXT_CORPFLOW_BRIDGE_CONTRACT_V1.md`; #701 CRM baseline | **PARTIAL** | No second CRM; no Postgres migration in this packet |
...
| R-ERP-10 | `#899` `MASTER_ADMIN_KEY` still injected into ordinary Cursor Cloud runs | Anton (UI-only removal) | Must not be used as ERPNext auth; ordinary packets fail closed on ERPNext secrets | Not accepted as a finished control | 2026-08-19 Factory Automation wake `bc-c67a9751-28cb-47e6-918a-29a13c213561`: **PRESENT**; `docs/erpnext/ERPNEXT_CURSOR_CLOUD_SECURITY_CORRECTION_899.md`; `JE-2026-08-19-1` | **NOT PROVEN** | Anton deletes the secret **name** from the remaining Cursor store that still injects Factory Automation workers, then a fresh wake must show **ABSENT** |
