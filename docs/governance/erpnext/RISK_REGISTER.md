...
| R-ERP-10 | `#899` `MASTER_ADMIN_KEY` still injected into ordinary Cursor Cloud runs | Anton (UI-only removal) | Must not be used as ERPNext auth; ordinary packets fail closed on ERPNext secrets | Not accepted as a finished control | #956 S-row; probe scripts | **NOT PROVEN** | Anton removes the secret from ordinary Cloud runs |
