...
| R-ERP-13 | Pre-revenue costs or the loan/current-account that funded them are dropped or re-booked as fresh post-cutover opex | Accountant + Anton | #1245 template keeps the pair; Method A vs B unchosen; no posting until classification | Not accepted as residual | `docs/erpnext/ERPNEXT_OPENING_BALANCE_CUTOVER_V1.md`; `SYNTHETIC_UNIT_PRE_REVENUE` | **REQUIRES DECISION** | Accountant classifies funding + cutover method before any Opening Entry |
