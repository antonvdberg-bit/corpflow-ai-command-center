...
| 5 | Business process configuration | **PARTIAL** | Lead → Opportunity → Customer, draft Quotation/Invoice, Project/Task/Timesheet, and Issue are proven on hosted test. Nothing is submitted or sent. Buying/AP and bank posting are not started. #1139/#1220 maps the manual/import-first recon path only. |
...
| P5-2 | Quotation → acceptance → invoice → payment evidence | **PARTIAL** | `#882` drafts; sandbox Phase C paid SI (2026-06, different site); `#1166` Proceed Approved fail-closed | Hosted-test drafts proven; hosted-test payment **not** proven | Submit/payment only after Anton + bank clearance | P0 | Keep drafts; Prestige uses Quotation first | Anton | YES — submit/payment |
| P5-2a | Payment evidence → later Payment Entry → bank import → recon | **PARTIAL** | `#1139` / `#1220` mapping + synthetic recon delta 0.00 | Operating model proven; hosted-test Bank Account rows = 0 | Accountant bank/cash ledgers (#1055) | P1 | Keep GET-only; do not post | Accountant then Anton | YES — Bank Account / PE submit / feed |
