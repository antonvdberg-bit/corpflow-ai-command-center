...
| P5-3 | Project → Task → Timesheet → billing | **PARTIAL** | `#920` P/T/TS; `#1097` operational proof (owner/next-action); `#1202` GET-only **OPERATIONALLY USABLE**. Timesheet **DEFER** | Structure and daily ops proven | Billing link not proven; CoA still packet A | P1 | Leave Timesheets draft | Cursor | YES to bill |
| P5-4 | Issue / support lifecycle | **DONE** (synthetic) | `#920` `ISS-2026-00001`; `#1097` close/reopen + description trail; `#1202` GET-only acceptance | Durable Issue lifecycle proven synthetic | Help Desk portal still off; SLA deferred | P2 | Do not replace `/change` | n/a | NO |
...
| P8-3 | CRM / project / support validation | **PARTIAL** | `#920` synthetic; `#1202` GET-only Project/Issue **OPERATIONALLY USABLE** | Synthetic ops accepted | Real engagement rehearsal | P1 | After Prestige accept | Cursor | NO for synthetic |
