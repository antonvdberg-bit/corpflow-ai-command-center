...
| 4 | Master data | **PARTIAL** | Synthetic Customer/Contact/Address, Items/prices, and a website Project Template are proven. #1098 / #1213 synthetic purchase Item `CF-AP-SYNTHETIC-OPEX` is proven. Supplier CREATE remains permission-blocked. Real-client masters are not. |
| 5 | Business process configuration | **PARTIAL** | Lead → Opportunity → Customer, draft Quotation/Invoice, Project/Task/Timesheet, and Issue are proven on hosted test. Nothing is submitted or sent. #1098 / #1213 mapped Buying/AP (PO **DEFER**); PI submit and workflows are not started. |
...
| 9 | Buying / AP / supplier process not started | Phase 1 order item 4 | **PARTIAL** — #1098 / #1213 mapped standard path; synthetic Item proven; Supplier CREATE 403; PI submit waits on #1055 | Continue accountant pack; Anton Role grant for Supplier create; AI still cannot approve suppliers | Cursor / Anton / accountant | **NO** for design; **YES** to create real suppliers or submit PI |
...
5. **Buying / AP design notes** — #1098 / #1213 landed the operating path on current `main`. No real supplier create; no PI submit.  
...
| 4 | Buying / AP | **PARTIAL** — #1098 / #1213 readiness packet; PO **DEFER** | After quotation fast lane; AI cannot approve suppliers; PI submit waits on #1055. |
...
| P2-8 | Default AR / AP accounts | **PARTIAL** | `Debtors USD - CFAI` used on `#882` USD draft; `#1098` / `#1213` read-back `Creditors - CFAI` / `Cost of Goods Sold - CFAI` | AR partial; AP skeleton unused for posting | Accountant must confirm payable + opex accounts (#1055) | P1 | Do not submit PI on skeleton defaults | Accountant | YES to change GL defaults |
...
| P4-2 | Suppliers | **PARTIAL** | #1098: LIST HTTP 200 empty; CREATE HTTP 403. #1213 GET: still count=0 | Synthetic name planned; not created | Anton Role Permissions Manager grant for Supplier create | P1 | Grant then create CF1098 synthetic only | Anton / Cursor | YES to approve/create real supplier |
...
| P5-3 | Project → Task → Timesheet → billing | **PARTIAL** | `#920` P/T/TS; `#1097` operational proof (owner/next-action). Timesheet **DEFER** | Structure and daily ops proven | Billing link not proven; CoA still packet A | P1 | Leave Timesheets draft | Cursor | YES to bill |
| P5-4 | Issue / support lifecycle | **DONE** (synthetic) | `#920` `ISS-2026-00001`; `#1097` close/reopen + description trail | Durable Issue lifecycle proven synthetic | Help Desk portal still off; SLA deferred | P2 | Do not replace `/change` | n/a | NO |
| P5-5 | Purchasing / payables | **PARTIAL** | #1098 / #1213 path mapped; PO **DEFER**; `po_required=No` | Draft mapping proven; no PI created | Accountant defaults then Draft PI; never pay from invoice existence | P1 | #1055 then capture-only Draft | Cursor / accountant | YES for real PO/bill/payment |
