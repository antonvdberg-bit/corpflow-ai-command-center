...
| Pro-forma / invoice | ERPNext Quotation/Sales Invoice when available (`docs/erpnext/ERPNEXT_COMMERCIAL_DOCUMENTS_V1.md`, #882 READY on hosted test). Names may fill `proposal_version`; they **never** set `financially_approved`. Else manual PDF per `docs/finance/AI_LEAD_RESCUE_MANUAL_PRO_FORMA_TEMPLATE_V1.md` | Payment evidence `invoice_ref` / pro-forma handoff section |
