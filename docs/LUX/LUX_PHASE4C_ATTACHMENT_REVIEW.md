...
- Public **GET** ` /api/lux/property-media?property=<slug>&attachment=<id>&slot=<slot>[&variant=…]` on the Lux hostname only: serves bytes **only** when the link is **published**, the attachment is **reviewed**, MIME is **`image/*`**, and `(property, slot)` matches the link. Wrong property/slot/unpublished/non-image → **404**; invalid slot → **400**; invalid **`variant`** (when provided) → **400**. **200** uses `Cache-Control: public, max-age=300, must-revalidate` (**no** `stale-while-revalidate`); denials use `private, no-store`; **`Content-Type`** normalized to a concrete `image/*` subtype; **no** raw storage path in the response.
- Public **GET** `/api/lux/property-media-list?property=<slug>` (Lux host, `luxe-maurice`): JSON list of **published** **hero + card + gallery** image entries with **safe** fields only (`slot`, `src`, `variant`, `content_type`, captions/alt, sort hints). No operator audit fields, no raw paths.
