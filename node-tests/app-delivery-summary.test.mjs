...
test('projector: Lead Rescue and Website Rescue become delivery items; general prospects do not', () => {
  ...
  assert.equal(lr?.links.clients, CLIENTS_SUMMARY_PATH);
  assert.equal(lr?.links.commercial, COMMERCIAL_SUMMARY_PATH);
...
test('payload: does not claim send/deploy/schema and keeps related surfaces', () => {
  ...
  assert.equal(payload.related_surfaces.commercial, COMMERCIAL_SUMMARY_PATH);
