...
test('handler: Tenant shell omits My Work and does not advertise the staff chooser', async () => {
  const prevNode = process.env.NODE_ENV;
  process.env.NODE_ENV = 'development';
  try {
    const res = mockRes();
    ...
    assert.ok(res.state.body.menus.some((m) => m.id === 'requests_progress'));
    assert.ok(res.state.body.menus.some((m) => m.id === 'service_change' && m.href === '/change'));
  } finally {
    process.env.NODE_ENV = prevNode;
  }
});
