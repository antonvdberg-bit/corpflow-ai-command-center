...
test('isHostAllowed allows exactly the umbrella sites and rejects everything else', () => {
  assert.equal(isHostAllowed('corpflowai.com'), true);
  assert.equal(isHostAllowed('aileadrescue.corpflowai.com'), true);
  assert.equal(isHostAllowed('AILeadRescue.CorpFlowAI.com'), true);
...
test('isPathAllowed denies factory/admin/login/master/api/_next/client under any allowed host', () => {
  for (const host of ['corpflowai.com', 'aileadrescue.corpflowai.com']) {
    for (const path of [
      '/change',
      '/change/queue',
      '/change-v2',
      ...
      assert.equal(
        isPathAllowed(host, path),
        false,
        `expected ${host}${path} to be denied`,
      );
    }
  }
});
...
test('apex-specific deny: /lead-rescue, /concierge, /properties, /property — denied only on apex', () => {
  for (const path of ['/lead-rescue', '/lead-rescue/v2', '/concierge', '/properties', '/properties/admin', '/property/abc']) {
    assert.equal(
      isPathAllowed('corpflowai.com', path),
      false,
      `expected apex ${path} to be denied`,
    );
  }
  for (const path of ['/lead-rescue', '/concierge', '/property/abc']) {
    assert.equal(
      isPathAllowed('aileadrescue.corpflowai.com', path),
      true,
      `expected ${path} on Lead Rescue host to NOT be apex-denied (allow-list logic still gates host separately)`,
    );
  }
});

test('apex-specific deny does not over-match: /lead-rescue-news (no slash) is allowed', () => {
  assert.equal(isPathAllowed('corpflowai.com', '/lead-rescue-news'), true);
  assert.equal(isPathAllowed('corpflowai.com', '/properties-overview'), true);
});

test('apex root and explicit allowed pages pass', () => {
  for (const path of ['/', '/about', '/standards', '/standards/seo']) {
...
test('isAnalyticsEnabledForHostPath = host AND path AND not denied', () => {
  ...
  assert.equal(isAnalyticsEnabledForHostPath('aileadrescue.corpflowai.com', '/'), true);
  ...
  assert.equal(isAnalyticsEnabledForHostPath('corpflowai.com', '/lead-rescue'), false);
...
test('getMarketingSurface returns the right label per host or null', () => {
  ...
  assert.equal(getMarketingSurface('aileadrescue.corpflowai.com'), 'lead_rescue');
