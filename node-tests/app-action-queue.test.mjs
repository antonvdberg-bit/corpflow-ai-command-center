...
test('handler: Action Queue default filter prioritises overdue before scheduled waiting', async () => {
  ...
  try {
    ...
    assert.equal(ids.includes('syn-772-lr-ada'), true);
...
test('handler: Action Queue named filters isolate overdue and awaiting prospect', async () => {
  const prevNode = process.env.NODE_ENV;
  const prevVercel = process.env.VERCEL_ENV;
  process.env.NODE_ENV = 'development';
  delete process.env.VERCEL_ENV;
  ...
  try {
    ...
    assert.ok(neu.includes('syn-772-rd-bea') || neu.includes('syn-996-gen-dee'));
  } finally {
    process.env.NODE_ENV = prevNode;
    if (prevVercel == null) delete process.env.VERCEL_ENV;
    else process.env.VERCEL_ENV = prevVercel;
    ...
    await handleAppProspectDetail(
      {
        ...
        body: {
          ...
          urgency: 'high',
          note_append: 'Updated from #1040 queue',
        },
      },
      save,
    );
