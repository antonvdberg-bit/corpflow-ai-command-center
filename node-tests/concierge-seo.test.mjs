...
test('buildConciergeSeo: Lux host returns Lux-branded SSR metadata', () => {
  ...
  assert.equal(seo.title, 'Private advisory · Rare & Exclusive Collection');
  ...
  assert.equal(seo.ogSiteName, 'Rare & Exclusive Collection');
...
test('buildConciergeSeo: propertyTitle is woven into title + description + ogTitle', () => {
  ...
  assert.equal(
    seo.title,
    'Private advisory · Bel Ombre Estate Villa 14 · Rare & Exclusive Collection',
  );
