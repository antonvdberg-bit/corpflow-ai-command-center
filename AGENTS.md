...
| **Codex Cloud install — operator playbook (Packet 7.2: completed activation path; ChatGPT Plus sign-in preferred; GitHub App least-privilege; bot username; first-packet smoke; rollback)** | **`docs/runbooks/CODEX_CLOUD_INSTALL.md`** |
...
CorpFlow operates a bounded autonomy model: Anton approves work **packets** once, and the assigned executor — Cursor as L1 primary executor, Codex Cloud as L2 bounded second executor, or a contractor under the same rules — may execute approved work up to the next gate without further line-by-line approval. Treat the three docs above as canonical for any "should I do this without asking?" decision.
