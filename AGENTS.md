...
### ERP strategy v2 (`APPROVED — VERSION 2`; consult, do not duplicate)

Small repo decisions that touch ERPNext, CRM/business records, finance, Projects, Support, proposals/quotations, automation authority, or source-of-truth boundaries must consult **`docs/governance/erpnext/VISION_AND_INTENDED_USE.md`** before implementation. Status is **`APPROVED — VERSION 2`**. Non-negotiables (zero default AI spend, Anton supplier approval, Anton approval before any external quotation, reconcile-don’t-duplicate, Prestige fast lane must not be blocked) are summarized in `.cursor/rules/erpnext-strategy.mdc`. Do not paste the full doctrine into other context files. Source lineage: issues #960 / #955 / #954 / #953.
...
| **ERP Strategy v2 (`APPROVED — VERSION 2`)** — consult before ERPNext, CRM/business records, finance, Projects, Support, quotations, automation authority, or source-of-truth work. Do not duplicate the doctrine. | **`docs/governance/erpnext/VISION_AND_INTENDED_USE.md`**, `.cursor/rules/erpnext-strategy.mdc` |
