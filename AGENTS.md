...
### ERP strategy v2 (draft — consult, do not duplicate)

Small repo decisions that touch ERPNext, CRM/business records, finance, Projects, Support, proposals/quotations, automation authority, or source-of-truth boundaries must consult **`docs/governance/erpnext/VISION_AND_INTENDED_USE.md`** before implementation. Status is **`DRAFT FOR ANTON APPROVAL`** until Anton explicitly approves the synthesized version. Non-negotiables (zero default AI spend, Anton supplier approval, Anton approval before any external quotation, reconcile-don’t-duplicate, Prestige fast lane must not be blocked) are summarized in `.cursor/rules/erpnext-strategy.mdc`. Do not paste the full doctrine into other context files. Source lineage: issues #955 / #954 / #953.
...
| Delivery reality (live URL = done) | `.cursor/rules/delivery-reality.mdc`, `.cursor/rules/predeploy-decision-checks.mdc` |
| **Environment classification (`corpflow_test` vs `client_production`) — CorpFlowAI-hosted tenant/factory surfaces are test; client_production is separately governed (#679)** | **`docs/operations/CORPFLOW_ENVIRONMENT_CLASSIFICATION_V1.md`**, `docs/decisions/20260729-corpflow-test-vs-client-production.md` |
...
| **ERP Strategy v2 (draft — Anton approval pending)** — consult before ERPNext, CRM/business records, finance, Projects, Support, quotations, automation authority, or source-of-truth work. Do not duplicate the doctrine. | **`docs/governance/erpnext/VISION_AND_INTENDED_USE.md`**, `.cursor/rules/erpnext-strategy.mdc` |
