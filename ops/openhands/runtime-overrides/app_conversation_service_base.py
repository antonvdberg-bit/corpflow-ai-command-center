"""CORPFLOWAI COMMISSIONING / SKILLS-REDUCTION OVERRIDE
====================================================
Bounded patch of upstream OpenHands 1.8
`openhands/app_server/app_conversation/app_conversation_service_base.py`
(tag 1.8.0).

Upstream hardcodes load_public=True (and load_user/project/org=True) when
fetching skills from agent-server. Public skills inflate first-request
context via <available_skills> / always-on legacy skills.

Env gates (default = minimal commissioning profile):
  CORPFLOWAI_LOAD_PUBLIC_SKILLS=0|1   (default 0)
  CORPFLOWAI_LOAD_USER_SKILLS=0|1     (default 0)
  CORPFLOWAI_LOAD_PROJECT_SKILLS=0|1  (default 0)
  CORPFLOWAI_LOAD_ORG_SKILLS=0|1      (default 0)
...
class AppConversationServiceBase(AppConversationService, ABC):
    ...
    async def load_and_merge_all_skills(
        ...
        try:
            ...
            # CORPFLOWAI: skill sources gated (default all OFF for free-tier TPM)
            all_skills = await load_skills_from_agent_server(
                ...
                load_public=_corpflowai_env_flag('CORPFLOWAI_LOAD_PUBLIC_SKILLS', '0'),
                load_user=_corpflowai_env_flag('CORPFLOWAI_LOAD_USER_SKILLS', '0'),
                load_project=_corpflowai_env_flag('CORPFLOWAI_LOAD_PROJECT_SKILLS', '0'),
                load_org=_corpflowai_env_flag('CORPFLOWAI_LOAD_ORG_SKILLS', '0'),
            )
