"""CORPFLOWAI COMMISSIONING / CONTEXT-REDUCTION OVERRIDE
=====================================================
Bounded patch of upstream OpenHands 1.8
`openhands/app_server/app_conversation/live_status_app_conversation_service.py`
(tag 1.8.0 / commit bc26df3).

Why a patch (same Option D pattern as docker_sandbox_service.py):
OpenHands 1.8 hardcodes on the conversation start path:
  - get_default_tools(enable_browser=True)  → ~14 browser tool schemas
  - register_builtins_agents(enable_browser=True)
  - _add_system_mcp_servers() always injects {web_url}/mcp/mcp
    (create_pr / create_mr / bitbucket / azure_devops tools)

Those schemas dominate the first Groq completion (~33k–47k tokens observed
on free-tier gpt-oss-20b with 8k TPM). No supported Settings API field turns
browser or default MCP off on this path.

Env gates (default = minimal commissioning profile):
  CORPFLOWAI_ENABLE_BROWSER=0|1          (default 0)
...
def _corpflowai_enable_browser() -> bool:
    return _corpflowai_env_flag('CORPFLOWAI_ENABLE_BROWSER', '0')


def _corpflowai_inject_default_mcp() -> bool:
    return _corpflowai_env_flag('CORPFLOWAI_INJECT_DEFAULT_MCP', '0')


def _corpflowai_enable_builtin_agents() -> bool:
    return _corpflowai_env_flag('CORPFLOWAI_ENABLE_BUILTIN_AGENTS', '0')
...
class LiveStatusAppConversationService(AppConversationServiceBase):
    ...
    async def _add_system_mcp_servers(
        ...
        """
        # CORPFLOWAI: default MCP tool schemas (create_pr/mr/...) are a major
        # first-request token cost. Skip injection unless explicitly enabled.
        # Conversation setup does NOT require these tools when mcpServers is empty.
        if not _corpflowai_inject_default_mcp():
            _logger.info(
                'CORPFLOWAI: skipping default MCP server injection '
    ...
    async def _build_start_conversation_request_for_user(
        ...
        if agent_type == AgentType.PLAN:
            ...
            # CORPFLOWAI: browser/builtin agents gated (default OFF for free-tier TPM)
            _cf_browser = _corpflowai_enable_browser()
            if _corpflowai_enable_builtin_agents():
                register_builtins_agents(enable_browser=_cf_browser)
            if _corpflowai_minimal_tools_only():
                # Arithmetic commissioning: terminal + file_editor only.
                # Avoid BrowserToolSet + TaskTrackerTool schema cost.
                ...
                _logger.info(
                    'CORPFLOWAI: minimal tools only '
                    '(terminal + file_editor; CORPFLOWAI_MINIMAL_TOOLS=1)'
                )
            else:
                tools = get_default_tools(
                    enable_browser=_cf_browser,
