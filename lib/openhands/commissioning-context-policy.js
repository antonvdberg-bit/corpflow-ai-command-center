...
/**
 * Absolute stop for combined_requested_tokens
 * (input_est + reserved_output) before calling Groq.
 */
export const COMMISSIONING_WIRE_HARD_STOP = 7000;
/** Preferred soft target for combined requested tokens. */
export const COMMISSIONING_WIRE_SOFT_TARGET = 5000;
/** Commissioning-only OpenHands LLM.max_output_tokens (maps to LiteLLM max_tokens). */
export const COMMISSIONING_MAX_OUTPUT_TOKENS = 1024;
/**
 * LiteLLM get_model_info('groq/openai/gpt-oss-20b').max_output_tokens when
 * OpenHands leaves max_output_tokens unset — counted into Groq TPM with input.
 */
export const LITELLM_GPT_OSS_20B_DEFAULT_MAX_OUTPUT = 32768;
...
export const GROQ_MINIMAL_TOOLS_OBSERVED_TOKENS = Object.freeze({
  ...
  note:
    'OpenAI-format input ~2749 + LiteLLM default max_output_tokens 32768 ≈ Groq TPM; cap to 1024',
});
...
export function auditLiveStatusCommissioningOverride(liveStatusSource) {
  ...
  if (!text.includes("llm_updates['max_output_tokens']") && !text.includes('max_output_tokens')) {
    findings.push('live_status must set commissioning max_output_tokens completion cap');
  }
  if (!text.includes("_corpflowai_completion_cap_tokens") || !text.includes('CORPFLOWAI_MAX_OUTPUT_TOKENS')) {
    findings.push('live_status must expose CORPFLOWAI_MAX_OUTPUT_TOKENS completion cap helper');
  }
  if (!/llm_updates\[['"]reasoning_effort['"]\]\s*=\s*['"]low['"]/.test(text)) {
    findings.push('commissioning profile must set reasoning_effort=low');
  }
  // Must not set max_output_tokens outside the short-prompt / commissioning gate
  if (
    /max_output_tokens/.test(text) &&
    !/_corpflowai_short_system_prompt\(\)/.test(text)
  ) {
    findings.push('completion cap must be gated by short/commissioning profile');
  }
...
export function estimateMinimalFirstRequestTokens() {
  ...
  // + user arithmetic (~100) + framing + capped completion (1024).
  const tokens_low = 1500;
  const tokens_high = 4000;
  const budget = COMMISSIONING_GROQ_TPM_LIMIT * COMMISSIONING_TPM_MARGIN;
  const combined_high =
    tokens_high + COMMISSIONING_MAX_OUTPUT_TOKENS; /* input + reserved output */
  return {
    tokens_low,
    tokens_high,
    under_70pct_of_8k: tokens_low <= budget,
    under_hard_stop: combined_high < COMMISSIONING_WIRE_HARD_STOP,
    combined_with_cap_high: combined_high,
    note:
      'Short system prompt + minimal tools + FinishTool + max_output_tokens=1024. ' +
      'Prior Groq TPM (~33k) matched input(~2.7k)+LiteLLM default max_tokens=32768. ' +
      'Run wire-capture-dry.sh and require combined_requested_tokens_est < 7000.',
  };
}
