...
/**
 * Sandbox resource targets — HostConfig limits enforced by the CorpFlowAI
 * bind-mounted spawn override (`ops/openhands/runtime-overrides/docker_sandbox_service.py`).
 * Upstream OpenHands 1.8 does NOT set these natively on the OSS Docker path;
 * the override injects mem_limit / nano_cpus / pids_limit on every containers.run.
 * Aggregate ceiling remains the systemd slice MemoryMax=4G.
 */
export const SANDBOX_MEM_LIMIT = '512m';
export const SANDBOX_NANO_CPUS = 500_000_000; // 0.5 CPU
export const SANDBOX_PIDS_LIMIT = 256;
/** @deprecated Prefer SANDBOX_MEM_LIMIT — kept for older doc cross-checks. */
export const SANDBOX_MEM_GUIDANCE_GIB = 0.5;
export const SANDBOX_MEM_HARD_MAX_GIB = 1;
export const SANDBOX_CPU_GUIDANCE = 0.5;
...
/**
 * Dedicated-daemon-safe sandbox↔control-plane callback base (no host-gateway).
 * Enforced by the runtime override's WEBHOOK_CALLBACK env, not by unread
 * upstream SANDBOX_LOCAL_RUNTIME_URL / SANDBOX_ADDITIONAL_NETWORKS knobs.
 */
export const SANDBOX_WEBHOOK_BASE = 'http://corpflowai-openhands-app:3000';
export const SANDBOX_ADDITIONAL_NETWORK = 'corpflowai-openhands-net';
/** @deprecated Alias — prefer SANDBOX_WEBHOOK_BASE. */
export const SANDBOX_LOCAL_RUNTIME_URL = SANDBOX_WEBHOOK_BASE;

/** Bind-mounted spawn override path (compose relative). */
export const SANDBOX_SPAWN_OVERRIDE_REL =
  './runtime-overrides/docker_sandbox_service.py';
export const SANDBOX_SPAWN_OVERRIDE_IN_CONTAINER =
