...
import {
  LIVING_WORD_LOGO_PATH,
  LIVING_WORD_THEME,
} from './living-word-site-theme.js';

export const DEMO_LABEL = '[LIVING WORD — TEST DEMO]';
export const LOGO_PATH = LIVING_WORD_LOGO_PATH;
export const COLOURS = { ...LIVING_WORD_THEME, muted: LIVING_WORD_THEME.textMuted };
    ...
    >
      ...
      <main style={{ maxWidth: 640, margin: '0 auto', padding: '0 20px' }}>
        <header style={{ textAlign: 'center', marginBottom: 32 }}>
          ...
          <h1 style={{ margin: '0 0 8px', fontSize: 26, color: COLOURS.heading }}>{title}</h1>
        </header>
        {children}
      </main>
    </div>
...
export function labelStyle() {
  return { display: 'block', fontWeight: 600, color: COLOURS.heading, fontSize: 14 };
}
...
export function buttonStyle(disabled = false) {
  return {
    ...
    padding: '12px 24px',
    background: disabled ? '#999' : COLOURS.primary,
    color: '#fff',
    border: 'none',
    borderRadius: 6,
    fontSize: 16,
