...
async function handleClientDecisionsGet(req, res) {
  ...
  try {
    ...
    if (!hasCd) {
      ...
      const cipcDeskStatus = getCipcDeskClientStatusFromConsoleJson(seeded);
      return res.status(200).json({
        ok: true,
        ticket_id: row.id,
        ...
        ...(cipcDeskStatus ? { cipc_desk_status: cipcDeskStatus } : null),
        source: 'postgres',
      });
    }
    ...
    const items = Array.isArray(cd.items) ? cd.items : [];
    const gate = evaluateClientDecisionsGateForTicket(row.id, items.map((x) => (x && typeof x === 'object' ? x : {})));
    const cipcDeskStatus = getCipcDeskClientStatusFromConsoleJson(stored);
