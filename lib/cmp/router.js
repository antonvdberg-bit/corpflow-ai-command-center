...
function getCipcDeskClientReplyDraftFromConsoleJson(consoleJson) {
  ...
  try {
    ...
    if (req.corpflowClientDecisionsMagicOk === true && (consumedIso || gate.sufficient_to_proceed)) {
      return res.status(200).json({
        ...
        thank_you_message:
          getCipcDeskClientReplyDraftFromConsoleJson(row.consoleJson) || getClientDecisionThankYouMessage(row.id),
        heading: spec.heading,
        explanation: spec.explanation,
        client_decisions: { items: [], sufficient_to_proceed: gate.sufficient_to_proceed },
  ...
  try {
    ...
    if (req.corpflowClientDecisionsMagicOk === true && req.corpflowClientDecisionsMagicConsumed === true) {
      return res.status(200).json({
        ...
        ticket_id: ticketId,
        thank_you_message:
          getCipcDeskClientReplyDraftFromConsoleJson(row.consoleJson) || getClientDecisionThankYouMessage(ticketId),
        source: 'postgres_magic_link',
      });
    }
    const scopeTenantId = resolveTenantIdForScopedTicketRead(req);
