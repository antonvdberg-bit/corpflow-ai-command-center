...
/**
 * List recent CMP tickets (Postgres only). Admin sees all; tenant sees rows for their tenant_id only.
 *
...
async function handlePromoteStatus(req, res) {
  ...
  await prisma.cmpTicket.update({ where: { id: ticketId }, data: { consoleJson: next } });
  return res.status(200).json({ ok: true, ticket_id: ticketId, promotion: next.promotion });
}
