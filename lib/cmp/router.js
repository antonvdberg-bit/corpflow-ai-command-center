...
} from './_lib/lux-lead-purchase-readiness.js';
    ...
    const leadsOut = rows.map((row) => {
      ...
      return {
        ...
        private_client_viewing: showLuxLeadCrm
          ...
          : null,
        private_client_purchase_readiness: showLuxLeadCrm
          ? buildLuxPurchaseReadinessPacket(leadLikeForQual)
          : null,
...
async function handleLuxLeadOperatorPatch(req, res) {
  ...
  const hasPurchaseViewingOutcome = b.purchase_viewing_outcome !== undefined;
  const hasPurchaseBuyerIntent = b.purchase_buyer_intent !== undefined;
  const hasPurchasePreferredResidence = b.purchase_preferred_residence !== undefined;
  const hasPurchaseReadiness = b.purchase_readiness !== undefined;
  const hasPurchaseNextAction = b.purchase_next_action !== undefined;
  const hasPurchaseDocumentsSelected = b.purchase_documents_selected !== undefined;
  const hasPurchaseDocumentsNotes = b.purchase_documents_notes !== undefined;
  const hasPurchaseTransactionNotes = b.purchase_transaction_notes !== undefined;
  const hasPurchaseOperatorNotes = b.purchase_operator_notes !== undefined;
  if (
    !hasStage &&
    !hasNote &&
    !hasFollow &&
    ...
    !hasPurchaseViewingOutcome &&
    !hasPurchaseBuyerIntent &&
    !hasPurchasePreferredResidence &&
    !hasPurchaseReadiness &&
    !hasPurchaseNextAction &&
    !hasPurchaseDocumentsSelected &&
    !hasPurchaseDocumentsNotes &&
    !hasPurchaseTransactionNotes &&
    !hasPurchaseOperatorNotes
  ) {
    return res.status(400).json({
      error: 'NO_UPDATES',
      hint:
        'Provide stage, note, follow_up_status, assign_owner, next_action_at, next_action_note, private_client_qualification, shortlist_slugs, invitation_operator_note, presentation_notes, viewing_next_step, viewing_format, viewing_proposed_datetime, viewing_access_notes, purchase_viewing_outcome, purchase_buyer_intent, purchase_preferred_residence, purchase_readiness, purchase_next_action, purchase_documents_selected, purchase_documents_notes, purchase_transaction_notes, and/or purchase_operator_notes.',
    });
  }
  ...
  try {
    ...
    if (
      ...
      const purchaseMerge = mergePrivateClientPurchaseReadinessPatch(
        qjPrev,
        {
          purchase_viewing_outcome: hasPurchaseViewingOutcome
            ? b.purchase_viewing_outcome
            : undefined,
          purchase_buyer_intent: hasPurchaseBuyerIntent ? b.purchase_buyer_intent : undefined,
          purchase_preferred_residence: hasPurchasePreferredResidence
            ? b.purchase_preferred_residence
            : undefined,
          purchase_readiness: hasPurchaseReadiness ? b.purchase_readiness : undefined,
          purchase_next_action: hasPurchaseNextAction ? b.purchase_next_action : undefined,
          purchase_documents_selected: hasPurchaseDocumentsSelected
            ? b.purchase_documents_selected
            : undefined,
          purchase_documents_notes: hasPurchaseDocumentsNotes
            ? b.purchase_documents_notes
            : undefined,
          purchase_transaction_notes: hasPurchaseTransactionNotes
            ? b.purchase_transaction_notes
            : undefined,
          purchase_operator_notes: hasPurchaseOperatorNotes
            ? b.purchase_operator_notes
            : undefined,
        },
        actorLabel,
        nowIso,
      ...
      if (!purchaseMerge.ok) {
        return res.status(400).json({ error: purchaseMerge.error, hint: purchaseMerge.hint });
      }
      qjPrev = purchaseMerge.qj;
      if (purchaseMerge.changed) {
        qjPrev = appendLuxLeadActivity(qjPrev, {
          at: nowIso,
          actor_label: actorLabel,
          kind: 'purchase_readiness_updated',
          detail: {
            purchase_viewing_outcome_set: hasPurchaseViewingOutcome,
            purchase_buyer_intent_set: hasPurchaseBuyerIntent,
            purchase_preferred_residence_set: hasPurchasePreferredResidence,
            purchase_readiness_set: hasPurchaseReadiness,
            purchase_next_action_set: hasPurchaseNextAction,
            purchase_documents_selected_set: hasPurchaseDocumentsSelected,
            purchase_documents_notes_set: hasPurchaseDocumentsNotes,
            purchase_transaction_notes_set: hasPurchaseTransactionNotes,
            purchase_operator_notes_set: hasPurchaseOperatorNotes,
          },
        });
      }
    }
    ...
    return res.status(200).json({
      ...
      private_client_purchase_readiness: buildLuxPurchaseReadinessPacket(leadLike),
    });
  } catch (e) {
