...
async function requireCipcCampaignAccess(req, res, action) {
  ...
  if (!isCipcCampaignOperatorScope(listScope)) {
    return res.status(403).json({
      ...
      hint: 'CIPC Desk tenant session or factory operator access is required. Campaign records are not public.',
    });
  }
  return true;
}
...
async function requireCipcResponseAccess(req, res, action) {
  ...
  if (!isCipcCampaignOperatorScope(listScope)) {
    return res.status(403).json({
      ...
      hint: 'CIPC Desk tenant session or factory operator access is required. Response records are not public.',
    });
  }
  return true;
}
