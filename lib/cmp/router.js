...
/**
 * GET `action=client-decisions-get` — client-safe payload for the six decision questions only.
 * Query: `id=<ticket_id>`
 */
  ...
  try {
    ...
    if (!hasCd) {
      return res.status(200).json({
        ...
        explanation: 'This is a larger growth system, so we are defining the first live version first.',
        client_decisions: { items: [], sufficient_to_proceed: false },
        source: 'postgres',
      });
    }

    const stored = ensureClientDecisionsOnConsoleJson(norm);
...
/**
 * POST `action=submit-client-decisions` — persist client answers to `console_json.client_decisions` only.
...
async function handleSubmitClientDecisions(req, res) {
  ...
  try {
    ...
    const applied = applyClientDecisionAnswers({
      ...
      answersByKey: /** @type {Record<string, { answer?: unknown, waive?: unknown }>} */ (answers),
    ...
    await prisma.cmpTicket.update({
      where: { id: ticketId },
      data: {
        consoleJson: applied.next,
        locale: applied.next.locale,
        brief: applied.next?.brief?.summary ? String(applied.next.brief.summary) : undefined,
      },
    });
    ...
    return res.status(200).json({
      ...
      ticket_id: ticketId,
      client_decisions: {
        items: applied.next.client_decisions?.items || [],
