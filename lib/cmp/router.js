...
import {
  ...
  clientDecisionsAnswersForOperator,
...
} from './_lib/cipc-desk-client-reply.js';
import {
...
import { withAuditActorFromRequest } from '../server/audit-actor-context.js';
...
} from './_lib/cmp-membership-enforcement.js';
...
import { classifyLuxLeadSystemTest } from './_lib/lux-lead-system-test-heuristic.js';
import {
  buildLuxPrivateClientQualificationView,
  mergePrivateClientQualificationPatch,
} from './_lib/lux-lead-qualification.js';
import {
  appendLuxLeadActivity,
  buildLuxInvitationPacketDraft,
  mergePrivateClientShortlistPatch,
} from './_lib/lux-lead-shortlist.js';
import {
  buildLuxConfidentialPresentationPacket,
  mergePrivateClientPresentationPatch,
} from './_lib/lux-lead-confidential-presentation.js';
import {
  buildLuxViewingByInvitationPacket,
  mergePrivateClientViewingPatch,
} from './_lib/lux-lead-viewing-by-invitation.js';
import { extractLuxSprintMetaForApi } from './_lib/lux-sprint-meta-extract.js';
import {
...
import { notifyLuxTicketOperatorEmail } from '../server/lux-ticket-operator-notify.js';
import {
  changeConsoleCheckpointLink,
  notifyOperatorCheckpoint,
  OPERATOR_CHECKPOINT_KINDS,
} from '../server/operator-checkpoint-alert.js';
import { mapApproveBuildServerException } from './_lib/cmp-operator-error-copy.js';
import {
  readLuxChangeNotifyPrefsPg,
  writeLuxChangeNotifyPrefsPg,
} from './_lib/lux-change-notify-prefs.js';
import { loadLuxAiHandoffContextForGroq } from '../server/lux-ai-handoff-context.js';
import {
  buildOperatorWithdrawalConsoleJson,
  isCmpTicketOperatorOpen,
  validateOperatorWithdrawalTarget,
} from './_lib/ticket-operator-withdraw.js';
import { denyIfTicketClosed } from './_lib/ticket-mutable-guard.js';
import {
...
/**
 * CIPC campaign MVP must not be a static import: that module previously used
 * `import.meta.url`, and a CJS `require()` of it crashed factory_router boot
 * so every `/api/*` returned Vercel HTML 500 (`x-matched-path: /500`). #1015.
 * Dynamic import() is the same pattern as `scripts/production-pulse.mjs`.
 * @returns {Promise<typeof import('../cipc-desk/campaign-mvp.js')>}
 */
async function loadCipcCampaignMvp() {
  return import('../cipc-desk/campaign-mvp.js');
}

/**
 * Same lazy-load pattern as campaign-mvp (#1015). Keep response-automation
 * off the static factory_router import graph.
 * @returns {Promise<typeof import('../cipc-desk/response-automation.js')>}
 */
async function loadCipcResponseAutomation() {
  return import('../cipc-desk/response-automation.js');
}
...
const TENANT_LOGIN_SESSION_CMP_ACTIONS = new Set([
  ...
  'ticket-withdraw',
  'cipc-desk-ticket-update',
  'cipc-campaign-list',
  'cipc-campaign-hydrate',
  'cipc-campaign-operator-patch',
  'cipc-response-list',
  'cipc-response-operator-patch',
  'cipc-response-link-reply',
  'reality-gate-run',
  'concierge-lead-operator-patch',
  'concierge-leads-list',
  'lux-client-request-create',
  'lux-client-requests-list',
  'lux-change-notify-prefs-get',
  'lux-change-notify-prefs-set',
...
function resolveTenantFromHost(req) {
  ...
  if (
    rawHost === 'cipc.corpflowai.com' ||
    rawHost === 'www.cipc.corpflowai.com' ||
    rawHost === 'cipc-desk.corpflowai.com' ||
    rawHost === 'www.cipc-desk.corpflowai.com'
  ) {
    return { host: rawHost, surface: 'tenant', tenant_id: 'cipc-desk' };
  }
...
function enforceCmpTenantBoundary(req, res, action) {
  ...
  const dbHostMismatch = evaluateDbBackedHostActingMismatch({
    hostSurface: hostTenant.surface,
    hostTenantId: hostTenant.tenant_id,
    sess,
  });
  if (dbHostMismatch) {
    return deny(res, 403, dbHostMismatch.error, {
      host: hostTenant.host,
      ...dbHostMismatch.extra,
    });
  }
...
async function handleConciergeLeadCreate(req, res) {
  ...
  // Tenant must come from host context (or explicit Lux/tenant host map).
  // Never default to luxe-maurice — that leaked Core/unknown hosts into the Lux queue (#693).
  const fromCtx = getClientIdFromHostContext(req);
  const hostResolved = resolveTenantFromHost(req);
  const tenantId =
    fromCtx || (hostResolved.surface === 'tenant' && hostResolved.tenant_id ? hostResolved.tenant_id : null);
  if (!tenantId) {
    return res.status(404).json({
      error: 'TENANT_NOT_FOUND',
      hint: 'Concierge lead create requires a mapped tenant host (e.g. lux.corpflowai.com).',
    });
  }
  ...
  // Public `/concierge` sends `contact` as `email | telephone` (no schema change).
  // Optional body.phone is accepted when present; otherwise parse from contact/message.
  const parsedContact = parseLuxConciergeContactFields({
    contact,
    phone: b.phone,
    message,
  });
  // Issue #685 Slice A — Lux public concierge requires both email and telephone server-side.
  if (tenantId === 'luxe-maurice' && (!parsedContact.email || !parsedContact.phone)) {
    return res.status(400).json({
      error: 'EMAIL_AND_TELEPHONE_REQUIRED',
      hint: 'Provide both a valid email address and a telephone number (contact as "email | telephone").',
    });
  }
  const email =
    parsedContact.email ||
    (contact.includes('@') ? contact.split('|')[0].trim().toLowerCase() : `concierge+${Date.now()}@placeholder.local`);
  const phone = parsedContact.phone || null;
...
async function handleConciergeLeadsList(req, res) {
  ...
  try {
    ...
    const leadsOut = rows.map((row) => {
      ...
      const contactFields = parseLuxConciergeContactFields({
        contact: row.contact,
        phone: row.phone,
        email: row.email,
        message: row.message,
      });
      // Server-side test / placeholder / smoke classification. The desk uses
      // `system_generated` to default-hide noise rows from LEADS · New without
      // dropping audit access (see `lib/cmp/_lib/lux-lead-system-test-heuristic.js`).
      const systemTestClass = classifyLuxLeadSystemTest({
        name: row.name,
        contact: row.contact,
        message: row.message,
        listing: row.listing,
      ...
      return {
        ...
        email: contactFields.email,
        phone: contactFields.phone,
        contact_display: contactFields.contact_display,
        message: row.message || '',
        status: row.status || 'new',
        listing: row.listing || null,
        intent: row.intent || null,
        // Issue #685 Slice A — operator desk "source" = enquiry intent channel.
        source: row.intent || null,
        property_interest: propertyInterest,
        operator_workflow: showLuxLeadCrm
          ...
          : null,
        private_client_qualification: showLuxLeadCrm
          ? buildLuxPrivateClientQualificationView(leadLikeForQual)
          : null,
        private_client_shortlist: showLuxLeadCrm
          ? buildLuxInvitationPacketDraft(leadLikeForQual)
          : null,
        private_client_presentation: showLuxLeadCrm
          ? buildLuxConfidentialPresentationPacket(leadLikeForQual)
          : null,
        private_client_viewing: showLuxLeadCrm
          ? buildLuxViewingByInvitationPacket(leadLikeForQual)
          : null,
        system_generated: systemTestClass.system_generated,
        system_generated_reason: systemTestClass.system_generated ? systemTestClass.reason || null : null,
      };
    });
    const counts = {
      total: leadsOut.length,
      real: leadsOut.filter((l) => l.system_generated !== true).length,
      system_generated: leadsOut.filter((l) => l.system_generated === true).length,
    };
    return res.status(200).json({ ok: true, leads: leadsOut, counts });
  } catch (e) {
...
async function handleLuxLeadOperatorPatch(req, res) {
  ...
  const hasQual =
    b.private_client_qualification !== undefined &&
    b.private_client_qualification != null &&
    typeof b.private_client_qualification === 'object';
  const hasShortlistSlugs = b.shortlist_slugs !== undefined;
  const hasInvitationNote = b.invitation_operator_note !== undefined;
  const hasPresentationNotes = b.presentation_notes !== undefined;
  const hasViewingNextStep = b.viewing_next_step !== undefined;
  const hasViewingFormat = b.viewing_format !== undefined;
  const hasViewingProposedDatetime = b.viewing_proposed_datetime !== undefined;
  const hasViewingAccessNotes = b.viewing_access_notes !== undefined;
  if (
    !hasStage &&
    !hasNote &&
    !hasFollow &&
    ...
    !hasShortlistSlugs &&
    !hasInvitationNote &&
    !hasPresentationNotes &&
    !hasViewingNextStep &&
    !hasViewingFormat &&
    !hasViewingProposedDatetime &&
    !hasViewingAccessNotes
  ) {
    return res.status(400).json({
      error: 'NO_UPDATES',
      hint:
        'Provide stage, note, follow_up_status, assign_owner, next_action_at, next_action_note, private_client_qualification, shortlist_slugs, invitation_operator_note, presentation_notes, viewing_next_step, viewing_format, viewing_proposed_datetime, and/or viewing_access_notes.',
    });
  }
  ...
  try {
    ...
    let qjPrev = row.qualificationJson && typeof row.qualificationJson === 'object' ? row.qualificationJson : {};
    const pi =
      qjPrev.property_interest && typeof qjPrev.property_interest === 'object' ? qjPrev.property_interest : null;

    if (hasStage || hasNote || hasFollow || hasAssign || hasNextAt || hasNextNote) {
      qjPrev = mergeLuxOperatorWorkflowPatch(
        qjPrev,
        {
    ...
    if (hasQual) {
      const qualMerge = mergePrivateClientQualificationPatch(
        qjPrev,
        b.private_client_qualification,
        actorLabel,
        nowIso,
      );
      qjPrev = qualMerge.qj;
      if (qualMerge.changed) {
        qjPrev = appendLuxLeadActivity(qjPrev, {
          at: nowIso,
          actor_label: actorLabel,
          kind: 'qualification_updated',
          detail: { keys: Object.keys(b.private_client_qualification || {}).slice(0, 12) },
        });
      }
    }

    if (hasShortlistSlugs || hasInvitationNote) {
      const shortMerge = mergePrivateClientShortlistPatch(
        qjPrev,
        {
          shortlist_slugs: hasShortlistSlugs ? b.shortlist_slugs : undefined,
          invitation_operator_note: hasInvitationNote ? b.invitation_operator_note : undefined,
        },
        actorLabel,
        nowIso,
        pi,
      ...
      if (!shortMerge.ok) {
        return res.status(400).json({ error: shortMerge.error, hint: shortMerge.hint });
      }
      qjPrev = shortMerge.qj;
      if (shortMerge.changed) {
        const parsedSlugs = Array.isArray(b.shortlist_slugs)
          ? b.shortlist_slugs.map((s) => String(s)).slice(0, 8)
          : undefined;
        qjPrev = appendLuxLeadActivity(qjPrev, {
          at: nowIso,
          actor_label: actorLabel,
          ...
          detail: {
            shortlist_slugs: parsedSlugs,
            invitation_note_set: hasInvitationNote,
          },
        });
      }
    }

    if (hasPresentationNotes || hasViewingNextStep) {
      const presentationMerge = mergePrivateClientPresentationPatch(
        qjPrev,
        {
          presentation_notes: hasPresentationNotes ? b.presentation_notes : undefined,
          viewing_next_step: hasViewingNextStep ? b.viewing_next_step : undefined,
        },
        actorLabel,
        nowIso,
      );
      if (!presentationMerge.ok) {
        return res.status(400).json({ error: presentationMerge.error, hint: presentationMerge.hint });
      }
      qjPrev = presentationMerge.qj;
      if (presentationMerge.changed) {
        qjPrev = appendLuxLeadActivity(qjPrev, {
          at: nowIso,
          actor_label: actorLabel,
          kind: 'presentation_updated',
          detail: {
            presentation_notes_set: hasPresentationNotes,
            viewing_next_step: hasViewingNextStep ? b.viewing_next_step : undefined,
          },
        });
      }
    ...
    if (hasViewingFormat || hasViewingProposedDatetime || hasViewingAccessNotes) {
      const viewingMerge = mergePrivateClientViewingPatch(
        qjPrev,
        {
          viewing_format: hasViewingFormat ? b.viewing_format : undefined,
          viewing_proposed_datetime: hasViewingProposedDatetime
            ? b.viewing_proposed_datetime
            : undefined,
          viewing_access_notes: hasViewingAccessNotes ? b.viewing_access_notes : undefined,
        },
        actorLabel,
        nowIso,
      );
      if (!viewingMerge.ok) {
        return res.status(400).json({ error: viewingMerge.error, hint: viewingMerge.hint });
      }
      qjPrev = viewingMerge.qj;
      if (viewingMerge.changed) {
        qjPrev = appendLuxLeadActivity(qjPrev, {
          at: nowIso,
          actor_label: actorLabel,
          kind: 'viewing_invitation_updated',
          detail: {
            viewing_format_set: hasViewingFormat,
            viewing_proposed_datetime_set: hasViewingProposedDatetime,
            viewing_access_notes_set: hasViewingAccessNotes,
          },
        });
      }
    }
    ...
    const qjSaved =
      saved.qualificationJson && typeof saved.qualificationJson === 'object' ? saved.qualificationJson : {};
    const piSaved =
      qjSaved.property_interest && typeof qjSaved.property_interest === 'object' ? qjSaved.property_interest : null;
    const leadLike = {
      name: saved.name,
      email: contactFields.email,
      phone: contactFields.phone,
      ...
      listing: saved.listing || null,
      property_interest: piSaved,
      qualificationJson: qjSaved,
    };
    return res.status(200).json({
      ok: true,
      lead_id: leadId,
      operator_workflow: ow,
      private_client_qualification: buildLuxPrivateClientQualificationView(leadLike),
      private_client_shortlist: buildLuxInvitationPacketDraft(leadLike),
      private_client_presentation: buildLuxConfidentialPresentationPacket(leadLike),
      private_client_viewing: buildLuxViewingByInvitationPacket(leadLike),
    });
  } catch (e) {
...
function requireLuxHostTenant(req, res, action) {
  const guard = requireDormantGate(req, res, action);
  if (guard !== true) return guard;
  const listScope = resolveCmpTicketListScope(req);
  if (listScope.kind !== 'tenant' || listScope.tenantId !== 'luxe-maurice') {
    return res.status(403).json({
      error: 'LUX_TENANT_ONLY',
      hint: 'This action requires a LuxeMaurice tenant login on the Lux hostname.',
    });
  }
  const hostTid = getClientIdFromHostContext(req);
  if (hostTid !== 'luxe-maurice') {
    return res.status(403).json({ error: 'LUX_HOST_REQUIRED' });
  }
  return true;
}

async function handleLuxChangeNotifyPrefsGet(req, res) {
  if (req.method !== 'GET') {
    res.setHeader('Allow', 'GET');
    return res.status(405).json({ error: 'Method not allowed' });
  }
  const guard = requireLuxHostTenant(req, res, 'lux-change-notify-prefs-get');
  if (guard !== true) return guard;
  try {
    const prefs = await readLuxChangeNotifyPrefsPg(prisma, 'luxe-maurice');
    return res.status(200).json({ ok: true, prefs });
  } catch (e) {
    return res.status(500).json({ error: 'LUX_NOTIFY_PREFS_GET_FAILED', detail: String(e?.message || e) });
  }
}

async function handleLuxChangeNotifyPrefsSet(req, res) {
  if (req.method !== 'POST') {
    res.setHeader('Allow', 'POST');
    return res.status(405).json({ error: 'Method not allowed' });
  }
  const guard = requireLuxHostTenant(req, res, 'lux-change-notify-prefs-set');
  if (guard !== true) return guard;
  const parsed = parseJsonBody(req);
  if (!parsed.ok) return res.status(400).json({ error: parsed.error });
  const body = parsed.body && typeof parsed.body === 'object' ? parsed.body : {};
  try {
    const prefs = await writeLuxChangeNotifyPrefsPg(prisma, 'luxe-maurice', {
    ...
    if (msg === 'INVALID_NOTIFY_EMAIL') {
      return res.status(400).json({ error: 'INVALID_NOTIFY_EMAIL' });
    }
    return res.status(500).json({ error: 'LUX_NOTIFY_PREFS_SET_FAILED', detail: msg });
  }
}
...
async function handleLuxAttachmentReviewSet(req, res) {
  ...
  try {
    ...
    try {
      await recordCmpAutomationEvent(prisma, req, {
        tenantId: 'luxe-maurice',
...
function requireFactoryMasterOnly(req, res, action) {
  ...
  if (sess?.ok === true && sess.payload?.typ === 'admin') {
    ...
    if (actingTid) {
      return deny(res, 403, 'FACTORY_MASTER_REQUIRES_PICKER_CONTEXT', {
        action,
...
function resolveTenantIdForScopedTicketRead(req) {
  const sess = getSessionFromRequest(req);
  const eff = req.corpflowEffectiveMemberships;
  const isFactoryMasterToken = verifyFactoryMasterToken(req);
  const isLegacyEnvMaster = sess?.ok === true && sess.payload?.typ === 'admin' && !readSessionUserId(sess);

  if (sess?.ok && sess.payload?.typ === 'admin') {
    const userId = readSessionUserId(sess);
    const actingTid = readActingTenantId(sess);
    if (userId && !actingTid && eff?.factory_master !== true) {
      req.corpflowInvalidAdminScope = true;
      return null;
    }
    return resolveAdminScopedTicketReadTenantId({
      sess,
      eff,
      isLegacyEnvMaster,
  ...
  if (sess?.ok && sess.payload?.typ === 'tenant') {
    ...
    if (sess.payload?.tenant_id) return String(sess.payload.tenant_id).trim() || null;
  }
...
function denyInvalidAdminScope(req, res) {
  if (!req.corpflowInvalidAdminScope) return null;
  return res.status(403).json({
    error: 'TENANT_SESSION_MISSING_TENANT_ID',
    hint: 'Operator session is not acting in a tenant workspace. Switch workspace on Core, then retry.',
  });
}
...
function resolveCmpTicketListScope(req) {
  ...
  if (sess?.ok && sess.payload?.typ === 'admin') {
    return resolveAdminTicketListScope({
      sess,
      eff,
      isLegacyEnvMaster,
      isFactoryMasterToken,
  ...
  if (sess?.ok && sess.payload?.typ === 'tenant') {
    ...
    const tid =
      actingTid ||
      (sess.payload?.tenant_id != null ? String(sess.payload.tenant_id).trim() : '');
    if (!tid) return { kind: 'invalid_tenant_session' };
    return { kind: 'tenant', tenantId: tid };
  }
  if (isFactoryMasterToken) {
...
/**
 * IM-7 — trusted automation write with server-resolved actor_user_id.
 *
 * @param {import('@prisma/client').PrismaClient} prisma
 * @param {import('http').IncomingMessage} req
 * @param {Parameters<typeof recordTrustedAutomationEvent>[1]} opts
 */
async function recordCmpAutomationEvent(prisma, req, opts) {
  return recordTrustedAutomationEvent(prisma, withAuditActorFromRequest(req, opts));
}
 ...
 *
 * When a tenant/operator session already satisfies the dormant gate **and** a magic token is present,
 * still validate the token so thank-you / consumption behave the same as a pure magic-link client.
 *
...
async function assertClientDecisionsMagicOrDormantGate(req, res, action) {
  ...
  const dormantOk = verifyDormantGateCredentials(req, action);
  const a = String(action || '').toLowerCase();
  const isMagicAction = a === 'client-decisions-get' || a === 'submit-client-decisions';

  if (!isMagicAction) {
    if (dormantOk) return true;
  ...
  if (a === 'client-decisions-get') {
    ...
    if (!parsed.ok) {
      if (dormantOk) return true;
      return deny(res, 400, parsed.error);
    }
...
async function handleTicketCreate(req, res) {
  ...
  try {
    ...
    const tidCreate = row.tenantId != null ? String(row.tenantId) : null;
    const handoffCreate =
      tidCreate === 'luxe-maurice' ? loadLuxAiHandoffContextForGroq() : { block: '' };
    const refinedCreate = await runGovernedGroqChangeRefiner({
      ...
      extraSystemContext: handoffCreate.block || null,
    });
    ...
    const missCreate = Array.isArray(storedCreate.brief?.missing_information)
      ...
      : [];
    void notifyLuxTicketOperatorEmail(
      req,
      {
        action: 'created',
        ticketId: row.id,
    ...
    if (withholdCreate.withheld) {
      void notifyOperatorCheckpoint({
        kind: OPERATOR_CHECKPOINT_KINDS.EXTERNAL_EMAIL_CLIENT_SEND_APPROVAL_NEEDED,
        whatNeedsApproval: `Review and approve withheld Change Console draft before client sees it (ticket ${row.id})`,
        link: changeConsoleCheckpointLink(row.id),
...
/**
 * POST `action=ticket-withdraw` — non-destructive operator withdrawal (hard-close without delete).
 * Tenant-scoped: Lux session can only withdraw its own tenant rows.
 */
async function handleTicketWithdraw(req, res) {
  if (req.method !== 'POST') {
    res.setHeader('Allow', 'POST');
    return res.status(405).json({ error: 'Method not allowed' });
  }

  const guard = requireDormantGate(req, res, 'ticket-withdraw');
  if (guard !== true) return guard;
  ...
  const targetCheck = validateOperatorWithdrawalTarget(ticketId);
  if (!targetCheck.ok) {
    return res.status(409).json({
      error: targetCheck.error,
      hint: targetCheck.hint,
  ...
  try {
    ...
    const row = await prisma.cmpTicket.findUnique({
      where: { id: ticketId },
      select: { id: true, tenantId: true, status: true, stage: true, consoleJson: true, description: true },
    });
    if (!row) {
      return res.status(404).json({ error: 'Ticket not found', ticket_id: ticketId });
    }

    const scopeTenantId = resolveTenantIdForScopedTicketRead(req);
    if (scopeTenantId && String(row.tenantId || '').trim() !== scopeTenantId) {
      return res.status(404).json({
    ...
    if (!isCmpTicketOperatorOpen(row)) {
      return res.status(409).json({
        error: 'TICKET_ALREADY_CLOSED',
        ticket_id: ticketId,
        hint: 'This ticket is already withdrawn or closed.',
      });
    }

    const nowIso = new Date().toISOString();
    const nextConsole = buildOperatorWithdrawalConsoleJson(row.consoleJson, nowIso);
    ...
    const ticketProgress = buildTicketProgress(nextConsole, 'Closed', 'Closed', {
      ticketId,
      tenantId: row.tenantId,
    });
    ...
    await recordCmpAutomationEvent(prisma, req, {
      tenantId: row.tenantId,
      eventType: 'cmp.ticket.withdrawn',
      payload: {
        ticket_id: ticketId,
        status: 'Closed',
        stage: 'Closed',
      ...
      idempotencyKey: `cmp:ticket:withdrawn:${ticketId}`,
      source: 'cmp',
    });
    ...
    void notifyLuxTicketOperatorEmail(
      req,
      {
        ...
        ticketId,
        tenantId: row.tenantId,
        descriptionPreview: row.description != null ? String(row.description) : null,
      },
      prisma,
    );
    ...
    return res.status(200).json({
      ...
      audit_note: nextConsole?.client_view?.closure?.context_note || null,
    });
  } catch (e) {
    emitLogicFailure({
      source: 'api/cmp/router.js:ticket-withdraw',
      severity: 'warning',
      error: e,
      cmp: { ticket_id: ticketId || 'n/a', action: 'ticket-withdraw' },
      recommended_action: 'Retry withdrawal; verify POSTGRES_URL and tenant session scope.',
    });
    return res.status(500).json({ error: 'ticket-withdraw failed', detail: String(e?.message || e) });
  }
}
 ...
 * @param {{ force?: boolean, req?: import('http').IncomingMessage }} opts
 */
async function applyRealityGateRefreshForTicketRow(prismaClient, row, opts = {}) {
  ...
  if (!run.all_ok && isClosed) {
    ...
    try {
      ...
      if (opts.req) {
        await recordCmpAutomationEvent(prismaClient, opts.req, auditOpts);
      } else {
        await recordTrustedAutomationEvent(prismaClient, auditOpts);
      }
    } catch (_) {
      /* non-blocking */
    }
...
async function handleTicketGet(req, res) {
  ...
  try {
    ...
    try {
      row = await applyRealityGateRefreshForTicketRow(prisma, row, { force: forceRg, req });
    } catch (e) {
      emitLogicFailure({
    ...
    const client_decisions_answers = clientDecisionsAnswersForOperator(row.id, itemsSum);
    ...
    /**
     * Sprint linkage metadata for `/change` — surfaces the LuxeMaurice Content
     * Population Sprint markers (`parent_sprint_ticket`, `lux_request_meta.sprint_code`)
     * so the operator desk can render the "Add content" panel and collapse generic
     * workflow controls without needing raw `console_json` (admin-only).
     *
     * Tenant-safe: only non-sensitive linkage strings + sprint code.
     */
    const luxSpriteMeta = extractLuxSprintMetaForApi(row.consoleJson);
    ...
    return res.status(200).json({
      ...
      client_decisions_answers,
      lux_programme_summary: luxProgrammeSummary,
      lux_sprint_meta: luxSpriteMeta,
      source: 'postgres',
    });
  } catch (e) {
    emitLogicFailure({
...
async function handleRealityGateRun(req, res) {
  ...
  try {
    ...
    const updated = await applyRealityGateRefreshForTicketRow(prisma, row, { force: true, req });
...
/**
 * CIPC campaign MVP (#985) — tenant `cipc-desk` or factory operator only.
 * @param {import('http').IncomingMessage} req
 * @param {import('http').ServerResponse} res
 * @param {string} action
 * @returns {Promise<true | import('http').ServerResponse>}
 */
async function requireCipcCampaignAccess(req, res, action) {
  const guard = requireDormantGate(req, res, action);
  if (guard !== true) return guard;
  const { isCipcCampaignOperatorScope } = await loadCipcCampaignMvp();
  const listScope = resolveCmpTicketListScope(req);
  if (!isCipcCampaignOperatorScope(listScope)) {
    return res.status(403).json({
      error: 'CIPC_CAMPAIGN_OPERATOR_REQUIRED',
      hint: 'Business Admin Desk tenant session or factory operator access is required. Campaign records are not public.',
    });
  }
  return true;
}

async function loadCipcCampaignLeadRows() {
  const pgUrl = String(cfg('POSTGRES_URL', '')).trim();
  if (!pgUrl) return [];
  try {
    return await prisma.lead.findMany({
      where: { tenantId: 'cipc-desk', intent: 'cipc-campaign' },
      orderBy: { createdAt: 'desc' },
      take: 200,
      select: {
        id: true,
...
/**
 * GET `action=cipc-campaign-list` — first 10 verified prospects + drafts. No send.
 */
async function handleCipcCampaignList(req, res) {
  if (req.method !== 'GET') {
    res.setHeader('Allow', 'GET');
    return res.status(405).json({ error: 'Method not allowed' });
  }
  const access = await requireCipcCampaignAccess(req, res, 'cipc-campaign-list');
  if (access !== true) return access;
  try {
    const { listCipcCampaignBoard } = await loadCipcCampaignMvp();
    const leadRows = await loadCipcCampaignLeadRows();
    const prospects = listCipcCampaignBoard({ leadRows });
    return res.status(200).json({
      ok: true,
      campaign_id: 'cipc-fractional-secretarial-v1',
      send: false,
      count: prospects.length,
      prospects,
    });
  } catch (e) {
    return res.status(500).json({ error: 'cipc-campaign-list failed', detail: String(e?.message || e) });
  }
}

/**
 * POST `action=cipc-campaign-hydrate` — idempotent upsert onto existing leads.
 */
async function handleCipcCampaignHydrate(req, res) {
  if (req.method !== 'POST') {
    res.setHeader('Allow', 'POST');
    return res.status(405).json({ error: 'Method not allowed' });
  }
  const access = await requireCipcCampaignAccess(req, res, 'cipc-campaign-hydrate');
  if (access !== true) return access;
  const pgUrl = String(cfg('POSTGRES_URL', '')).trim();
  if (!pgUrl) return res.status(503).json({ error: 'POSTGRES_URL_MISSING' });
  try {
    const { buildCipcCampaignLeadUpsert, listCipcCampaignBoard, mergeCipcCampaignQualificationJson } =
      await loadCipcCampaignMvp();
    const leadRows = await loadCipcCampaignLeadRows();
    const board = listCipcCampaignBoard({ leadRows });
    let upserted = 0;
    for (const row of board) {
      if (row.duplicate_of) continue;
      const mapped = buildCipcCampaignLeadUpsert(row);
      if (row.lead_id) {
        const existing = await prisma.lead.findUnique({
          where: { id: String(row.lead_id) },
          select: { id: true, tenantId: true, qualificationJson: true },
        });
        if (!existing || String(existing.tenantId || '') !== 'cipc-desk') continue;
        await prisma.lead.update({
          ...
          data: {
            ...
            score: mapped.score,
            qualificationJson: mergeCipcCampaignQualificationJson(existing.qualificationJson, mapped.qualificationJson),
          },
        });
        upserted += 1;
        continue;
      ...
      await prisma.lead.create({
        data: {
          ...
          score: mapped.score,
          qualificationJson: mapped.qualificationJson,
        },
      });
      upserted += 1;
    }
    const refreshed = listCipcCampaignBoard({ leadRows: await loadCipcCampaignLeadRows() });
    return res.status(200).json({
      ok: true,
      send: false,
      upserted,
    ...
    return res.status(500).json({ error: 'cipc-campaign-hydrate failed', detail: String(e?.message || e) });
  }
}

/**
 * POST `action=cipc-campaign-operator-patch` — approve / reject / do-not-contact.
 * Send remains blocked.
 */
async function handleCipcCampaignOperatorPatch(req, res) {
  if (req.method !== 'POST') {
    res.setHeader('Allow', 'POST');
    return res.status(405).json({ error: 'Method not allowed' });
  }
  const access = await requireCipcCampaignAccess(req, res, 'cipc-campaign-operator-patch');
  if (access !== true) return access;
  const parsed = parseJsonBody(req);
  if (!parsed.ok) return res.status(400).json({ error: parsed.error });
  const body = parsed.body && typeof parsed.body === 'object' ? parsed.body : {};
  const prospectId = String(body.prospect_id || body.id || '').trim();
  const intent = String(body.intent || body.action || '').trim();
  if (!prospectId) return res.status(400).json({ error: 'prospect_id is required' });
  ...
  try {
    ...
    const leadRows = await loadCipcCampaignLeadRows();
    const board = listCipcCampaignBoard({ leadRows });
    const current = board.find((row) => String(row.prospect_id || '') === prospectId);
    if (!current) return res.status(404).json({ error: 'PROSPECT_NOT_FOUND' });
    const result = applyCipcCampaignIntent(current, intent, body);
    if (!result.applied) {
      return res.status(result.protected_gate_encountered ? 409 : 400).json({
        ok: false,
        error: result.protected_gate_encountered ? 'PROTECTED_SEND_BLOCKED' : 'CAMPAIGN_INTENT_REJECTED',
        reason: result.reason || null,
        protected_gate_encountered: result.protected_gate_encountered,
        exact_protected_action: result.exact_protected_action,
        send: false,
      });
    }
    const pgUrl = String(cfg('POSTGRES_URL', '')).trim();
    if (pgUrl) {
      const mapped = buildCipcCampaignLeadUpsert(result.record);
      if (result.record.lead_id) {
        const existing = await prisma.lead.findUnique({
          where: { id: String(result.record.lead_id) },
          select: { id: true, tenantId: true, qualificationJson: true },
        });
        if (existing && String(existing.tenantId || '') === 'cipc-desk') {
          await prisma.lead.update({
            where: { id: existing.id },
            data: {
              score: mapped.score,
              qualificationJson: mergeCipcCampaignQualificationJson(
                existing.qualificationJson,
                mapped.qualificationJson,
              ),
            },
          });
        }
    ...
    return res.status(200).json({
      ...
      prospect: result.record,
    });
  } catch (e) {
    return res.status(500).json({ error: 'cipc-campaign-operator-patch failed', detail: String(e?.message || e) });
  }
}

async function requireCipcResponseAccess(req, res, action) {
  const guard = requireDormantGate(req, res, action);
  if (guard !== true) return guard;
  const { isCipcCampaignOperatorScope } = await loadCipcCampaignMvp();
  const listScope = resolveCmpTicketListScope(req);
  if (!isCipcCampaignOperatorScope(listScope)) {
    return res.status(403).json({
      error: 'CIPC_RESPONSE_OPERATOR_REQUIRED',
      hint: 'Business Admin Desk tenant session or factory operator access is required. Response records are not public.',
    });
  }
  return true;
}

async function loadCipcResponseTicketRows() {
  const pgUrl = String(cfg('POSTGRES_URL', '')).trim();
  if (!pgUrl) return [];
  try {
    return await prisma.cmpTicket.findMany({
      where: { tenantId: 'cipc-desk' },
      orderBy: { createdAt: 'desc' },
      take: 100,
      select: {
        id: true,
...
/**
 * GET `action=cipc-response-list` — website/campaign response queue. No send.
 */
async function handleCipcResponseList(req, res) {
  if (req.method !== 'GET') {
    res.setHeader('Allow', 'GET');
    return res.status(405).json({ error: 'Method not allowed' });
  }
  const access = await requireCipcResponseAccess(req, res, 'cipc-response-list');
  if (access !== true) return access;
  try {
    const { listCipcCampaignBoard } = await loadCipcCampaignMvp();
    const { listCipcResponseBoard } = await loadCipcResponseAutomation();
    const [ticketRows, leadRows] = await Promise.all([loadCipcResponseTicketRows(), loadCipcCampaignLeadRows()]);
    const campaignRecords = listCipcCampaignBoard({ leadRows });
    const records = listCipcResponseBoard({ ticketRows, campaignRecords });
    return res.status(200).json({
      ok: true,
      send: false,
    ...
    return res.status(500).json({ error: 'cipc-response-list failed', detail: String(e?.message || e) });
  }
}

function findCipcResponseRow(board, id) {
  const wanted = String(id || '').trim();
  if (!wanted) return null;
  return (
    board.find((row) => String(row.ticket_id || '') === wanted) ||
    board.find((row) => String(row.public_reference || '') === wanted) ||
    board.find((row) => String(row.campaign_prospect_id || '') === wanted) ||
    null
  );
}

/**
 * POST `action=cipc-response-operator-patch` — approve / reject / do-not-contact.
 * Send, quotation, payment, and CIPC submission remain blocked.
 */
async function handleCipcResponseOperatorPatch(req, res) {
  if (req.method !== 'POST') {
    res.setHeader('Allow', 'POST');
    return res.status(405).json({ error: 'Method not allowed' });
  }
  const access = await requireCipcResponseAccess(req, res, 'cipc-response-operator-patch');
  if (access !== true) return access;
  const parsed = parseJsonBody(req);
  if (!parsed.ok) return res.status(400).json({ error: parsed.error });
  const body = parsed.body && typeof parsed.body === 'object' ? parsed.body : {};
  const recordId = String(body.record_id || body.ticket_id || body.prospect_id || body.id || '').trim();
  const intent = String(body.intent || body.action || '').trim();
  if (!recordId) return res.status(400).json({ error: 'record_id is required' });
  if (!intent) return res.status(400).json({ error: 'intent is required' });
  try {
    const { applyCipcCampaignIntent, listCipcCampaignBoard, mergeCipcCampaignQualificationJson, buildCipcCampaignLeadUpsert } =
      await loadCipcCampaignMvp();
    const {
    ...
    } = await loadCipcResponseAutomation();
    const [ticketRows, leadRows] = await Promise.all([loadCipcResponseTicketRows(), loadCipcCampaignLeadRows()]);
    const campaignRecords = listCipcCampaignBoard({ leadRows });
    const board = listCipcResponseBoard({ ticketRows, campaignRecords });
    const current = findCipcResponseRow(board, recordId);
    if (!current) return res.status(404).json({ error: 'RESPONSE_RECORD_NOT_FOUND' });
    const result = applyCipcResponseIntent(current, intent, body);
    if (!result.applied) {
      return res.status(result.protected_gate_encountered ? 409 : 400).json({
        ok: false,
        error: result.protected_gate_encountered ? 'PROTECTED_SEND_BLOCKED' : 'RESPONSE_INTENT_REJECTED',
        reason: result.reason || null,
        protected_gate_encountered: result.protected_gate_encountered,
        exact_protected_action: result.exact_protected_action,
        send: false,
    ...
    const pgUrl = String(cfg('POSTGRES_URL', '')).trim();
    if (pgUrl && result.record.ticket_id) {
      const existing = await prisma.cmpTicket.findUnique({
        where: { id: String(result.record.ticket_id) },
        select: { id: true, tenantId: true, consoleJson: true },
      });
      if (existing && String(existing.tenantId || '') === 'cipc-desk') {
        await prisma.cmpTicket.update({
          where: { id: existing.id },
          data: {
            consoleJson: mergeCipcResponseJson(existing.consoleJson, result.record),
          },
        });
      }
    }
    if (pgUrl && result.record.lead_id) {
      const existingLead = await prisma.lead.findUnique({
        where: { id: String(result.record.lead_id) },
        select: { id: true, tenantId: true, qualificationJson: true },
      });
      if (existingLead && String(existingLead.tenantId || '') === 'cipc-desk') {
        await prisma.lead.update({
          where: { id: existingLead.id },
          data: {
            qualificationJson: mergeCipcResponseQualificationJson(existingLead.qualificationJson, {
              cipc_response: result.record,
            }),
          },
        });
      }
    }
    if (pgUrl && result.record.source === 'campaign' && result.record.campaign_prospect_id) {
      const campaignCurrent = campaignRecords.find(
        (row) => String(row.prospect_id || '') === String(result.record.campaign_prospect_id),
      );
      if (campaignCurrent && ['approve', 'reject', 'do_not_contact', 'unsubscribe'].includes(intent)) {
        const campaignIntent = intent === 'unsubscribe' ? 'do_not_contact' : intent;
        const campaignResult = applyCipcCampaignIntent(campaignCurrent, campaignIntent, body);
        if (campaignResult.applied && campaignResult.record.lead_id) {
          const existingLead = await prisma.lead.findUnique({
            where: { id: String(campaignResult.record.lead_id) },
            select: { id: true, tenantId: true, qualificationJson: true },
          });
          if (existingLead && String(existingLead.tenantId || '') === 'cipc-desk') {
            const mapped = buildCipcCampaignLeadUpsert(campaignResult.record);
            await prisma.lead.update({
              where: { id: existingLead.id },
              data: {
                qualificationJson: mergeCipcCampaignQualificationJson(
                  existingLead.qualificationJson,
                  mapped.qualificationJson,
                ),
              },
            });
    ...
    return res.status(500).json({ error: 'cipc-response-operator-patch failed', detail: String(e?.message || e) });
  }
}

/**
 * POST `action=cipc-response-link-reply` — attach reply metadata to an existing matter.
 * Idempotent. Does not create a second ticket/lead. Does not send.
 */
async function handleCipcResponseLinkReply(req, res) {
  if (req.method !== 'POST') {
    res.setHeader('Allow', 'POST');
    return res.status(405).json({ error: 'Method not allowed' });
  }
  const access = await requireCipcResponseAccess(req, res, 'cipc-response-link-reply');
  if (access !== true) return access;
  const parsed = parseJsonBody(req);
  if (!parsed.ok) return res.status(400).json({ error: parsed.error });
  const body = parsed.body && typeof parsed.body === 'object' ? parsed.body : {};
  try {
    const { listCipcCampaignBoard } = await loadCipcCampaignMvp();
    const { linkCipcResponseReply, listCipcResponseBoard, mergeCipcResponseJson, mergeCipcResponseQualificationJson } =
      await loadCipcResponseAutomation();
    const [ticketRows, leadRows] = await Promise.all([loadCipcResponseTicketRows(), loadCipcCampaignLeadRows()]);
    const campaignRecords = listCipcCampaignBoard({ leadRows });
    const board = listCipcResponseBoard({ ticketRows, campaignRecords });
    const linked = linkCipcResponseReply(board, body);
    if (!linked.linked || !linked.record) {
      return res.status(404).json({
        ok: false,
        error: 'REPLY_NOT_LINKED',
    ...
    const pgUrl = String(cfg('POSTGRES_URL', '')).trim();
    if (pgUrl && linked.record.ticket_id) {
      const existing = await prisma.cmpTicket.findUnique({
        where: { id: String(linked.record.ticket_id) },
        select: { id: true, tenantId: true, consoleJson: true },
      });
      if (existing && String(existing.tenantId || '') === 'cipc-desk') {
        await prisma.cmpTicket.update({
          where: { id: existing.id },
          data: { consoleJson: mergeCipcResponseJson(existing.consoleJson, linked.record) },
        });
      }
    }
    if (pgUrl && linked.record.lead_id) {
      const existingLead = await prisma.lead.findUnique({
        where: { id: String(linked.record.lead_id) },
        select: { id: true, tenantId: true, qualificationJson: true },
      });
      if (existingLead && String(existingLead.tenantId || '') === 'cipc-desk') {
        await prisma.lead.update({
    ...
    return res.status(500).json({ error: 'cipc-response-link-reply failed', detail: String(e?.message || e) });
  }
}

/**
 * POST `action=cipc-desk-ticket-update` — update CIPC Desk preview checklist + client reply draft.
 *
 * Body:
 * {
 *   ticket_id: string,
 *   checklist_items: Array<{ key: string, status: string }>,
 *   client_reply_draft?: string
 * }
 */
async function handleCipcDeskTicketUpdate(req, res) {
  if (req.method !== 'POST') {
    res.setHeader('Allow', 'POST');
    return res.status(405).json({ error: 'Method not allowed' });
  }

  const guard = requireDormantGate(req, res, 'cipc-desk-ticket-update');
  if (guard !== true) return guard;
  ...
  const clientReplyDraftRaw = body?.client_reply_draft;
  const clientReplyDraft =
    typeof clientReplyDraftRaw === 'string' ? clientReplyDraftRaw.trim() : undefined;
  ...
  try {
    ...
    const cv = existing.client_view && typeof existing.client_view === 'object' ? existing.client_view : {};
    const cipc = cv.cipc_desk && typeof cv.cipc_desk === 'object' ? cv.cipc_desk : null;
    if (!cipc) return res.status(409).json({ error: 'CIPC_DESK_TICKET_REQUIRED' });

    const curChecklist = cipc.checklist && typeof cipc.checklist === 'object' ? cipc.checklist : {};
    const itemsCur = Array.isArray(curChecklist.items) ? curChecklist.items : [];

    /** @type {Map<string,string>} */
    const updatesMap = new Map();
    for (const it of checklistItems) {
      const k = it && typeof it === 'object' ? it.key : null;
      const s = it && typeof it === 'object' ? it.status : null;
      const key = typeof k === 'string' ? k.trim() : '';
      const status = typeof s === 'string' ? s.trim().toLowerCase() : '';
      if (!key) continue;
      if (!status) continue;
      updatesMap.set(key, status);
    }

    const updatedItems = itemsCur.map((it) => {
      const key = it && typeof it === 'object' ? String(it.key || '').trim() : '';
      if (!key) return it;
      if (!updatesMap.has(key)) return it;
      const nextStatus = updatesMap.get(key);
    ...
    const allReviewed =
      ...
      updatedItems.every((it) => {
        const st = it && typeof it === 'object' ? String(it.status || '').trim().toLowerCase() : '';
        return st === 'reviewed' || st === 'done' || st === 'complete' || st === 'ok';
      });
    ...
    const updatedCipc = {
      ...
      ...(clientReplyDraft !== undefined ? { client_reply_draft: clientReplyDraft } : null),
    };

    const updatedBrief = existing.brief && typeof existing.brief === 'object' ? { ...existing.brief } : {};
    if (allReviewed) {
      updatedBrief.missing_information = [];
    }
    ...
    const updated = {
      ...
      brief: updatedBrief,
      client_view: {
        ...(existing.client_view && typeof existing.client_view === 'object' ? existing.client_view : {}),
        cipc_desk: updatedCipc,
      },
    };
    ...
    await prisma.cmpTicket.update({
      ...
      data: {
        consoleJson: updated,
        locale: updated.locale ? String(updated.locale) : row.locale,
        brief: updated?.brief?.summary ? String(updated.brief.summary) : row.brief,
      },
    });

    const ticketProgress = buildTicketProgress(updated, row.status || 'Approved', row.stage || 'Build', {
      ticketId,
      tenantId: row.tenantId,
    });
    ...
    return res.status(500).json({ error: 'cipc-desk-ticket-update failed', detail: String(e?.message || e) });
  }
}
...
async function handleApproveBuild(req, res) {
  ...
  try {
    ...
    // Best-effort only — serverless hosts often lack Python; never block approve on provisioning.
    try {
      ...
      emitLogicFailure({
        ...
        recommended_action:
          'Skipping filesystem tenant provisioning. Rely on Postgres tenant rows if used.',
      ...
      if (!Number.isFinite(tokenBalanceUsd) || tokenBalanceUsd <= 0) {
        return deny(
          ...
          'FACTORY_DORMANT: INSUFFICIENT_CREDITS',
          {
            ...
            operator_message:
              'Insufficient token credits to approve this build. Top up the tenant wallet or set billing_exempt.',
    ...
    const closedDenyApprove = denyIfTicketClosed(res, exists, deny);
    if (closedDenyApprove) return closedDenyApprove;
    ...
    const normExistsPre = normalizeConsoleJson(exists.consoleJson);
    const cvPre =
      normExistsPre.client_view && typeof normExistsPre.client_view === 'object'
        ? normExistsPre.client_view
        : {};
    const wfPre = String(cvPre.workflow_state || '').trim().toLowerCase();
    const hasEstimateRecord =
      Boolean(cvPre.last_estimate_at) ||
      cvPre.actual_cost_to_client_usd != null ||
      cvPre.market_reference_usd != null ||
      cvPre.effort_hours_low != null ||
      cvPre.effort_hours_high != null;
    if (wfPre !== 'estimated' && !hasEstimateRecord) {
      return deny(res, 409, 'This ticket cannot be approved until an estimate exists.', {
        operator_message: 'This ticket cannot be approved until an estimate exists.',
        workflow_state: wfPre || null,
        hint: 'Generate and save an estimate on this ticket before using Proceed.',
      });
    }

    const baseDescription = await resolveCmpDescriptionFromTicket(ticketId, body?.description);
    const normExists = normExistsPre;
    ...
    if (!description) {
      ...
      return deny(res, 400, 'Approve blocked: missing description.', {
        operator_message: 'Add a change description before approval.',
    ...
    if (!verdict?.ok) {
      ...
      return deny(
        ...
        {
          operator_message: `Approval blocked by ethical verification: ${verdict?.reject_reason || 'ethical sentinel rejected this build'}.`,
    ...
    if (!isAdminApprove && !billingExempt) {
      try {
        ...
        return deny(
          ...
          {
            operator_message:
              'Insufficient token credits to approve this build. Top up the tenant wallet before Proceed.',
            cost_estimate_usd: verdict?.cost_estimate_usd ?? costUsd,
          }
        );
      }
    ...
    return res.status(500).json({
      ...
      operator_message: mapApproveBuildServerException(pMsg, 500, pMsg),
      detail: pMsg,
...
async function handleCostingPreview(req, res) {
  ...
  if (ticketRef) {
    ...
    const tCheck = await prisma.cmpTicket.findUnique({
      where: { id: ticketRef },
      select: { tenantId: true, status: true, stage: true, consoleJson: true },
    });
    if (!tCheck) {
      return res.status(404).json({ error: 'Ticket not found' });
    }
    if (scopeCost && String(tCheck.tenantId || '') !== scopeCost) {
      return res.status(404).json({ error: 'Ticket not found' });
    }
    const closedDenyCost = denyIfTicketClosed(res, tCheck, deny);
    if (closedDenyCost) return closedDenyCost;
  }
...
async function handleChangeChat(req, res) {
  ...
  try {
    ...
    const closedDenyChat = denyIfTicketClosed(res, row, deny);
    if (closedDenyChat) return closedDenyChat;
    const existing = normalizeConsoleJson(row?.consoleJson);
    ...
    if (withholdChat.withheld) {
      void notifyOperatorCheckpoint({
        kind: OPERATOR_CHECKPOINT_KINDS.EXTERNAL_EMAIL_CLIENT_SEND_APPROVAL_NEEDED,
        whatNeedsApproval: `Review and approve withheld Change Console reply before client sees it (ticket ${ticketId})`,
        link: changeConsoleCheckpointLink(ticketId),
        tenant_id: row.tenantId,
        ticket_id: ticketId,
      });
    }
  ...
  try {
    ...
    if (req.corpflowClientDecisionsMagicOk === true && (consumedIso || gate.sufficient_to_proceed)) {
      return res.status(200).json({
        ...
        thank_you_message: resolveClientDecisionsThankYouMessage(
          row.consoleJson,
          row.id,
          getClientDecisionThankYouMessage,
        ),
...
async function handleSubmitClientDecisions(req, res) {
  ...
  try {
    ...
    if (prevCd.sufficient_to_proceed === true) {
      // Complete tickets: always return thank-you (session or magic). Magic keeps already_submitted.
      return res.status(200).json({
        ok: true,
        already_submitted: true,
        ticket_id: ticketId,
        ...
        source: req.corpflowClientDecisionsMagicOk === true ? 'postgres_magic_link' : 'postgres',
      });
    }
    ...
    const operator_signal = enrichOperatorSignalForTicket(ticketId, {
    ...
    const out = {
      ...
      source: 'postgres',
      // Always return thank-you after a successful submit (session and/or magic link).
      // CIPC Desk prefers console_json.client_view.cipc_desk.client_reply_draft when present.
      thank_you_message: resolveClientDecisionsThankYouMessage(
        persisted,
        ticketId,
        getClientDecisionThankYouMessage,
...
async function handleClientDecisionsLinkMint(req, res) {
  ...
  try {
    ...
    const pathBase = resolveClientDecisionsMintPath(
      ticketId,
      typeof body?.client_path === 'string' ? body.client_path : '',
    );
...
async function handleAutomationCallback(req, res) {
  ...
  const prevPreviewUrl =
    typeof prevAuto.preview_url === 'string' ? prevAuto.preview_url.trim() : '';
  if (previewIn && !prevPreviewUrl) {
    void notifyOperatorCheckpoint({
      kind: OPERATOR_CHECKPOINT_KINDS.CLIENT_APPROVAL_NEEDED,
      whatNeedsApproval: `Client preview review required for ticket ${ticketId}`,
      link: changeConsoleCheckpointLink(ticketId, previewIn),
      tenant_id: row.tenantId,
      ticket_id: ticketId,
      meta: { preview_url: previewIn },
    });
  }
...
async function handlePromoteMerge(req, res) {
  ...
  if (!rgResult.all_ok) {
    ...
    void notifyOperatorCheckpoint({
      kind: OPERATOR_CHECKPOINT_KINDS.PRODUCTION_VALIDATION_FAILURE,
      whatNeedsApproval: `Reality Gate failed after promote-merge on ticket ${ticketId}`,
      link: urls[0] || changeConsoleCheckpointLink(ticketId),
      tenant_id: row.tenantId,
      ticket_id: ticketId,
      meta: { checks: rgResult.checks, required_urls: urls },
    });
  }
...
async function handlePreviewReview(req, res) {
  ...
  if (decision === 'approve') {
    void notifyOperatorCheckpoint({
      ...
      whatNeedsApproval: `Client approved preview — operator merge/promote to CorpFlowAI test spine (corpflow_test) required for ticket ${ticketId}. Not client_production.`,
      link: changeConsoleCheckpointLink(ticketId, previewUrl),
      tenant_id: row.tenantId,
      ticket_id: ticketId,
      meta: { preview_url: previewUrl, environment: 'corpflow_test' },
    });
  }
...
export default async function handler(req, res) {
  ...
  const membership = await assertMembershipEnforcement(req, res, action, {
    getEffectiveMembershipsFn: getEffectiveMemberships,
    factoryOnlyActions: FACTORY_ONLY_CMP_ACTIONS,
    deny,
  });
  ...
  switch (action) {
    ...
    case 'ticket-withdraw':
      return handleTicketWithdraw(req, res);
    case 'cipc-desk-ticket-update':
      return handleCipcDeskTicketUpdate(req, res);
    case 'cipc-campaign-list':
      return handleCipcCampaignList(req, res);
    case 'cipc-campaign-hydrate':
      return handleCipcCampaignHydrate(req, res);
    case 'cipc-campaign-operator-patch':
      return handleCipcCampaignOperatorPatch(req, res);
    case 'cipc-response-list':
      return handleCipcResponseList(req, res);
    case 'cipc-response-operator-patch':
      return handleCipcResponseOperatorPatch(req, res);
    case 'cipc-response-link-reply':
      return handleCipcResponseLinkReply(req, res);
    case 'ticket-get':
    ...
    case 'lux-change-notify-prefs-get':
      return handleLuxChangeNotifyPrefsGet(req, res);
    case 'lux-change-notify-prefs-set':
      return handleLuxChangeNotifyPrefsSet(req, res);
