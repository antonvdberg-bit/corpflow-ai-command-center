...
import { isBillingExemptTenant } from '../server/billing-exempt.js';
import fs from 'fs';
...
async function handleApproveBuild(req, res) {
  ...
  try {
    ...
    const billingExempt = isBillingExemptTenant(clientId);
    ...
    // Admin sessions bypass; CORPFLOW_BILLING_EXEMPT_TENANT_IDS skips credits for demos / partners.
    ...
    // Admin and billing-exempt tenants skip debit.
    if (!isAdminApprove && !billingExempt) {
      try {
...
async function handleCostingPreview(req, res) {
  ...
  const costUsd = Number(cost?.full_market_value_usd);
  const billingExemptPreview = isBillingExemptTenant(clientId);
...
async function handleTenantHostnameUpsert(req, res) {
  ...
  try {
    ...
    recordSovereignAuditEvent({
      ...
      meta: { host: hostRaw, mode, enabled: enabled === true },
    });
    return res.status(200).json({ ok: true, mapping: row });
  } catch (e) {
    emitLogicFailure({
