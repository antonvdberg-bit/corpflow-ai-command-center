...
async function handleConciergeLeadCreate(req, res) {
  ...
  const contactRaw = str(b.contact);
  const message = str(b.message);
  const phoneFromBody = str(b.phone);
  if (!name) return res.status(400).json({ error: 'name is required' });
  if (!contactRaw) return res.status(400).json({ error: 'contact is required' });
  if (!message) return res.status(400).json({ error: 'message is required' });

  // Prefer explicit phone body field; else parse `email | phone` from contact (Issue #673).
  const parsedContact = parseConciergeContactFields(contactRaw, phoneFromBody || null);
  const emailCandidate = parsedContact.email || (contactRaw.includes('@') ? contactRaw : '');
  const email = emailCandidate.includes('@')
    ? emailCandidate.toLowerCase().slice(0, 254)
    : `concierge+${Date.now()}@placeholder.local`;
  const phone = parsedContact.phone ? parsedContact.phone.slice(0, 64) : null;
  // Keep operator-readable contact (email | phone) when phone was captured.
  const contact = phone && !contactRaw.includes('|') ? `${email} | ${phone}` : contactRaw;
...
async function handleConciergeLeadsList(req, res) {
  ...
  try {
    ...
    const leadsOut = rows.map((row) => {
      ...
      const contactFields = parseConciergeContactFields(row.contact || row.email, row.phone);
      ...
      return {
        ...
        name: row.name,
        email: contactFields.email || row.email || null,
        phone: contactFields.phone || null,
        contact: contactFields.contact_display || row.contact || row.name,
        message: row.message || '',
...
async function handlePreviewReview(req, res) {
  ...
  if (decision === 'approve') {
    void notifyOperatorCheckpoint({
      ...
      whatNeedsApproval: `Client approved preview — operator promote-merge required for ticket ${ticketId}`,
