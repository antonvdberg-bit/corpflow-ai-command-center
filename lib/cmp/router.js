      ...
      data: {
        ...
        consoleJson: {
          ...
          client_view: {
            ...
            progress_message: 'Ticket created. Use Refine (chat) to confirm scope before estimating.',
          },
        },
      },
    });
...
/** @typedef {'intake'|'refining'|'ready_for_estimate'|'estimated'|'approved_for_build'|'building'|'preview_ready'|'in_review'|'changes_requested'|'client_approved'|'publishing'|'published'} ChangeWorkflowState */

/**
 * @param {unknown} s
 * @returns {ChangeWorkflowState | null}
 */
function normalizeWorkflowState(s) {
...
/**
 * Derive the required explicit workflow state from ticket + console_json signals.
 * Prefers an explicitly persisted state if present.
 *
 * @param {{
 *   status: string,
 *   stage: string,
...
function deriveWorkflowState(args) {
  ...
  if (summary || acOk) return 'ready_for_estimate';
 ...
 * @returns {string}
 */
function nextActionForWorkflowState(st) {
  switch (st) {
    case 'intake':
...
async function callGroqChangeRefiner({ messages, locale }) {
  ...
  /** Stable prefix for prompt-cache friendly providers; locale is appended to the last turn only. */
  const system = [
...
async function handleChangeChat(req, res) {
  ...
  try {
    ...
    // Explicit workflow state: refining -> ready_for_estimate when the brief has substance.
    try {
      ...
      const nextState = summary || hasAc ? 'ready_for_estimate' : 'refining';
      ...
      stored.client_view = {
        ...
        progress_message:
          nextState === 'ready_for_estimate'
            ? 'Brief is ready. Run Estimate to size cost and impact.'
            : 'Refining the request — answer the follow-up question.',
      };
    } catch (_) {}
...
async function handleAutomationCallback(req, res) {
  ...
  automation = withClientSitePreviewFields(automation, effectivePreview || null, row.tenantId);
  const client_view = {
    ...prevCv,
    automation,
    ...(previewIn
...
async function handlePromoteStatus(req, res) {
  ...
  if (vercelPreview.ok && vercelPreview.preview_url) {
    ...
    const automation = withClientSitePreviewFields(autoBase, vercelPreview.preview_url, row.tenantId);
    next = {
      ...next,
      client_view: {
        ...prevCv,
    ...
    if (pu) {
      next = {
        ...
        client_view: {
          ...prevCv,
          automation: withClientSitePreviewFields({ ...prevAuto, updated_at: new Date().toISOString() }, pu, row.tenantId),
        },
      };
    }
  }
