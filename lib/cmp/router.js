...
} from './_lib/lux-lead-qualification-shortlist.js';
...
async function handleConciergeLeadCreate(req, res) {
  ...
  // Issue #685 Slice A: both email and telephone are required (server-enforced).
  ...
  if (!parsedContact.email || !parsedContact.phone) {
    return res.status(400).json({
      ...
      hint: 'Provide both a valid email address and a telephone number.',
    });
  }
  const email = parsedContact.email;
  const phone = parsedContact.phone;
...
async function handleConciergeLeadsList(req, res) {
  ...
  try {
    ...
    const leadsOut = rows.map((row) => {
      ...
      const operatorWorkflow = showLuxLeadCrm
        ? luxOperatorWorkflowForApiList(owRaw, { lead_created_at: row.createdAt, lead_updated_at: row.updatedAt })
        : null;
      const qualShort = showLuxLeadCrm
        ? luxQualificationShortlistForApi(qj, {
            stage: operatorWorkflow?.stage,
            lead_name: row.name,
      ...
      return {
        ...
        // Slice A (#685): operator-facing "source" alias for intent (concierge / property / private-access).
        ...
        private_client_qualification: qualShort?.private_client_qualification || null,
        curated_shortlist: qualShort?.curated_shortlist || null,
        invitation_draft: qualShort?.invitation_draft || null,
...
async function handleLuxLeadOperatorPatch(req, res) {
  ...
  const qualPatch =
    b.private_client_qualification && typeof b.private_client_qualification === 'object'
      ? b.private_client_qualification
      : b.qualification && typeof b.qualification === 'object'
        ? b.qualification
        : null;
  const hasQualification = !!qualPatch;
  const hasShortlist = Array.isArray(b.shortlist_slugs) || Array.isArray(b.curated_shortlist_slugs);
  if (
    !hasStage &&
    !hasNote &&
    !hasFollow &&
    ...
    return res.status(400).json({
      error: 'NO_UPDATES',
      hint:
        'Provide stage, note, follow_up_status, assign_owner, next_action_at, next_action_note, private_client_qualification, and/or shortlist_slugs.',
    });
  }
  ...
  try {
    ...
    let merged = row.qualificationJson && typeof row.qualificationJson === 'object' ? row.qualificationJson : {};
    if (hasStage || hasNote || hasFollow || hasAssign || hasNextAt || hasNextNote) {
      merged = mergeLuxOperatorWorkflowPatch(
        merged,
        {
    ...
    if (hasQualification) {
      merged = mergeLuxPrivateClientQualificationPatch(merged, qualPatch, actorLabel, nowIso);
    }
    if (hasShortlist) {
      const slugs = Array.isArray(b.shortlist_slugs)
        ? b.shortlist_slugs
        : Array.isArray(b.curated_shortlist_slugs)
          ? b.curated_shortlist_slugs
          : [];
      merged = mergeLuxCuratedShortlistPatch(
        merged,
        {
          slugs,
          notes_by_slug:
            b.shortlist_notes && typeof b.shortlist_notes === 'object' ? b.shortlist_notes : null,
        },
        actorLabel,
        nowIso,
      );
    ...
    const qualShort = luxQualificationShortlistForApi(saved.qualificationJson, {
      stage: ow.stage,
    ...
    return res.status(200).json({
      ...
      private_client_qualification: qualShort.private_client_qualification,
      curated_shortlist: qualShort.curated_shortlist,
      invitation_draft: qualShort.invitation_draft,
    });
  } catch (e) {
