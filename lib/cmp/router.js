...
import { lowCostCap } from '../server/low-cost-mode.js';
import { runGovernedGroqChangeRefiner } from '../server/groq-change-refiner-service.js';
import {
  computeRefinementWorkflowPatch,
  shouldReopenTicketForActiveRefinement,
} from './_lib/change-refine-workflow.js';
import { computeRealityPanel } from './_lib/reality-panel-compute.js';
import {
  CHANGE_STAGE_ORDER,
  STAGE_RIBBON_TITLES,
  deriveChangeStage,
} from './_lib/change-stage-engine.js';
import { assertApproveBuildAllowed } from './_lib/change-stage-guards.js';
import { buildChangeStageHistory, enrichConsoleJsonWithChangeAudit } from './_lib/change-transition-log.js';
import {
...
/**
 * @param {import('http').IncomingMessage} req
 * @returns {'client' | 'operator' | 'system'}
 */
function resolveChangeStageActor(req) {
  const sess = getSessionFromRequest(req);
  if (sess?.ok === true && sess.payload?.typ === 'admin') return 'operator';
  return 'client';
}
...
async function handleTicketCreate(req, res) {
  ...
  try {
    ...
    let storedCreateAudited = storedCreate;
    try {
      storedCreateAudited = await enrichConsoleJsonWithChangeAudit(prisma, {
        rowBefore: {
          id: row.id,
        ...
        consoleJsonAfter: storedCreate,
        statusAfter: row.status || 'Open',
        stageAfter: row.stage || 'Intake',
        actor: resolveChangeStageActor(req),
    ...
    await prisma.cmpTicket.update({
      where: { id: row.id },
      data: {
        consoleJson: storedCreateAudited,
        locale: storedCreateAudited.locale,
        brief: storedCreateAudited?.brief?.summary ? String(storedCreateAudited.brief.summary) : undefined,
      },
    });

    const ticketProgressCreate = buildTicketProgress(storedCreateAudited, row.status || 'Open', row.stage || 'Intake', {
      ticketId: row.id,
      tenantId: row.tenantId,
    });
    const normCreate = normalizeConsoleJson(storedCreateAudited);
    const briefStructuredCreate =
      normCreate.brief && typeof normCreate.brief === 'object' ? normCreate.brief : null;
    const refinementMessagesCreate = buildRefinementMessagesTailFromConsoleJson(storedCreateAudited);
    ...
    const missCreate = Array.isArray(storedCreateAudited.brief?.missing_information)
      ? storedCreateAudited.brief.missing_information
      : [];
    return res.status(200).json({
      ...
      mode: storedCreateAudited.mode || undefined,
      pending_operator_draft: withholdCreate.withheld ? storedCreateAudited.pending_operator_draft : undefined,
      operator_escalation_reasons: escalationCreate.reasons.length ? escalationCreate.reasons : undefined,
      brief: storedCreateAudited.brief,
...
/**
 * Refresh stored Reality Gate checks for closed tickets (stale) or when `force` is true (reality-gate-run only).
...
async function handleTicketGet(req, res) {
  ...
  try {
    ...
    const refinement_messages = buildRefinementMessagesTailFromConsoleJson(row.consoleJson);

    let change_stage = null;
    try {
      const rpBad = reality_panel && typeof reality_panel === 'object' && reality_panel.error;
      const rpSafe = rpBad
        ? { blocker: { code: 'none', label: '' }, next_action: 'FIX' }
        : /** @type {Record<string, unknown>} */ (reality_panel);
      change_stage = deriveChangeStage({
        ticket: {
          id: row.id,
        ...
        consoleJson: row.consoleJson && typeof row.consoleJson === 'object' ? row.consoleJson : {},
        reality_panel: rpSafe,
      });
    } catch {
      change_stage = null;
    }

    const change_stage_ribbon = CHANGE_STAGE_ORDER.map((id, i) => ({
      id,
      short: STAGE_RIBBON_TITLES[i] || id,
    }));

    const change_stage_history = buildChangeStageHistory(row.consoleJson);
    const change_stage_progress_feedback =
      change_stage && typeof change_stage === 'object'
        ? [change_stage.progress_situation, change_stage.progress_hint].filter(Boolean).join(' ').trim()
        : '';

    const lastTx =
      change_stage_history.transitions.length > 0
        ? change_stage_history.transitions[change_stage_history.transitions.length - 1]
        : null;
    const rpBlockerCode =
      reality_panel && typeof reality_panel === 'object' && !reality_panel.error && reality_panel.blocker
        ? String(reality_panel.blocker.code || '')
        : '';

    const change_stage_debug =
      isAdmin && change_stage
        ? {
            current_stage: change_stage.current_stage,
            blocker_code: rpBlockerCode,
            dominant_action: change_stage.dominant_action,
            primary_cta_kind: change_stage.primary_cta_kind,
            last_transition: lastTx,
          }
        : undefined;
    ...
    return res.status(200).json({
      ...
      change_stage: change_stage || undefined,
      change_stage_ribbon: change_stage_ribbon.length ? change_stage_ribbon : undefined,
      change_stage_history,
      change_stage_progress_feedback: change_stage_progress_feedback || undefined,
      change_stage_debug,
...
async function handleRealityGateRun(req, res) {
  ...
  /** Minimum seconds between full reality-gate runs for one ticket (prevents DB + fetch spam). */
  const REALITY_GATE_MIN_INTERVAL_MS = 45_000;
  ...
  try {
    ...
    const cjTh = row.consoleJson && typeof row.consoleJson === 'object' ? row.consoleJson : {};
    const rgTh = cjTh.reality_gate && typeof cjTh.reality_gate === 'object' ? cjTh.reality_gate : {};
    const lrTh = rgTh.last_run && typeof rgTh.last_run === 'object' ? rgTh.last_run : {};
    const checkedAtTh = lrTh.checked_at != null ? String(lrTh.checked_at).trim() : '';
    const parsedAt = checkedAtTh ? Date.parse(checkedAtTh) : NaN;
    if (Number.isFinite(parsedAt)) {
      const elapsed = Date.now() - parsedAt;
      if (elapsed >= 0 && elapsed < REALITY_GATE_MIN_INTERVAL_MS) {
        const retry_after_seconds = Math.ceil((REALITY_GATE_MIN_INTERVAL_MS - elapsed) / 1000);
        return res.status(200).json({
          ok: true,
          throttled: true,
          retry_after_seconds,
          ticket_id: ticketId,
          all_ok: lrTh.all_ok === true,
          last_run: lrTh,
          required_urls: Array.isArray(rgTh.required_urls) ? rgTh.required_urls : [],
        });
      }
    }

    const updated = await applyRealityGateRefreshForTicketRow(prisma, row, { force: true });
    let finalCj = updated.consoleJson && typeof updated.consoleJson === 'object' ? updated.consoleJson : {};
    try {
      const audited = await enrichConsoleJsonWithChangeAudit(prisma, {
        rowBefore: row,
        consoleJsonAfter: finalCj,
        statusAfter: updated.status != null ? String(updated.status) : String(row.status || ''),
        stageAfter: updated.stage != null ? String(updated.stage) : String(row.stage || ''),
        actor: resolveChangeStageActor(req),
        action: 'reality-gate-run',
      });
      finalCj = audited;
      await prisma.cmpTicket.update({
        where: { id: ticketId },
        data: { consoleJson: finalCj },
      });
    } catch {
      /* non-blocking audit */
    }
    const cjx = finalCj && typeof finalCj === 'object' ? finalCj : {};
...
async function handleApproveBuild(req, res) {
  ...
  try {
    ...
    const exists = await prisma.cmpTicket.findUnique({
      where: { id: ticketId },
      select: { id: true, tenantId: true, consoleJson: true, description: true, status: true, stage: true },
    });
    if (!exists) {
    ...
    try {
      assertApproveBuildAllowed({
        status: exists.status,
        stage: exists.stage,
        consoleJson: exists.consoleJson,
      });
    } catch (guardErr) {
      const code =
        guardErr && typeof guardErr === 'object' && guardErr !== null && 'code' in guardErr
          ? String(guardErr.code)
          : '';
      return deny(res, 409, guardErr instanceof Error ? guardErr.message : String(guardErr), {
        ticket_id: ticketId,
        code: code || undefined,
      });
    }
    ...
    let nextConsoleFinal = nextConsole;
    try {
      nextConsoleFinal = await enrichConsoleJsonWithChangeAudit(prisma, {
        rowBefore: {
          id: exists.id,
    ...
    const ticket_progress = buildTicketProgress(nextConsoleFinal, 'Approved', 'Build');
  ...
  if (ticketRef) {
    try {
      ...
      if (row) {
        ...
        const nextEstCj = { ...norm, client_view };
        let nextEstFinal = nextEstCj;
        try {
          nextEstFinal = await enrichConsoleJsonWithChangeAudit(prisma, {
            rowBefore: row,
            consoleJsonAfter: nextEstCj,
            statusAfter: row.status || 'Open',
            stageAfter: row.stage || 'Intake',
            actor: resolveChangeStageActor(req),
            action: 'costing-preview',
...
/**
 * Last N chat turns for Change Console UIs (same shape as ticket-get).
 *
 * @param {unknown} consoleJson
 * @returns {{ role: string; content: string; ts: string | null }[]}
 */
function buildRefinementMessagesTailFromConsoleJson(consoleJson) {
  const norm = normalizeConsoleJson(consoleJson);
  if (!Array.isArray(norm.messages)) return [];
...
async function handleChangeChat(req, res) {
  ...
  try {
    ...
    /** @type {{ workflow_state: string; workflow_next_action: string; progress_message: string } | null} */
    let refinementWfPatch = null;
    try {
      const briefObj = stored.brief && typeof stored.brief === 'object' ? stored.brief : {};
      refinementWfPatch = computeRefinementWorkflowPatch(/** @type {Record<string, unknown>} */ (briefObj));
      const prevCv = stored.client_view && typeof stored.client_view === 'object' ? stored.client_view : {};
      stored.client_view = { ...prevCv, ...refinementWfPatch };
    } catch (_) {}
    Object.assign(
      stored,
    ...
    const reopenRow = shouldReopenTicketForActiveRefinement(row.status, row.stage, refinementWfPatch);
    /** @type {{ consoleJson: typeof stored; locale?: string; brief?: string; status?: string; stage?: string }} */
    const ticketUpdate = {
      consoleJson: stored,
      locale: stored.locale,
      brief: stored?.brief?.summary ? String(stored.brief.summary) : undefined,
    };
    if (reopenRow) {
      ticketUpdate.status = 'Open';
      ticketUpdate.stage = 'Intake';
    }

    try {
      ticketUpdate.consoleJson = await enrichConsoleJsonWithChangeAudit(prisma, {
        rowBefore: {
          id: row.id,
          tenantId: row.tenantId,
          status: row.status,
        ...
        consoleJsonAfter: stored,
        statusAfter: ticketUpdate.status != null ? String(ticketUpdate.status) : String(row.status || ''),
        stageAfter: ticketUpdate.stage != null ? String(ticketUpdate.stage) : String(row.stage || ''),
        actor: resolveChangeStageActor(req),
        action: 'change-chat',
      });
    } catch {
      ticketUpdate.consoleJson = stored;
    }
    ...
    const storedOut =
      ticketUpdate.consoleJson && typeof ticketUpdate.consoleJson === 'object' ? ticketUpdate.consoleJson : stored;
    const missChat = Array.isArray(storedOut.brief?.missing_information) ? storedOut.brief.missing_information : [];
    const progressStatus = reopenRow ? 'Open' : row.status || 'Open';
    const progressStage = reopenRow ? 'Intake' : row.stage || 'Intake';
    const ticketProgressChat = buildTicketProgress(storedOut, progressStatus, progressStage, {
      ticketId,
      tenantId: row.tenantId,
    });
    const refinement_messages_chat = buildRefinementMessagesTailFromConsoleJson(storedOut);
    return res.status(200).json({
      ok: true,
      ticket_id: ticketId,
      ...
      mode: storedOut.mode || undefined,
      pending_operator_draft: withholdChat.withheld ? storedOut.pending_operator_draft : undefined,
      operator_escalation_reasons: escalationChat.reasons.length ? escalationChat.reasons : undefined,
      brief: storedOut.brief,
      stored_messages: Array.isArray(storedOut.messages) ? storedOut.messages.length : 0,
      source: 'postgres',
      ticket_progress: ticketProgressChat,
      row_status: progressStatus,
      row_stage: progressStage,
      refinement_messages: refinement_messages_chat.length ? refinement_messages_chat : undefined,
      ...
      ticket_reopened_for_refinement: reopenRow === true,
    });
  } catch (e) {
...
async function handleChangeChatCommitDraft(req, res) {
  ...
  try {
    ...
    let storedOut = stored;
    try {
      storedOut = await enrichConsoleJsonWithChangeAudit(prisma, {
        rowBefore: {
          id: row.id,
          tenantId: row.tenantId,
          status: row.status,
        ...
        stageAfter: row.stage || 'Intake',
        actor: resolveChangeStageActor(req),
        action: 'change-chat-commit-draft',
      });
    } catch {
      /* non-blocking audit */
    }
    ...
    await prisma.cmpTicket.update({
      where: { id: ticketId },
      data: {
        consoleJson: storedOut,
        locale: storedOut.locale,
        brief: storedOut?.brief?.summary ? String(storedOut.brief.summary) : undefined,
      },
    });

    const ticketProgress = buildTicketProgress(storedOut, row.status || 'Open', row.stage || 'Intake', {
      ticketId,
      tenantId: row.tenantId,
    });
    return res.status(200).json({
...
async function handleOverseerSweep(req, res) {
  ...
  let limit = Math.min(50, Math.max(1, parseInt(String(limitRaw || '10'), 10) || 10));
  limit = lowCostCap(limit, 3);
