...
async function handleTicketOperatorQueue(req, res) {
  ...
  const hostScope = resolveTenantFromHost(req);
  const isCoreHost = hostScope && hostScope.surface === 'core';
  if (listScope.kind !== 'tenant' && !(listScope.kind === 'core' && isCoreHost)) {
    return res.status(403).json({
      error: 'TICKET_OPERATOR_QUEUE_DENIED',
      hint:
        listScope.kind === 'core'
          ? 'Operator queue on core requires a core hostname (core.corpflowai.com).'
          : 'Operator queue is tenant-scoped. Sign in on a tenant hostname to view the operator queue.',
    });
  }
  ...
  try {
    ...
    return res.status(200).json({
      ok: true,
      list_scope: listScope.kind === 'core' ? 'core' : 'tenant',
      scope_tenant_id: listScope.kind === 'core' ? null : listScope.tenantId,
      count: items.length,
      tickets: items,
    });
  } catch (e) {
    emitLogicFailure({
      ...
      meta: { tenant_id: listScope.kind === 'tenant' ? listScope.tenantId : null },
    });
