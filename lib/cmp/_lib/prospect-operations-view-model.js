...
/** Canonical Prospect Action Queue (#995). Replaces /admin/rapid-delivery as the UX owner. */
export const OPERATING_ACTION_QUEUE_PATH = '/app/queue';

/** Shared Prospect Workbench inside the Operating Workspace (#996 / #721 Slice 3). */
export const OPERATING_WORKBENCH_PATH = '/app/workbench';

/**
 * Action Queue filter ids. Default `needs_action` answers: what needs action now?
 * @type {readonly string[]}
 */
export const ACTION_QUEUE_FILTERS = Object.freeze([
  'needs_action',
  'all',
  'new',
  'overdue',
...
/** Signals that mean the operator should act now (default queue). */
export const ACTION_QUEUE_NEEDS_ACTION_SIGNALS = Object.freeze([
  'overdue_action',
  'due_today',
  'no_next_action',
  'missing_owner',
...
const ACTION_QUEUE_FILTER_TO_SIGNAL = Object.freeze({
  new: 'new_unreviewed',
  overdue: 'overdue_action',
  due_today: 'due_today',
  no_next_action: 'no_next_action',
  awaiting_prospect: 'awaiting_prospect',
  awaiting_operator: 'awaiting_operator',
  awaiting_protected_approval: 'awaiting_protected_approval',
});

/**
 * Standard reusable Workbench filters. Not a saved-view platform.
 * @type {readonly string[]}
 */
export const WORKBENCH_FILTERS = Object.freeze([
  'all',
  'lead_rescue',
  'website_rescue',
  'general',
...
/** Default Workbench sort uses exception priority (same vocabulary as the Action Queue). */
export const WORKBENCH_SORT_PRIORITY = 'priority';

/**
 * Sortable Workbench columns for multi-record processing.
 * @type {readonly string[]}
 */
export const WORKBENCH_SORT_COLUMNS = Object.freeze([
  WORKBENCH_SORT_PRIORITY,
  'prospect',
  'product',
  'owner',
  'stage',
...
export function prospectSourceSurfaces(args = {}) {
  ...
  return {
    ...
    action_queue: OPERATING_ACTION_QUEUE_PATH,
    workbench: OPERATING_WORKBENCH_PATH,
    kanban: '/app/pipeline',
...
/**
 * Labelled Lead Rescue enquiry identity (#1171).
 *
 * The current named-product buyer path posts `meta.product = corpflow-rapid-delivery`
 * with `offer_slug = ai-lead-rescue` (DiscoveryIntakeForm locked offer). That row
 * must still project as Lead Rescue in the canonical Prospect / Commercial views,
 * not as Website Rescue.
 *
 * Do not treat `service_path = client-lead-service` alone as Lead Rescue — that
 * path also covers non-product enquiries (e.g. AI receptionist).
 *
 * @param {unknown} qj
 * @returns {boolean}
 */
export function isLabelledLeadRescueEnquiry(qj) {
  const root = qj && typeof qj === 'object' ? qj : {};
  const intake =
    root.intake_meta && typeof root.intake_meta === 'object'
      ? /** @type {Record<string, unknown>} */ (root.intake_meta)
      : {};
  const offer = String(intake.offer_slug || '').trim();
  const interest = String(intake.service_interest || '').trim();
  const ctx = intake.lead_rescue_context;
  if (offer === 'ai-lead-rescue' || offer === AI_LEAD_RESCUE_PRODUCT) return true;
  if (interest === 'lead_rescue') return true;
  if (ctx === true || ctx === 'true' || ctx === 1 || ctx === '1') return true;
  return false;
}
...
export function detectProspectProduct(qj) {
  ...
  if (isLabelledLeadRescueEnquiry(qj)) return AI_LEAD_RESCUE_PRODUCT;
...
export function computeProspectExceptionSignals(row, now = new Date(), opts = {}) {
  ...
  if (!closed && !owner) signals.push('missing_owner');
 ...
 * @returns {string}
 */
export function normalizeActionQueueFilter(raw) {
  const f = String(raw || '')
    .trim()
    .toLowerCase()
    .replace(/[-\s]+/g, '_');
  if (f === 'new_unreviewed') return 'new';
  if (f === 'awaiting_client') return 'awaiting_prospect';
  if (f === 'awaiting_corpflow' || f === 'awaiting_corpflowai') return 'awaiting_operator';
  if (f === 'awaiting_protected' || f === 'protected_approval') return 'awaiting_protected_approval';
  if (ACTION_QUEUE_FILTERS.includes(f)) return f;
  return 'needs_action';
}

/**
 * Action Queue filter: default needs-action, named signal slices, or all.
 *
 * @param {Record<string, unknown>} row
 * @param {string} [filter]
 * @param {Date} [now]
 * @returns {boolean}
 */
export function matchesActionQueueFilter(row, filter = 'needs_action', now = new Date()) {
  ...
  const signals = Array.isArray(row.exception_signals)
    ...
    : computeProspectExceptionSignals(row, now);
  if (id === 'needs_action') {
    return ACTION_QUEUE_NEEDS_ACTION_SIGNALS.some((signal) => signals.includes(signal));
  }
  const mapped = ACTION_QUEUE_FILTER_TO_SIGNAL[id];
  return mapped ? signals.includes(mapped) : false;
}
 ...
 * @returns {string}
 */
export function normalizeWorkbenchFilter(raw) {
  const f = String(raw || '')
    .trim()
    .toLowerCase()
    .replace(/[-\s]+/g, '_');
  if (f === 'lr' || f === 'ai_lead_rescue' || f === 'ai-lead-rescue') return 'lead_rescue';
  if (f === 'wr' || f === 'rapid_delivery' || f === 'website' || f === 'corpflow-rapid-delivery') {
    return 'website_rescue';
  }
  if (f === 'unknown' || f === 'other' || f === 'market') return 'general';
  if (f === 'overdue_action') return 'overdue';
  if (f === 'stalled_no_activity') return 'stalled';
  if (WORKBENCH_FILTERS.includes(f)) return f;
  return 'all';
}
 ...
 * @returns {string}
 */
export function normalizeWorkbenchSort(raw) {
  const s = String(raw || '')
    .trim()
    .toLowerCase()
    .replace(/[-\s]+/g, '_');
  if (s === 'business' || s === 'name' || s === 'organisation') return 'prospect';
  if (s === 'source' || s === 'product_service_path') return 'product';
  if (s === 'status' || s === 'canonical_stage') return 'stage';
  if (s === 'priority') return WORKBENCH_SORT_PRIORITY;
  if (s === 'next_action_due' || s === 'due_date') return 'due';
  if (s === 'qualification_complete' || s === 'qual') return 'qualification';
  if (s === 'last_meaningful_activity_at' || s === 'activity') return 'last_activity';
  if (s === 'estimated_value') return 'value';
  if (WORKBENCH_SORT_COLUMNS.includes(s)) return s;
  return WORKBENCH_SORT_PRIORITY;
}
 ...
 * @returns {'asc'|'desc'}
 */
export function normalizeWorkbenchSortDir(raw) {
  const d = String(raw || '')
    .trim()
    .toLowerCase();
...
/**
 * Standard Workbench filters: product slices + exception signals. Not saved views.
 *
 * @param {Record<string, unknown>} row
 * @param {string} [filter]
 * @param {Date} [now]
 * @returns {boolean}
 */
export function matchesWorkbenchFilter(row, filter = 'all', now = new Date()) {
  const id = normalizeWorkbenchFilter(filter);
  if (id === 'all') return true;
  const product = String(row?.product || 'unknown');
  if (id === 'lead_rescue') return product === AI_LEAD_RESCUE_PRODUCT;
  if (id === 'website_rescue') return product === RAPID_DELIVERY_PRODUCT;
  if (id === 'general') return product !== AI_LEAD_RESCUE_PRODUCT && product !== RAPID_DELIVERY_PRODUCT;
  const signals = Array.isArray(row.exception_signals)
    ? row.exception_signals
    : computeProspectExceptionSignals(row, now);
  if (id === 'overdue') return signals.includes('overdue_action');
  if (id === 'due_today') return signals.includes('due_today');
  if (id === 'no_next_action') return signals.includes('no_next_action');
  if (id === 'stalled') return signals.includes('stalled_no_activity');
  if (id === 'missing_qualification') return signals.includes('missing_qualification');
  return false;
}
 ...
 * @returns {boolean}
 */
export function matchesWorkbenchQuery(row, q) {
 ...
 */
function workbenchSortValue(row, column) {
  const col = normalizeWorkbenchSort(column);
  if (col === 'prospect') return String(row?.organisation_name || row?.person_name || row?.id || '');
  if (col === 'product') return String(row?.product || row?.product_service_path || '');
  if (col === 'owner') return String(row?.owner || '');
  if (col === 'stage') return String(row?.canonical_stage || row?.native_status || '');
  if (col === 'urgency') return String(row?.urgency || row?.priority || '');
  if (col === 'next_action') return String(row?.next_action || '');
  if (col === 'due') {
    const due = row?.next_action_due ? new Date(String(row.next_action_due)).getTime() : 0;
    return Number.isFinite(due) ? due : 0;
  }
  if (col === 'qualification') return row?.qualification_complete === true ? 1 : 0;
  if (col === 'last_activity') {
    const at = row?.last_meaningful_activity_at
      ? new Date(String(row.last_meaningful_activity_at)).getTime()
      : 0;
...
/**
 * @param {Array<Record<string, unknown>>} rows
 * @param {{ sort?: string, dir?: string }} [opts]
 * @param {Date} [now]
 * @returns {Array<Record<string, unknown>>}
 */
export function sortProspectsForWorkbench(rows, opts = {}, now = new Date()) {
  const list = Array.isArray(rows) ? [...rows] : [];
  const sort = normalizeWorkbenchSort(opts.sort);
  const dir = normalizeWorkbenchSortDir(opts.dir);
  if (sort === WORKBENCH_SORT_PRIORITY) {
    const ordered = sortProspectsForActionQueue(list, now);
    return dir === 'desc' ? ordered.reverse() : ordered;
  }
  list.sort((a, b) => {
    const va = workbenchSortValue(a, sort);
    const vb = workbenchSortValue(b, sort);
    let cmp = 0;
...
export function leadRowToProspectViewModel(row, now = new Date()) {
  ...
  if (product === AI_LEAD_RESCUE_PRODUCT) {
    ...
    const rdOp =
      qj && typeof qj === 'object' && qj.rapid_delivery_operator && typeof qj.rapid_delivery_operator === 'object'
        ? /** @type {Record<string, unknown>} */ (qj.rapid_delivery_operator)
        : {};
    ...
    const nextActionDue = resolveNextActionDue({
      next_action_due: /** @type {any} */ (op).next_action_due || rdOp.next_action_due || null,
      activity,
    });
    const lastActivity =
      activity.length > 0
    ...
    const base = {
      ...
      source: intake.page || intake.source || intake.host || null,
      product_service_path: 'ai-lead-rescue',
      owner: list.owner || rdOp.owner || null,
      native_status: nativeStatus,
      native_status_label: list.status_label,
      canonical_stage: canonicalStage,
      priority: /** @type {any} */ (op).priority || rdOp.priority || null,
      urgency: /** @type {any} */ (op).urgency || intake.urgency || null,
      next_action: list.next_action || rdOp.next_action || null,
      ...
      notes: /** @type {any} */ (op).notes || rdOp.notes || '',
      recommended_next_action: list.next_action || rdOp.next_action || null,
  ...
  const generalBase = {
    ...
    organisation_name: intake.business_name || null,
    email: row?.email || null,
    phone: row?.phone || row?.contact || null,
    source: intake.source || intake.page || intake.host || null,
    product_service_path: intake.service_path || 'general',
    owner: null,
    ...
    qualification_complete: Boolean(intake.business_name && row?.email),
    ...
    offer_slug: null,
    offer_title: null,
    enquiry_channels: intake.enquiry_channels || null,
    notes: '',
    recommended_next_action: 'Review general enquiry and set next action',
    related_refs: {},
    closure_reason: null,
    waiting_on: null,
    ...
    detail_path: `${OPERATING_PROSPECTS_PATH}?id=${encodeURIComponent(String(row?.id || ''))}`,
  ...
  return {
    ...generalBase,
    exception_signals: computeProspectExceptionSignals(generalBase, now),
  };
}
...
function resolveCurrentBlocker(list) {
  ...
  if (signals.includes('missing_owner')) return 'Missing owner';
...
function recommendFromSignals(signals) {
  ...
  if (list.includes('missing_owner')) return 'Assign owner';
