 ...
 *
 * Concierge enquiry progression (issue #673):
 *   new → contacted → qualified → invited → closed
 *
 * Legacy stages from earlier CRM slices are normalized on read (no schema change).
 */
...
/** Operator-facing next-action copy for the current stage (computed, not stored). */
const STAGE_NEXT_ACTION = {
  new: 'Contact the enquirer (email or telephone).',
  contacted: 'Qualify fit and private-client requirements.',
  qualified: 'Send or confirm a private invitation.',
  invited: 'Await response, then close when complete.',
  closed: 'No further action — enquiry closed.',
};
...
export function normalizeLuxLeadCrmStage(s) {
  ...
  const map = {
    ...
    invited: 'invited',
    closed: 'closed',
    // Legacy Phase 3 stages → enquiry progression (read-time only).
    viewing_requested: 'invited',
    'viewing-requested': 'invited',
    viewingrequested: 'invited',
    follow_up: 'contacted',
    'follow-up': 'contacted',
    followup: 'contacted',
    lost: 'closed',
...
/**
 * Clear next-action guidance for the operator desk (issue #673).
 * Prefer a scheduled next_action_note when present; else stage default.
 * @param {string} stage
 * @param {{ next_action_at?: string | null, next_action_note?: string | null, overdue_follow_up?: boolean }} [opts]
 */
export function luxLeadCrmNextActionHint(stage, opts = {}) {
  const k = normalizeLuxLeadCrmStage(stage) || 'new';
  const note = opts.next_action_note != null ? String(opts.next_action_note).trim() : '';
  const at = opts.next_action_at != null ? String(opts.next_action_at).trim() : '';
  if (note || at) {
    const when = at
      ? (() => {
          try {
      ...
      : '';
    const overdue = opts.overdue_follow_up === true ? ' (overdue)' : '';
    if (note && when) return `${note} · due ${when}${overdue}`;
    if (note) return note;
    return `Scheduled follow-up · ${when}${overdue}`;
  }
  return STAGE_NEXT_ACTION[k] || STAGE_NEXT_ACTION.new;
}

/**
 * Split public concierge `contact` (`email | phone`) plus optional phone column / message line.
 * No schema change — uses existing lead.contact / lead.phone / lead.email / lead.message.
 * @param {{ contact?: unknown, phone?: unknown, email?: unknown, message?: unknown }} row
 * @returns {{ email: string | null, phone: string | null, contact_display: string }}
 */
export function parseLuxConciergeContactFields(row) {
  const contactRaw = row?.contact != null ? String(row.contact).trim() : '';
  const phoneCol = row?.phone != null ? String(row.phone).trim() : '';
  ...
  if (contactRaw.includes('|')) {
    ...
    for (const p of parts) {
      ...
      else if (!phone && p.replace(/[^\d+]/g, '').length >= 7) phone = p;
      else if (!email && !p.includes('@') && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(p)) email = p;
    }
    if (!email && parts[0] && parts[0].includes('@')) email = parts[0];
    if (!phone && parts[1]) phone = parts[1];
  } else if (contactRaw.includes('@')) {
    email = contactRaw;
  } else if (contactRaw && contactRaw.replace(/[^\d+]/g, '').length >= 7 && !phone) {
    phone = contactRaw;
  }

  if (!email && emailCol && emailCol.includes('@') && !emailCol.includes('placeholder.local')) {
    email = emailCol;
  } else if (!email && emailCol && emailCol.includes('@') && emailCol.includes('|')) {
    const left = emailCol.split('|')[0].trim();
    if (left.includes('@')) email = left;
  }

  if (!phone) {
    const m = message.match(/^\s*Phone:\s*(.+)$/im);
    if (m && m[1]) phone = String(m[1]).trim().slice(0, 80);
  }

  const emailOut = email ? email.toLowerCase().slice(0, 254) : null;
  const phoneOut = phone ? phone.slice(0, 80) : null;
  const contact_display =
    emailOut && phoneOut ? `${emailOut} · ${phoneOut}` : emailOut || phoneOut || contactRaw || '—';
  return { email: emailOut, phone: phoneOut, contact_display };
}
...
export function computeLuxLeadCrmSignals(ow, opts) {
  ...
  const terminal = ow.stage === 'closed';
...
export function luxOperatorWorkflowForApiList(ow, opts) {
  ...
  return {
    ...
    next_action_hint: luxLeadCrmNextActionHint(ow.stage, {
      next_action_at: ow.next_action_at,
      next_action_note: ow.next_action_note,
...
export function activityKindLabel(kind) {
  ...
  const map = {
    ...
    presentation_updated: 'Confidential presentation updated',
  };
