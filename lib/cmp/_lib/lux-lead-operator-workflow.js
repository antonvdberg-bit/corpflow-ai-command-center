 ...
 *
 * Issue #673 enquiry lifecycle (operator-facing):
 *   new → contacted → qualified → invited → closed
 *
 * Legacy stages written before #673 (viewing_requested, follow_up, lost) still
 * normalize into this path — no DB/schema migration.
 */
...
/** Operator-facing next step for the current stage (computed, not persisted). */
const STAGE_NEXT_ACTION = {
  new: 'Contact the enquirer (email or telephone).',
  contacted: 'Qualify interest and capture requirements.',
  qualified: 'Issue private invitation / next access step.',
  invited: 'Confirm outcome and close the enquiry.',
  closed: 'No further action — enquiry closed.',
};

const STAGE_SUGGESTED_NEXT = {
  new: 'contacted',
  contacted: 'qualified',
  qualified: 'invited',
  invited: 'closed',
...
export function normalizeLuxLeadCrmStage(s) {
  ...
  const map = {
    ...
    invited: 'invited',
    closed: 'closed',
    // Legacy aliases (pre-#673) → current enquiry lifecycle
...
/**
 * Suggested next stage in the enquiry path (null when closed).
 * @param {string} stage
 * @returns {string | null}
 */
export function luxLeadCrmSuggestedNextStage(stage) {
  const k = normalizeLuxLeadCrmStage(stage) || 'new';
  return STAGE_SUGGESTED_NEXT[k] ?? null;
}

/**
 * Human-readable operator next-action hint for the current stage.
 * @param {string} stage
 */
export function luxLeadCrmNextActionHint(stage) {
  const k = normalizeLuxLeadCrmStage(stage) || 'new';
  return STAGE_NEXT_ACTION[k] || STAGE_NEXT_ACTION.new;
}

/**
 * Split concierge `contact` (+ optional phone column) into email / phone for the operator queue.
 * Supports `email | phone` (current /concierge submit) and legacy email-only contact.
 * @param {unknown} contact
 * @param {unknown} [phoneCol]
 * @returns {{ email: string, phone: string, contact_display: string }}
 */
export function parseConciergeContactFields(contact, phoneCol) {
  const raw = contact != null ? String(contact).trim() : '';
  const phoneDirect = phoneCol != null ? String(phoneCol).trim() : '';
  if (raw.includes('|')) {
    const parts = raw.split('|').map((p) => String(p || '').trim()).filter(Boolean);
    const emailPart = parts.find((p) => p.includes('@')) || parts[0] || '';
    const phonePart =
      phoneDirect ||
      parts.find((p) => p !== emailPart && /[\d+]/.test(p)) ||
      parts.slice(1).join(' | ') ||
      '';
    return {
      email: emailPart,
      phone: phonePart,
      contact_display: emailPart || raw,
    };
  }
  return {
...
export function luxOperatorWorkflowForApiList(ow, opts) {
  ...
  const suggested = luxLeadCrmSuggestedNextStage(ow.stage);
  return {
    stage: ow.stage,
    ...
    /** Issue #673 — clear operator next-action state (computed). */
    next_action_hint: luxLeadCrmNextActionHint(ow.stage),
    suggested_next_stage: suggested,
    suggested_next_stage_label: suggested ? luxLeadCrmStageLabel(suggested) : null,
