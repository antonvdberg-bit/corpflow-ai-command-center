...
const ACTIVITY_TYPE_LABEL = Object.freeze({
  ...
  note: 'Note',
  intake_closed_test_data: 'Test intake closed (operator)',
});
...
export function parseIntakeMeta(qj) {
  ...
  const consentRaw = m.consent_contact;
  const consentContact =
    consentRaw === true || consentRaw === 'true' || consentRaw === 1 || consentRaw === '1'
      ? true
      : consentRaw === false || consentRaw === 'false' || consentRaw === 0 || consentRaw === '0'
        ? false
        : null;
  return {
    ...
    website: m.website != null ? String(m.website).trim() : '',
    urgency: m.urgency != null ? String(m.urgency).trim() : '',
    consent_contact: consentContact,
    source: m.source != null ? String(m.source).trim() : '',
...
/** Default operator next action when intake lands and no operator override is stored (#712). */
export const AI_LEAD_RESCUE_DEFAULT_NEXT_ACTION = 'Review and reply within 2 business hours.';

/**
 * Human label for intake urgency / timing (shared market urgency ids).
 * @param {string | null | undefined} urgency
 * @returns {string}
 */
export function aiLeadRescueUrgencyLabel(urgency) {
  const id = String(urgency || '').trim();
  switch (id) {
    case 'asap':
...
export function parseAiLeadRescueOperator(qj) {
  ...
  return {
    ...
    next_action_due: parseOptionalIso(ow.next_action_due),
    ...
    priority: ow.priority != null ? String(ow.priority).trim().slice(0, 64) : null,
    urgency: ow.urgency != null ? String(ow.urgency).trim().slice(0, 64) : null,
    waiting_on: ow.waiting_on != null ? String(ow.waiting_on).trim().slice(0, 64) : null,
    internal_notes,
    activity,
...
export function leadRowToAiLeadRescueListItem(row) {
  ...
  const nextAction =
    op.next_action ||
    (status === 'NEW_INTAKE' ? AI_LEAD_RESCUE_DEFAULT_NEXT_ACTION : null);
  return {
    id: row.id,
    ...
    urgency_label: intake.urgency ? aiLeadRescueUrgencyLabel(intake.urgency) : '',
    consent_contact: intake.consent_contact,
    source: intake.source || intake.page || '',
...
export function mergeAiLeadRescueOperatorPatch(qualificationJson, patch, actorLabel, nowIso) {
  ...
  const next = {
    ...
    next_action_due: prev.next_action_due,
    owner: prev.owner,
  ...
  assign('next_action_due', patch.next_action_due, (v) => parseOptionalIso(v));
  assign('owner', patch.owner, (v) => (v == null || v === '' ? null : String(v).trim().slice(0, 200)));
  assign('priority', patch.priority, (v) => (v == null || v === '' ? null : String(v).trim().slice(0, 64)));
  assign('urgency', patch.urgency, (v) => (v == null || v === '' ? null : String(v).trim().slice(0, 64)));
  assign('waiting_on', patch.waiting_on, (v) => (v == null || v === '' ? null : String(v).trim().slice(0, 64)));
...
export function buildAiLeadRescueIntakeNotification(args) {
  ...
  const regionRaw = String(p.region_path || '').trim();
  const region = regionRaw ? aiLeadRescueRegionPathLabel(regionRaw) : '';
  const sources = String(p.lead_sources || '').trim() || '(not provided)';
  const payment = String(p.preferred_payment_path || '').trim();
  ...
  // Public /lead-rescue no longer collects region or payment path — omit empty
  // "Not selected" noise from operator alerts (keep lines when values exist).
  if (region) lines.push(`Region: ${region}`);
  lines.push(`Lead sources: ${sources}`);
  if (payment) lines.push(`Preferred payment path: ${payment}`);
  lines.push(
    `Submitted at: ${submittedAtIso}`,
    `Admin detail link: ${adminUrl}`,
    `Next action: ${AI_LEAD_RESCUE_DEFAULT_NEXT_ACTION}`,
  );

  const notification_text = lines.join('\n');
  ...
  return {
    ...
    prospect: {
      ...
      email,
      phone,
      region_path: regionRaw || 'not_selected',
      region_label: region || 'Not selected',
      lead_sources: sources,
      preferred_payment_path: payment || '(not selected)',
      source_host: String(p.source_host || '').trim() || null,
    },
    next_action: AI_LEAD_RESCUE_DEFAULT_NEXT_ACTION,
    notification_text,
  };
}
