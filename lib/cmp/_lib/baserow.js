 ...
 * All outbound requests are wrapped in a 3s timeout (Hobby / serverless safety).
 * @see https://baserow.io/user-docs/database-api
 */

import { cfg } from '../../server/runtime-config.js';

/** Neutral default when `BASEROW_URL` is unset (Baserow Cloud). Never commit tenant-specific hosts. */
const DEFAULT_BASE_URL = 'https://api.baserow.io';
export const BASEROW_REQUEST_TIMEOUT_MS = 3000;
 ...
 * @property {string} [tenantFieldName]
 */
 ...
 * @returns {Promise<T>}
 */
export function withBaserowTimeout(promise, ms = BASEROW_REQUEST_TIMEOUT_MS) {
  return Promise.race([
    promise,
    new Promise((_, reject) =>
      setTimeout(
        () => reject(new BaserowError(`Baserow request timed out after ${ms}ms`, 504)),
        ms,
      ),
    ),
  ]);
...
/**
 * Generic row read for any table id (e.g. BASEROW_TENANT_TABLE_ID).
 *
 * @param {object} opts
 * @param {string} opts.tableId
 * @param {string|number} opts.rowId
 * @param {string|null} [opts.tenantId] - when set, enforces tenant field match on row
 * @param {string} [opts.tenantFieldName]
 * @returns {Promise<unknown>}
 */
export async function getBaserowRowGeneric(opts) {
  const client = createBaserowClient({
    defaultTableId: opts.tableId,
    tenantId: opts.tenantId ?? null,
    tenantFieldName: opts.tenantFieldName,
  });
  return client.getRow(opts.tableId, opts.rowId);
}

/**
 * Generic row patch for any table id.
 *
 * @param {object} opts
 * @param {string} opts.tableId
 * @param {string|number} opts.rowId
 ...
 * @returns {Promise<unknown>}
 */
export async function updateBaserowRowGeneric(opts) {
  const client = createBaserowClient({
    defaultTableId: opts.tableId,
    tenantId: opts.tenantId ?? null,
    tenantFieldName: opts.tenantFieldName,
  });
  return client.updateRow(opts.tableId, opts.rowId, opts.fields);
}
...
export function createBaserowClient(options) {
  const baseUrl = (options.baseUrl || cfg('BASEROW_URL', '') || DEFAULT_BASE_URL).replace(/\/$/, '');
  const token = options.token || cfg('BASEROW_TOKEN', '');
  ...
  const defaultTableId =
    ...
    cfg('BASEROW_CMP_TABLE_ID', '') ||
    cfg('BASEROW_TABLE_ID', '') ||
    null;
  ...
  async function request(path, init = {}) {
    ...
    return withBaserowTimeout(inner, BASEROW_REQUEST_TIMEOUT_MS);
  }
...
/**
 * Factory-only: list all rows in a table (no tenant filter). Caller MUST enforce factory master auth.
 *
 * @param {string} tableId
 * @param {{ page?: number, size?: number }} [opts]
 * @returns {Promise<unknown>}
 */
export async function listBaserowTableAllRows(tableId, opts = {}) {
  const baseUrl = (process.env.BASEROW_URL || DEFAULT_BASE_URL).replace(/\/$/, '');
  const token = process.env.BASEROW_TOKEN;
  if (!token) {
    throw new BaserowError('BASEROW_TOKEN is not configured');
  }
  if (!tableId || !String(tableId).trim()) {
    throw new BaserowError('listBaserowTableAllRows: tableId is required');
  }
  const page = opts.page != null ? Number(opts.page) : 1;
  const size = Math.min(500, Math.max(1, opts.size != null ? Number(opts.size) : 200));
  const params = new URLSearchParams();
  params.set('user_field_names', 'true');
  params.set('page', String(page));
  params.set('size', String(size));
  const path = `/api/database/rows/table/${String(tableId)}/?${params.toString()}`;
