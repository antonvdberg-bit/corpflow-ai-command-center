...
/**
 * Canonical tenant-relative URL for published Lux property image bytes (Phase 5A).
 ...
 * @param {{ variant?: string | null }} [opts] — explicit variant token; must be allowlisted if set
 * @returns {string}
 */
export function buildLuxPublicPropertyMediaSrc(propertyRef, attachmentId, normalizedSlot, opts = {}) {
  ...
  return `/api/lux/property-media?property=${ref}&attachment=${aid}&slot=${sl}&variant=${encodeURIComponent(variantToken)}`;
}
