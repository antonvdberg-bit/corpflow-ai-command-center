 ...
 *
 * Missing Vercel preview is not a corpflow_test gap (#973).
 */
...
import { shouldFlagMissingVercelPreview } from '../../server/corpflow-test-evidence-policy.js';
...
export function evaluateTechnicalLeadChecklistV1(evidence) {
  ...
  if (ve.skipped === true) {
    ...
    if (
      ...
      dispatchOk === true &&
      dfb === 0 &&
      shouldFlagMissingVercelPreview(evidence.environment)
    ) {
      gaps.push({
        id: 'vercel_preview_missing',
        severity: 'warning',
        detail: `No preview deployment found for branch ${String(ve.branch || 'cmp/…')} (preview-sandbox packets only; corpflow_test does not require *.vercel.app).`,
      });
    }
