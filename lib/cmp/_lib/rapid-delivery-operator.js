...
export function mergeRapidDeliveryOperatorPatch(existing, patch, nowIso, actor) {
  ...
  const assignTrimmed = (key, val, max) => {
    if (val === undefined) return;
    if (val == null || val === '') next[key] = null;
    else next[key] = String(val).trim().slice(0, max);
  };
  assignTrimmed('owner', patch.owner, 200);
  assignTrimmed('next_action', patch.next_action, 500);
  assignTrimmed('priority', patch.priority, 64);
  assignTrimmed('urgency', patch.urgency, 64);
  assignTrimmed('waiting_on', patch.waiting_on, 64);
  if (patch.next_action_due !== undefined) {
    if (patch.next_action_due == null || String(patch.next_action_due).trim() === '') {
      next.next_action_due = null;
    } else {
      const d = new Date(String(patch.next_action_due));
      next.next_action_due = Number.isNaN(d.getTime()) ? next.next_action_due || null : d.toISOString();
    }
  }
  if (patch.note_append != null && String(patch.note_append).trim()) {
    const extra = String(patch.note_append).trim().slice(0, 4000);
    const prior = String(next.notes || '').trim();
    next.notes = [prior, extra].filter(Boolean).join('\n\n').slice(0, 4000);
  }
  next.updated_at = nowIso;
  const activity = Array.isArray(next.activity) ? [...next.activity] : [];
  const activityNote =
    patch.note_append != null && String(patch.note_append).trim()
      ? String(patch.note_append).trim().slice(0, 200)
      : patch.notes != null
        ? String(patch.notes).slice(0, 200)
        : undefined;
  activity.push({
    at: nowIso,
    ...
    status: next.status,
    note: activityNote,
    next_action: next.next_action || undefined,
    next_action_date: next.next_action_due || undefined,
  });
...
export function leadRowToRapidDeliveryDetail(row) {
  ...
  return {
    ...
    intake_meta: {
      ...
      page: meta.page ?? null,
      host: meta.host ?? null,
      buyer_need: meta.buyer_need ?? null,
      service_interest: meta.service_interest ?? null,
      locked_product: meta.locked_product ?? null,
      discovery_form: meta.discovery_form ?? null,
    },
...
export function buildRapidDeliveryProposalSummary(row) {
  ...
  const nextSteps = [
    'Confirm fit on /app/prospects (Operating Workspace). Product desks remain at /admin/rapid-delivery.',
  ...
  const sections = {
    ...
    starting_price: offer?.priceIsFixed ? mur : `from ${mur}`,
