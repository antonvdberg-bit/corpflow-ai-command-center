...
/** @typedef {'intake'|'refining'|'ready_for_estimate'|'estimated'|'approved_for_build'|'awaiting_client_programme_decisions'|'building'|'preview_ready'|'in_review'|'changes_requested'|'client_approved'|'publishing'|'published'|'closed'} ChangeWorkflowState */
...
export function normalizeWorkflowState(s) {
  ...
  if (
    ...
    v === 'awaiting_client_programme_decisions' ||
    v === 'building' ||
    v === 'preview_ready' ||
    v === 'in_review' ||
    v === 'changes_requested' ||
...
/**
 * Multi-system programme tickets may persist `console_json.client_decisions` with required answers.
 * While required answers are incomplete, Approved/Build should not present as "waiting for automation"
 * even if no sandbox dispatch has occurred yet.
 *
 * @param {Record<string, unknown>} cj
 * @returns {boolean}
 */
function clientDecisionsGateBlocksBuildStart(cj) {
  const cd = cj.client_decisions && typeof cj.client_decisions === 'object' ? cj.client_decisions : null;
  if (!cd) return false;
  const items = Array.isArray(cd.items) ? cd.items : [];
  if (!items.length) return false;
  if (cd.sufficient_to_proceed === true) return false;
  return true;
}
...
export function deriveWorkflowState(args) {
  ...
  if (explicit && !isApprovedBuildPath(args.status, args.stage)) return explicit;
  ...
  if (approvedBuild) {
    if (clientDecisionsGateBlocksBuildStart(cj)) return 'awaiting_client_programme_decisions';
...
export function nextActionForWorkflowState(st) {
  switch (st) {
    ...
    case 'ready_for_estimate':
      return 'Run Estimate.';
    case 'estimated':
      return 'Review the estimate and proceed when ready.';
    ...
    case 'awaiting_client_programme_decisions':
      return 'Answer the programme decisions so we can safely plan the first live slice.';
    case 'building':
...
export function buildTicketProgress(consoleJson, status, stage, auditOpts) {
  ...
  const luxProg = cj.lux_programme && typeof cj.lux_programme === 'object' ? cj.lux_programme : {};
  const customNext =
    luxProg.operator_next_action != null && String(luxProg.operator_next_action).trim()
      ? String(luxProg.operator_next_action).trim()
      : '';
  const workflow_next_action = customNext || nextActionForWorkflowState(wf);

  return {
    status: status || '',
    stage: stage || '',
    client_view: { ...cv, workflow_state: wf, workflow_next_action },
  };
}
