...
export async function dispatchCmpSandboxStart(opts) {
  ...
  try {
    ...
    return {
      ...
      repo_full_name: owner && repo ? `${owner}/${repo}` : undefined,
...
export async function fetchCmpTicketActivity(opts) {
  ...
  /** When the sandbox branch was deleted after a merged PR, absence is normal — avoid false "automation broken" alerts. */
  let merged_sandbox_pr = null;
  if (branch_exists === false && token && owner && repo) {
    try {
      const head = `${owner}:${branchName}`;
      const pullsUrl = `https://api.github.com/repos/${owner}/${repo}/pulls?state=all&head=${encodeURIComponent(head)}&base=${encodeURIComponent(baseRef)}&per_page=10`;
      const pulls = await githubApiJson(pullsUrl, token);
      if (Array.isArray(pulls)) {
        const m = pulls.find((p) => p.merged_at);
        if (m) {
          merged_sandbox_pr = {
  ...
  if (branch_exists === false) {
    if (merged_sandbox_pr) {
      needs_attention = false;
      const prBit = merged_sandbox_pr.html_url ? ` ${merged_sandbox_pr.html_url}` : '';
      summary_line = `Sandbox branch ${branchName} was removed after merge into ${baseRef} (expected).${prBit}`;
    } else {
      summary_line =
      ...
      if (diag) {
        ...
        if (diag.conclusion === 'failure' || diag.conclusion === 'timed_out') {
          ...
          const hbStr = String(diag.head_branch || '');
          if (hbStr === baseRef || hbStr === 'main' || hbStr === 'master') {
            needs_attention = false;
            summary_line = `CMP workflow completed on "${hbStr}"; ${branchName} is absent — usual after merge and deleting the sandbox branch.`;
          } else {
            needs_attention = true;
  ...
  return {
    ...
    branch_exists,
    base_ref: baseRef,
    merged_sandbox_pr: merged_sandbox_pr,
    actions_filter_url,
    workflow_filter_url,
...
/**
 * Add a label to a PR (issue) by PR number.
 *
 * @param {{ prNumber: number, labels: string[] }} opts
 * @returns {Promise<{ ok: boolean, error?: string }>}
 */
export async function addLabelsToCmpPullRequest(opts) {
  const { token, owner, repo } = resolveGithubDispatchAuth();
  if (!token || !owner || !repo) {
    return {
      ok: false,
  ...
  const labels = Array.isArray(opts.labels)
    ? opts.labels.map((s) => String(s || '').trim()).filter(Boolean).slice(0, 10)
    : [];
  if (!labels.length) return { ok: false, error: 'labels are required' };
