 ...
 * @param {{
 *   attachmentCountByTicketId?: Map<string, number>,
 *   tenantHeroByTenantId?: Map<string, { hero_image_url: string | null }>,
 * }} [queryCache] Same-request cache: avoids duplicate Prisma reads when computing “before” and “after” panels for one ticket.
 * @returns {Promise<Record<string, unknown>>}
 */
export async function computeRealityPanel(prisma, ticketRow, queryCache) {
  ...
  const attMap = queryCache && typeof queryCache === 'object' ? queryCache.attachmentCountByTicketId : undefined;
  if (attMap && attMap.has(ticketId)) {
    attachmentCount = attMap.get(ticketId) || 0;
  } else {
    try {
      attachmentCount = await prisma.cmpTicketAttachment.count({ where: { ticketId } });
      if (attMap) attMap.set(ticketId, attachmentCount);
    } catch {
      attachmentCount = 0;
      if (attMap) attMap.set(ticketId, 0);
    }
  }
  ...
  const heroMap = queryCache && typeof queryCache === 'object' ? queryCache.tenantHeroByTenantId : undefined;
  if (tenantId) {
    if (heroMap && heroMap.has(tenantId)) {
      prodHero = heroMap.get(tenantId) || { hero_image_url: null };
    } else {
      try {
        ...
        if (heroMap) heroMap.set(tenantId, prodHero);
      } catch {
