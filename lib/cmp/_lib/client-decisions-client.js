...
} from '../../client/lux-product-direction-content.js';
...
/** Luxe Maurice recovery programme ticket — Release 1 roadmap + decision capture (#544). */
export const LUX_RECOVERY_ROADMAP_TICKET_ID = 'cmr7a244f0000l505x5vne2s0';

/** Tickets that use recovery Release 1 client roadmap decision surface. */
export const LUX_RECOVERY_ROADMAP_TICKET_IDS = new Set([LUX_RECOVERY_ROADMAP_TICKET_ID]);
...
export const RECOVERY_LUX_CLIENT_DECISION_KEYS = [
  'lux_product_direction_overall',
  ...LUX_PRODUCT_DIRECTION_PILLAR_KEYS,
  'lux_product_direction_misunderstood',
];

/** @type {Record<string, string>} */
export const RECOVERY_LUX_CLIENT_DECISION_QUESTIONS = {
  lux_product_direction_overall:
    'Confirm or correct our understanding of the LuxeMaurice product direction as summarised below.',
  lux_product_direction_misunderstood: 'What have we misunderstood?',
  ...Object.fromEntries(
    LUX_PRODUCT_DIRECTION_PILLARS.map((p) => [
      p.key,
      `${p.title} — your priority choice`,
    ]),
  ),
};

export const RECOVERY_LUX_HEADING = 'Product direction confirmation';

export const RECOVERY_LUX_EXPLANATION =
  'Review our working understanding of each product pillar. Tell us what belongs in a first release, what can wait, and what we have wrong.';

export const RECOVERY_LUX_THANK_YOU =
  "Thank you — we've received your product direction confirmation. Your team will review and follow up.";
export const RECOVERY_LUX_CLIENT_DECISION_KEYS = ['lux_recovery_release1_decision', 'lux_recovery_release1_notes'];

/** @type {Record<string, string>} */
export const RECOVERY_LUX_CLIENT_DECISION_QUESTIONS = {
  lux_recovery_release1_decision: 'Do you approve Release 1 — First Real Opportunity — as proposed below?',
  lux_recovery_release1_notes:
    'Notes, changes requested, or concerns (optional if you fully approve — you can write “none” or leave blank).',
};

export const RECOVERY_LUX_HEADING = 'Your LuxeMaurice recovery roadmap';

export const RECOVERY_LUX_EXPLANATION =
  'Review the proposed first release below. When you are ready, tell us whether to proceed, request changes, or hold — and add any notes.';

export const RECOVERY_LUX_THANK_YOU =
  "Thank you — we've received your decision on Release 1. Your team will review and follow up.";

/** Client-safe paths for one-time decision links. */
export const ALLOWED_CLIENT_DECISION_PATHS = new Set(['/client/change-decisions', '/client/recovery-roadmap']);
 ...
 * @returns {string}
 */
export function resolveClientDecisionsMintPath(ticketId, requestedPath) {
  const req = String(requestedPath || '').trim();
  if (req && ALLOWED_CLIENT_DECISION_PATHS.has(req)) return req;
  if (LUX_RECOVERY_ROADMAP_TICKET_IDS.has(String(ticketId || '').trim())) {
    return '/client/recovery-roadmap';
  }
  return '/client/change-decisions';
}

/**
 * @param {string} ticketId
 * @param {Record<string, unknown>} operator_signal
 * @param {Record<string, { answer?: unknown, waive?: unknown }>} filteredAnswers
 * @returns {Record<string, unknown>}
 */
export function enrichOperatorSignalForTicket(ticketId, operator_signal, filteredAnswers) {
  const id = String(ticketId || '').trim();
  if (!LUX_RECOVERY_ROADMAP_TICKET_IDS.has(id)) return operator_signal;
  const overallRaw = filteredAnswers?.lux_product_direction_overall;
  const notesRaw = filteredAnswers?.lux_product_direction_misunderstood;
  const overall =
    overallRaw && typeof overallRaw === 'object' && typeof overallRaw.answer === 'string'
      ? overallRaw.answer.trim()
  const decisionRaw = filteredAnswers?.lux_recovery_release1_decision;
  const notesRaw = filteredAnswers?.lux_recovery_release1_notes;
  const decision =
    decisionRaw && typeof decisionRaw === 'object' && typeof decisionRaw.answer === 'string'
      ? decisionRaw.answer.trim()
      : '';
  const client_notes =
    notesRaw && typeof notesRaw === 'object' && typeof notesRaw.answer === 'string'
      ? notesRaw.answer.trim()
      : '';
  /** @type {Record<string, string>} */
  const overallLabels = {
    confirm: 'Confirm product direction understanding',
    correct: 'Correct product direction — priorities need changes',
  };
  const pillarNeedsCorrection = LUX_PRODUCT_DIRECTION_PILLAR_KEYS.some((key) => {
    const raw = filteredAnswers?.[key];
    const ans =
      raw && typeof raw === 'object' && typeof raw.answer === 'string' ? raw.answer.trim() : '';
    return ans === 'needs_correction';
  });
  return {
    ...operator_signal,
    type: 'client_product_direction_confirmation',
    decision_set_id: 'client_product_direction_confirmation',
    source_page: '/client/recovery-roadmap',
    decision_status: overall || 'pending',
    decision_label: overallLabels[overall] || overall || 'Pending',
    client_notes: client_notes || null,
    anton_action_required:
      overall === 'correct' || pillarNeedsCorrection || Boolean(client_notes),
  const labels = {
    approve: 'Approve Release 1 scope',
    request_changes: 'Request changes',
    not_approved: 'Not approved — explain concerns',
  };
  return {
    ...operator_signal,
    source_page: '/client/recovery-roadmap',
    decision_status: decision || 'pending',
    decision_label: labels[decision] || decision || 'Pending',
    client_notes: client_notes || null,
    anton_action_required: decision === 'request_changes' || decision === 'not_approved',
  };
}

/**
 * Tenant-safe client decision answers for operator `/change` (no internal_decisions).
 *
 * @param {string} ticketId
 * @param {Array<Record<string, unknown>>} items
 * @returns {Array<{ key: string, question: string, answer: string, status: string, answered_at: string|null }>|undefined}
 */
export function clientDecisionsAnswersForOperator(ticketId, items) {
  const id = String(ticketId || '').trim();
  const spec = getClientDecisionSpecForTicket(id);
  ...
  const rows = (Array.isArray(items) ? items : [])
    .map((it) => {
      ...
      return {
        key,
        question: typeof o.question === 'string' ? o.question : key,
        answer: typeof o.answer === 'string' ? o.answer : '',
        status: typeof o.status === 'string' ? o.status : 'pending',
        answered_at: o.answered_at != null ? String(o.answered_at) : null,
      };
    })
    .filter(Boolean);
...
export function getClientDecisionSpecForTicket(ticketId) {
  const id = String(ticketId || '').trim();
  if (LUX_RECOVERY_ROADMAP_TICKET_IDS.has(id)) {
  /** @type {Record<string, Array<{ value: string, label: string }>>} */
    const pillarSelectOptions = Object.fromEntries(
      LUX_PRODUCT_DIRECTION_PILLAR_KEYS.map((key) => [key, LUX_PILLAR_CLIENT_CHOICES]),
    );
    return {
      id: 'client_product_direction_confirmation',
    return {
      id: 'lux_recovery_release1',
      keys: [...RECOVERY_LUX_CLIENT_DECISION_KEYS],
      questions: { ...RECOVERY_LUX_CLIENT_DECISION_QUESTIONS },
      heading: RECOVERY_LUX_HEADING,
      explanation: RECOVERY_LUX_EXPLANATION,
      thank_you_message: RECOVERY_LUX_THANK_YOU,
      waiveKeys: new Set(),
      optionalGateKeys: new Set(['lux_product_direction_misunderstood']),
      selectOptions: {
        lux_product_direction_overall: [
          { value: '', label: 'Choose one…' },
          { value: 'confirm', label: 'Confirm our understanding of the product direction' },
          { value: 'correct', label: 'Correct our understanding — priorities need changes' },
        ],
        ...pillarSelectOptions,
      optionalGateKeys: new Set(['lux_recovery_release1_notes']),
      selectOptions: {
        lux_recovery_release1_decision: [
          { value: '', label: 'Choose one…' },
