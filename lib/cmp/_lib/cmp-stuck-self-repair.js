...
import { isLowCostMode } from '../../server/low-cost-mode.js';
...
export async function runStuckSelfRepair(prisma, options = {}) {
  ...
  let sweepLimit = Math.min(50, Math.max(1, options.limit != null ? options.limit : cfgSweepLimit));
  if (isLowCostMode()) sweepLimit = Math.min(sweepLimit, 3);
