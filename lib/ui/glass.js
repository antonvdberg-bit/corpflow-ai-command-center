...
export function glassElevationShadow(elevation = 2) {
  const drop =
    {
      1: '0 12px 34px rgba(0,0,0,0.30)',
      2: '0 24px 64px rgba(0,0,0,0.42)',
      3: '0 40px 104px rgba(0,0,0,0.52)',
    }[elevation] || '0 24px 64px rgba(0,0,0,0.42)';
  // drop (depth) + crisp top light edge + hairline rim + soft inner bottom
  // shade so the lower lip of the glass reads as thickness, not a flat card.
  return [
    drop,
    'inset 0 1px 0 rgba(255,255,255,0.22)',
    'inset 0 0 0 1px rgba(255,255,255,0.05)',
    'inset 0 -22px 44px rgba(0,0,0,0.12)',
  ].join(', ');
}
 ...
 *
 * @param {{fill?:string,border?:string,blur?:string,radius?:number,padding?:number|string,shadow?:string,color?:string,elevation?:1|2|3,sheen?:string|false}} [opts]
...
export function glassPanelStyle(opts = {}) {
  ...
  return {
    ...
    background,
    boxShadow: opts.shadow || glassElevationShadow(opts.elevation ?? 2),
