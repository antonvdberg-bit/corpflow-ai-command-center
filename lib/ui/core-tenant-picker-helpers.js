...
/**
 * Merge server `acting_tenant_id` with a client override after switch/leave.
 * `undefined` override means "use prop"; `null` override means explicitly
 * not acting on any tenant (after leave).
 *
 * @param {string | null | undefined} overrideActingTenantId
 * @param {string | null | undefined} propActingTenantId
 * @returns {string | null}
 */
export function resolveEffectiveActingTenantId(overrideActingTenantId, propActingTenantId) {
  if (overrideActingTenantId !== undefined) {
    return overrideActingTenantId != null ? String(overrideActingTenantId) : null;
  }
  if (propActingTenantId != null && String(propActingTenantId).trim()) {
    return String(propActingTenantId).trim();
  }
  return null;
}
