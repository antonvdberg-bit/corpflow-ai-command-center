...
export function parseCipcEnquiryFromIntake(emailText, body) {
  ...
  const company = str(extra.company || extra.firm || extra.business_name || line('Firm'));
  ...
  const asked = str(extra.need || extra.asked || extra.message || line('Approximate client portfolio or immediate need')) || text;
  ...
  if (!source) {
    if (campaignProspectId || /^campaign:/i.test(text)) source = 'campaign';
      ...
      source = 'partner_web';
    } else if (/annual returns review|director changes review|beneficial ownership review/i.test(text)) {
