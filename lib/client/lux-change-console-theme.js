  ...
  return {
    ...
    badge(kind) {
      const map = {
        ...
        active_client: { bg: 'rgba(59,130,246,0.08)', border: '#3b82f6', color: '#1e40af' },
        client_request: { bg: 'rgba(59,130,246,0.08)', border: '#3b82f6', color: '#1e40af' },
        crm_leads: { bg: 'rgba(14,165,233,0.1)', border: '#0ea5e9', color: '#0369a1' },
        property_media: { bg: 'rgba(34,197,94,0.1)', border: '#22c55e', color: '#14532d' },
        media: { bg: 'rgba(168,85,247,0.08)', border: '#a855f7', color: '#6b21a8' },
        property: { bg: 'rgba(34,197,94,0.1)', border: '#22c55e', color: '#14532d' },
        internal: { bg: 'rgba(100,116,139,0.1)', border: '#94a3b8', color: '#64748b' },
        archived_smoke: { bg: 'rgba(100,116,139,0.12)', border: '#94a3b8', color: '#475569' },
    ...
    /** Readable action + estimate tokens for the Lux ivory/sand desk (not dark-operator slate). */
    deskInk: {
      sectionLabel: T.heroDeep,
      body: T.ink,
      muted: T.inkMuted,
      success: '#166534',
      successSoft: '#14532d',
      ...
      infoSoft: '#1d4ed8',
      danger: '#9f1239',
      dangerBg: 'rgba(190,18,60,0.10)',
      dangerBorder: 'rgba(190,18,60,0.35)',
      footnoteBg: 'rgba(28,25,23,0.04)',
      footnoteBorder: T.border,
      /** @param {boolean} busy */
      withdrawBtn(busy) {
        return {
          padding: '8px 12px',
          borderRadius: T.radiusMd,
          border: `1px solid ${busy ? T.border : 'rgba(190,18,60,0.45)'}`,
          background: busy ? T.sand : 'rgba(190,18,60,0.10)',
          color: busy ? T.inkMuted : '#9f1239',
          fontWeight: 800,
          fontSize: 12,
          cursor: busy ? 'not-allowed' : 'pointer',
        };
      ...
      proceedBtn(disabled) {
        return {
          padding: '12px 14px',
          borderRadius: T.radiusLg,
          border: `1px solid ${disabled ? T.border : 'rgba(22,163,74,0.45)'}`,
          background: disabled ? T.sand : 'rgba(22,163,74,0.12)',
          color: disabled ? T.inkMuted : '#14532d',
          textAlign: 'left',
      ...
      changeRequestBtn(busy) {
        return {
          ...
          background: busy ? T.sand : T.white,
          color: T.heroDeep,
          textAlign: 'left',
          cursor: busy ? 'not-allowed' : 'pointer',
        };
      ...
      estimateCtaBtn(disabled) {
        return {
          padding: '10px 14px',
          borderRadius: T.radiusLg,
          border: `1px solid ${disabled ? T.border : T.goldDeep}`,
          background: disabled ? T.sand : 'rgba(168,132,44,0.12)',
          color: disabled ? T.inkMuted : T.goldDeep,
          fontWeight: 900,
          fontSize: 12,
          cursor: disabled ? 'not-allowed' : 'pointer',
        };
