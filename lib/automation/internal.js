...
export async function recordTrustedAutomationEvent(prisma, opts) {
  ...
  const actorUserId =
    opts.actorUserId != null && String(opts.actorUserId).trim()
      ? String(opts.actorUserId).trim()
      : null;
  ...
  try {
    ...
    try {
      console.error('[recordTrustedAutomationEvent] write_failed', {
        event_type: eventType,
        tenant_scope: tenantScope,
        actor_user_id_set: Boolean(actorUserId),
        source,
