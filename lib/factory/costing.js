...
import { isBillingExemptAllTenantsEnv, isBillingExemptEnvOverride } from '../server/billing-exempt.js';
...
export async function getTenantWalletSnapshot({ tenantId }) {
  ...
  const exemptAll = isBillingExemptAllTenantsEnv();
  ...
  if (!hasPostgres()) {
    return {
      ...
      billingExemptEffective: exemptAll || envExempt,
    };
  }
  ...
  try {
    ...
    return {
      ...
      billingExemptEffective: exemptAll || envExempt || dbExempt,
    };
  } catch (_) {
    return {
      tokenCreditBalanceUsd: fileBalance,
