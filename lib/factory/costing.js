...
export async function getTokenCreditBalance({ tenantId }) {
  ...
  if (hasPostgres()) {
    try {
      ...
      if (row && Number.isFinite(Number(row.tokenCreditBalanceUsd))) {
        return Number(row.tokenCreditBalanceUsd);
      }
      const seedUsd = initialWalletUsdForNewPersona(tid, fileBalance, defaultBalance);
      // If missing, initialize from file/default so new installs converge to DB.
