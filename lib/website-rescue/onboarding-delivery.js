 ...
 * CJS-wrapped factory_router graph (#1015 / #716).
 */
 ...
 */
export function loadWebsiteRescueOnboardingDeliveryConfig(repoRoot = process.cwd()) {
  if (cachedConfig && repoRoot === process.cwd()) return cachedConfig;
  const raw = readFileSync(path.join(repoRoot, CONFIG_REL), 'utf8');
  const parsed = JSON.parse(raw);
  if (repoRoot === process.cwd()) cachedConfig = parsed;
  return parsed;
}
