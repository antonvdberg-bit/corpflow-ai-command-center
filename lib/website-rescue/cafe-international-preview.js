...
/**
 * Build priced menu rows from preferred category ids (fixture-backed only).
 * Does not invent demand/sales claims — selection is presentation order only.
 */
function selectCafeInternationalPricedRows(menu, preferredIds, limit) {
  const categories = Array.isArray(menu?.categories) ? menu.categories : [];
...
/**
 * Homepage featured favourites — real fixture items, not invented best-sellers.
 * Prefers grill / starters / burgers / platters for appetite-led preview.
 */
export function selectCafeInternationalHomeMenuPreview(menu, limit = 6) {
  return selectCafeInternationalPricedRows(
    menu,
    ['from-our-grill', 'starters', 'build-a-burger', 'platters'],
    limit,
  );
}

/**
 * Takeaway featured picks — platters first for local collection use-case,
 * then grill / burger favourites. Presentation order only; no demand claims.
 */
export function selectCafeInternationalTakeawayFeatured(menu, limit = 6) {
  return selectCafeInternationalPricedRows(
    menu,
    ['platters', 'from-our-grill', 'build-a-burger', 'starters'],
    limit,
  );
}
