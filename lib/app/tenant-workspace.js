...
export const TENANT_WORKSPACE_ROUTE_MATRIX = Object.freeze([
  ...
  Object.freeze({
    ...
    disposition: 'STAFF_ONLY_FAIL_CLOSED',
    reason: 'Operating Workspace only. Tenant session cannot see cross-client delivery oversight.',
  }),
]);
...
export const OPERATING_WORKSPACE_STAFF_PATHS = Object.freeze([
  ...new Set([
    ...
    '/api/app/commercial-quotation-pdf',
    '/api/app/delivery',
    '/api/app/overview',
  ]),
]);
...
/**
 * #1201 — Requests & Progress panel kind.
 * A list-load failure must not collapse into the empty “no work yet” panel,
 * which a client could read as false completion.
 *
 * @typedef {'loading'|'error'|'empty'|'ready'} TenantProgressPanelKind
 */

export const TENANT_PROGRESS_LOADING_TITLE = 'Loading Requests & Progress…';
export const TENANT_PROGRESS_LIST_ERROR_TITLE = 'Requests & Progress unavailable';
export const TENANT_PROGRESS_LIST_ERROR_BODY =
  'We could not load your requests. This does not mean your work is complete. Retry, or use Service & change if you need to reach CorpFlowAI.';
 ...
 *   busy?: boolean,
 * }} args
 * @returns {TenantProgressPanelKind}
 */
export function tenantProgressPanelKind(args = {}) {
  const listReady = args.listReady === true;
  if (!listReady) return 'loading';
  const count = Number(args.requestCount) || 0;
  const hasError = Boolean(String(args.error || '').trim());
  if (count === 0 && hasError) return 'error';
  if (count === 0) return 'empty';
  return 'ready';
}

/**
 * Loading / empty / list-error copy must not claim the delivery is finished.
 * @param {unknown} text
 * @returns {boolean}
 */
export function tenantPanelCopyImpliesFalseCompletion(text) {
  const s = String(text || '').toLowerCase();
  if (!s.trim()) return false;
  if (s.includes('does not mean your work is complete')) return false;
  return (
    s.includes('live verified') ||
    s.includes('delivery is complete') ||
    s.includes('no remaining client action') ||
    /\bwork is complete\b/.test(s)
  );
}
