/**
 * Core/Tenant access — separately authenticated environments (Slice 1 + Slice 2).
 *
 * AUTHORITATIVE:
 ...
 * - Slice 2: normal authenticated sessions are the operator path; proof is harness-only.
 */
...
export function environmentDescriptorForActor(actor) {
  if (canAccessCore(actor)) {
    return { environment: 'core', label: OPERATING_WORKSPACE_LABEL, tenant_id: null };
  }
  const tid = actor.tenant_id || actor.can_tenant_ids[0] || REFERENCE_TENANT_ID;
  return {
    environment: 'tenant',
    label: tid === REFERENCE_TENANT_ID ? TENANT_WORKSPACE_LABEL : `Tenant Workspace — ${tid}`,
    tenant_id: tid,
  };
}
...
/**
 * Resolve the tenant id a Tenant actor may use for this request.
 * Query/body tenant is accepted only when authorised; otherwise primary session tenant.
 * Never silently grants a tenant outside can_tenant_ids.
 *
 * @param {AppActor | null | undefined} actor
 * @param {string} [requestedTenantId]
 * @returns {{ ok: true, tenant_id: string } | { ok: false, error: string, http_status: number }}
 */
export function resolveAuthorisedTenantId(actor, requestedTenantId) {
  if (!actor) {
    return { ok: false, error: 'authentication_required', http_status: 401 };
  }
  if (actor.environment !== 'tenant' || actor.can_core === true) {
    return { ok: false, error: 'tenant_access_denied', http_status: 403 };
  }
  const requested = String(requestedTenantId || '').trim();
  if (requested) {
    if (canAccessTenant(actor, requested)) {
      return { ok: true, tenant_id: requested };
    }
    return { ok: false, error: 'tenant_access_denied', http_status: 403 };
  }
  const primary = String(actor.tenant_id || actor.can_tenant_ids[0] || '').trim();
  if (primary && canAccessTenant(actor, primary)) {
    return { ok: true, tenant_id: primary };
  }
  return { ok: false, error: 'tenant_access_denied', http_status: 403 };
}
