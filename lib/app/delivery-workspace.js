 ...
 * Change / request tickets. Consumes the existing #714 / #551 commercial
 * clearance boolean — no second approval model. No schema. No send.
 */
...
import { projectCommercialRowsFromLeads } from './commercial-summary.js';
...
const COMMERCIAL_CLEARANCE_REQUIRED = 'Commercial clearance required';
const COMMERCIAL_CLEARANCE_NEXT = 'Record commercial clearance before build.';
...
/**
 * Overlay the existing #714 / #551 commercial gate onto a Delivery item.
 * Does not copy quotation, payment, or acceptance amounts. Does not create
 * a second commercial approval model.
 *
 * @param {Record<string, unknown>} item
 * @param {Record<string, unknown> | null | undefined} commercialRow
 * @returns {Record<string, unknown>}
 */
export function applyCommercialClearanceToDeliveryItem(item, commercialRow) {
  if (!item || typeof item !== 'object') return item;
  const commercial =
    commercialRow && typeof commercialRow === 'object' && !Array.isArray(commercialRow)
      ? commercialRow
      : null;
  const cleared = commercial?.financially_approved === true;
  const blockers = Array.isArray(commercial?.blockers)
    ? commercial.blockers.map((code) => String(code))
    : [];
  const commercialNext = asText(commercial?.next_action) || COMMERCIAL_CLEARANCE_NEXT;
  const commercialState =
    asText(commercial?.commercial_state) ||
    (cleared ? 'financially_approved' : 'financial_gate_blocking');
  ...
  if (item.protected_gate === true) {
    return blocked;
  }

  return {
    ...blocked,
    current_blocker: commercialNext,
    next_action: commercialNext,
    next_action_due: null,
    review_approval_state: COMMERCIAL_CLEARANCE_REQUIRED,
    exception_signals: ['blocked'],
 ...
 * @param {Date} [now]
 * @param {Array<Record<string, unknown>>} [clients]
 * @param {Record<string, unknown> | null} [commercialRow]
 * @returns {Record<string, unknown> | null}
 */
export function projectProspectToDeliveryItem(
  prospect,
  now = new Date(),
  clients = [],
  commercialRow = undefined,
) {
  ...
  const evidence = [
    ...
    { label: 'Commercial', href: COMMERCIAL_EXISTING_PATH, kind: 'safe' },
  ...
  if (commercialRow === undefined) return item;
  return applyCommercialClearanceToDeliveryItem(item, commercialRow);
}
...
export function projectDeliveryItems(args = {}) {
  ...
  const commercialLookup = Array.isArray(args.commercialRows);
  /** @type {Map<string, Record<string, unknown>>} */
  const commercialById = new Map();
  if (commercialLookup) {
    for (const row of args.commercialRows) {
      const id = asText(row?.prospect_id || row?.id);
      if (id) commercialById.set(id, row);
    }
  }
  /** @type {Array<Record<string, unknown>>} */
  const items = [];
  for (const row of Array.isArray(args.prospects) ? args.prospects : []) {
    const commercialRow = commercialLookup ? commercialById.get(asText(row?.id)) || null : undefined;
    const projected = projectProspectToDeliveryItem(row, now, clients, commercialRow);
...
export async function loadDeliverySummary(args) {
  ...
  let commercialRows = [];
  try {
    const commercialLoaded = await loadCommercialSummaryList({
      proofMode: args.proofMode === true,
      now: args.now,
    });
    if (commercialLoaded.ok) {
      commercialRows = projectCommercialRowsFromLeads(commercialLoaded.leads, args.now);
    }
  } catch {
    commercialRows = [];
  }
