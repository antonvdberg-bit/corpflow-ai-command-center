...
import {
  ...
  projectBoundedErpnextBlock,
  resolveBoundedErpnextStatus,
  sourceIdMatchesExistingContract,
  unlinkedErpnextBlock,
} from '../erpnext/delivery-continuity.js';
...
function mergeErpnextEvidence(item) {
  const evidence = Array.isArray(item.evidence) ? [...item.evidence] : [];
  const erpnext = item.erpnext && typeof item.erpnext === 'object' ? item.erpnext : null;
  const projectName = asText(erpnext?.project?.name);
  if (!projectName) return item;
  const href = `${DELIVERY_PATH}?item=${encodeURIComponent(String(item.id))}`;
  if (!evidence.some((link) => asText(link?.kind) === 'erpnext')) {
    evidence.push({
      label: `ERPNext ${projectName}`,
      href,
      kind: 'erpnext',
    });
  }
...
/**
 * Attach an existing ERPNext Project/Issue pointer. Does not guess by client name.
 *
 * @param {Record<string, unknown>} record
 * @param {ReturnType<typeof loadExistingDeliveryErpnextContract> | null} contract
 * @param {Record<string, unknown> | null} [status]
 */
export function attachErpnextBlock(record, contract, status = null) {
  const extracted = extractExistingDeliveryPointer(record);
  const matchesExtracted = contract ? pointerMatchesExistingContract(extracted, contract) : Boolean(extracted?.project || extracted?.issue_name);
  const matchesId = contract
    ? sourceIdMatchesExistingContract(record?.id || record?.source_id, contract)
    : false;
  if (!matchesExtracted && !matchesId) return unlinkedErpnextBlock();
  const pointer = matchesExtracted && extracted
    ? {
        ...contract,
        ...extracted,
        project: extracted.project || contract?.project,
        issue_name: extracted.issue_name || contract?.issue_name,
      }
    : contract;
  return projectBoundedErpnextBlock({ pointer, status: status || undefined });
}

/**
 * Existing #1097 synthetic delivery as a Delivery Workspace row.
 * Uses the recorded contract only — not a second project system.
 *
 * @param {ReturnType<typeof loadExistingDeliveryErpnextContract>} contract
 * @param {Record<string, unknown> | null} [status]
 */
export function projectExistingErpnextDeliveryItem(contract, status = null) {
  if (!contract?.project && !contract?.issue_name) return null;
  const erpnext = projectBoundedErpnextBlock({ pointer: contract, status: status || undefined });
  const stage = asText(erpnext.project?.status) || asText(erpnext.issue?.status) || 'referenced';
  const item = {
    id: `erpnext:${contract.delivery_ref}`,
    source_id: contract.delivery_ref,
    record_kind: 'general_delivery',
    record_kind_label: 'ERPNext Project / Issue',
    client_business: contract.customer || 'Synthetic delivery',
    product_service: contract.project_name || 'Existing ERPNext Project',
    delivery_stage: stage,
    owner: 'ERPNext owner (authoritative on Project/Issue)',
    current_blocker: 'None recorded',
    next_action: 'Use the ERPNext Project/Issue reference — do not copy tasks here',
    next_action_due: null,
    review_approval_state: 'ERPNext is authoritative for project/support status',
    exception_signals: [],
    primary_exception: '',
    protected_gate: false,
    protected_action_label: '',
    tenant_id: null,
    client_id: null,
    evidence: [
    ...
    source_contract: contract.source_contract || contract.schema,
    fabricated: false,
    is_synthetic: true,
    erpnext,
  };
  return mergeErpnextEvidence(item);
}
...
export function matchClientForProspect(prospect, clients = []) {
  ...
  const org = asText(prospect?.organisation_name).toLowerCase();
  for (const client of Array.isArray(clients) ? clients : []) {
    const related = Array.isArray(client?.related_prospects) ? client.related_prospects : [];
    if (prospectId && related.some((row) => asText(row?.id) === prospectId)) return client;
    const names = [client?.trading_name, client?.legal_name].map((v) => asText(v).toLowerCase()).filter(Boolean);
    if (org && names.includes(org)) return client;
  }
  return null;
}
...
export function projectProspectToDeliveryItem(prospect, now = new Date(), clients = []) {
  ...
  return {
    ...
    fabricated: false,
    erpnext: attachErpnextBlock(prospect, null),
  };
}
...
export function projectRequestToDeliveryItem(request, now = new Date()) {
  ...
  return {
    ...
    fabricated: false,
    erpnext: attachErpnextBlock(request, null),
  };
}
...
export function projectDeliveryItems(args = {}) {
  ...
  const status = args.erpnextStatus || null;
  ...
  for (const row of Array.isArray(args.prospects) ? args.prospects : []) {
    ...
    projected.erpnext = attachErpnextBlock(row, contract, status);
    items.push(mergeErpnextEvidence(projected));
  }
  ...
  if (contract && !existingReferenceBlocker(contract)) {
    const already = items.some(
      (row) =>
        asText(row.source_id) === contract.delivery_ref ||
        asText(row.id) === `erpnext:${contract.delivery_ref}`,
    );
    if (!already) {
      const synthetic = projectExistingErpnextDeliveryItem(contract, status);
...
export function buildDeliveryPayload(args) {
  ...
  const selectedId = asText(args.item);
  const selected =
    selectedId && items.find((row) => asText(row.id) === selectedId)
      ? items.find((row) => asText(row.id) === selectedId)
      : selectedId
        ? all.find((row) => asText(row.id) === selectedId) || null
        : null;
  return {
    ok: true,
    workspace: 'operating',
    ...
    erpnext: {
      ...
      pointer_schema: 'corpflow.delivery.erpnext.v1',
      note: 'ERPNext Project/Issue are authoritative where an existing pointer already exists. Delivery does not copy task or support history.',
    },
    protected_actions: {
...
export async function loadDeliverySummary(args) {
  ...
  const allowLiveGet =
    args.allowLiveGet === true || (args.allowLiveGet !== false && nodeEnv !== 'test' && args.proofMode !== true);
  const allowRecordedReadback = args.allowRecordedReadback !== false && (args.proofMode === true || nodeEnv === 'test');
  const erpnextStatus = args.erpnextStatus
    ? args.erpnextStatus
    : await resolveBoundedErpnextStatus({
        pointer: contract,
        allowLiveGet,
        allowRecordedReadback,
        client: args.erpnextClient || null,
      });
  ...
  return {
    ...
    erpnext_contract: {
      delivery_ref: contract.delivery_ref,
      project: contract.project,
      issue_name: contract.issue_name,
      blocker: existingReferenceBlocker(contract),
    },
  };
}
