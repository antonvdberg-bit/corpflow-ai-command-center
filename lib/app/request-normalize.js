 ...
 * Fail-closed: missing/invalid tenant_id never becomes REFERENCE_TENANT_ID / CorpFlowAI.
 *
 * @param {unknown} row
...
export function normalizeCmpTicketRow(row, opts = {}) {
  const result = normalizeCmpTicketRowDetailed(row, opts);
  return result.ok ? result.request : null;
}

/**
 * Detailed normalize result for Core data-quality reporting.
 * @param {unknown} row
 * @param {{ source?: 'fixture'|'cmp_ticket' }} [opts]
 * @returns {{ ok: true, request: AppRequest } | { ok: false, error: string, request_id?: string | null }}
 */
export function normalizeCmpTicketRowDetailed(row, opts = {}) {
  ...
  const tenantId = rowTenantId(r);
  if (!tenantId) {
    return { ok: false, error: 'missing_tenant_id', request_id: id };
  }
