...
/**
 * Workbench list: Lead Rescue + Website Rescue + general enquiries.
 *
 * @param {{
 *   proofMode?: boolean,
 *   forceFixture?: boolean,
 ...
 * }} [opts]
 */
export async function loadProspectWorkbenchList(opts = {}) {
  const dataSource = resolveProspectOperationsDataSource(opts);
  const now = opts.now instanceof Date ? opts.now : new Date();
  if (dataSource === DATA_SOURCE_FIXTURE) {
    return {
      ok: true,
      data_source: DATA_SOURCE_FIXTURE,
      prospects: projectProspectWorkbenchRows(getProspectFixtureRows(), now),
    };
  }
  try {
    const db = opts.prisma || getDefaultPrisma();
    ...
    return {
      ok: true,
      data_source: DATA_SOURCE_LEADS_READ,
      prospects: projectProspectWorkbenchRows(rows, now),
    };
  } catch {
    return {
      ok: false,
