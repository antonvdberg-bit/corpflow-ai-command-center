...
/** Canonical Prospect Action Queue (#995 / #1040). Operating Workspace only. */
 ...
 * @returns {boolean}
 */
export function isProspectActionQueuePath(path) {
  const p = String(path || '').split('?')[0].replace(/\/+$/, '') || '/';
...
export const WORKSPACE_SURFACE_MATRIX = Object.freeze([
  ...
  Object.freeze({
    ...
    data_source: 'fixture | leads_read + existing qualificationJson write paths',
    disposition: 'CANONICAL',
    notes: 'Staff-only #996. Cross-product processing grid. Opens #994 shared detail. Temporary Lead Rescue desk remains for unique product fields.',
  }),
  Object.freeze({
    path: '/app/pipeline',
  ...
  Object.freeze({
    ...
    disposition: 'CANONICAL',
    notes: 'Staff-only #995 / #1040. Cross-product queue. Opens #994 shared detail. Rapid Delivery desk stays temporary for unique proposal/offer tools.',
  }),
  Object.freeze({
    ...
    notes: 'Unique Rapid Delivery proposal copy and offer desk. Canonical Action Queue is /app/queue. No hard redirect.',
  }),
  Object.freeze({
    ...
    disposition: 'TEMPORARY',
    notes: 'Unique Lead Rescue commercial / activity / payment fields. Canonical Workbench is /app/workbench. No hard redirect.',
  }),
  Object.freeze({
    ...
    data_source: 'Postgres leads via admin-lead-rescue-api',
    disposition: 'TEMPORARY',
    notes: 'Unique product-detail fields remain here. Canonical shared detail is /app/prospects/[id]. No hard redirect.',
  }),
  Object.freeze({
    path: '/change/revenue',
    ...
    disposition: 'TEMPORARY',
    notes: 'Unique browser-only personal checklist. Canonical pipeline is /app/pipeline. Not a second CRM.',
  }),
  Object.freeze({
    path: '/change',
    name: 'Change Console',
  ...
  Object.freeze({
    ...
    disposition: 'REUSE',
    notes: 'Future Clients summary should reuse Company Master, not duplicate identity.',
  }),
]);
 ...
 * @returns {boolean}
 */
export function navIncludesProspectActionQueue(items) {
  return (items || []).some((item) => {
    const id = String(item?.id || '');
    const href = String(/** @type {{ href?: string }} */ (item).href || '');
 ...
 * @returns {boolean}
 */
export function operatingNavIncludesProspectActionQueue() {
  return navIncludesProspectActionQueue(CORE_NAV_ITEMS);
}
 ...
 * @returns {boolean}
 */
export function tenantNavOmitsProspectActionQueue() {
  return !navIncludesProspectActionQueue(TENANT_NAV_ITEMS);
}
