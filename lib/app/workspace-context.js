...
/** Commercial ERPNext quotation evidence drilldown (#1160). Operating Workspace only. */
export const COMMERCIAL_QUOTATION_PATH_PREFIX = '/app/commercial/';
export const COMMERCIAL_QUOTATION_API_PATH = '/api/app/commercial-quotation';
export const COMMERCIAL_QUOTATION_PDF_API_PATH = '/api/app/commercial-quotation-pdf';
...
/** Operating Workspace action overview landing (#1159). Same page as the Core shell. */
export const OPERATING_OVERVIEW_PATH = '/app/core';
export const OPERATING_OVERVIEW_API_PATH = '/api/app/overview';
 ...
 * @returns {string}
 */
export function commercialQuotationPath(commercialId) {
  const id = String(commercialId || '').trim();
  return id ? `${COMMERCIAL_QUOTATION_PATH_PREFIX}${encodeURIComponent(id)}` : COMMERCIAL_SUMMARY_PATH;
}

/**
 * Keep the local/preview proof harness on Operating Workspace `/app` routes.
 * Staff cookie sessions ignore this (proofWanted is false). `/change` and
 * `/admin` stay unchanged — they are not the proof actor.
 *
 * @param {unknown} href
 * @param {boolean} [proofWanted]
 * @returns {string}
 */
export function appendProofQuery(href, proofWanted) {
  const raw = href == null ? '' : String(href).trim();
  if (!raw || proofWanted !== true) return raw;
  const hashIdx = raw.indexOf('#');
  const beforeHash = hashIdx === -1 ? raw : raw.slice(0, hashIdx);
  const hashSuffix = hashIdx === -1 ? '' : raw.slice(hashIdx);
  const pathOnly = beforeHash.split('?')[0];
  if (!pathOnly.startsWith('/app')) return raw;
  if (/(?:^|[?&])proof=/.test(beforeHash)) return raw;
  return `${beforeHash}${beforeHash.includes('?') ? '&' : '?'}proof=1${hashSuffix}`;
}
...
export function isCommercialSummaryPath(path) {
  const p = String(path || '').split('?')[0].replace(/\/+$/, '') || '/';
  return p === COMMERCIAL_SUMMARY_PATH || p === '/api/app/commercial' || isCommercialQuotationPath(p);
}

/**
 * Commercial ERPNext quotation evidence (#1160). Operating Workspace only.
 * Tenant Workspace must not receive this route or API.
 *
 * @param {string} path
 * @returns {boolean}
 */
export function isCommercialQuotationPath(path) {
  const p = String(path || '').split('?')[0].replace(/\/+$/, '') || '/';
  if (
    p === COMMERCIAL_QUOTATION_API_PATH ||
    p === COMMERCIAL_QUOTATION_PDF_API_PATH ||
    p === '/api/app/commercial/quotation' ||
    p === '/api/app/commercial/quotation-pdf'
  ) {
    return true;
  }
  if (!p.startsWith(COMMERCIAL_QUOTATION_PATH_PREFIX)) return false;
  return p.length > COMMERCIAL_QUOTATION_PATH_PREFIX.length;
}
...
export const WORKSPACE_SURFACE_MATRIX = Object.freeze([
  ...
  Object.freeze({
    path: '/app/core',
    name: 'Operating Workspace action overview (Core environment)',
    environment: 'core',
    record_type: 'leads + company_master + commercial rail + cmp_tickets',
    data_source: 'fixture | existing Prospect / Client / Commercial / Delivery readers',
    disposition: 'CANONICAL',
    notes: '#1159 action overview landing. Counts and short exception lists reuse merged Prospect, Client, Commercial and Delivery contracts. Requests remain an in-shell list. GET /api/app/overview is staff-only.',
  }),
  Object.freeze({
    path: '/app/tenant',
  ...
  Object.freeze({
    path: '/app/commercial/[id]',
    name: 'Commercial ERPNext quotation evidence',
    environment: 'core',
    record_type: 'existing erpnext_quotation reference + ERPNext GET',
    data_source: 'fixture proof readback | hosted ERPNext GET/read-only',
    disposition: 'CANONICAL',
    notes: 'Staff-only #1160 drilldown. Bounded identifier/docstatus/status/currency/total + printable PDF. No ERPNext write, no Postgres copy, no billing model.',
  }),
  Object.freeze({
    path: '/app/delivery',
...
export function classifyWorkspaceSurface(path) {
  ...
  if (isCommercialQuotationPath(raw) && raw.startsWith('/app/commercial/')) {
    return WORKSPACE_SURFACE_MATRIX.find((row) => row.path === '/app/commercial/[id]') || null;
  }
  return null;
}
...
/**
 * Action overview lands on the Operating Workspace Core shell (#1159).
 * @param {string} path
 * @returns {boolean}
 */
export function isOperatingOverviewPath(path) {
  const p = String(path || '').split('?')[0].replace(/\/+$/, '') || '/';
  return p === OPERATING_OVERVIEW_PATH || p === OPERATING_OVERVIEW_API_PATH;
}
 ...
 * @returns {boolean}
 */
export function navIncludesOperatingOverview(items) {
  return (items || []).some((item) => {
    const id = String(item?.id || '');
    const href = String(/** @type {{ href?: string }} */ (item).href || '');
    return id === 'overview' && href === OPERATING_OVERVIEW_PATH;
  });
}

/** @returns {boolean} */
export function operatingNavIncludesOverview() {
  return navIncludesOperatingOverview(CORE_NAV_ITEMS);
}

/** @returns {boolean} */
export function tenantNavOmitsOverview() {
  return !navIncludesOperatingOverview(TENANT_NAV_ITEMS);
}
