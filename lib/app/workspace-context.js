...
/** Cross-client Operating Workspace overview landing (#1071). Reuses /app/core. */
export const OPERATING_OVERVIEW_PATH = '/app/core';

/** Overview aggregator — Operating Workspace / Core only. */
...
/**
 * Cross-client overview is an Operating Workspace capability (#1071).
 * Tenant Workspace must not receive this nav or API.
 *
 * @param {string} path
 * @returns {boolean}
...
export const WORKSPACE_SURFACE_MATRIX = Object.freeze([
  ...
  Object.freeze({
    path: '/app/core',
    name: 'Operating Workspace overview landing',
    environment: 'core',
    record_type: 'leads + company_master + cmp_tickets (summary only)',
    data_source: 'existing /app/today /queue /clients /change contracts',
    disposition: 'CANONICAL',
    notes: 'Staff-only #1071 overview. Counts + short exception lists over existing records. Requests remain an in-shell list. No second analytics store.',
  }),
  Object.freeze({
    path: '/app/tenant',
...
/**
 * Operating Workspace nav includes the cross-client overview landing.
 * @returns {boolean}
 */
export function operatingNavIncludesOperatingOverview() {
  return navIncludesOperatingOverview(CORE_NAV_ITEMS);
}

/**
 * Tenant Workspace must not expose the Operating Workspace overview.
 * @returns {boolean}
 */
export function tenantNavOmitsOperatingOverview() {
