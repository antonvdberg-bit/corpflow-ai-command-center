...
/** Staff-only lifecycle continuity API (#1072). Not a Tenant surface. */
export const LIFECYCLE_CONTINUITY_API_PATH = '/api/app/lifecycle';
...
/**
 * Lifecycle continuity is an Operating Workspace capability.
 * Tenant Workspace must not receive this API.
 *
 * @param {string} path
 * @returns {boolean}
 */
export function isLifecycleContinuityPath(path) {
  const p = String(path || '').split('?')[0].replace(/\/+$/, '') || '/';
  return p === LIFECYCLE_CONTINUITY_API_PATH;
}
...
export const WORKSPACE_SURFACE_MATRIX = Object.freeze([
  ...
  Object.freeze({
    ...
    disposition: 'CANONICAL',
    notes: 'Staff-only #999. Reuses Company Master identity. No second Client table. #1072 bidirectional lifecycle rail reuses existing prospect/commercial/delivery contracts.',
  }),
  Object.freeze({
    path: '/app/clients/[id]',
