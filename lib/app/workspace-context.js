...
/** Canonical Delivery summary (#1005). Operating Workspace only. */
export const DELIVERY_SUMMARY_PATH = '/app/delivery';
 ...
 * @returns {boolean}
 */
export function isDeliverySummaryPath(path) {
  const p = String(path || '').split('?')[0].replace(/\/+$/, '') || '/';
  return p === DELIVERY_SUMMARY_PATH || p === '/api/app/delivery';
}
 ...
 * @returns {boolean}
 */
export function navIncludesDeliverySummary(items) {
  return (items || []).some((item) => {
    const id = String(item?.id || '');
    const href = String(/** @type {{ href?: string }} */ (item).href || '');
    return id === 'delivery' && href === DELIVERY_SUMMARY_PATH;
  });
}
...
export const WORKSPACE_SURFACE_MATRIX = Object.freeze([
  ...
  Object.freeze({
    ...
    disposition: 'CANONICAL',
    notes: 'Staff-only #1005. Lead Rescue, Website Rescue, and Change/request delivery in one surface. No second project system. Tenant denied.',
  }),
  Object.freeze({
 ...
 * @returns {boolean}
 */
export function operatingNavIncludesDeliverySummary() {
  return navIncludesDeliverySummary(CORE_NAV_ITEMS);
}
 ...
 * @returns {boolean}
 */
export function tenantNavOmitsDeliverySummary() {
  return !navIncludesDeliverySummary(TENANT_NAV_ITEMS);
}
