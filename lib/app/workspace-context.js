...
/**
 * Keep Operating Workspace hops inside the proof harness when that is the
 * operator session. Only `/app/*` routes accept `proof=1`. `/change` and
 * `/admin/*` stay on their existing auth.
 *
 * @param {unknown} href
 * @param {boolean} [proofWanted]
 * @returns {string}
 */
export function withOperatingWorkspaceProof(href, proofWanted) {
