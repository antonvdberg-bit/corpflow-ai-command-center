...
export function applySharedProspectOperatorPatch(row, patch, opts = {}) {
  ...
  if (product === AI_LEAD_RESCUE_PRODUCT) {
    ...
    if (patch.setup_checklist_item != null) {
      const checklistResult = mergeAiLeadRescueChecklistItemPatch(
        merged,
        /** @type {Record<string, unknown>} */ (patch.setup_checklist_item),
        actorLabel,
        nowIso,
      );
      if (!checklistResult.ok) {
        return { ok: false, error: checklistResult.error, http_status: 400 };
      }
      merged = checklistResult.qj;
    }
    if (patch.activity_append != null) {
      const activityResult = appendAiLeadRescueActivity(
        merged,
        /** @type {Record<string, unknown>} */ (patch.activity_append),
        actorLabel,
        nowIso,
      );
      if (!activityResult.ok) {
  ...
  if (patch.setup_checklist_item != null) {
    return { ok: false, error: 'setup_checklist_not_applicable', http_status: 400 };
  }
  if (patch.activity_append != null) {
    return { ok: false, error: 'activity_append_not_applicable', http_status: 400 };
  }
