...
import {
  ...
  buildLeadRescueTenantRequestRow,
  hydrateLeadRescueTenantRequest,
  resetLeadRescueDeliveryStore,
} from '../lead-rescue/tenant-delivery-progress.js';
...
export function listAppRequests() {
  return ticketRows
    ...
    .filter(Boolean)
    .map((r) => hydrateLeadRescueTenantRequest(r))
    .filter(Boolean)
...
export function getAppRequest(id) {
  ...
  const hydrated = hydrateLeadRescueTenantRequest(normalized);
  return hydrated ? structuredClone(hydrated) : null;
}
...
export function updateAppRequest(id, mutator) {
  ...
  const current = hydrateLeadRescueTenantRequest(
    normalizeCmpTicketRow(ticketRows[idx], { source: 'fixture' }),
  );
  ...
  const next = hydrateLeadRescueTenantRequest(
    normalizeCmpTicketRow(ticketRows[idx], { source: 'fixture' }),
  );
