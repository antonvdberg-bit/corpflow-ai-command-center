...
export function projectClientSummary(company, prospects = []) {
  ...
  return {
    ...
    commercial_references: {
      status: related.length ? 'existing_commercial_rail' : 'no_related_prospect',
      issue: COMMERCIAL_SUMMARY_ISSUE,
      source_of_truth:
        'leads row commercial_approval namespace (#714 rail / #551). ERPNext names are pointers only.',
      existing_identity_path: COMPANY_MASTER_PATH,
      prospect_commercial_path: related[0]?.shared_detail_path
        ? `${related[0].shared_detail_path}#commercial-clearance`
        : null,
      note: 'Commercial truth stays on the existing rail. A dedicated /app/commercial module remains later (#1004).',
    },
    delivery_references: {
      status: related.length ? 'existing_product_delivery_contract' : 'no_related_prospect',
      issue: DELIVERY_SUMMARY_ISSUE,
      source_of_truth:
        'Same leads row Lead Rescue / Website Rescue operator JSON. Tenant service-request stays at /change.',
      existing_delivery_path: TENANT_DELIVERY_PATH,
      action_queue_path: ACTION_QUEUE_PATH,
      pipeline_path: PROSPECT_PIPELINE_PATH,
      prospect_delivery_path: related[0]?.shared_detail_path
        ? `${related[0].shared_detail_path}#delivery-state`
        : null,
      note: 'Delivery truth stays on the existing product contract. A dedicated /app/delivery module remains later (#1005).',
    },
  };
}
