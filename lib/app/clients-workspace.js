...
export function matchProspectsToClient(company, prospects) {
  ...
  if (explicit.size > 0) {
    return list.filter((prospect) => {
      ...
      return Boolean(id && explicit.has(id));
    });
  }
  ...
  return list.filter((prospect) => {
    ...
    return Boolean(org && names.includes(org));
  });
}

/**
 * Recorded hops into existing Prospect / Commercial / Delivery routes.
 * Uses already-linked prospect ids only — never a name-invented join.
 *
 * @param {Array<Record<string, unknown>>} related
 * @returns {{
 *   prospect: string,
 *   commercial: string,
 *   delivery: string,
 ...
 * }}
 */
export function recordedClientHopPaths(related = []) {
  const rows = Array.isArray(related) ? related : [];
  const recorded = rows.find((row) => String(row?.shared_detail_path || '').trim());
  return {
    prospect: recorded ? String(recorded.shared_detail_path) : PROSPECT_OPERATIONS_PATH,
    ...
    change: TENANT_DELIVERY_PATH,
  };
}

export { withOperatingWorkspaceProof };
...
export function projectClientSummary(company, prospects = []) {
  ...
  return {
    ...
    hop_paths: recordedClientHopPaths(related),
