...
} from './staff-workspace-load-state.js';
...
export function projectCommercialExceptionItems(rows) {
  return take(
    (Array.isArray(rows) ? rows : []).map((row) => {
      ...
      const quotationHref = asText(row.quotation_evidence_path);
      const href = quotationHref || `${COMMERCIAL_SUMMARY_PATH}?filter=needs_attention`;
      return projectOverviewExceptionItem({
        id: `commercial:${id}`,
        kind: 'commercial',
...
export function projectDeliveryExceptionItems(rows) {
  return take(
    (Array.isArray(rows) ? rows : []).map((row) => {
      const id = asText(row.id);
      const exception = asText(row.primary_exception);
      const href = exception
        ? `${DELIVERY_PATH}?filter=${encodeURIComponent(exception)}`
        : DELIVERY_PATH;
