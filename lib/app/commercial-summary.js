  ...
  const nested =
    row.commercial_approval && typeof row.commercial_approval === 'object'
      ...
      : row.commercial && typeof row.commercial === 'object'
        ...
        : readCommercialApprovalFromQualification(row.qualificationJson);
...
export function projectCommercialRow(record, prospect = null) {
  ...
  const quotation = asTrimmedString(row.erpnext_quotation);
 ...
 * prospect identity only — no fabricated prices or ERPNext names.
 *
 * @param {Record<string, unknown>} lead
 * @param {Date} [now]
 * @returns {Record<string, unknown>}
