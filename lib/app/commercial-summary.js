...
/**
 * Authoritative ERPNext Customer name already stored on the qualification pointer.
 * Commercial Workspace reads this; it does not copy Customer/Contact/Address into a second ledger
 * and does not invent a join from legal or trading names.
 *
 * @param {Record<string, unknown> | null | undefined} lead
 * @returns {string}
 */
export function erpnextCustomerPointerFromLead(lead) {
  const row = asRecord(lead);
  const qj = asRecord(row.qualificationJson || row.qualification_json);
  const erp = asRecord(qj.erpnext);
  return asTrimmedString(erp.customer || erp.customer_name || row.erpnext_customer);
}
...
export function commercialRecordFromProspect(prospect) {
  ...
  const pointerCustomer = erpnextCustomerPointerFromLead(row);
  return {
    ...nested,
    ...
    erpnext_customer: asTrimmedString(nested.erpnext_customer) || pointerCustomer || null,
  };
}
...
export function projectCommercialRow(record, prospect = null) {
  ...
  const customer =
    asTrimmedString(row.erpnext_customer || prospectRow?.erpnext_customer) ||
    erpnextCustomerPointerFromLead(prospectRow) ||
    null;
  ...
  return {
    ...
    quotation_evidence_path:
      quotation && (prospectId || asTrimmedString(row.id))
        ? `/app/commercial/${encodeURIComponent(prospectId || asTrimmedString(row.id))}`
        : null,
    ...
    erpnext: {
      ...
      authoritative: Boolean(quotation || proforma || invoice || customer),
      mutated: false,
    },
    related_refs: {
  ...
  return projectCommercialRow(
    {
      ...
      erpnext_customer: erpnextCustomerPointerFromLead(lead) || null,
