 ...
 *
 * Repository selector: fixture (proof/test/no DB) or read-only cmp_tickets via Prisma.
...
/**
 * POST /api/app/component-review — Tenant only; fixture mutations only; no external send.
 */
export async function handleAppComponentReview(req, res) {
  if (req.method !== 'POST') {
    return send(res, 405, { ok: false, error: 'method_not_allowed' });
  }
  const { actor, proof } = resolveActor(req, 'tenant');
  if (!actor) {
    return send(res, 401, { ok: false, error: 'authentication_required' });
  }
  const body =
  ...
  const repo = repoForRequest(req, proof);
  if (!repo.supportsMutations) {
    return send(res, 409, {
      ok: false,
      error: 'persistence_unavailable',
      hint: 'Review writes are fixture-proof only in this slice; DB-backed cmp_tickets path is read-only.',
      data_source: repo.dataSource,
      external_send: false,
    });
  }
  ...
  const got = await repo.getForTenant(requestId, tenantId);
  if (!got.request) {
    return send(res, 404, { ok: false, error: 'request_not_found' });
  }
...
/**
 * POST /api/app/component-expose — Core only; fixture mutations only.
 */
export async function handleAppComponentExpose(req, res) {
  if (req.method !== 'POST') {
    return send(res, 405, { ok: false, error: 'method_not_allowed' });
  }
  const { actor, proof } = resolveActor(req, 'core');
  if (!actor) {
    return send(res, 401, { ok: false, error: 'authentication_required' });
  }
  ...
  if (!repo.supportsMutations) {
    return send(res, 409, {
      ok: false,
      error: 'persistence_unavailable',
      hint: 'Expose/hide writes are fixture-proof only in this slice; DB-backed cmp_tickets path is read-only.',
      data_source: repo.dataSource,
      external_send: false,
    });
  }
