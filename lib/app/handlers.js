...
import {
  ...
  mergeTenantProgressLists,
} from './website-rescue-tenant-progress.js';
...
export async function handleAppRequestsList(req, res) {
  ...
  try {
    if (environment === 'tenant') {
      ...
      const ticketList = projectTenantRequestList(listed.requests, tenantId);
      const wrListed = await listTenantWebsiteRescueProgress({
        tenantId,
        proofMode: proof === true,
        forceFixture: listed.data_source === 'fixture',
      });
      if (!wrListed.ok) {
        return send(res, 503, {
          ok: false,
          error: wrListed.error || 'repository_unavailable',
          environment,
          data_source: wrListed.data_source,
        });
      }
      return send(res, 200, {
        ok: true,
        ...
        tenant_id: tenantId,
        auth_mode: proof === true ? 'proof_harness' : 'session',
        requests: mergeTenantProgressLists(ticketList, wrListed.items),
...
export async function handleAppRequestDetail(req, res) {
  ...
  try {
    if (environment === 'tenant') {
      ...
      const wr = await getTenantWebsiteRescueProgress({
        id,
        tenantId,
        proofMode: proof === true,
        forceFixture: got.data_source === 'fixture',
      });
      if (!wr.ok || !wr.request) {
        const status = wr.error === 'repository_unavailable' ? 503 : 404;
        return send(res, status, {
          ok: false,
          error: wr.error || 'request_not_found',
 ...
 * Delivery summary (#1005 / #1184). Lead Rescue + Website Rescue leads plus
 * existing Change / request tickets, with existing ERPNext Project/Issue
 * references projected read-only. No ERPNext write. No schema.
 */
export async function handleAppDelivery(req, res) {
  ...
  const item = typeof q.item === 'string' ? q.item : '';
  const testClient = /** @type {{ __testFrappeClient?: object }} */ (req).__testFrappeClient;
  const repo = repoForRequest(req, proof);
  const loaded = await loadDeliverySummary({
    ...
    erpnextClient: testClient || undefined,
  });
  if (!loaded.ok) {
    return send(res, 503, {
      ok: false,
