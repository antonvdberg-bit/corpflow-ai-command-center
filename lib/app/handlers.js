...
import { buildOperatingOverviewPayload } from './operating-overview.js';
 ...
 * Cross-client overview (#1071). Summarises existing Prospect, Client and
 * Delivery records. No second store. No fabricated KPIs. Tenant → 403.
 */
export async function handleAppOverview(req, res) {
  ...
  /** @type {Array<{ section: string, error: string }>} */
  const sectionErrors = [];
  ...
  const dataSources = { prospects: null, clients: null, requests: null };

  const prospectsLoaded = await loadProspectWorkbenchList({ proofMode: proof === true });
  if (prospectsLoaded.ok) {
    prospects = prospectsLoaded.prospects;
    dataSources.prospects = prospectsLoaded.data_source;
  } else {
    sectionErrors.push({ section: 'prospects', error: String(prospectsLoaded.error || 'prospects_unavailable') });
  }

  const clientsLoaded = await loadClientsList({ proofMode: proof === true });
  if (clientsLoaded.ok) {
    clients = clientsLoaded.clients;
    dataSources.clients = clientsLoaded.data_source;
  } else {
    sectionErrors.push({ section: 'clients', error: String(clientsLoaded.error || 'clients_unavailable') });
  }

  const repo = repoForRequest(req, proof);
  try {
    const listed = await repo.listForCore({ tenantFilter: null });
    requests = projectCoreRequestList(listed.requests || [], {
      tenantFilter: null,
      statusFilter: null,
      waitingPartyFilter: null,
    });
    dataSources.requests = String(listed.data_source || repo.dataSource || '');
  } catch (err) {
    sectionErrors.push({
      section: 'requests',
      error: repositoryErrorCode(err),
  ...
  if (!dataSources.prospects && !dataSources.clients && !dataSources.requests) {
    return send(res, 503, {
      ok: false,
