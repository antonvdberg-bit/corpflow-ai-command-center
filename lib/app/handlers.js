...
import {
  attachLifecycleToClient,
  attachLifecycleToProspect,
  loadLifecycleTrace,
} from './lifecycle-continuity-list.js';
import {
  assertClientsAccess,
...
export async function handleAppClientDetail(req, res) {
  ...
  const client = await attachLifecycleToClient(loaded.client, { proofMode: proof === true });
  return send(
    res,
    200,
    buildClientDetailPayload({
...
export async function handleAppProspectDetail(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    const prospect = await attachLifecycleToProspect(loaded.prospect, { proofMode: proof === true });
    return send(
      res,
      200,
      buildProspectDetailPayload({
  ...
  const prospect = await attachLifecycleToProspect(saved.prospect, { proofMode: proof === true });
  return send(res, 200, {
    ...buildProspectDetailPayload({
      prospect,
      data_source: saved.data_source,
...
/**
 * GET /api/app/lifecycle — Operating Workspace continuity proof (#1072).
 * Core only. Read-only. No schema. No ERPNext write. No external send.
 */
export async function handleAppLifecycle(req, res) {
  if (req.method !== 'GET') {
    return send(res, 405, { ok: false, error: 'method_not_allowed' });
  }
  const { actor, proof } = await resolveActor(req, 'core');
  ...
  const looksLikeCompany = rawId.startsWith('cmp_');
  const loaded = await loadLifecycleTrace({
    prospectId: String(q.prospect_id || '').trim() || (looksLikeCompany ? '' : rawId),
    companyId: String(q.company_id || '').trim() || (looksLikeCompany ? rawId : ''),
    proofMode: proof === true,
  });
  if (!loaded.ok) {
    return send(res, loaded.http_status || 500, {
  ...
  return send(res, loaded.payload.ok ? 200 : 409, loaded.payload);
}
...
export async function tryHandleAppApi(req, res, pathSeg) {
  ...
  if (pathSeg === 'app/lifecycle') {
    await handleAppLifecycle(req, res);
    return true;
  }
