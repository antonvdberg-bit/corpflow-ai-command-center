...
export function proofQuotationDocs(repoRoot) {
  ...
  if (sellingName) {
    docs[sellingName] = {
      ...
      valid_till: asTrimmed(sellingProof.valid_till) || null,
      tc_name: asTrimmed(selling.quotation?.terms_name) || null,
      letter_head: asTrimmed(selling.quotation?.letter_head) || null,
      item_code: asTrimmed(sellingProof.item_code) || null,
      item_name: 'Premium Landing Page Rescue',
      description:
        'Premium Landing Page Rescue (Website Rescue T1). A bounded landing-page rescue that states the offer clearly, is mobile-ready, and routes buyer intent into a working enquiry path. Two structured preview rounds included. 50% deposit before design/build; balance before production release. No SEO ranking or sales-volume guarantees.',
      terms_present: false,
      company: asTrimmed(selling.company) || 'CorpFlowAI LTD',
    };
  }
  return docs;
}
...
function asTermsPresent(row) {
  if (typeof row?.terms_present === 'boolean') return row.terms_present;
  const terms = row?.terms;
  if (terms == null) return false;
  return String(terms).replace(/<[^>]+>/g, ' ').trim().length > 0;
}
...
export function projectBoundedQuotationEvidence(doc, extra = {}) {
  ...
  const projected = {
    ...
    company: asTrimmed(row.company) || null,
    transaction_date: asTrimmed(row.transaction_date) || null,
    valid_till: asTrimmed(row.valid_till) || null,
    tc_name: asTrimmed(row.tc_name) || null,
    letter_head: asTrimmed(row.letter_head) || null,
    item_code: asTrimmed(item.item_code || row.item_code) || null,
    item_name: asTrimmed(item.item_name || row.item_name) || null,
    description: asTrimmed(String(item.description || row.description || '').replace(/<[^>]+>/g, ' ')) || null,
    terms_present: asTermsPresent(row),
