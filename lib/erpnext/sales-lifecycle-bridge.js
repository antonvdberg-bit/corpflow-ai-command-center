...
function notesFor(leadId, idempotencyKey, extra = '') {
  return [
    'synthetic=true',
    'issue=1018',
    `ref=${asString(leadId)}`,
    `idempotency_key=${asString(idempotencyKey)}`,
    extra,
  ]
    .filter(Boolean)
    .join(' | ');
...
function buildLeadPayload(intake, plan, leadId, idempotencyKey) {
  ...
  return {
    ...
    notes: notesFor(leadId, idempotencyKey),
    utm_source: 'corpflowai',
    utm_medium: 'sales_lifecycle_bridge',
    utm_campaign: 'cf1018-wp2',
  };
}
...
function buildOpportunityPayload(intake, plan, leadId, leadName, idempotencyKey) {
  ...
  return {
    ...
    notes: notesFor(leadId, idempotencyKey, `lead=${leadName}`),
  };
}
...
export async function searchExistingLeadIdentity(client, candidate) {
  ...
  const searches = [
    client.list('Lead', {
      fields,
      filters: [['notes', 'like', `%${candidate.idempotency_key}%`]],
      limit: 20,
    }),
    client.list('Lead', {
      fields,
  ...
  const [byKey, byEmail, byCompany] = results;
  const leads = uniqueRows([...(byKey.rows || []), ...(byEmail.rows || []), ...(byCompany.rows || [])]);
...
export async function searchExistingOpportunityIdentity(client, candidate) {
  const fields = ['name', 'title', 'opportunity_from', 'party_name', 'status', 'notes', 'currency'];
  const searches = [
  ...
  const opportunities = uniqueRows(results.flatMap((result) => result.rows || []));
...
function decideLeadAction(leads, candidate) {
  ...
  const sameKey = leads.filter((row) => asString(row.notes).includes(candidate.idempotency_key));
  if (sameKey.length) {
  ...
  for (const hit of emailHits) {
    const otherId = extractCorpflowLeadIdFromText(hit.notes);
...
function decideOpportunityAction(opportunities, candidate) {
  const sameKey = opportunities.filter((row) => asString(row.notes).includes(candidate.idempotency_key));
  if (sameKey.length) {
    return { action: 'UPDATE', reason: 'MATCH_IDEMPOTENCY_KEY', canonical: asString(sameKey[0].name), row: sameKey[0] };
  }
...
export async function reconcileSalesLifecycle(event, opts) {
  ...
  if (plan.create_customer) {
    ...
    if (customerResult.customer && liveLeadName) {
      const linked = await client.update('Customer', customerResult.customer, {
        lead_name: liveLeadName,
        ...(opportunityRow?.name ? { opportunity_name: opportunityRow.name } : {}),
      });
      audit.push(
        auditEvent({
          at: now,
    ...
    if (customerResult.customer && opportunityRow?.name) {
      const existingNotes = asString(opportunityRow.notes);
      const linkedNotes = existingNotes.includes(`customer=${customerResult.customer}`)
        ? existingNotes
        : `${existingNotes} | customer=${customerResult.customer}`.trim();
      const linkedOpp = await client.update('Opportunity', opportunityRow.name, { notes: linkedNotes });
      if (linkedOpp.ok && linkedOpp.row) opportunityRow = linkedOpp.row;
      audit.push(
        auditEvent({
          at: now,
          action: 'LINK_OPPORTUNITY_CUSTOMER',
          ok: linkedOpp.ok === true,
          reason: 'OPPORTUNITY_NOTES_CUSTOMER',
          http: linkedOpp.http,
          doctype: 'Opportunity',
          name: opportunityRow?.name,
          error: linkedOpp.error,
        }),
      );
    }
  }
  ...
  const matchingLeads = (recountLeads.leads || []).filter((row) => asString(row.notes).includes(idempotencyKey) || normalizeEmail(row.email_id) === normalizeEmail(email));
  const matchingOpps = (recountOpps.opportunities || []).filter(
    (row) => asString(row.notes).includes(idempotencyKey) || asString(row.title).includes(leadId),
  );
  ...
  if (!asString(gotLead.row.notes).includes(idempotencyKey)) mismatches.push('lead_idempotency_key');
  if (plan.create_opportunity) {
    if (!gotOpportunity.row) mismatches.push('opportunity_missing');
    else if (!asString(gotOpportunity.row.notes).includes(idempotencyKey)) mismatches.push('opportunity_idempotency_key');
  ...
  if (plan.create_customer && customerResult?.customer && !asString(gotOpportunity.row?.notes).includes(`customer=${customerResult.customer}`)) {
    mismatches.push('opportunity_customer_link');
  }
