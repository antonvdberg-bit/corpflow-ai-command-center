/**
 * ERPNext selling / quote-to-cash bridge (#1056 / onboarding C; current-main #1166).
 *
...
export function buildDraftQuotationPayload(event, opts = {}) {
  ...
  return {
    ...
    terms: approvedCommercialTermsHtml(loadCommercialDocumentsConfig(repoRoot)),
...
export async function reconcileDraftQuotation(event, opts) {
  ...
  if (!asString(got.row.terms)) mismatches.push('terms_body');
  ...
  return {
    ...
    readback: {
      ...
      terms_present: Boolean(asString(got.row.terms)),
