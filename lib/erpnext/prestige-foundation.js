 ...
 *   issue_type_http?: number | null,
 *   timesheet_create_http?: number | null,
 *   timesheet_project_linked?: boolean,
 *   project_task_count?: number | null,
 *   ids?: { project?: string | null, issue?: string | null, project_template?: string | null },
 * }} evidence
 * @param {string} [repoRoot]
 */
export function evaluateFoundationReadiness(evidence = {}, repoRoot = REPO_ROOT) {
  const cfg = loadPrestigeFoundationConfig(repoRoot);
  const blockers = [];
  const ids = evidence.ids && typeof evidence.ids === 'object' ? evidence.ids : {};
  ...
  const issueDenied = evidence.issue_http === 403 || evidence.issue_type_http === 403;
  ...
  if (!asString(ids.project)) {
    if (!blockers.includes('PROJECT_TASK_WRITE_DENIED') && !blockers.includes('PROJECT_NOT_VERIFIED')) {
      blockers.push('PROJECT_NOT_VERIFIED');
    }
  }
  if (!asString(ids.project_template)) {
    if (!blockers.includes('PROJECT_TASK_WRITE_DENIED') && !blockers.includes('PROJECT_NOT_VERIFIED')) {
      blockers.push('PROJECT_TEMPLATE_NOT_VERIFIED');
    }
  }
  if (Number(evidence.project_task_count || 0) < 12) {
    if (!blockers.includes('PROJECT_TASK_WRITE_DENIED')) blockers.push('PROJECT_TASKS_INCOMPLETE');
  }
  if (!evidence.issue_http || evidence.issue_http !== 200) {
    if (!blockers.includes('ISSUE_WRITE_DENIED')) blockers.push('ISSUE_NOT_VERIFIED');
  }
  if (!asString(ids.issue)) {
    if (!blockers.includes('ISSUE_WRITE_DENIED') && !blockers.includes('ISSUE_NOT_VERIFIED')) {
      blockers.push('ISSUE_NOT_VERIFIED');
    }
  }
  if (evidence.timesheet_project_linked === false) {
    blockers.push('TIMESHEET_PROJECT_LINK_FAILED');
  }
