...
export const ACCEPTANCE_VERDICT = 'ERPNext PROJECTS / SUPPORT OPERATIONALLY USABLE';
export const POINTER_SCHEMA = 'corpflow.delivery.erpnext.v1';
export const BRIDGE_IDS = ['project_task_timesheet', 'issue_support'];
const CLOSE_REOPEN_CONTRACT_REL = 'artifacts/erpnext/projects-support-ops-1097/apply-log.json';
const USABLE_ISSUE_STATUSES = new Set(['Open', 'Closed', 'Replied', 'On Hold', 'Resolved']);
...
/**
 * Wrap a Frappe client so create/update cannot mutate ERPNext.
 * GET/list/getLoggedUser pass through. Writes return ERPNEXT_WRITE_FORBIDDEN.
 * @param {object} client
 */
export function asReadOnlyFrappeClient(client) {
  const writes = [];
  function forbid(method) {
    return async function forbidden(doctype, nameOrPayload) {
      writes.push({
        method,
        doctype: asString(doctype),
        name: typeof nameOrPayload === 'string' ? asString(nameOrPayload) : null,
      });
      return { ok: false, http: 0, row: null, rows: [], error: 'ERPNEXT_WRITE_FORBIDDEN' };
    };
  }
  return {
    kind: 'read-only',
...
/**
 * Close/reopen was proved by the #1097 mutating apply. Acceptance must not re-close.
 * @param {string} [repoRoot]
 */
export function loadCloseReopenContract(repoRoot = REPO_ROOT) {
  try {
    const log = JSON.parse(readFileSync(path.join(repoRoot, CLOSE_REOPEN_CONTRACT_REL), 'utf8'));
    const closed = asString(log?.issue_lifecycle?.closed_status);
    const reopened = asString(log?.issue_lifecycle?.reopened_status);
    const proven = log?.issue_lifecycle?.proven === true && closed === 'Closed' && reopened === 'Open';
    return {
      present: true,
      artifact: CLOSE_REOPEN_CONTRACT_REL,
...
function trailFromReadback(issueRow, commRows, sentinel) {
  const rows = Array.isArray(commRows) ? commRows : [];
  const comm = rows.find((row) => includesSentinel(row.subject || row.content, sentinel));
  if (comm) return 'communication_present';
  if (includesSentinel(issueRow?.description, sentinel)) return 'issue_description';
  return null;
}

/**
 * GET-only operational acceptance. Repo-only config is never USABLE.
 * @param {Record<string, unknown>} evidence
 */
export function evaluateOpsAcceptance(evidence = {}, repoRoot = REPO_ROOT) {
  const cfg = loadProjectsSupportOpsConfig(repoRoot);
  const blockers = [];
  if (evidence.write_attempted === true) blockers.push('ERPNEXT_WRITE_ATTEMPTED');
  if (evidence.postgres_written === true) blockers.push('POSTGRES_WRITTEN');
  if (evidence.send_attempted === true) blockers.push('EXTERNAL_SEND_ATTEMPTED');
  if (evidence.timesheet_submitted === true) blockers.push('TIMESHEET_SUBMITTED');
  if (asString(evidence.project) !== asString(cfg.reuse.project)) blockers.push('PROJECT_NOT_REUSED');
  ...
  if (evidence.project_owner_proven !== true) blockers.push('PROJECT_OWNER_UNPROVEN');
  if (evidence.task_readback !== true) blockers.push('TASK_READBACK_MISSING');
  if (evidence.next_action_proven !== true) blockers.push('NEXT_ACTION_UNPROVEN');
  if (evidence.timesheet_readback !== true) blockers.push('TIMESHEET_READBACK_MISSING');
  if (evidence.issue_readback !== true) blockers.push('ISSUE_READBACK_MISSING');
  if (evidence.issue_owner_proven !== true) blockers.push('ISSUE_OWNER_UNPROVEN');
  if (evidence.issue_trail_proven !== true) blockers.push('ISSUE_TRAIL_UNPROVEN');
  if (evidence.issue_lifecycle_contract_proven !== true) blockers.push('ISSUE_LIFECYCLE_CONTRACT_UNPROVEN');
  if (asString(evidence.timesheet_verdict) !== 'DEFER') blockers.push('TIMESHEET_VERDICT_NOT_DEFER');
  if (asString(evidence.pointer_postgres_persist) !== 'not_written') blockers.push('POINTER_COPIES_TRUTH');
  if (evidence.created_on_replay === true) blockers.push('CREATED_ON_REPLAY');
  const expectedVerdict = asString(cfg.acceptance_verdict) || ACCEPTANCE_VERDICT;
  return {
    ok: blockers.length === 0,
    verdict: blockers.length === 0 ? expectedVerdict : `NOT READY — ${blockers[0]}`,
    blockers,
  };
}
...
/**
 * GET/read-only operational acceptance (#1202).
 * Reuses #920/#1097 synthetic IDs. Never creates, updates, or submits.
 *
 * @param {object} client
 * @param {{ repoRoot?: string, nowIso?: string }} [opts]
 */
export async function inspectProjectsSupportOps(client, opts = {}) {
  const repoRoot = opts.repoRoot || REPO_ROOT;
  const cfg = loadProjectsSupportOpsConfig(repoRoot);
  const nowIso = asString(opts.nowIso) || new Date().toISOString().replace(/\.\d{3}Z$/, 'Z');
  ...
  const projectSearch = await listSafe(readonly, 'Project', {
    fields: ['name', 'project_name', 'customer', 'status'],
    filters: [['project_name', '=', cfg.reuse.project_name]],
    limit: 10,
  });
  ...
  const projectGet = await readonly.get('Project', cfg.reuse.project);
  http.project_get = asNumber(projectGet?.http, 0);
  const projectRow = projectGet?.row || null;

  const taskList = await listSafe(readonly, 'Task', {
    fields: [
      'name',
      'subject',
      'status',
  ...
  http.task_list = taskList.http;
  const tasks = taskList.rows.map(summarizeTask).filter(Boolean);
  const nextAction = nextActionFromTasks(tasks);

  const firstTaskName = asString(cfg.reuse.tasks[0]);
  const taskTodos = await listSafe(readonly, 'ToDo', {
    fields: ['name', 'status', 'allocated_to', 'reference_type', 'reference_name', 'description'],
    filters: [
      ['reference_type', '=', 'Task'],
      ['reference_name', '=', firstTaskName],
  ...
  const timesheetGet = await readonly.get('Timesheet', cfg.reuse.timesheet);
  http.timesheet_get = asNumber(timesheetGet?.http, 0);
  const timesheetClass = classifyTimesheet(timesheetGet?.row || {}, repoRoot);

  const issueSearch = await listSafe(readonly, 'Issue', {
    fields: ['name', 'subject', 'status', 'customer', 'project'],
    filters: [['subject', '=', cfg.reuse.issue_subject]],
    limit: 10,
  });
  ...
  const issueGet = await readonly.get('Issue', cfg.reuse.issue);
  http.issue_get = asNumber(issueGet?.http, 0);
  const issueRow = issueGet?.row || null;

  const commList = await listSafe(readonly, 'Communication', {
    fields: ['name', 'subject', 'communication_medium', 'sent_or_received', 'reference_doctype', 'reference_name'],
    filters: [
      ['reference_doctype', '=', 'Issue'],
      ['reference_name', '=', cfg.reuse.issue],
  ...
  http.communication_list = commList.http;
  const trailKind = trailFromReadback(issueRow, commList.rows, cfg.synthetic.sentinel);

  const issueTodos = await listSafe(readonly, 'ToDo', {
    fields: ['name', 'status', 'allocated_to', 'reference_type', 'reference_name', 'description'],
    filters: [
      ['reference_type', '=', 'Issue'],
      ['reference_name', '=', cfg.reuse.issue],
  ...
  const versionList = await listSafe(readonly, 'Version', {
    fields: ['name', 'ref_doctype', 'docname'],
    filters: [
      ['ref_doctype', '=', 'Issue'],
      ['docname', '=', cfg.reuse.issue],
    ],
    limit: 5,
  });
  http.version_list = versionList.http;
  if (!versionList.ok) gaps.push('VERSION_TRAIL_UNREADABLE');

  const slaList = await listSafe(readonly, 'SLA', { fields: ['name'], limit: 1 });
  const assignmentRuleList = await listSafe(readonly, 'Assignment Rule', { fields: ['name'], limit: 1 });
  http.sla = slaList.http;
  http.assignment_rule = assignmentRuleList.http;

  const projectReplay = await listSafe(readonly, 'Project', {
    fields: ['name', 'project_name', 'customer', 'status'],
    filters: [['project_name', '=', cfg.reuse.project_name]],
    limit: 10,
  });
  const issueReplay = await listSafe(readonly, 'Issue', {
    fields: ['name', 'subject', 'status'],
    filters: [['subject', '=', cfg.reuse.issue_subject]],
    limit: 10,
  });
  ...
  const contract = loadCloseReopenContract(repoRoot);
  const issueStatus = asString(issueRow?.status);
  const lifecycleContractProven =
    contract.proven === true && Boolean(trailKind) && USABLE_ISSUE_STATUSES.has(issueStatus);
  ...
  const writeAttempted = readonly.writes.length > 0;
  const evidence = {
    write_attempted: writeAttempted,
    postgres_written: false,
    send_attempted: false,
    timesheet_submitted: asNumber(timesheetGet?.row?.docstatus, 0) !== 0,
    project: asString(projectRow?.name || projectDecision.name),
    issue: asString(issueRow?.name || issueDecision.name),
    project_duplicate_count: replayProject.duplicate_count,
    issue_duplicate_count: replayIssue.duplicate_count,
    project_readback: Boolean(projectRow),
    project_owner_proven: Boolean(asString(projectRow?.owner)),
    task_readback: tasks.length >= 12,
    next_action_proven: Boolean(nextAction?.task),
    timesheet_readback: Boolean(timesheetGet?.row),
    issue_readback: Boolean(issueRow),
    issue_owner_proven: Boolean(asString(issueRow?.owner)),
    issue_trail_proven: Boolean(trailKind),
    issue_lifecycle_contract_proven: lifecycleContractProven,
    timesheet_verdict: timesheetClass.verdict,
    pointer_postgres_persist: asString(pointer.postgres_persist),
    created_on_replay: replayProject.action !== 'REUSE' || replayIssue.action !== 'REUSE',
  };
  const readiness = evaluateOpsAcceptance(evidence, repoRoot);
  ...
  return {
    ...
    created_on_replay: evidence.created_on_replay,
    project: summarizeProject(projectRow),
    tasks,
    ...
    issue_lifecycle: {
      ...
      close_reopen_this_run: 'not_attempted',
      contract_artifact: contract.artifact,
      contract_closed_status: contract.closed_status,
      contract_reopened_status: contract.reopened_status,
      contract_proven: contract.proven,
      proven: lifecycleContractProven,
    },
    issue_trail: trailKind,
    issue_history: {
      communication_http: commList.http,
      version_http: versionList.http,
      version_count: versionList.rows.length,
      todo_count: issueTodos.rows.length,
      task_todo_count: taskTodos.rows.length,
    },
    sla: {
      decision: slaDecision(repoRoot),
      sla_http: slaList.http,
