 ...
 * @returns {Promise<{
 *   ref: string,
 *   published_hero: { src: string, alt: string, caption: string | null, content_type: string, variant: string } | null,
 *   published_gallery: { src: string, alt: string, caption: string | null, gallery_order: number | null, is_gallery_cover: boolean, content_type: string, variant: string }[],
 *   published_card: { src: string, alt: string, caption: string | null, intended_slot: 'card', content_type: string, variant: string } | null,
 * }>}
 */
export async function collectPublishedLuxPropertyMedia(prisma, propertySlugOrRef, options = {}) {
  ...
  if (firstHero) {
    ...
    if (att && String(att.tenantId || '').trim() === 'luxe-maurice' && String(att.contentType || '').toLowerCase().startsWith('image/')) {
      publishedHero = {
        src: buildLuxPublicPropertyMediaSrc(resolved.ref, att.id, 'hero'),
  ...
  const published_gallery = filtered.map((g) => ({
    src: buildLuxPublicPropertyMediaSrc(resolved.ref, g.attachment_id, 'gallery'),
  ...
  if (cardCandidates.length) {
    ...
    if (firstCard) {
      ...
      publishedCard = {
        src: buildLuxPublicPropertyMediaSrc(resolved.ref, firstCard.attachment_id, 'card'),
...
export async function collectPublishedLuxCardMediaByPropertyRefs(prisma, propertyRefsRaw, options = {}) {
  ...
  for (const [refLower, cands] of cardCandidatesByRef) {
    ...
    out.set(refLower, {
      src: buildLuxPublicPropertyMediaSrc(canonical, cand.attachment_id, 'card'),
