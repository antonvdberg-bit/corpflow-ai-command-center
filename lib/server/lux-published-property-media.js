...
export async function collectPublishedLuxPropertyMedia(prisma, propertySlugOrRef, options = {}) {
  ...
  if (firstHero) {
    ...
    if (att && String(att.tenantId || '').trim() === 'luxe-maurice' && String(att.contentType || '').toLowerCase().startsWith('image/')) {
      const { src, srcSet } = buildLuxPublicPropertyMediaSrcSet(resolved.ref, att.id, 'hero');
      publishedHero = {
        src,
        src_set: srcSet,
  ...
  const published_gallery = filtered.map((g) => {
    const { src, srcSet } = buildLuxPublicPropertyMediaSrcSet(resolved.ref, g.attachment_id, 'gallery');
    return {
      src,
      src_set: srcSet,
  ...
  if (cardCandidates.length) {
    ...
    if (firstCard) {
      ...
      const { src, srcSet } = buildLuxPublicPropertyMediaSrcSet(resolved.ref, firstCard.attachment_id, 'card');
      publishedCard = {
        src,
        src_set: srcSet,
