...
export const FACTORY_GENERATION_BOUNDARY_MARKER =
  'corpflow.factory_generation_boundary_required.v1';

/**
 * Durable explanation when a ready label is removed because prior-generation
 * completion/review evidence still owns the issue. Prevents silent pickup loss.
 *
 * @param {{ issueNumber: number, reason?: string | null }} input
 */
export function formatGenerationBoundaryRequiredComment(input) {
  const issueNumber = toPositiveIssueNumber(input.issueNumber);
  if (!issueNumber) throw new Error('issueNumber required');
  const reason = emptyToNull(input.reason) ||
    'prior generation remains review/terminal inventory';
  return `CURSOR GENERATION BOUNDARY REQUIRED

Issue: #${issueNumber}
State: NOT CLAIMED
Reason: ${reason}

The current \`dispatch:cursor-ready\` label cannot start a new Cursor generation because durable completion/review evidence exists from an older generation.

Required next action: use one explicit authorised \`CURSOR REQUEUE\` generation boundary for the current execution packet, then let the existing canonical Handoff acquire the new generation.

Do not label-cycle, create a duplicate agent, or bypass the canonical Handoff.

<!-- ${FACTORY_GENERATION_BOUNDARY_MARKER} {"source_issue":${issueNumber},"state":"GENERATION_BOUNDARY_REQUIRED"} -->
`;
}


/**
 * Ready labels are activation inputs, not review inventory markers. Return
 * source issues whose durable review/terminal evidence makes a lingering ready
 * label unsafe and misleading. This deliberately excludes protected-gate holds:
 * those retain their ready label so an exact authorization can resume them.
 *
 * @param {{ decisions?: Array<{ issue?: { number?: unknown }, reason?: string | null }> | null }} plan
 * @returns {number[]}
 */
export function findStaleReadyReviewIssueNumbers(plan) {
  const decisions = Array.isArray(plan?.decisions) ? plan.decisions : [];
  const issueNumbers = new Set();
  for (const decision of decisions) {
    const reason = String(decision?.reason || '');
    const isReviewInventory =
      /dispatch:operator-review/i.test(reason) ||
      /review-ready linked PR or terminal completion/i.test(reason);
    if (!isReviewInventory) continue;
    const issueNumber = toPositiveIssueNumber(decision?.issue?.number);
