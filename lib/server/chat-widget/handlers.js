...
import { getNode, sanitiseFieldInput, publicNodeView } from './flow.js';
import { resolveChatWidgetFlowForTenant } from './living-word-flow-resolve.js';
import { extractClientIp, hashIpForTenant, checkAndConsume } from './rate-limit.js';
import { emitChatWidgetSubmitted } from './notify.js';
import { tryProcessAutomationEventForWorkflows } from '../tenant-workflow/process-event.js';
...
export async function handleChatWidgetStart(req, res) {
  ...
  let flow;
  try { flow = resolveChatWidgetFlowForTenant(cfg.tenantId, cfg.flowJson); } catch (e) {
...
export async function handleChatWidgetStep(req, res) {
  ...
  const thread = await prisma.chatWidgetThread.findFirst({
    ...
    select: {
      ...
      leadName: true, leadSurname: true, leadEmail: true, leadPhone: true,
      preferredContactMethod: true, requestType: true, leadMessage: true,
  ...
  if (currentNode.type === 'menu' || (currentNode.type === 'info' && Array.isArray(currentNode.options))) {
    ...
    if (typeof valid.widget_action === 'string' && valid.widget_action.trim()) {
      await prisma.chatWidgetMessage.create({
        data: {
          threadId: thread.id,
          tenantId: cfg.tenantId,
          ...
          content: String(valid.label || choice).slice(0, 2000),
        },
      });
      return res.status(200).json({
        ok: true,
        widget_action: valid.widget_action.trim(),
      });
    }
    nextNodeId = valid.next;
    userText = String(valid.label || choice);
    if (typeof valid.store_field === 'string' && typeof valid.store_value === 'string') {
      const sf = valid.store_field;
      const sv = valid.store_value;
      if (sf === 'preferred_contact_method') updateLeadFields.preferredContactMethod = sv;
      else if (sf === 'request_type') updateLeadFields.requestType = sv;
    }
    ...
    userText = cleaned;
    if (currentNode.field === 'name' || currentNode.field === 'first_name') updateLeadFields.leadName = cleaned;
    else if (currentNode.field === 'surname') updateLeadFields.leadSurname = cleaned;
    ...
    else if (currentNode.field === 'preferred_contact_method') {
      updateLeadFields.preferredContactMethod = cleaned;
    }
  ...
  const submitted = nextNode.type === 'submit';

  if (submitted) {
    const afterId = typeof nextNode.next_after === 'string' ? nextNode.next_after.trim() : '';
    if (afterId && getNode(flow, afterId)) {
      responseNodeId = afterId;
      responseNode = getNode(flow, afterId);
    }
  }
  ...
  await prisma.chatWidgetMessage.create({
    data: {
      ...
      content: String(responseNode.prompt || ''),
    },
  });
  ...
  if (submitted) {
    ...
    if (responseNodeId !== nextNodeId) {
      leadUpdate.status = 'active';
      leadUpdate.currentNode = responseNodeId;
    } else {
  ...
  if (submitted) {
    ...
    const notify = await emitChatWidgetSubmitted(prisma, {
      ...
      leadSurname: fresh && fresh.leadSurname,
      leadEmail: fresh && fresh.leadEmail,
      leadPhone: fresh && fresh.leadPhone,
      preferredContactMethod: fresh && fresh.preferredContactMethod,
    ...
    if (notify.ok && notify.eventId) {
      await tryProcessAutomationEventForWorkflows(prisma, notify.eventId);
    }
  }

  const completed = submitted && responseNodeId === nextNodeId;

  return res.status(200).json({
    ok: true,
    next: publicNodeView(responseNodeId, responseNode),
    completed,
    submitted,
  });
}
