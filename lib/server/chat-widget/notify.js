 ...
 * @returns {string | null}
 */
export function buildChatWidgetLeadDisplayName(lead) {
  const first = lead.leadName != null ? String(lead.leadName).trim() : '';
  const last = lead.leadSurname != null ? String(lead.leadSurname).trim() : '';
  const combined = [first, last].filter(Boolean).join(' ').trim();
  return combined || first || last || null;
}
 ...
 */
export function buildChatWidgetLeadPayload(ctx) {
  ...
  const displayName = buildChatWidgetLeadDisplayName(ctx);
  return {
    ...
    lead: {
      name: displayName,
      first_name: ctx.leadName || null,
      surname: ctx.leadSurname || null,
      email: ctx.leadEmail || null,
      phone: ctx.leadPhone || null,
      whatsapp_or_mobile: ctx.leadPhone || null,
      preferred_contact_method: ctx.preferredContactMethod || null,
...
export async function emitChatWidgetSubmitted(prisma, ctx) {
  ...
  const payload = buildChatWidgetLeadPayload(ctx);
