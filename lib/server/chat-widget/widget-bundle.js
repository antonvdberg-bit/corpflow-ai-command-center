...
(function(){
  ...
  function makeStyles(accent){
    ...
    styleEl.appendChild(document.createTextNode([
      ...
      '.cfcw-btn-primary { background: ' + ac + '; color: #fff; border: none; border-radius: 10px; padding: 10px 12px; font: inherit; font-weight: 600; cursor: pointer; text-align: center; width: 100%; }',
      '.cfcw-btn-primary:hover { filter: brightness(1.05); }',
  ...
  function isAskOnlyAiMenu(node){
    if (!node || node.type !== 'menu' || !Array.isArray(node.options)) return false;
    return node.options.length > 0 && node.options.every(function(o){ return o && o.widget_action === 'ai_ask'; });
  }

  function renderNode(node){
    if (!node) return;
    state.currentNode = node;
    if (isAskOnlyAiMenu(node)) {
      pushMessage('bot', node.prompt);
      showAiAskPanel({ skipIntro: true });
      return;
    }
    pushMessage('bot', node.prompt);
    setActions(null);
    ...
    if (node.type === 'menu' || (node.type === 'info' && Array.isArray(node.options) && node.options.length)) {
      var btns = (node.options || []).map(function(opt){
        ...
        b.className = opt.widget_action === 'ai_ask' ? 'cfcw-btn-primary' : 'cfcw-btn';
