 ...
 * If Groq is configured but the call fails, fall back to retrieval-preview whenever
 * approved context exists so visitors still receive the approved records.
 * Retrieval-first: Groq when GROQ_API_KEY is configured and the call succeeds;
 * retrieval-preview when Groq is unset **or** the live call fails but approved
 * records already produced context. `provider_unavailable` is only for the
 * remaining case where Groq failed and preview text is empty.
 */
 ...
 *   threadId: string;
 *   question: string;
 *   ctx: import('./context-builder.js').RetrievalContext;
 *   meta: { contextAtomIds: string[]; scheduleEntryIds: string[] };
 * }} args
 * @returns {Promise<AiAnswerResult>}
 */
async function answerFromRetrievalPreview(prisma, args) {
  const { cfg, threadId, question, ctx, meta } = args;
 ...
 * @returns {Promise<AiAnswerResult>}
 */
async function finishWithRetrievalPreview(prisma, args) {
  const { cfg, threadId, question, ctx, meta } = args;
  const preview = formatRetrievalPreviewAnswer(ctx);
  const answer = sanitiseModelAnswer(preview || UNKNOWN_ANSWER_FALLBACK, MAX_ANSWER_CHARS);
  await logChatWidgetAiUsage(prisma, {
...
export async function answerChatWidgetQuestion(prisma, args) {
  ...
  if (!isGroqConfigured()) {
    return answerFromRetrievalPreview(prisma, { cfg, threadId, question, ctx, meta });
  }
