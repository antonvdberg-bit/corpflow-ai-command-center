 ...
 */
import { buildEvidenceManifest, RARE_EXCLUSIVE_TARGET_REPO, normalizeSha, str } from './jan-approval-control.js';
...
export function createEvidenceHash(evidence) {
  return buildEvidenceManifest(evidence).hash;
}
...
export async function fetchBoundedJanReviewPackage({ repo, prNumber, token = '', fetchFn = globalThis.fetch, contextPaths = [] }) {
  ...
  return {
    evidence,
    manifest: {
      ...buildEvidenceManifest(evidence),
      repository: repo,
      pr_number: number,
