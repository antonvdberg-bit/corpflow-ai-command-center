...
export default async function tenantIntakeHandler(req, res) {
  ...
  const buyerNeed = str(meta.buyer_need);
  const lockedProduct =
    meta.locked_product === true ||
    meta.locked_product === 'true' ||
    meta.locked_product === 1 ||
    meta.locked_product === '1';

  const routed = isRapidDelivery
    ? resolveMarketEnquiryRouting({
        buyer_need: buyerNeed || null,
        service_path: meta.service_path,
        offer_slug: meta.offer_slug,
        locked_offer: lockedProduct,
      })
    : null;

  const servicePath = routed?.ok
    ? routed.service_path
    : str(meta.service_path);
  const offerSlug = routed?.ok
    ? routed.offer_slug
    : resolveOfferSlugForMarketEnquiry(str(meta.service_path), meta.offer_slug) ||
      str(meta.offer_slug);
  ...
  if (isRapidDelivery) {
    ...
    if (routed && !routed.ok) {
      return res.status(400).json({
        error: routed.reason || 'INVALID_ROUTING',
        hint: 'Provide a valid buyer_need, or a consistent service_path / offer_slug pair.',
      });
    }
    if (!servicePath && !isRapidDeliveryOfferSlug(offerSlug)) {
      return res.status(400).json({
        error: 'INVALID_OFFER_SLUG',
        hint: 'Provide meta.buyer_need, meta.service_path, or a valid meta.offer_slug.',
      });
    }
    ...
    const pairCheck = isConsistentServiceProductPair(servicePath, offerSlug);
    if (!pairCheck.ok) {
      return res.status(400).json({
        error: pairCheck.reason || 'CONTRADICTORY_SERVICE_PRODUCT',
        hint: 'service_path and offer_slug must not contradict each other.',
      });
    }
  ...
  if (isAiLeadRescue || isRapidDelivery || Object.keys(meta).length > 0) {
    ...
    if (isRapidDelivery) {
      ...
      if (routed?.ok) {
        intakeMeta.buyer_need = routed.buyer_need;
        intakeMeta.service_interest = routed.service_interest;
      } else if (buyerNeed) {
        intakeMeta.buyer_need = buyerNeed;
      }
    }
    if (isAiLeadRescue) {
      intakeMeta.website = str(meta.website) || null;
    ...
    if (isRapidDelivery) {
      ...
      await recordTrustedAutomationEvent(prisma, {
        ...
        payload: {
          ...
          operator_path: '/app/prospects',
          change_revenue_path: '/app/prospects',
