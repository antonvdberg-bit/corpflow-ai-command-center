...
export default async function tenantIntakeHandler(req, res) {
  ...
  const isMarketEnquiry = isRapidDelivery && (meta.market_enquiry === true || isCorpFlowServicePathId(servicePath));
  ...
  if (isRapidDelivery) {
    ...
    if (meta.consent_to_contact !== true && str(meta.consent_to_contact) !== 'true') {
      return res.status(400).json({
        error: 'CONSENT_REQUIRED',
        hint: 'meta.consent_to_contact must be true.',
      });
    }
    if (isMarketEnquiry) {
      if (!isCorpFlowServicePathId(servicePath)) {
        return res.status(400).json({
          error: 'INVALID_SERVICE_PATH',
          hint: 'meta.service_path must be one of the CorpFlowAI service path ids.',
        });
      }
      if (str(meta.urgency) && !isCorpFlowEnquiryUrgency(str(meta.urgency))) {
        return res.status(400).json({
          error: 'INVALID_URGENCY',
          hint: 'meta.urgency must be a known timing option.',
        });
      }
      if (offerSlug && !isRapidDeliveryOfferSlug(offerSlug)) {
        return res.status(400).json({
          error: 'INVALID_OFFER_SLUG',
          hint: 'meta.offer_slug must be one of the rapid-delivery offer slugs when provided.',
        });
      }
    } else if (!isRapidDeliveryOfferSlug(offerSlug)) {
      return res.status(400).json({
        error: 'INVALID_OFFER_SLUG',
  ...
  try {
    const lead = await prisma.lead.create({
      data: {
        ...
        intent: isRapidDelivery
          ? isMarketEnquiry
            ? `rapid_delivery:path:${servicePath}`
    ...
    if (isRapidDelivery) {
      const offer = offerSlug && isRapidDeliveryOfferSlug(offerSlug) ? getRapidDeliveryOffer(offerSlug) : null;
      ...
      await recordTrustedAutomationEvent(prisma, {
        ...
        payload: {
          ...
          market_enquiry: isMarketEnquiry,
