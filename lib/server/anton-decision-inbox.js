 ...
 * Contract (#684): send only when Anton required: yes; include workstream, issue/PR
 * link, why needed now, exact action, recommendation, consequence of delay.
 *
...
export function formatExceptionNotifyMessage(fields = {}) {
  const errors = [];
  const project = str(fields.project_workstream) || str(fields.workstream);
  const issueOrPr = str(fields.issue_or_pr);
  const action =
    str(fields.exact_action_required) || str(fields.exact_action) || str(fields.approval_type);
  const why =
    str(fields.why_needed_now) || str(fields.why) || 'protected decision required now';
  const recommendation = str(fields.recommendation) || str(fields.recommended_decision);
  const consequence =
    str(fields.consequence_of_delay) ||
    str(fields.consequence) ||
    str(fields.consequence_of_reject_or_defer);
  const link = str(fields.github_link) || str(fields.link);
  ...
  if (!why) errors.push('missing why_needed_now');
  if (!recommendation) errors.push('missing recommendation');
  if (!consequence) errors.push('missing consequence_of_delay');
  if (
    /secret|password|token|api[_-]?key|bearer\s/i.test(
      `${project}\n${action}\n${link}\n${why}\n${recommendation}\n${consequence}`,
    )
  ) {
