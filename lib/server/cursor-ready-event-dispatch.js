 ...
 * - workflow_call scheduled whole-queue reconciliation (#1023)
...
export function resolveFactoryDispatcherRunPlan(opts = {}) {
  ...
  // Reusable workflow call from lifecycle capacity release, scheduled
  // whole-queue reconcile (#1023), or explicit continuation wake.
  if (eventName === 'workflow_call') {
    const wakeReason = String(opts.wakeReasonInput || '')
      .trim()
      .toLowerCase();
    const scheduledReconcile =
      wakeReason === 'scheduled_reconciliation' || wakeReason === 'schedule_fallback';
