 ...
 *
 * Pure predicates for factory-dispatcher-activate wake sources:
 ...
 * - schedule fallback / manual workflow_dispatch
 *
 * #929: schedule is an authoritative cursor_live recovery path. It does not
 * require CURSOR_LIVE_ENABLED. Manual workflow_dispatch remains the dry_run
 * path. Fail-closed still applies when CURSOR_API_KEY is absent.
 *
...
export function resolveFactoryDispatcherRunPlan(opts = {}) {
  ...
  // cursorLiveEnabled is accepted for evidence/compat only. #929: it must not
  // gate scheduled cursor_live. Manual workflow_dispatch still chooses dry_run.
  void opts.cursorLiveEnabled;
  ...
  // Reusable workflow call from lifecycle capacity release (or explicit wake).
  ...
  if (eventName === 'issue_comment') {
    if (!shouldWakeOnOperatorAuthorizationComment(opts)) {
    ...
    return {
      ...
      path: 'event_operator_authorization',
