 ...
 * @returns {string | null}
 */
export function buildClientSitePreviewUrl(previewUrl, tenantId, opts = {}) {
  ...
  try {
    ...
    const preferAppRoot =
      opts?.preferAppRoot === true || tid === 'cipc-desk';
    // Preview deployments reliably serve `/lux-landing-static` via vercel.json → static HTML.
    // Root `/` is handled by Next.js and can 404 on some Preview builds while `/api/*` still works.
    // CIPC Desk (and preferAppRoot) must use Next `/` so website_draft SSR runs.
    if (isVercelAppDeploymentHost(u.hostname) && !preferAppRoot) {
      const rawPath = (u.pathname || '/').replace(/\/+$/, '') || '/';
      if (rawPath === '/' || rawPath === '') {
        u.pathname = '/lux-landing-static';
      }
    }
