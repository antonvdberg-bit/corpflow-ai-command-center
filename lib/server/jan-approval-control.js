...
export const JAN_APPROVAL_SCHEMA = 'corpflow.jan_durable_decision.v1';
...
export const JAN_DECISIONS = Object.freeze(['APPROVE', 'CHANGES', 'HOLD', 'ASK_AI']);
...
export const SAFE_NEXT_STEPS = Object.freeze({
  ...
  ASK_AI: 'request_ai_review_comment',
});
...
export function formatJanDurableDecisionComment(fields) {
  ...
  const payload = {
    ...
    note: str(fields.note).slice(0, 500),
  };
  const lines = [
...
export function listJanDecisionsFromComments(comments) {
  ...
  for (const row of Array.isArray(comments) ? comments : []) {
    ...
    if (parsed.ok && parsed.record) out.push(parsed.record);
  }
  return out;
}
