...
async function fetchJsonWithTimeout(url, init, ms) {
  ...
  try {
    ...
    return {
      ...
      body: { error: isAbort ? 'timeout' : 'fetch_failed', message: msg, name },
    };
  } finally {
    clearTimeout(t);
  }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    return json(res, 500, {
      error: 'ENSURE_CONSOLE_FIELD_CRASH',
      detail: e instanceof Error ? e.message : String(e),
      url: listUrl,
      table_id: tableId,
      field_name: fieldName,
      hint:
        'Check BASEROW_URL points to your Baserow instance origin (no trailing /api). Also verify BASEROW_TOKEN has permission to edit table schema.',
    });
  }
}
