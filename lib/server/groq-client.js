...
import { emitProductionGroqTrace } from './langfuse-production-observability.js';
 ...
 * @param {{ workflow?: string, product?: string, workstream_id?: string, work_packet_id?: string }} [opts.observability]
 * @returns {Promise<Response>}
 */
export async function groqChatCompletionsFetch(opts) {
  const key = getGroqApiKey();
  const startedAtMs = Date.now();
  const response = await fetch(GROQ_OPENAI_CHAT_COMPLETIONS_URL, {
    method: 'POST',
    headers: {
  ...
  // Observability is fail-open: it must never change the provider response or expose raw content.
  try {
    const providerData = await response.clone().json().catch(() => ({}));
    await emitProductionGroqTrace({
      model: opts.model,
      messages: opts.messages,
      providerData,
      ...
      startedAtMs,
      endedAtMs,
      context: opts.observability,
    });
  } catch {
    // Deliberately ignore telemetry errors; the business call remains authoritative.
  }
