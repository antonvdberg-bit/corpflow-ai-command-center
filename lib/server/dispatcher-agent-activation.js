...
export function buildDispatcherActivationPlan(dispatcherReport, opts = {}) {
  ...
  const skip = plan.anton.length + plan.n8n.length + plan.no_action.length;
  ...
  if (mode === DISPATCHER_ACTIVATION_MODE_CURSOR_LIVE) {
    ...
    if (liveDecision) {
      ...
      const sourceIssue = Number(
        String(liveDecision.routing?.objectRef || '').match(/^issue:(\d+)$/i)?.[1] || 0,
      );
      const modelSelection = resolveCursorExecutionModelSelection(
        await listCursorCloudAgentModels(apiKey, opts.cursorDeps),
...
/**
 * Direct-issue activation: manual workflow_dispatch input, issue-scan handoff,
 * or event-driven `issues:labeled` ready path (Phase A).
 * Bare `target_issue` on schedule remains forbidden unless scan/event handoff.
 *
 * @param {{
