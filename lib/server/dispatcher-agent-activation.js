 ...
 * @param {{ mode?: DispatcherActivationMode, dedupeKeys?: string[], maxCursorLive?: number }} [opts]
  ...
  for (const routing of list) {
    ...
    if (owner === 'cursor') {
      ...
      if (dedupeKeys.has(dedupeKey)) {
        decisions.push({ routing, action: 'SKIP_DEDUPE', dedupeKey });
        continue;
      }
      ...
      if (cursorLiveCount < maxCursorLive) {
        decisions.push({ routing, action: 'ACTIVATE_CURSOR', dedupeKey });
        cursorLiveCount += 1;
        continue;
      }

      decisions.push({ routing, action: 'SKIP_CURSOR_CAP', dedupeKey });
    }
  }
...
export async function runDispatcherActivation(dispatcherReport, opts = {}) {
  ...
  const report = injectSmokeInternalCursorRouting(
    dispatcherReport && typeof dispatcherReport === 'object' ? dispatcherReport : {},
    { smokeInternal: Boolean(opts.smokeInternal), smokePrompt: opts.smokePrompt },
  );
  const routings = Array.isArray(report.routings) ? report.routings : [];
  ...
  if (mode === DISPATCHER_ACTIVATION_MODE_CURSOR_LIVE) {
    ...
    if (liveDecision) {
      const { buildCursorAgentCreatePayload, createCursorCloudAgent } = await import(
        './cursor-cloud-agent-client.js'
      );
      ...
      const agent = apiResult?.agent && typeof apiResult.agent === 'object' ? apiResult.agent : {};
      cursorLive = {
        objectRef: liveDecision.routing.objectRef,
        dedupeKey: liveDecision.dedupeKey,
        agentId: agent.id ?? null,
        agentUrl: agent.url ?? null,
        runId: apiResult?.run?.id ?? null,
      };
