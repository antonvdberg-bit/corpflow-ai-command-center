...
export function buildCursorAgentCreatePayload(routing, opts = {}) {
  ...
  const model = opts.model || resolveCursorExecutionTier({ tier: opts.executionTier }).model;
...
export async function createCursorCloudAgent(apiKey, payload, deps = {}) {
  ...
  if (
    ...
    throw new Error('Cursor Cloud Agent create requires one explicit model selection');
  }
...
export async function createCursorRepairAgentForPr(apiKey, opts, deps = {}) {
  ...
  const payload = {
    ...
    model: opts.model || resolveCursorExecutionTier({ tier: opts.executionTier }).model,
    repos: [
      {
        url: opts.repoUrl || CORPFLOW_CURSOR_REPO_URL,
        prUrl,
