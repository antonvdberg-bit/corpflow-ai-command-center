 ...
 */
import {
  isLiveCursorExecutionModelSelection,
 ...
 * @param {{ repoUrl?: string, startingRef?: string, namePrefix?: string, executionTier?: string, sourceIssue?: number, comments?: Array<object>, modelSelection?: { id: string, params: Array<{ id: string, value: string }> }, autoCreatePR?: boolean }} [opts]
 */
export function buildCursorAgentCreatePayload(routing, opts = {}) {
  ...
  const model = opts.modelSelection;
  if (model && (!String(model.id || '').trim() || !Array.isArray(model.params))) {
    throw new Error(
      'Cursor Cloud Agent create payload requires a live catalogue-resolved model selection',
    );
  }
...
/**
 * Return bounded model-only metadata suitable for a GitHub failure receipt.
 * The catalogue is account-scoped, but model names and supported parameter
 * shapes are not credentials. Do not include descriptions or unknown fields:
 * an API response must never be treated as safe-to-log wholesale.
 *
 * @param {unknown} modelCatalog
 * @param {unknown} [requestedModelId]
 * @returns {{ itemCount: number, requestedItem: { id: string, displayName: string | null, aliases: string[], parameterIds: string[], variantCount: number, variantParams: string[] } | null, items: Array<{ id: string, displayName: string | null }> }}
 */
export function summarizeCursorModelCatalog(modelCatalog, requestedModelId) {
  const sourceItems = Array.isArray(modelCatalog?.items) ? modelCatalog.items : [];
  const requestedId = String(requestedModelId || '').trim();
  const requestedItem = sourceItems.find((item) => String(item?.id || '') === requestedId);
  const summarizeItem = (item) => ({
    id: String(item?.id || '').slice(0, 120),
    displayName: String(item?.displayName || '').trim().slice(0, 120) || null,
    aliases: (Array.isArray(item?.aliases) ? item.aliases : [])
      .slice(0, 10)
      .map((alias) => String(alias || '').slice(0, 120)),
    parameterIds: (Array.isArray(item?.parameters) ? item.parameters : [])
      .slice(0, 10)
      .map((parameter) => String(parameter?.id || '').slice(0, 120)),
    variantCount: Array.isArray(item?.variants) ? item.variants.length : 0,
    variantParams: (Array.isArray(item?.variants) ? item.variants : [])
      .slice(0, 24)
      .map((variant) =>
        (Array.isArray(variant?.params) ? variant.params : [])
          .map((parameter) => `${String(parameter?.id || '').slice(0, 80)}=${String(parameter?.value || '').slice(0, 80)}`)
          .join(','),
      ),
  });
  return {
    itemCount: sourceItems.length,
    requestedItem: requestedItem ? summarizeItem(requestedItem) : null,
    items: sourceItems.slice(0, 30).map((item) => ({
      id: String(item?.id || '').slice(0, 120),
      displayName: String(item?.displayName || '').trim().slice(0, 120) || null,
    })),
  };
...
/**
 * Evaluate whether the account model catalogue permits this exact policy
 * selection. `variants` is optional in the v1 API: a parameterless model
 * without a variants list is valid, while a parameterized selection must
 * still be represented by an explicit matching variant.
 *
 * @param {unknown} modelCatalog
 * @param {{ id?: unknown, params?: unknown } | null | undefined} model
 * @returns {{
 *   available: boolean,
 *   reason: 'model_id_missing' | 'parameterless_model_without_variants' | 'matching_variant' | 'params_not_available',
 *   modelId: string,
 *   modelFound: boolean,
 ...
 * }}
 */
export function evaluatePolicyModelAvailability(modelCatalog, model) {
  const items = Array.isArray(modelCatalog?.items) ? modelCatalog.items : [];
  const modelId = String(model?.id || '');
  const requestedParams = Array.isArray(model?.params) ? model.params : [];
  const item = items.find((candidate) => String(candidate?.id || '') === modelId);
  if (!item) {
    return {
      available: false,
  ...
  if (variants.some((variant) => matchingParams(variant?.params, requestedParams))) {
    return {
      available: true,
      reason: 'matching_variant',
      modelId,
      modelFound: true,
      requestedParams,
  ...
  if (requestedParams.length === 0 && variants.length === 0) {
    return {
      available: true,
      reason: 'parameterless_model_without_variants',
      modelId,
      modelFound: true,
      requestedParams,
      availableVariantCount: 0,
...
export async function createCursorCloudAgent(apiKey, payload, deps = {}) {
  ...
  if (
    ...
    !isLiveCursorExecutionModelSelection(model)
  ) {
    throw new Error('Cursor Cloud Agent create requires one live catalogue-resolved explicit model selection');
  }
...
/**
 * Read provider-reported usage for one known run. This is intentionally
 * read-only: lifecycle/backfill callers must never create a new run merely
 * to obtain economics evidence.
 *
 * @param {string} apiKey
 * @param {string} agentId
 * @param {string} runId
 * @param {{ fetch?: typeof fetch, policyFetch?: typeof fetch, timeoutMs?: number }} [deps]
 */
export async function getCursorCloudAgentUsage(apiKey, agentId, runId, deps = {}) {
  const agent = String(agentId || '').trim();
  const run = String(runId || '').trim();
  if (!agent) throw new Error('getCursorCloudAgentUsage requires agentId');
  if (!run) throw new Error('getCursorCloudAgentUsage requires runId');
  return cursorApiRequest(
    apiKey,
    `/agents/${encodeURIComponent(agent)}/usage?runId=${encodeURIComponent(run)}`,
    deps,
  );
}
 ...
 *
 * @param {string} apiKey
 * @param {{ promptText: string, prUrl: string, name?: string, repoUrl?: string, executionTier?: string, sourceIssue?: number, comments?: Array<object>, modelSelection: { id: string, params: Array<{ id: string, value: string }> } }} opts
 * @param {{ fetch?: typeof fetch, policyFetch?: typeof fetch, timeoutMs?: number }} [deps]
 */
