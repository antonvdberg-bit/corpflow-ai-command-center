...
export function extractCursorGitDetails(apiResult) {
  ...
  const topLevelBcId =
    result.id != null && String(result.id).startsWith('bc-') ? String(result.id) : null;
  const topLevelRunId = result.id != null && String(result.id).startsWith('run-') ? String(result.id) : null;

  return {
    agentId: agent.id != null ? String(agent.id) : topLevelAgentId || topLevelBcId,
...
const CURSOR_AGENT_ID_RE = /^bc-[0-9a-f-]{20,}$/i;
const CURSOR_RUN_ID_RE = /^run-[0-9a-f-]{20,}$/i;

/**
 * Fail closed unless Cloud Agents API v1 returned a concrete agent identity.
 * A HTTP 200 with no `bc-…` id is not execution.
 *
 * @param {Record<string, unknown> | null | undefined} apiResult
 * @returns {{
 *   ok: true,
 ...
 * }}
 */
export function extractValidatedCursorAgentIdentity(apiResult) {
  const git = extractCursorGitDetails(apiResult);
  const agentId = git.agentId && CURSOR_AGENT_ID_RE.test(git.agentId) ? git.agentId : null;
  const runId = git.runId && CURSOR_RUN_ID_RE.test(git.runId) ? git.runId : null;
  if (!agentId) {
    return {
      ok: false,
      agentId: null,
      ...
      prUrl: null,
      prNumber: null,
      reason: 'missing_or_invalid_agent_identity',
    };
  }
  return {
    ok: true,
