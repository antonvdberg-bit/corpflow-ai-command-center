...
export function inferIssueClassification(issue) {
  ...
  const forbidsSecretsChange =
    /do not expose secrets|approved secrets-management|env\/secret change|no env\/secret|no secret change|no env or secret|do not change(?:[\w\s.\/,-]{0,60})?env\/secrets|do not:[\s\S]{0,900}?change (?:vercel )?env\/secrets|without anton approval|without explicit approval/i.test(
      blob,
    );
  ...
  /** Doctrine / control-design issues that describe gates must not self-block claim (#676). */
  ...
  // Explicit client_production intent — never infer from public URL / Vercel Production alone (#679).
  ...
  if (!forbidsProduction && !isProtectedGateControlDesign && clientProductionIntent) {
    ...
    !/no env|no secret|no \.env|do not expose secrets|do not change(?:[\w\s.\/,-]{0,60})?env\/secrets|approved secrets-management|anton.*secret|operator.*secret/i.test(
      blob,
    )
  ) {
