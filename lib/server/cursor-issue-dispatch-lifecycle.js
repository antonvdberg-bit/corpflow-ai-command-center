...
/**
 * Resolve WIP limits for a scan. The default 3-slot ceiling is preserved
 * unless the Temporal pilot activation contract is live, in which case the
 * effective ceiling is 5. Callers that pass `wipLimits` still win.
 *
 * @param {{
 *   pilotActive?: boolean,
 ...
 * ("no production deploy") or as a later comma/list item in the same
 * sentence ("No schema, env/secrets, payment, production deploy, ...").
 *
...
export function prohibitionAppliesToPhrase(text, phrase) {
  ...
  if (new RegExp(`\\b(?:no|not|without|never|do not|don't)\\s+${escaped}\\b`).test(blob)) {
    return true;
  }

  const listItemBoundary = '(?=\\s*[,;]|\\s+(?:or|and)\\b|[.!?\\n]|$)';
  const listForm = new RegExp(
    `\\b(?:no|do not|don't|without|never)\\b[^.!?\\n]{0,400}?[;,]\\s*(?:or\\s+|and\\s+)?${escaped}${listItemBoundary}`,
  );
