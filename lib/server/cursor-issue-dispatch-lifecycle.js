...
export const DEFAULT_WIP_LIMITS = Object.freeze({
  maxActiveCursorImplementationIssues: 1,
...
/**
 * Resolve WIP limits for a scan. The one-lane spend-control ceiling wins
 * even when Temporal is marked active or a caller requests a higher max.
 *
 * @param {{
 *   pilotActive?: boolean,
 *   env?: NodeJS.ProcessEnv | Record<string, string | undefined> | null,
 *   wipLimits?: Partial<typeof DEFAULT_WIP_LIMITS>,
 * }} [input]
 */
export function resolveCursorDispatchWipLimits(input = {}) {
  const policyMax = resolveEffectiveCursorWipMaxSlots({
    pilotActive: input.pilotActive,
    env: input.env,
  });
  const requestedMax = Number(input.wipLimits?.maxActiveCursorImplementationIssues);
  const maxActiveCursorImplementationIssues =
    Number.isFinite(requestedMax) && requestedMax > 0
      ? Math.min(CURSOR_WIP_MAX_SLOTS, policyMax, Math.floor(requestedMax))
      : Math.min(CURSOR_WIP_MAX_SLOTS, policyMax);
  return {
    ...DEFAULT_WIP_LIMITS,
    ...(input.wipLimits || {}),
    maxActiveCursorImplementationIssues,
  };
}
...
/**
 * Adjacent + related production-deploy prohibitions that predate list-form
 * parsing. Kept as a fallback so historical wording still clears the gate.
 */
const FORBIDS_PRODUCTION_RELATED =
  /without.*production|not.*production deploy|no db\/schema|no schema change|no deployment into any client|no client.?production|not.*client.?production|classification\/documentation\/test-workflow correction only|open a pr only|do not merge|do not deploy/;

/**
 * @param {string} phrase
 * @returns {string}
 */
function escapePhraseForRegExp(phrase) {
...
/**
 * True when a prohibition lead applies to `phrase` adjacently
 * ("no production deploy" or "does not authorize: production deploys") or
 * as a later comma/list item in the same sentence ("No schema, env/secrets,
 * payment, production deploy, ...").
 *
 * List items must be bounded by comma/semicolon, "or"/"and", or sentence
 * end so "No waiting, production deploy is required" stays an authorization.
 * Sentence-ending punctuation stops the lead so "No schema. Then production
 * deploy …" remains fail-closed (#962 / #950).
 *
 * @param {string} text
 * @param {string} phrase
 * @returns {boolean}
 */
export function prohibitionAppliesToPhrase(text, phrase) {
  const blob = String(text || '').toLowerCase();
  const escaped = escapePhraseForRegExp(phrase);
  if (!blob || !escaped) return false;

  if (
    new RegExp(
      `\\b(?:no|not|without|never|do not|don't|does not authorize)\\s*:?\\s*(?:[-*]\\s*)?${escaped}s?\\b`,
    ).test(blob)
  ) {
    return true;
  }

  const listItemBoundary = '(?=\\s*[,;]|\\s+(?:or|and)\\b|[.!?\\n]|$)';
  const listForm = new RegExp(
    `\\b(?:no|do not|don't|without|never|does not authorize)\\b[^.!?\\n]{0,400}?[;,]\\s*(?:or\\s+|and\\s+)?${escaped}s?${listItemBoundary}`,
  );
  return listForm.test(blob);
}

/**
 * Production-deploy prohibition, including #950-style list-form constraints.
 *
 * @param {string} text
 * @returns {boolean}
 */
export function textForbidsProduction(text) {
  const blob = String(text || '').toLowerCase();
  if (prohibitionAppliesToPhrase(blob, 'production deploy')) return true;
  if (prohibitionAppliesToPhrase(blob, 'production deployment')) return true;
  return FORBIDS_PRODUCTION_RELATED.test(blob);
}

/**
 * Governance prose such as "approval remains required before production
 * deploy" names a protected boundary; it is not a request to deploy. The
 * actual deployment wording still has to be evaluated independently.
 *
 * @param {string} text
 * @returns {boolean}
 */
export function textContainsProductionApprovalPrerequisite(text) {
  return /\b(?:explicit\s+)?(?:anton\s+)?(?:operator\s+)?(?:approval|authorization)\s+(?:remains\s+)?required\s+before\s+(?:a\s+)?production\s+deploy(?:ment)?\b/i.test(
    String(text || ''),
  );
}
...
export function inferProtectedSubjectsAndConsequentialGate(blob) {
  ...
  const forbidsSecretsChange =
    /do not expose secrets|approved secrets-management|no env\/secret change|no secret change|no env or secret changes|\bno[^\n]{0,120}env\/secrets\b|do not change(?:[\w\s.\/,-]{0,60})?env\/secrets|do not:[\s\S]{0,900}?change (?:vercel )?env\/secrets|mentions credentials\/secrets as things not to expose/.test(
      text,
    ) &&
    !/(?:authorized|approved).{0,30}(?:change|configure|set).{0,80}(environment\/settings|env\/secrets)|explicitly approved.{0,120}(environment\/settings|secure cursor)/.test(
      text,
    );
  ...
  const productionConsequential =
    !forbidsProduction &&
    (!textContainsProductionApprovalPrerequisite(text) || clientProductionIntent) &&
    !isProtectedGateControlDesign &&
      ...
      (!corpflowTestPublishIntent &&
        (/deploy to (client )?production\b|production deploy to client/.test(text) ||
          (/production deploy|deploy to production/.test(text) &&
            !textForbidsProduction(text) &&
            !/vercel production spine|corpflow.?test/.test(text) &&
...
/**
 * An explicit, authorised CURSOR REQUEUE starts a fresh generation. It is the
 * only safe automatic path that can clear a prior dispatcher block; a normal
 * scheduled scan must continue to leave blocked work untouched.
 *
 * @param {Array<string | { name?: string }> | null | undefined} labels
 * @param {boolean} requeueAccepted
 * @returns {{ restoreReady: boolean, removeBlocked: boolean, reason: string }}
 */
export function planCursorRequeueDispatchState(labels, requeueAccepted) {
  const names = normalizeIssueLabels(labels).map((label) => label.toLowerCase());
  const blocked = names.includes(DISPATCH_LABEL_BLOCKED.toLowerCase());
  if (!requeueAccepted || !blocked) {
    return {
      restoreReady: false,
      removeBlocked: false,
      reason: requeueAccepted ? 'not_blocked' : 'requeue_not_accepted',
    };
  }
  return {
    restoreReady: true,
...
export function inferIssueClassification(issue) {
  ...
  if (
    labels.includes('lux') ||
    labels.includes('luxe-maurice') ||
    ((/\blux\b|luxe|rare\s*&\s*exclusive/.test(blob)) &&
      !/\bnot a rare\s*&\s*exclusive one-off\b/.test(blob))
  ) {
    systemBoundary = 'tenant';
    tenantOrClient = 'lux / Rare & Exclusive';
  } else if (labels.includes('cipc') || /cipc\s*desk/.test(blob)) {
    systemBoundary = 'tenant';
    tenantOrClient = 'Business Admin Desk';
  ...
  const forbidsProduction = textForbidsProduction(blob);
  const forbidsDatabaseChange =
    /no db\/schema|no schema change|\bno[^\n]{0,120}db\/schema\b|do not(?:\s+\w+){0,4}\s+(?:change|touch|alter|mutate|modify|run)(?:[\w\s.\/,-]{0,80})?(?:db\/schema|prisma\s+migrate|schema)|(?:do not|don't)[\s\S]{0,240}?(?:change|touch|alter|mutate|modify)\s+db\/schema|do not:[\s\S]{0,900}?(?:change|touch|alter|mutate|modify)\s+db\/schema|without(?:\s+\w+){0,12}\s+db\/schema(?:\s+change)?|db\/schema changes?(?:\s+are)?\s+(?:not\s+)?(?:authori[sz]ed|allowed)|unrelated db\/schema|not a corpflowai database\/schema task|do not touch corpflowai postgres|do not run prisma migrate|determine whether schema changes are needed/.test(
      blob,
    );
...
export function planCursorIssueClaims(input) {
  const limits = resolveCursorDispatchWipLimits(input);
  ...
  const reviewInventoryNumbers = new Set(wip.reviewDecisionInventoryIssueNumbers);
  ...
  for (const issue of sorted) {
    ...
    // A non-draft linked PR (or terminal completion evidence) is review
    // inventory even when the label reconciliation has not yet caught up.
    // Do not repeatedly select it: the Cloud Agents transport correctly
    // suppresses that duplicate but a successful no-op handoff can otherwise
    // starve later ready work.
    if (reviewInventoryNumbers.has(Number(issue.number))) {
      decisions.push({
        issue,
        classification,
        decision: 'discover_only',
    ...
    if (remainingSlots <= 0) {
      const temporalNote =
        wip.pilotActive && activationLane === CURSOR_WIP_LANE_TEMPORAL
          ? `Temporal extra ${wip.temporalUsed}/${wip.temporalExtraSlots}; production reserve ${wip.ordinaryUsed}/${wip.productionReserveSlots} preserved; `
          : wip.pilotActive
            ? `production reserve ${wip.ordinaryUsed}/${wip.productionReserveSlots}; `
            : '';
      decisions.push({
        issue,
        classification,
        ...
        reason: `WIP cap reached (${temporalNote}max ${limits.maxActiveCursorImplementationIssues} verified active Cursor runs)`,
      });
      continue;
    }
