...
import {
  ANTON_DECISION_INBOX_LABELS,
  APPROVAL_LABEL_CREATE_DEFAULTS,
...
/**
 * Lifecycle labels plus Anton Decision Inbox approval:* reason labels.
 * Ensured together so `needs:anton` items always have reason labels available.
 */
export const DISPATCH_AND_DECISION_INBOX_LABELS = Object.freeze([
  ...DISPATCH_LIFECYCLE_LABELS,
  ...ANTON_DECISION_INBOX_LABELS.filter((l) => l !== DISPATCH_LABEL_NEEDS_ANTON),
]);
...
export function inferIssueClassification(issue) {
  ...
  if (
    ...
    !/requires? (production|secret|payment|deploy|schema)|must (deploy|change)/i.test(blob)
  ) {
    protectedGate = 'none';
  }
...
/**
 * Ensure all dispatch lifecycle + Decision Inbox approval labels exist; auto-create missing ones.
...
export async function ensureDispatchLifecycleLabels(token, repo, fetchFn = globalThis.fetch) {
  const required = DISPATCH_AND_DECISION_INBOX_LABELS;
  const existing = new Set((await listRepoLabelNames(token, repo, fetchFn)).map((l) => l.toLowerCase()));
  const missing = required.filter((l) => !existing.has(l.toLowerCase()));
  const created = [];
  ...
  const stillMissing = required.filter((l) => !after.has(l.toLowerCase()));
  if (stillMissing.length) {
    throw new Error(
      `Missing GitHub labels required for dispatch lifecycle / Decision Inbox (auto-create failed — grant issues:write or create manually): ${stillMissing.join(', ')}`,
    );
  }
  return { ok: true, labels: [...required], created };
}
