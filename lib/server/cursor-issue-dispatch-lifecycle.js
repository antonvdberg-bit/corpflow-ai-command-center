...
export const PROTECTED_GATES = Object.freeze([
  'none',
  /** Client production only — not CorpFlowAI-hosted corpflow_test publish. */
  'production',
  'database',
  'secrets',
  'messaging',
 ...
 *   environment: 'local' | 'test' | 'preview' | 'corpflow_test' | 'production' | 'client_production',
 *   workTypes: string[],
...
export function inferIssueClassification(issue) {
  ...
  const isEnvironmentDoctrine =
    /environment classification|corpflowai-hosted tenant surfaces as test|treat all corpflowai-hosted/.test(
      blob,
    ) ||
    (/corpflow_test/.test(blob) &&
      /client_production/.test(blob) &&
      /test environments?/.test(blob) &&
      /classification|doctrine|operating rule|business purpose/.test(blob));

  if (isEnvironmentDoctrine) {
    systemBoundary = 'corpflowai_business_system';
    tenantOrClient = 'N/A';
  } else if (
    labels.includes('lux') ||
  ...
  // Vercel "Production" channel serving CorpFlowAI hosts is corpflow_test, not client_production.
  const mentionsClientProductionDeploy =
    textRequestsClientProduction(blob) ||
    (/deploy(?:ment)? (?:to|into) (?:client )?production/.test(blob) &&
      !textForbidsClientProduction(blob));
  if (
    mentionsClientProductionDeploy ||
    (/deploy|production deployment/.test(blob) &&
      !textForbidsClientProduction(blob) &&
      !/corpflow(?:ai)?(?:-hosted)? test|test environment|corpflow_test/.test(blob))
  ) {
    workTypes.push('deployment');
  }
  if (/validat|smoke|live verify|delivery reality|live test url/.test(blob)) {
    workTypes.push('validation');
  }
  if (workTypes.length === 0) workTypes.push('documentation');

  let protectedGate = 'none';
  const forbidsClientProduction = textForbidsClientProduction(blob);
  const forbidsDbSchema =
    /(?:no|without)[^\n.]{0,160}(?:db\/schema|schema change|database\/schema)/.test(blob) ||
    /db\/schema changes? (?:are )?not authori[sz]ed/.test(blob) ||
    /no env\/secrets? or db\/schema/.test(blob);
  const forbidsPaidTool =
    /no.*paid tool|paid tool or public|without.*paid tool|paid tool or public launch/i.test(blob);
  const forbidsPaymentRuntime =
    /no.*payment runtime|payment runtime,|without.*payment runtime|no payment|payment runtime, messaging/i.test(
      blob,
    );
  const forbidsSecretsChange =
    /do not expose secrets|approved secrets-management|env\/secret change|no env\/secret|no secret change|without anton approval/i.test(
      blob,
    );

  // production protected gate = client_production only (never ordinary corpflow_test publish).
  if (!forbidsClientProduction && textRequestsClientProduction(blob)) {
    protectedGate = 'production';
  } else if (
    !forbidsClientProduction &&
    !forbidsDbSchema &&
    ...
    (/payment runtime|checkout|stripe/.test(blob) ||
      (/\bpayment\b/.test(blob) && !/payment\/deposit trigger wording/i.test(blob)))
  ) {
  ...
  } else if (/paid tool|new vendor/.test(blob) && !forbidsPaidTool) {
    protectedGate = 'paid_tool';
  }

  // Protected-gates sections that list prohibitions "without Anton approval" are not activation requests.
  if (
    /protected gates[\s\S]{0,500}without anton approval/i.test(blob) &&
    !/requires? (?:client[_ ]?)?(production|secret|payment|deploy|schema)|must (deploy|change)/i.test(
      blob,
    )
  ) {
    protectedGate = 'none';
  ...
  // Drop false-positive deployment/database work types when client production / schema is forbidden.
  if (forbidsClientProduction || forbidsDbSchema) {
    const filtered = workTypes.filter((t) => {
      if (t === 'database' && forbidsDbSchema) return false;
      if (t === 'deployment' && forbidsClientProduction) return false;
      return true;
    });
    workTypes.length = 0;
    workTypes.push(...(filtered.length ? filtered : ['documentation']));
    if (forbidsClientProduction && protectedGate === 'production') protectedGate = 'none';
  }

  const environment = /** @type {IssueWorkClassification['environment']} */ (
    inferBusinessEnvironment({
      blob,
      systemBoundary,
      protectedGate,
  ...
  const mayRunConcurrently =
    ...
    !isClientProductionEnvironment(environment);
  ...
  return {
    ...
    concurrencyReason: mayRunConcurrently
      ? 'Non-tenant, non-schema, non-client-production work; still requires separate branch/PR and WIP check.'
...
export function canRunConcurrently(a, b) {
  ...
  if (
    isClientProductionEnvironment(a.environment) &&
    isClientProductionEnvironment(b.environment)
  ) {
