...
/** Lifecycle labels that must exist before claim mutation (fail closed with one error). */
...
export async function assertDispatchLifecycleLabelsExist(token, repo, fetchFn = globalThis.fetch) {
  ...
  if (missing.length) {
    throw new Error(
      `Missing GitHub labels required for dispatch lifecycle — create in repo Settings → Labels: ${missing.join(', ')}`,
    );
  }
  return { ok: true, labels: [...DISPATCH_LIFECYCLE_LABELS] };
}
...
export async function finalizeIssueClaimAfterActivation(opts) {
  ...
  await assertDispatchLifecycleLabelsExist(opts.token, opts.repo, opts.fetch);
