...
export function inferIssueClassification(issue) {
  ...
  const forbidsProduction =
    /no production deploy|without.*production|not.*production deploy|no db\/schema|no schema change|no deployment into any client|no client.?production|not.*client.?production|classification\/documentation\/test-workflow correction only/.test(
    /no production deploy|without.*production|not.*production deploy|no db\/schema|no schema change|open a pr only|do not merge|do not deploy/.test(
      blob,
    );
  ...
  const forbidsPaymentRuntime =
    /no.*payment runtime|payment runtime,|without.*payment runtime|no payment|no live messaging,\s*payments|messaging,\s*payments,\s*outreach|payment runtime, messaging|approval:payment/i.test(
      blob,
    );
  const forbidsSecretsChange =
    /do not expose secrets|approved secrets-management|env\/secret change|no env\/secret|no secret change|no env or secret|without anton approval|without explicit approval/i.test(
      blob,
    );

  // Explicit client_production intent — never infer from public URL / Vercel Production alone.
  const clientProductionIntent =
    ...
    !/not .*client.?production|until a separate production transition|future client production|treat.*as test|corpflow.?test|corpflowai-hosted.*test/.test(
      blob,
    );
  ...
  const mentionsDeployOrVercelProd =
    /deploy|production deployment|vercel production|publish to/.test(blob);

  if (clientProductionIntent || (mentionsDeployOrVercelProd && !forbidsProduction && !corpflowTestPublishIntent)) {
    workTypes.push('deployment');
  } else if (mentionsDeployOrVercelProd && corpflowTestPublishIntent && !forbidsProduction) {
    // Publishing to CorpFlowAI test spine is deployment work, not client_production.
    workTypes.push('deployment');
  }
  ...
  if (!forbidsProduction && clientProductionIntent) {
    protectedGate = 'production';
  } else if (
    !forbidsProduction &&
    !corpflowTestPublishIntent &&
    (/deploy to (client )?production\b|production deploy to client/.test(blob) ||
      (/production deploy|deploy to production/.test(blob) &&
        !/no production deploy|without.*production|vercel production spine|corpflow.?test/.test(blob) &&
        !/test environment|test surface|sign-off|demonstration/.test(blob)))
  const forbidsOutreach =
    /no.*outreach|without.*outreach|no live messaging,\s*payments,\s*outreach|outreach, or public launch/i.test(
      blob,
    );
  const forbidsPublicLaunch =
    /no.*public launch|without.*public launch|or public launch|open a pr only/i.test(blob);
  /** Doctrine / control-design issues that describe gates must not self-block claim. */
  const isProtectedGateControlDesign =
    /central anton decision inbox|enforceable protected-action gates|decision inbox|protected-action gates/i.test(
      blob,
    ) && forbidsProduction;
  ...
  if (
    ...
    // Legacy phrasing that clearly means client/live production — not CorpFlowAI test publish.
    protectedGate = 'production';
  } else if (
    !forbidsProduction &&
    !isProtectedGateControlDesign &&
    ...
    !isProtectedGateControlDesign &&
    /secret|env var|\.env|infisical/.test(blob) &&
    !/no env|no secret|no \.env|do not expose secrets|approved secrets-management|anton.*secret|operator.*secret/i.test(
      blob,
    )
  ) {
    ...
    !/no.*whatsapp|no.*sms|no.*messaging|no email|no live messaging/.test(blob)
  ) {
    ...
    (/payment runtime|checkout|stripe/.test(blob) ||
      (/\bpayment\b/.test(blob) &&
        !/payment\/deposit trigger wording|approval:payment|payments?, outreach/i.test(blob)))
  ) {
    protectedGate = 'payment';
  } else if (
    !forbidsOutreach &&
    !isProtectedGateControlDesign &&
    /outreach|cold email|bulk send/.test(blob)
  ) {
    protectedGate = 'outreach';
  } else if (/paid tool|new vendor/.test(blob) && !forbidsPaidTool && !isProtectedGateControlDesign) {
    protectedGate = 'paid_tool';
  } else if (
    !forbidsPublicLaunch &&
    !isProtectedGateControlDesign &&
    /public launch|public client-facing launch/.test(blob)
  ) {
    protectedGate = 'public_launch';
  }

  // Protected-gates / governance sections that list prohibitions are not activation requests.
  if (
    /protected gates[\s\S]{0,500}without anton approval/i.test(blob) &&
    !/requires? (client.?production|secret|payment|schema)|must (change secrets|migrate)/i.test(blob)
    (/protected gates[\s\S]{0,500}without anton approval|## governance[\s\S]{0,800}no production deployment/i.test(
      blob,
    ) ||
      isProtectedGateControlDesign) &&
    !/\brequires? (production deploy|secret change|payment runtime|schema migration)\b|\bmust (deploy to production|change secrets)\b/i.test(
      blob,
    )
  ) {
    protectedGate = 'none';
  ...
  if (forbidsProduction && !clientProductionIntent) {
    ...
    if (protectedGate === 'production' || protectedGate === 'database') protectedGate = 'none';
  }
  ...
  if (protectedGate === 'production' || clientProductionIntent) {
    ...
    // Core factory host is CorpFlowAI-controlled test/operator spine, not client_production.
    environment = 'test';
  }
...
export function planCursorIssueClaims(input) {
  ...
  const activeClassifications = claimed.map((issue) => inferIssueClassification(issue));
  /** @type {IssueWorkClassification[]} */
