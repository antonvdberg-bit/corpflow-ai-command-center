...
/**
 * Public image serving for Lux properties (Phase 4C.3 + 5A variant scaffold).
 *
 * Route: GET /api/lux/property-media?property=<slug>&attachment=<id>&slot=<hero|gallery|card|...>&variant=<original|card|hero|gallery>
 *
...
export async function handleLuxPropertyMedia(req, res, prisma) {
  ...
  if (variantRaw) {
    const v = normalizeLuxPublicMediaVariantParam(variantRaw);
    if (v === false) {
      sendDeny(res, 400);
      return;
    }
  ...
  try {
    ...
    const mime = normalizeLuxPublicImageContentTypeMime(att.contentType) || 'image/jpeg';
