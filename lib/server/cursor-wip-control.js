 ...
 * (`CURSOR_WIP_MAX_SLOTS`). That default is unchanged outside the Temporal
 * real-production pilot. When and only when `CORPFLOW_TEMPORAL_PILOT=active`,
 * the effective ceiling becomes 5 with 3+2 isolation (#1145):
 * - 3 slots reserved for ordinary PRODUCT / CLIENT / REVENUE / ERP wakes
 *   (issue events, Queue Reconcile, capacity_released, manual dispatch);
 * - +2 incremental slots for Temporal-supervised wakes
 *   (`wake_reason=temporal_supervisory` only);
 * - Temporal cannot consume the production reserve;
 * - unsetting the variable returns the ceiling to 3 automatically.
 *
...
export const CURSOR_WIP_TEMPORAL_EXTRA_SLOTS = 2;
/** Effective ceiling while `CORPFLOW_TEMPORAL_PILOT=active` (3 + 2). */
...
/**
 * Effective Cursor execution ceiling: 3 outside the pilot, 5 while active.
 *
 * @param {{
 *   pilotActive?: boolean,
 *   env?: NodeJS.ProcessEnv | Record<string, string | undefined> | null,
 ...
 * When the Temporal pilot is active, `availableSlots` is lane-aware: ordinary
 * wakes see the 3-lane production reserve; Temporal-supervised wakes see only
 * the two incremental extras. Unsetting the pilot variable returns max=3.
 *
 * @param {{
...
export function evaluateCursorWipCapacity(input = {}) {
  ...
  const maxSlots =
    input.maxSlots != null && Number.isFinite(Number(input.maxSlots)) && Number(input.maxSlots) > 0
      ? Math.max(1, Math.floor(Number(input.maxSlots)))
