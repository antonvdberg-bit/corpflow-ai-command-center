...
import { ensureClientDecisionsForTicket, resolveClientDecisionsMintPath } from '../cmp/_lib/client-decisions-client.js';
...
export default async function handleClientDecisionsLinkMint(req, res) {
  ...
  try {
    ...
    const withItems = ensureClientDecisionsForTicket(ticketId, norm);
    const cd = withItems.client_decisions && typeof withItems.client_decisions === 'object' ? withItems.client_decisions : {};
    ...
    const merged = ensureClientDecisionsForTicket(ticketId, {
      ...withItems,
    ...
    const pathBase = resolveClientDecisionsMintPath(
      ticketId,
      typeof parsed.body?.client_path === 'string' ? parsed.body.client_path : '',
    );
