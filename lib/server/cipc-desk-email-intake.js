...
function inferCipcDeskFromEmailText(emailText) {
  ...
  if (has(/(director\s+appointment|appointment\s+of\s+director|appoint(ed)?\s+director)/i) || has(/resign(ed|ation)?\s+director|resign(ed|ation)?\s+of\s+director/i)) {
    service = {
...
function inferPublicBaseUrl(req) {
  ...
  return resolveCipcDeskPublicBaseUrl({
    host,
    proto,
    explicitBase: cfg('CORPFLOW_PUBLIC_BASE_URL', ''),
  });
}
