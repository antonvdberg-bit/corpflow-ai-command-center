...
/**
 * Lazy-load so `response-automation.js` stays off the CJS-wrapped
 * factory_router static import graph (#1015).
 * @returns {Promise<typeof import('../cipc-desk/response-automation.js')>}
 */
function loadCipcResponseAutomation() {
...
export function inferCipcDeskFromEmailText(emailText) {
  ...
  if (has(/partner overflow\s*\/\s*white-label enquiry|overflow\s*\/\s*white-label|fractional\s*\/\s*white-label/i)) {
    ...
    has(/director\s+(change|amendment|appointment|resignation|retirement|particular)/i) ||
    has(/(appointment\s+of\s+director|appoint(ed)?\s+director)/i) ||
    has(/resign(ed|ation)?\s+(of\s+)?director/i)
  ) {
    service = {
      serviceSlug: 'director-appointments-resignations',
      serviceTitle: 'Director changes (provisional)',
      requestedChange: 'Process director-change matter (provisional)',
    };
  ...
  const clientDecisionsItems = [
    ...
    },
    {
      ...
      status: 'waived',
      answer: 'Not applicable for Business Admin Desk',
    },
    {
...
export default async function cipcDeskEmailIntakeHandler(req, res) {
  ...
  try {
    ...
    const { applyCipcResponseIntake, hydrateCipcResponseFromTicket, mergeCipcResponseJson, mergeCipcResponseQualificationJson } =
    ...
    const existingOverlays = candidates.map((r) => hydrateCipcResponseFromTicket(r));
    const intakePreview = applyCipcResponseIntake({
      emailText,
      body,
      interpreted,
      existingRecords: existingOverlays,
    });
    const existing = candidates.find((r) => {
      const cj = asObj(r.consoleJson);
      const cv = asObj(cj.client_view);
      const cipc = asObj(cv.cipc_desk);
      if (cipc.source_email_hash === emailHash && cipc.preview_source === 'email-intake') return true;
      return intakePreview.duplicate && String(intakePreview.overlay?.ticket_id || '') === String(r.id);
    });

    let ticketId = existing?.id ? String(existing.id) : '';
    let linkedExistingLeadId = str(intakePreview.overlay?.lead_id);
    const senderEmail = str(intakePreview.enquiry?.email);
    if (senderEmail) {
      try {
        ...
        if (existingLead?.id) linkedExistingLeadId = String(existingLead.id);
      } catch {
        /* lead lookup is best-effort; ticket overlay remains the source of truth */
      }
    }
    if (!ticketId) {
      const checklistItems = interpreted.checklistItems;
      const overlayForCreate = {
        ...asObj(intakePreview.overlay),
        lead_id: linkedExistingLeadId,
      };

      const consoleJson = mergeCipcResponseJson(
        {
          locale: 'en',
          seed_source: 'cipc-desk-email-intake',
          brief: {
            summary: `Business Admin Desk · ${interpreted.service.serviceTitle}`,
          ...
          client_view: {
            cipc_desk: {
              ...
              company: str(intakePreview.enquiry?.company) || null,
              checklist: { items: checklistItems },
              client_reply_draft:
                str(overlayForCreate?.draft?.body) || interpreted.clientReplyDraft,
      ...
      const created = await prisma.cmpTicket.create({
        data: {
          ...
          status: 'Approved',
          stage: 'Build',
          title: `Business Admin Desk · ${interpreted.service.serviceTitle}`,
          description: 'Business Admin Desk · email-first intake (fictional data preview)',
          locale: 'en',
    ...
    const overlayForStore = applyCipcResponseIntake({
      emailText,
      body,
      interpreted,
      existingRecords: existingOverlays,
    ...
    overlayForStore.ticket_id = ticketId;
    overlayForStore.lead_id = linkedExistingLeadId;
    overlayForStore.public_reference = overlayForStore.public_reference || ticketId.slice(-8).toUpperCase();
    if (!String(overlayForStore.public_reference || '').startsWith('CD-')) {
      overlayForStore.public_reference = `CD-${String(ticketId).replace(/[^a-zA-Z0-9]/g, '').slice(-8).toUpperCase()}`;
    }

    const updated = ensureClientDecisionsForTicket(ticketId, {
      ...mergeCipcResponseJson(seeded || {}, overlayForStore),
    ...
    if (linkedExistingLeadId) {
      try {
        const existingLead = await prisma.lead.findUnique({
          where: { id: linkedExistingLeadId },
          select: { id: true, tenantId: true, qualificationJson: true },
        });
        if (existingLead && String(existingLead.tenantId || '') === tenantId) {
          await prisma.lead.update({
            where: { id: existingLead.id },
            data: {
              qualificationJson: mergeCipcResponseQualificationJson(existingLead.qualificationJson, {
                intake_meta: {
                  product: 'cipc-response',
                  source: 'cipc-response-automation-#987',
                  page: str(body.client_path) || '/change',
                  host: reqHost,
                },
                cipc_response: overlayForStore,
              }),
            },
          });
        }
      } catch {
        /* overlay on the ticket is sufficient if lead merge fails */
      }
    }
    ...
    const confirmation = {
      reference: String(overlayForStore.public_reference || ''),
      message:
        `Thank you. We have recorded your enquiry under reference ${overlayForStore.public_reference}. ` +
    ...
    return res.status(200).json({
      ...
      expires_at,
      source: 'email_intake',
      public_reference: confirmation.reference,
      confirmation,
      classification: overlayForStore.classification,
