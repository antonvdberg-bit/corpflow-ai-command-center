 ...
 *
 * Audits: GET /api/factory/technical-lead/audits?ticket_id=… — factory master **or** cron Bearer auth (read-only).
 */
...
import {
  ...
  verifyFactoryMasterAuth,
  verifyFactoryMasterOrCronBearer,
} from './factory-master-auth.js';
...
async function handleTechnicalLeadRun(req, res, mode) {
  ...
  try {
    ...
    if (mode.cron && out?.ok) {
      emitTelemetry({
        event_type: 'technical_lead.cron_completed',
        cmp: { ticket_id: 'n/a', action: 'technical-lead-observer' },
        payload: {
