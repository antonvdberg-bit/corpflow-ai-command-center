 ...
 *
 * Scope: factory-only writes (host-map upsert, CMP tenant-hostname-upsert, tenant bootstrap). Does **not** run on
 * `GET /login`, `/change`, or other tenant traffic. Apex `CORPFLOW_ROOT_DOMAIN` skips tenant-id prefix enforcement
 * (`h === root`), so e.g. https://corpflowai.com/login is unaffected; apex tenant comes from host map / defaults (see `attachTenantFromHostPg`).
 *
 * Env:
