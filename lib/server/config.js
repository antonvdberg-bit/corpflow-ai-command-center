...
/**
 * Optional n8n webhook for Product A US clinic audit intake (`POST /api/product-a/intake`).
 * Follows the same optional-webhook pattern as `N8N_CMP_WEBHOOK_URL`.
 *
 * @returns {string}
 */
export function getN8nProductAIntakeWebhookUrl() {
  return String(cfg('N8N_PRODUCT_A_INTAKE_WEBHOOK_URL', '')).trim();
}
