...
export function planStaleFactoryBlockRecovery(input = {}) {
  ...
  if (!currentMainSha) return { recover: false, reason: 'current_main_sha_missing' };
  ...
  for (const comment of comments) {
    ...
    if (evidence.current_main_sha === currentMainSha) {
      return { recover: false, reason: 'current_factory_block' };
    }
    return {
      recover: true,
