 ...
 * request wins; otherwise the selected GitHub issue receives a stable request
 * record before agent creation.
 */
export function buildFactoryCloudAgentsExecutionEnvelope(input = {}) {
  ...
  const existing = findAiWorkRequest(input.issue, input.comments);
  const request =
    existing ||
    buildAiWorkRequest({
  ...
  return {
    ...
    create_payload: buildCursorAgentCreatePayload(
      {
...
export function validateCloudAgentCreateResponse(apiResult) {
  ...
  if (!/^bc-[a-z0-9-]{6,}$/i.test(agentId)) {
    return {
      ok: false,
      reason: 'cursor_create_response_missing_valid_agent_id',
      details: { ...details, agentId: null },
    };
  }
  return { ok: true, details: { ...details, agentId } };
}
