...
export function buildFactoryCloudAgentsExecutionEnvelope(input = {}) {
  ...
  const prompt = [
    ...
    '',
    'Execute only the validated current packet below. Do not fetch or inject historical issue material by default. Open one PR only.',
  ...
  const createPayload = buildCursorAgentCreatePayload(
    ...
    },
    {
      ...
      executionTier: economicRoute.tier,
      sourceIssue,
      comments: input.comments,
    },
  );
