...
function currentGenerationComments(comments) {
  ...
  for (let i = 0; i < list.length; i += 1) {
    if (isCursorRequeueBoundaryComment(list[i])) latestRequeueIndex = i;
  }
  return {
...
/**
 * Keep heading detection in sync with looksLikeExplicitCursorRequeueInstruction.
 * Durable HTML markers always bound. Prose headings require an authorised actor
 * when author metadata is present.
 *
 * @param {{ body?: string | null, author?: string | null, user?: { login?: string | null } } | null | undefined} comment
 */
function isCursorRequeueBoundaryComment(comment) {
  const text = String(comment?.body || '');
  if (/<!--\s*corpflow\.cursor_requeue\.v1\s+\{/i.test(text)) return true;
  const stripped = text.replace(/^\uFEFF/, '').replace(/^\s*(?:#{1,6}\s+)?/, '');
  if (!/^CURSOR REQUEUE\b/i.test(stripped)) return false;
  const author = comment?.author || comment?.user?.login || null;
  if (!author) return true;
  if (isTrustedOperatorIdentity(author, null)) return true;
  const assoc = String(comment?.author_association || comment?.authorAssociation || '')
    .trim()
    .toUpperCase();
