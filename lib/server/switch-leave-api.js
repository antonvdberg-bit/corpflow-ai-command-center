 ...
 * the CSRF token. No database writes. No audit/event writes (per Anton's IM-5
 * approval correction #1 — audit/event writing is a separate IM-7 packet).
 *
...
/* TODO (IM-7 audit packet — NOT in IM-5 scope):
 *
 * When the dedicated audit packet ships, instrument the success paths of
 * handleMembershipSwitch / handleMembershipLeave with non-blocking writes to
 * `automation_events`:
 *   - event_type: membership.switch.success | membership.leave.success
 *   - payload_json: { user_id, from_tenant_id, to_tenant_id, session_version, host }
 *
 * And the rejection paths with `membership.switch.rejected` events. Writes
 * MUST stay best-effort (try/catch swallows on failure; the request still
 * succeeds). Per Anton's IM-5 approval correction #1, this is OUT OF SCOPE
 * for IM-5 — no production writes of any kind here.
 */
