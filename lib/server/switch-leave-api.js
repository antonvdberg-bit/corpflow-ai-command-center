 ...
 */
function reissueSessionCookie(res, req, prevPayload, newActingTenantId) {
  ...
  const csrfToken = issueCsrfCookie(res, cookieOpts);
...
export async function handleMembershipSwitch(req, res, deps = {}) {
  ...
  let reissue;
  try {
    reissue = reissueSessionCookie(res, req, sess.payload, targetTid);
  } catch (err) {
    try {
...
export async function handleMembershipLeave(req, res) {
  ...
  let reissue;
  try {
    reissue = reissueSessionCookie(res, req, sess.payload, null);
  } catch (err) {
    try {
