...
export async function runGovernedGroqChangeRefiner(args) {
  const { prisma, ticketId, tenantId, messages, locale, existingBrief, extraSystemContext } = args;
  const key = getGroqApiKey();
  const model = resolveGroqModel('primary');

  const systemBase = buildChangeRefinerSystemPrompt(locale);
  const extra = String(extraSystemContext || '').trim();
  const system = extra ? `${systemBase}\n\n${extra}` : systemBase;
  ...
  const logSession = (final) => {
    emitTelemetry({
      ...
      payload: {
        ...
        handoff_loaded: Boolean(extra),
        handoff_char_count: extra ? extra.length : 0,
        attempts: attemptLog,
        ...final,
      },
    });
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    ...
    try {
      const r = await groqChatCompletionsFetch({
        ...
        temperature: 0.2,
        observability: {
          workflow: 'change_refiner.refine',
          product: 'coreflow-change-console',
          workstream_id: 'change-refiner',
          work_packet_id: `ticket:${ticketId}`,
        },
