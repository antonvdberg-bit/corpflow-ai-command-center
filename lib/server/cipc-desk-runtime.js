...
/** Public client-production hosts for the Business Admin Desk brand. */
export const BUSINESS_ADMIN_DESK_PUBLIC_HOSTS = Object.freeze([
  'businessadmindesk.co.za',
  'www.businessadmindesk.co.za',
]);
...
export function isBusinessAdminDeskPublicHost(host) {
  const h = normalizeHostname(host);
  if (!h) return false;
  return BUSINESS_ADMIN_DESK_PUBLIC_HOSTS.includes(h);
}
...
export function resolveCipcDeskTenantIdFromHost(host) {
  return isCipcDeskStandingTestHost(host) || isBusinessAdminDeskPublicHost(host)
    ? CIPCDESK_TENANT_ID
    : null;
}
...
export function resolveCipcDeskWorkflowAccess(args = {}) {
  ...
  if (isBusinessAdminDeskPublicHost(host)) {
    return { allowed: true, reason: 'business_admin_desk_public_host' };
  }
