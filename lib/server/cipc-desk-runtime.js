...
/**
 * Prefer the request's standing CIPC Desk host for client-facing magic links
 * so links stay on `cipc.corpflowai.com` / `cipc-desk.corpflowai.com` instead of apex.
 *
 * Falls back to `explicitBase` (e.g. CORPFLOW_PUBLIC_BASE_URL) only when the
 * request host is not a standing CIPC Desk host.
 *
 * @param {{
 *   host?: string | null,
 *   proto?: string | null,
 ...
 */
export function resolveCipcDeskPublicBaseUrl(args = {}) {
  const host = normalizeHostname(args.host);
  const protoRaw = String(args.proto || 'https')
    .split(',')[0]
    .trim()
    .toLowerCase();
  ...
  const explicit = String(args.explicitBase || '')
    .trim()
    .replace(/\/+$/, '');
