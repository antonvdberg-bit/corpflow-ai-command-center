 ...
 *
 * Issue #645 (PR A): when no published DB rows exist, marketing SSR may fall
 * back to the staged curated catalog (demo entries stripped). No schema change.
 */

import { getPublicLuxStagedListingRows } from '../client/luxe-maurice-staged-properties.js';
...
/**
 * Marketing opportunity list for `/properties` (and similar SSR).
 * Prefer published `lux_listings`. When that set is empty, fall back to the
 * public staged curated catalog so the Jan-approved MVP journey remains visible
 * without schema work. Demo staged entries are never included.
 *
 * @param {import('@prisma/client').PrismaClient} prisma
 * @returns {Promise<Array<object>>}
 */
export async function fetchLuxMarketingOpportunityListings(prisma) {
  const published = await fetchPublishedLuxListingsPublic(prisma);
  if (Array.isArray(published) && published.length > 0) {
    return published;
  }
  return getPublicLuxStagedListingRows();
}
