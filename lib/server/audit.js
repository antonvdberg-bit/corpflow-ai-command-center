...
/**
 * Records a sovereign / UAT audit line without an HTTP round-trip (JSONL via emitTelemetry).
 *
 * @param {object} params
 * @param {string} params.tenant_id
 * @param {string} params.action - e.g. signoff
 * @param {Record<string, unknown>} [params.meta]
 * @returns {ReturnType<typeof emitTelemetry>}
 */
export function recordSovereignAuditEvent({ tenant_id, action, meta }) {
  return emitTelemetry({
    event_type: 'sovereign_audit',
    cmp: { ticket_id: 'n/a', action: String(action || 'unknown') },
    payload: {
      tenant_id: tenant_id != null ? String(tenant_id) : '',
