...
async function monitorOneTicket(ticketId) {
  ...
  const outcomes = extractOutcomesFromDescription(str(row.description || ''));
  const actions = [];
...
export default async function cmpMonitorCronHandler(req, res) {
  ...
  if (!ids.length) {
    return res.status(200).json({
      ok: true,
      skipped: true,
      reason: 'CMP_MONITOR_TICKET_IDS_EMPTY',
      hint: 'Set CMP_MONITOR_TICKET_IDS to a comma-separated list of CMP ticket IDs to monitor.',
    });
  }
