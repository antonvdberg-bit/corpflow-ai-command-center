...
import { isLowCostMode } from './low-cost-mode.js';
import {
...
import { fetchCmpOverseerSummary, fetchCmpPullRequest, fetchCmpTicketActivity } from '../cmp/_lib/github-dispatch.js';
import { forwardOpsAlert, sendTelegramOpsAlert } from './ops-alerts.js';
...
async function monitorOneTicket(ticketId) {
  ...
  const prevCj = row.consoleJson && typeof row.consoleJson === 'object' ? row.consoleJson : {};
  const prevCv = prevCj?.client_view && typeof prevCj.client_view === 'object' ? prevCj.client_view : {};
  const prevAuto = prevCv?.automation && typeof prevCv.automation === 'object' ? prevCv.automation : {};
  const prevVerdict =
    prevCv?.delivery_verdict && typeof prevCv.delivery_verdict === 'object' ? prevCv.delivery_verdict : null;
  ...
  // Delivery verdict: evidence of whether work is actually progressing (PR exists, non-empty diff, preview exists, CI status).
  const st = str(row.status || '');
  const sg = str(row.stage || '');
  const approvedBuild =
    (st + ' ' + sg).toLowerCase().includes('approved') && (st + ' ' + sg).toLowerCase().includes('build');
  let delivery_verdict = prevVerdict;
  if (approvedBuild) {
    const baseRef = str(cfg('CMP_SANDBOX_BASE_REF', 'main') || 'main') || 'main';
    const pr = await fetchCmpPullRequest({ ticketId });
    const act = await fetchCmpTicketActivity({ ticketId, baseRef });
    const diff = await fetchCmpOverseerSummary({ ticketId, baseRef });
    const filesChanged = diff.ok && Array.isArray(diff.files) ? diff.files.length : null;
    const previewUrl =
      typeof prevAuto.preview_url === 'string' && prevAuto.preview_url.trim() ? prevAuto.preview_url.trim() : '';
    const hasClientPreview =
      typeof prevAuto.client_site_preview_url === 'string' && prevAuto.client_site_preview_url.trim();
    const automationForCv =
      previewUrl && row.tenantId && !hasClientPreview
        ? withClientSitePreviewFields(
            { ...prevAuto, updated_at: new Date().toISOString() },
            previewUrl,
            row.tenantId,
          )
        : prevAuto;
    const lastReview = prevCv.preview_review && typeof prevCv.preview_review === 'object' ? prevCv.preview_review : null;
    const clientApproved = lastReview && String(lastReview.decision || '') === 'approve';
    const ok = Boolean(pr.ok && pr.pr_number) && Boolean(filesChanged != null && filesChanged > 0) && Boolean(previewUrl);
    const reasons = [];
    if (!(pr.ok && pr.pr_number)) reasons.push('PR_NOT_FOUND');
    if (!(filesChanged != null && filesChanged > 0)) reasons.push('NO_FILE_CHANGES_YET');
    if (!previewUrl) reasons.push('PREVIEW_URL_MISSING');
    if (act && act.ok === true && act.needs_attention === true) reasons.push('GITHUB_ACTIONS_NEEDS_ATTENTION');
    if (clientApproved) reasons.push('CLIENT_APPROVED');

    delivery_verdict = {
      ok,
      computed_at: new Date().toISOString(),
      base_ref: baseRef,
      pr_number: pr.ok ? pr.pr_number ?? null : null,
      pr_url: pr.ok ? pr.pr_url ?? null : null,
      files_changed_count: filesChanged,
      preview_url: previewUrl || null,
      latest_run: act && act.ok === true ? act.latest_run || null : null,
      needs_attention: act && act.ok === true ? Boolean(act.needs_attention) : false,
      reasons,
    };

    // Persist verdict into console_json so /change can render it without external tabs.
    const nextCv = { ...prevCv, automation: automationForCv, delivery_verdict };
    const nextCj = { ...prevCj, client_view: nextCv };
    await prisma.cmpTicket.update({ where: { id: ticketId }, data: { consoleJson: nextCj } }).catch(() => {});

    // Alert when we newly become blocked or need attention.
    const prevOk = prevVerdict && typeof prevVerdict.ok === 'boolean' ? Boolean(prevVerdict.ok) : null;
    const nowOk = Boolean(delivery_verdict.ok);
    const newlyBlocked = prevOk === true && nowOk === false;
    const firstVerdict = prevOk === null;
    const needsAttention = Boolean(delivery_verdict.needs_attention);
    if ((firstVerdict && !nowOk) || newlyBlocked || needsAttention) {
      const msg =
        `CMP blocked: ${ticketId} (${row.tenantId || 'tenant'})\\n` +
        `Reasons: ${(delivery_verdict.reasons || []).join(', ') || 'unknown'}\\n` +
        (delivery_verdict.pr_url ? `PR: ${delivery_verdict.pr_url}\\n` : '') +
        (delivery_verdict.preview_url ? `Preview: ${delivery_verdict.preview_url}\\n` : '') +
        (act && act.workflow_filter_url ? `Actions: ${act.workflow_filter_url}\\n` : '');
      await sendTelegramOpsAlert({ text: msg });
      await forwardOpsAlert({
        kind: 'cmp_delivery_blocked',
        title: `CMP blocked: ${ticketId}`,
        message: msg,
        tenant_id: row.tenantId || null,
        ticket_id: ticketId,
        meta: { delivery_verdict },
      });
    }
  }
...
export default async function cmpMonitorCronHandler(req, res) {
  ...
  let ids = resolveMonitorTicketIds();
  if (isLowCostMode() && ids.length > 1) {
    ids = ids.slice(0, 1);
  }
