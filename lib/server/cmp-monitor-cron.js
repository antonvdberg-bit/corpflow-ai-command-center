...
} from './operator-checkpoint-alert.js';
...
async function monitorOneTicket(ticketId) {
  ...
  if (approvedBuild) {
    ...
    if ((firstVerdict && !nowOk) || newlyBlocked || needsAttention) {
      const reasons = (delivery_verdict.reasons || []).join(', ') || 'unknown';
      const link =
        delivery_verdict.preview_url ||
        delivery_verdict.pr_url ||
        changeConsoleCheckpointLink(ticketId);
      void notifyOperatorCheckpoint({
        kind: OPERATOR_CHECKPOINT_KINDS.PRODUCTION_VALIDATION_FAILURE,
        whatNeedsApproval: `Production delivery validation failed for ticket ${ticketId} (${reasons})`,
        link,
