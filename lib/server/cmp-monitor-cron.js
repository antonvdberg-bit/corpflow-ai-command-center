...
import { shouldEmitDeliveryCheckpointAlert } from './ops-notification-policy.js';
...
async function monitorOneTicket(ticketId) {
  ...
  if (approvedBuild) {
    ...
    // Exception-only: alert on transition, not unchanged blocked/needs_attention (#658).
    if (shouldEmitDeliveryCheckpointAlert({ prevVerdict, deliveryVerdict: delivery_verdict })) {
