...
/**
 * Build the same payload as GET `/api/tenant/site`, but by tenant id.
 * Used by internal gates (preview truth, delivery integrity) without requiring a request object.
 *
 * @param {import('@prisma/client').PrismaClient} prisma
 * @param {string} tenantId
 * @param {{ host?: string | null }=} opts
 * @returns {Promise<{ ok: true, tenant_id: string, tenant: Record<string, unknown>, site: Record<string, unknown> } | { ok: true, tenant_id: string, tenant: null, site: null, reason: string }>}
 */
export async function buildTenantSiteJsonForTenantId(prisma, tenantId, opts = {}) {
  const tid = str(tenantId);
  if (!tid) {
    return { ok: true, tenant_id: '', tenant: null, site: null, reason: 'TENANT_ID_MISSING' };
  }

  const host = opts.host != null ? str(opts.host) : '';
  ...
  if (!tenantRow) {
    return { ok: true, tenant_id: tid, tenant: null, site: null, reason: 'TENANT_NOT_REGISTERED' };
  }
  ...
  const base = defaultPublicSite(tid, host, { dbName: tenantPayload.name });
  ...
  return { ok: true, tenant_id: tid, tenant: tenantPayload, site };
}
