/**
 * CorpFlowAI Cursor Factory Handoff — permanent wake packet (#913 / #1062).
 *
 * GitHub eligibility/capacity event → dedicated workflow success on main with
 * exactly one source issue. Current live executor is Factory Wake Proof v2
 * (MODE B webhook). Cloud Agents API v1 is implemented behind this path and
 * stays dormant until FACTORY_CURSOR_EXECUTOR=cloud_agents_v1. The Background
 * Agents API dispatcher remains legacy / diagnostic only. Reuses scan/WIP
 * eligibility decisions. Scheduled whole-queue recovery (#1023) is
 * factory-queue-reconcile.yml, which workflow_calls this file only when a real
 ...
 */
import { formatFactoryCursorHandoffReceiptComment } from './factory-cursor-handoff-receipt.js';
...
export function formatFactoryHandoffComment(opts = {}) {
  ...
  const executorMode = emptyToNull(opts.executorMode);
  const executeLine =
    executorMode === 'cloud_agents_v1'
      ? 'Cursor Cloud Agents API v1 must execute exactly this one issue, open one PR, then terminate. Handoff selection alone is not execution.'
...
${executeLine}
Do not treat stale labels or merge-ready/operator-review inventory as execution WIP. Protected subject mentions alone do not block ordinary work.
${capacity ? `\nCapacity packet:\n\`\`\`\n${capacity.trim()}\n\`\`\`\n` : ''}${receipt}`;
}
