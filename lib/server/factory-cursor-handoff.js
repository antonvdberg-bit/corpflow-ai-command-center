/**
 * CorpFlowAI Cursor Factory Handoff — legacy diagnostic packet (#913 / #929).
 *
 * Not an execution authority. Production Cursor work starts via
 * Factory dispatcher activate → Background Agents API.
 * Manual workflow_dispatch may still emit a diagnostic artifact.
 *
...
/**
 * Displayed GitHub Actions workflow name. Must not equal the former production
 * Automation trigger `CorpFlowAI Cursor Factory Handoff`.
 */
export const FACTORY_CURSOR_HANDOFF_WORKFLOW_NAME =
  'CorpFlowAI Cursor Factory Handoff (legacy diagnostic)';

/** Former production Automation trigger name — must not be reused. */
export const FACTORY_CURSOR_HANDOFF_LEGACY_PRODUCTION_WORKFLOW_NAME =
...
This packet is diagnostic only. It is not an execution authority.
Production Cursor work starts via Factory dispatcher activate → Background Agents API.
...
/**
 * Decide whether this diagnostic handoff workflow run may conclude successfully.
 *
 * Success is reserved for a real single-issue diagnostic packet. This workflow
 * is not an execution authority and must not wake native Automation.
 *
 * @param {{
