...
export default async function handler(req, res) {
  // Safety: this endpoint was originally drafted against a Prisma `Tenant` model and BASEROW_DB_ID.
  // In this repo's current schema.prisma, `Tenant` does not exist. Fail-closed unless explicitly enabled.
  const enabled = String(process.env.PROVISIONING_ENABLED || 'false').toLowerCase() === 'true';
  if (!enabled) {
    return res.status(501).json({
      error: 'PROVISIONING_DISABLED',
      hint: 'Set PROVISIONING_ENABLED=true and configure BASEROW_DB_ID + Prisma Tenant model before using this endpoint.',
    });
  }

  if (req.method !== 'POST') return res.status(405).json({ error: 'Method Not Allowed' });

  // Factory-only: prevents tenant-level callers from provisioning infrastructure.
  if (!verifyFactoryMasterAuth(req)) {
    return res.status(403).json({ error: 'Factory master authentication required.' });
  }

  const baserowDbId = (process.env.BASEROW_DB_ID || '').toString().trim();
  if (!baserowDbId) {
    return res.status(503).json({
      error: 'BASEROW_DB_ID_MISSING',
      hint: 'Set BASEROW_DB_ID (Baserow database id) to enable provisioning.',
    });
  }
  ...
  try {
    ...
    if (!tenant) {
      ...
      const response = await fetch(`${process.env.BASEROW_URL}/api/database/tables/database/${baserowDbId}/`, {
        method: 'POST',
        headers: { 
