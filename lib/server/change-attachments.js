...
/**
 * Returns true when the ticket row + console_json look like a Lux client-request
 * ticket (i.e. carries `lux_request_meta`). We only annotate attachments for these
 * — generic CMP tickets keep their existing minimal attachment shape.
 *
 * @param {{ tenantId?: string | null, consoleJson?: unknown }} ticketRow
 * @returns {boolean}
 */
function isLuxRequestTicket(ticketRow) {
  if (!ticketRow) return false;
  const tid = String(ticketRow.tenantId || '').trim();
  if (tid !== LUX_TENANT_ID) return false;
  const cj = ticketRow.consoleJson && typeof ticketRow.consoleJson === 'object' ? ticketRow.consoleJson : null;
  if (!cj || typeof cj !== 'object') return false;
  return cj.lux_request_meta && typeof cj.lux_request_meta === 'object';
}
 ...
 * @returns {Promise<{ ok: true, row: { tenantId: string | null, consoleJson: unknown } } | { ok: false, status: number, error: string }>}
 */
...
export async function handleChangeAttachmentUpload(req, res) {
  ...
  const intendedUse = normalizeAttachmentIntendedUse(body.intended_use);
  const notes = normalizeAttachmentNotes(body.notes);
  ...
  try {
    ...
    let metaEntry = null;
    if (isLuxRequestTicket(access.row)) {
      try {
        metaEntry = buildLuxAttachmentEntry({
          ...
          notes,
          created_at: created.createdAt instanceof Date ? created.createdAt.toISOString() : null,
          created_by: scope.tenantId === LUX_TENANT_ID ? 'tenant_session' : 'admin_session',
        });
        const nextConsoleJson = upsertLuxAttachmentEntry(access.row.consoleJson, metaEntry);
        await prisma.cmpTicket.update({
          where: { id: ticketId },
          data: { consoleJson: nextConsoleJson },
          select: { id: true },
        });
      } catch (annotateErr) {
        // Fail-soft: the binary upload succeeded; metadata annotation is recoverable
        // via the lux-attachment-review-set path. Surface the error in the response so
        // the operator UI can warn, but do not 500 the whole upload.
        const annotateMsg = annotateErr instanceof Error ? annotateErr.message : String(annotateErr);
        res.status(200).json({
          ok: true,
          attachment_id: created.id,
          ...
          content_type: contentType,
          byte_size: buf.length,
          lux_meta_warning: annotateMsg.slice(0, 240),
        });
        return;
      }
    }
    ...
    res.status(200).json({
      ...
      lux_meta: metaEntry
        ? {
            media_type: metaEntry.media_type,
            intended_use: metaEntry.intended_use,
            notes: metaEntry.notes,
            review_status: metaEntry.review_status,
          }
        : null,
    });
  } catch (e) {
...
export async function handleChangeAttachmentList(req, res) {
  ...
  try {
    ...
    const lux = isLuxRequestTicket(access.row);
    const metaList = lux ? readLuxAttachmentEntries(access.row.consoleJson) : [];
    const metaById = new Map(metaList.map((m) => [String(m.attachment_id), m]));
    ...
    const attachments = rows.map((r) => {
      ...
      return safeLuxAttachmentShape(dbRow, metaById.get(r.id) || null);
    });

    res.status(200).json({
      ok: true,
      ticket_id: ticketId,
      lux_request_ticket: lux,
      attachments,
    });
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e);
