...
/**
 * Parent-domain cookie scope for CorpFlow production hosts.
 * Spec: docs/operations/OPERATOR_MULTI_TENANT_CREDENTIAL_V1.md — session must
 * carry from Core to tenant hosts (e.g. core → lux) after workspace switch.
 *
 * @param {import('http').IncomingMessage | { headers?: { host?: string } } | null | undefined} req
 * @returns {string | undefined} e.g. `.corpflowai.com`, or undefined for localhost / unknown hosts
 */
export function resolveSessionCookieDomain(req) {
...
/**
 * Standard Set-Cookie options for session + CSRF cookies on this request host.
 *
 * @param {import('http').IncomingMessage | { headers?: { host?: string } } | null | undefined} req
 * @param {{ maxAgeSec?: number, httpOnly?: boolean }} [extra]
 */
export function buildCorpflowAuthCookieOpts(req, extra = {}) {
  const domain = resolveSessionCookieDomain(req);
  /** @type {{ maxAgeSec?: number, httpOnly?: boolean, sameSite?: string, secure?: boolean, path?: string, domain?: string }} */
  const opts = { ...extra };
...
export function clearCookie(res, name, opts = {}) {
  setCookie(res, name, '', { maxAgeSec: 0, ...opts });
}
