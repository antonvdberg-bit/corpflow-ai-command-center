/**
 * Read-only factory API for tenant workflow runs and operator steps.
 *
 * GET /api/factory/tenant-workflows/runs
 * GET /api/factory/tenant-workflows/steps
 *
 * Factory master auth required (same gate as /api/automation/events).
 */
...
export async function handleTenantWorkflowStepsList(req, res, prisma = defaultPrisma) {
  ...
  const workflowRunId = pickQuery(q, 'workflow_run_id');
  const limitRaw = pickQuery(q, 'limit');
  const limit = Math.min(MAX_LIMIT, Math.max(1, parseInt(String(limitRaw || DEFAULT_LIMIT), 10) || DEFAULT_LIMIT));
  ...
  try {
    ...
    return res.status(200).json({ ok: true, count: rows.length, steps: rows });
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e);
    return res.status(500).json({ ok: false, error: 'workflow_steps_list_failed', detail: msg });
  }
}
