...
export const MPGS_ENV_KEYS = Object.freeze({
  ...
  TEST_ALLOWED_PUBLIC_HOSTS: 'CORPFLOW_MPGS_TEST_ALLOWED_PUBLIC_HOSTS',
});

/** Default PAY-SBM-4 stable TEST host (factory core surface on Production). */
const DEFAULT_TEST_ALLOWED_CORPFLOW_PUBLIC_HOSTS = ['test.corpflowai.com'];

/**
 * Hostnames on CORPFLOW_ROOT_DOMAIN allowed as CORPFLOW_MPGS_PUBLIC_BASE_URL in TEST mode.
 * All other *.corpflowai.com hosts stay blocked (lux, core, apex, etc.).
 *
 * @returns {Set<string>}
 */
export function getMpgsTestAllowedCorpflowPublicHosts() {
  const raw = String(cfg(MPGS_ENV_KEYS.TEST_ALLOWED_PUBLIC_HOSTS, '')).trim();
  const hosts = raw
    ? raw.split(',').map((h) => h.trim().toLowerCase()).filter(Boolean)
    : [...DEFAULT_TEST_ALLOWED_CORPFLOW_PUBLIC_HOSTS];
...
export function validateMpgsPublicBaseUrl() {
  ...
  if (getMpgsMode() === 'test') {
    ...
    if (host === 'corpflowai.com' || host.endsWith('.corpflowai.com')) {
      if (!getMpgsTestAllowedCorpflowPublicHosts().has(host)) {
