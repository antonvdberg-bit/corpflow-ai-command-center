...
export async function handleMpgsCreatePaymentLink(req, res, prisma) {
  ...
  try {
    ...
    if (code === 'mpgs_not_configured' && e.details) {
      body.diagnostics = e.details;
    }
    if (code === 'mpgs_initiate_failed' || code === 'mpgs_initiate_incomplete') {
      if (e.status != null) body.gateway_http_status = e.status;
      if (e.gateway && typeof e.gateway === 'object') body.gateway_hint = e.gateway;
    }
