...
/**
 * Parse INITIATE_CHECKOUT response. Payment Link mode returns paymentLink.id/url
 * without session.id (SBM TEST v66 shape).
 *
 * @param {Record<string, unknown>} raw
 * @returns {{ ok: true, sessionId: string, successIndicator: string, paymentLinkUrl: string } | { ok: false, error: 'mpgs_initiate_incomplete' }}
 */
export function parseMpgsInitiateCheckoutResponse(raw) {
  const paymentLinkUrl = raw?.paymentLink?.url ? String(raw.paymentLink.url) : null;
  const paymentLinkId = raw?.paymentLink?.id ? String(raw.paymentLink.id) : null;
  const sessionId = raw?.session?.id ? String(raw.session.id) : paymentLinkId;
  const successIndicator = raw?.successIndicator ? String(raw.successIndicator) : null;
  if (!successIndicator || !paymentLinkUrl || !sessionId) {
    return { ok: false, error: 'mpgs_initiate_incomplete' };
  }
  return {
    ok: true,
    sessionId,
...
export async function initiateMpgsCheckoutSession(params) {
  ...
  const parsed = parseMpgsInitiateCheckoutResponse(raw);
  if (!parsed.ok) {
