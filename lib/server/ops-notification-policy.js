 ...
 * Publishing or validating work on CorpFlowAI-hosted corpflow_test surfaces
 * (lux / cipc / core, etc.) is NOT an exception by itself (#679).
 *
 * @param {{
 *   now?: number,
...
/**
 * Hour-bucket dedupe for heartbeat Telegram paging (kind × target × hour).
 * Suppresses repeat pages for the same exception within the same UTC hour.
 *
 * @param {HeartbeatAlert[]} alerts
 * @param {Record<string, true> | null | undefined} seenKeys prior keys (mutated copy returned)
 * @param {number} [now]
...
export function filterHeartbeatAlertsByHourDedupe(alerts, seenKeys, now = Date.now()) {
  ...
  for (const alert of list) {
    ...
    const key = `${kind}|${target}|${hourBucket}`;
    if (nextSeen[key]) continue;
    nextSeen[key] = true;
    paging.push(alert);
  }

  // Drop keys from older hour buckets so staticData does not grow forever.
  for (const key of Object.keys(nextSeen)) {
    const parts = key.split('|');
    const bucket = Number(parts[parts.length - 1]);
    if (Number.isFinite(bucket) && bucket < hourBucket) delete nextSeen[key];
  }
