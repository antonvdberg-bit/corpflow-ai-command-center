...
import { closeLeadRescueTestIntakes } from './close-lead-rescue-test-intakes.js';
 ...
 * @param {string} pathSeg
 */
async function handleCloseTestIntakes(req, res) {
  setCommonHeaders(res);
  const parsed = parseJsonBody(req);
  const body = parsed.ok ? parsed.body : {};
  const dryRun = body.execute !== true;
  const sess = getSessionFromRequest(req);
  const actorLabel = aiLeadRescueActorLabelFromPayload(sess?.payload || null) || 'factory-admin';

  const result = await closeLeadRescueTestIntakes({
    dryRun,
    actorLabel,
    prismaClient: prisma,
    applyPatch: applyAiLeadRescuePatch,
  });
...
export async function adminLeadRescueHandler(req, res, pathSeg) {
  ...
  if (route === 'close-test-intakes') {
    ...
    return handleCloseTestIntakes(req, res);
  }
