...
export async function handleAuthLogin(req, res) {
  ...
  if (level === 'admin') {
    ...
    // Preferred: Postgres auth_users table (Elestio).
    try {
      ...
      return deny(res, 503, 'AUTH_BACKEND_ERROR', { detail: String(e?.message || e), backend: 'postgres' });
    }
    ...
    try {
      ...
      // Fail closed to env fallback only if table isn't configured; otherwise surface the error.
      const msg = String(e?.message || '');
      if (!msg.includes('BASEROW_AUTH_USERS_TABLE_ID')) return deny(res, 503, 'AUTH_BACKEND_ERROR', { detail: msg });
    }
    ...
    if (!creds.username) {
      return deny(res, 503, 'ADMIN_LOGIN_DISABLED', {
        hint:
          'Either set BASEROW_AUTH_USERS_TABLE_ID (recommended) OR set CORPFLOW_ADMIN_USERNAME and CORPFLOW_ADMIN_PASSWORD (or *_HASH/_SALT) in Vercel.',
      });
    }
