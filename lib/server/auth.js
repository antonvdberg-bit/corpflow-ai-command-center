...
export async function handleAuthLogin(req, res) {
  ...
  if (level === 'admin') {
    ...
    try {
      ...
      if (pg.ok) {
        /**
         * IM-5 (2026-06-15) — DB-backed admin session payload gains two new
         * additive fields:
         *   - acting_tenant_id: null   (admin starts in "no acting tenant" state;
         *                               sets via POST /api/membership/switch later)
         *   - session_version: 1       (monotonic; bumped by /switch and /leave)
         * Legacy env-master payload (mint point below) is intentionally NOT
         * extended (Anton's IM-5 approval correction #3: "Do not change legacy
         * env-master/PIN session shape").
         */
        ...
        const cookieOpts = buildCorpflowAuthCookieOpts(req, { maxAgeSec: ttlSec });
        setCookie(res, CORPFLOW_SESSION_COOKIE, signed.token, cookieOpts);
        /**
         * IM-5 — CSRF cookie is issued at the same call sites where the session
         * cookie is minted/re-issued. NOT HttpOnly so client JS can read it back
         * to populate the X-CorpFlow-CSRF header on /switch + /leave POSTs.
         * Scope is intentionally minimal (Anton's IM-5 approval correction #2).
         */
        issueCsrfCookie(res, cookieOpts);
    ...
    if (!signed.ok) return deny(res, 503, 'SESSION_SECRET_MISSING');
    setCookie(res, CORPFLOW_SESSION_COOKIE, signed.token, buildCorpflowAuthCookieOpts(req, { maxAgeSec: ttlSec }));
  ...
  if (level === 'tenant') {
    ...
    if (username && password) {
      ...
      if (pg.ok) {
        /**
         * IM-5 (2026-06-15) — DB-backed tenant session gains the same two
         * additive fields. The initial `acting_tenant_id` is the user's bound
         * `tenant_id` (so a single-tenant client's session naturally reads as
         * "acting as their own tenant"). Legacy PIN mint point below is NOT
         * extended (correction #3).
         */
        const signed = signSession(
          {
            typ: 'tenant',
            ...
            username: pg.user.username,
            user_id: pg.user.id,
            acting_tenant_id: pg.user.tenant_id != null ? String(pg.user.tenant_id) : null,
            session_version: 1,
          },
          { ttlSec },
        );
        ...
        const cookieOpts = buildCorpflowAuthCookieOpts(req, { maxAgeSec: ttlSec });
        setCookie(res, CORPFLOW_SESSION_COOKIE, signed.token, cookieOpts);
        /** IM-5 — CSRF cookie at the DB-backed mint point (see admin comment above). */
        issueCsrfCookie(res, cookieOpts);
...
export async function handleAuthLogout(req, res) {
  ...
  clearCookie(res, CORPFLOW_SESSION_COOKIE, buildCorpflowAuthCookieOpts(req));
  return res.status(200).json({ ok: true });
}
