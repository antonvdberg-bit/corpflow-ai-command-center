 ...
 *     findPrForBranch?: (branch: string) => Promise<{ number: number, url: string, headSha?: string | null } | null>,
...
export async function runCursorAgentLifecycleTick(input) {
  ...
  const normalized = normalizeCursorAgentLifecycleStatus(apiResult, {
    ...
    hasPr: Boolean(git.prUrl || git.prNumber || prior.prNumber),
  });
  ...
  /** @type {string | null} */
  let ciResult = null;
  const actions = [`normalized:${phase}`];
  ...
  // COMPLETED — discover PR/checks + review packet once
  if (branch && !prNumber && input.github?.findPrForBranch) {
