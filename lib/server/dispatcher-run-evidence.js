/**
 * Durable evidence packets for the LEGACY diagnostic Factory dispatcher activate runs.
 * Posted to the control issue (#661 by default) so silent/missing diagnostic runs are detectable.
 *
 * This path is NOT production execution (#930). Production is CorpFlowAI Cursor Factory Handoff.
 *
...
export function classifyMissingDispatcherRunEvidence(input = {}) {
  ...
  if (!Number.isFinite(latest)) {
    return {
      ...
      message:
        'No LEGACY Factory dispatcher activate runs found via GitHub API. Paste the Actions run URL or re-open Actions → LEGACY Factory dispatcher activate (diagnostic - not production execution) and confirm a run ID appears.',
    };
  }
  ...
  if (Number.isFinite(merged) && latest < merged) {
    return {
      ...
      anton_action_required: true,
      message:
        'Latest diagnostic dispatcher run predates the repair merge. No post-merge workflow_dispatch run exists. Confirm the Actions run was created (branch main, workflow LEGACY Factory dispatcher activate) and paste the run URL.',
