...
/**
 * Cross-executor gate: Cursor ownership → SKIP_ALREADY_CLAIMED.
 *
 * @param {{
 *   issueNumber: number,
 *   labels?: unknown,
...
export function evaluateCodexClaimGate(input) {
  ...
  const codexClaims = parseCodexClaimsFromComments(comments).sort((a, b) => {
    const tb = Date.parse(b.claimed_at || 0) || 0;
    const ta = Date.parse(a.claimed_at || 0) || 0;
    if (tb !== ta) return tb - ta;
    return (b.attempt || 0) - (a.attempt || 0);
  });
  const latestCodex = codexClaims[0] || null;
  const activeCodex =
    latestCodex &&
    ['AWAITING_HUMAN_TRIGGER', 'RUNNING', 'pending', 'activated'].includes(String(latestCodex.status))
      ? latestCodex
      : null;
