...
function buildCipcDeskWebsiteDraft() {
  // Catalogue lives in cipc-desk-catalogue.js; dedicated CipcDeskLanding renders it.
  // Persona draft remains a fallback for generic TenantSite / operator tooling.
  const seedNote =
    'CorpFlowAI test desk (corpflow_test) — fictional data only. All service requirements, documents, turnaround and pricing must be validated by Serah Fourie before any work starts.';

  return {
    meta: {
      page_title: 'CIPC Desk · CorpFlow test desk',
    ...
    hero: {
      ...
      cta_href: `mailto:swart829@gmail.com?subject=${encodeURIComponent('CIPC Desk · SME enquiry')}`,
    },
    sections: {
      about: {
        ...
        body:
          `${seedNote}\n\n` +
          'How it works:\n' +
          '1) You email a matter request (direct SME or professional partner).\n' +
          '2) Serah reviews and confirms the exact scope in /change.\n' +
          '3) You receive a drafted status/update after guided decisions — not auto-sent.\n\n' +
          'CorpFlowAI is not CIPC, not a law firm, and not an authorised CIPC representative.',
      },
      services: {
    ...
    client_view: {
      cipc_desk: {
        ...
        onboarding: {
          ...
          communication_model: 'email-first (corpflow_test)',
...
function buildInitialCipcDeskTicketRow() {
  ...
  const briefSummary =
    consoleJson?.brief && typeof consoleJson.brief === 'object' && typeof consoleJson.brief.summary === 'string'
      ...
      : 'CIPC Desk · test request';

  return {
    description: 'CIPC Desk · email-first corpflow_test workflow (fictional data)',
    title: 'CIPC Desk · Private-company registration (test)',
    status: 'Approved',
    stage: 'Build',
    locale: 'en',
    brief: briefSummary,
