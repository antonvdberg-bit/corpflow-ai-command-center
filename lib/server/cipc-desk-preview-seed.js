...
} from './cipc-desk-landing-content.js';

export { CIPC_DESK_LANDING_CONTENT_VERSION, buildCipcDeskWebsiteDraftForLanding };
...
function buildCipcDeskWebsiteDraft() {
  return buildCipcDeskWebsiteDraftForLanding();
}
 ...
 * Also refreshes website_draft when content_version is stale (presentation only).
 *
...
export async function ensureCipcDeskPreviewTenantSeeded(opts = {}) {
  ...
  try {
    ...
    const draftOk = draftVersion === CIPC_DESK_LANDING_CONTENT_VERSION;
    ...
    await prisma.$transaction(async (tx) => {
      ...
      if (!personaOk) {
        ...
        // Refresh visible catalogue / CTAs without recreating the persona row.
