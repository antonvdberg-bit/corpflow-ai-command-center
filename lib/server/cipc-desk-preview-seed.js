...
function buildInitialCipcDeskTicketConsoleJson() {
  ...
  return {
    ...
    brief: {
      summary: 'CIPC Desk · Private-company registration (standing test request)',
    ...
    client_view: {
      cipc_desk: {
        ...
        onboarding: {
          ...
          communication_model: 'email-first (standing corpflow_test)',
...
function buildInitialCipcDeskTicketRow() {
  ...
  const briefSummary =
    consoleJson?.brief && typeof consoleJson.brief === 'object' && typeof consoleJson.brief.summary === 'string'
      ...
      : 'CIPC Desk · standing test request';

  return {
    description: 'CIPC Desk · email-first standing test workflow (fictional data)',
    title: 'CIPC Desk · Private-company registration (standing test)',
    status: 'Approved',
    stage: 'Build',
    locale: 'en',
    brief: briefSummary,
  ...
  try {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      if (!personaOk) {
        ...
        // Refresh visible catalogue / dual-route CTAs without recreating the persona row.
