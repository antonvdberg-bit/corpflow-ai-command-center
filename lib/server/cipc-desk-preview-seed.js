...
/**
 * Marker used to confirm the ticket row belongs to this synthetic Business Admin Desk slice.
 * Stored in `cmp_tickets.console_json.client_view.cipc_desk.seed_marker`.
 */
const CIPC_DESK_SEED_MARKER = 'cipc-desk-preview-seed-v2-business-admin-desk';
...
function buildInitialCipcDeskTicketConsoleJson() {
  ...
  return {
    ...
    brief: {
      summary: 'Business Admin Desk · Private-company registration (preview request)',
...
function buildInitialCipcDeskTicketRow() {
  ...
  const briefSummary =
    consoleJson?.brief && typeof consoleJson.brief === 'object' && typeof consoleJson.brief.summary === 'string'
      ...
      : 'Business Admin Desk · preview request';

  return {
    description: 'Business Admin Desk · email-first preview workflow (fictional data)',
    title: 'Business Admin Desk · Private-company registration (preview)',
    status: 'Approved',
    stage: 'Build',
    locale: 'en',
    brief: briefSummary,
...
/**
 * Ensure the Business Admin Desk tenant + seed ticket exist for the standing test tenant
...
export async function ensureCipcDeskPreviewTenantSeeded(opts = {}) {
  ...
  try {
    ...
    const normalizedHost = normalizeHostname(opts.host);
    const publicHost = isBusinessAdminDeskPublicHost(normalizedHost);
    const hostnameRow = publicHost
      ? await prisma.tenantHostname.findUnique({
          where: { host: normalizedHost },
          select: { host: true, tenantId: true, mode: true, enabled: true },
        })
      : null;
    ...
    const hostMappingOk =
      !publicHost ||
      (hostnameRow?.tenantId === tenantId &&
        hostnameRow?.mode === 'client_production' &&
        hostnameRow?.enabled === true);

    if (tenantOk && personaOk && markerOk && draftOk && hostMappingOk) {
    ...
    await prisma.$transaction(async (tx) => {
      if (publicHost && !hostMappingOk) {
        await tx.tenantHostname.upsert({
          where: { host: normalizedHost },
          create: {
            host: normalizedHost,
            tenantId,
            mode: 'client_production',
            enabled: true,
          },
          update: {
            tenantId,
    ...
    return {
      ...
      host_mapping_refreshed: publicHost && !hostMappingOk,
      tenant_id: tenantId,
