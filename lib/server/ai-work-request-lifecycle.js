...
const FACTORY_COMPACT_RUN_MARKER = 'corpflow.factory_cloud_agents_executor.v1';
...
export function extractCursorPickupEvidence(text) {
  ...
  // Cloud Agents v1 stores current lifecycle state in one compact updatable
  // comment. Read it directly here to avoid a circular dependency on the
  // executor (which itself builds AI work-request records).
  const compactMatch = body.match(
    new RegExp(`<!--\\s*${FACTORY_COMPACT_RUN_MARKER}\\s+(\\{[\\s\\S]*?\\})\\s*-->`, 'i'),
  );
  if (compactMatch) {
    try {
      const compact = JSON.parse(compactMatch[1]);
      const status = String(compact?.status || '').toUpperCase();
      const agent = emptyToNull(compact?.cursor_agent_id);
      const run = emptyToNull(compact?.cursor_run_id);
      if (agent || run || status === 'BLOCKED') {
        return {
          ...empty,
          cursor_agent_id: agent,
          cursor_run_id: run,
          branch: emptyToNull(compact?.branch),
          pr_number: toPositiveInt(compact?.pr_number),
          pr_url: emptyToNull(compact?.pr_url),
          head_sha: emptyToNull(compact?.head_sha),
          ci_state: emptyToNull(compact?.ci_state),
          blocker: status === 'BLOCKED' ? emptyToNull(compact?.blocker) : null,
          lifecycle_phase: status === 'COMPLETED' ? 'COMPLETED' : null,
          completion_status: status === 'COMPLETED' ? 'COMPLETED' : null,
          picked_up: Boolean(agent || run),
          active_execution: status === 'IN_PROGRESS',
        };
      }
    } catch {
      // Invalid externally stored evidence must not become execution proof.
    }
  }
