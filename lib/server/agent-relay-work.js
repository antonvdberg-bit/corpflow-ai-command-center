...
export const AGENT_RELAY_READ_OPERATIONS = Object.freeze(Object.keys(OPERATIONS));
...
async function performOperation(envelope, { fetchFn, configOverrides } = {}) {
...
function safeErrorCode(error) {
  ...
  return /^(?:AGENT_RELAY_[A-Z_]+|EXPECTED_SHA_MISMATCH|GITHUB_HEAD_SHA_MISSING|GITHUB_READ_FAILED)$/.test(code)
    ? code
...
export async function executeAgentRelayWork(envelopeInput, deps = {}) {
  ...
  try {
    ...
    return { status: 200, body: resultEnvelope(envelope, { ok: true, accepted: true, evidence: result.evidence }) };
  } catch (error) {
