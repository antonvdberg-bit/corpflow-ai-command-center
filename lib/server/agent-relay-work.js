...
function relayIdempotencyKey(envelope) {
  const identity = `${envelope.repository}:${envelope.target.number}:${envelope.replayIdentity}`;
  return `agent-relay:issue.add_comment:v1:${crypto.createHash('sha256').update(identity).digest('hex')}`;
}
...
async function claimCommentWrite(prisma, envelope) {
  if (!prisma?.automationEvent) throw relayError('RELAY_DURABLE_STORE_UNAVAILABLE', 503);
  const tenantScope = 'global';
  const idempotencyKey = relayIdempotencyKey(envelope);
  ...
  try {
    const row = await prisma.automationEvent.create({
      data: {
        ...
        tenantScope,
        source: 'agent-relay',
        eventType: 'agent_relay.issue_comment.claim.v1',
        ...
        payload: {
          ...
          target_number: envelope.target.number,
          request_id: envelope.requestId,
          replay_identity: envelope.replayIdentity,
          marker,
          comment_sha256: crypto.createHash('sha256').update(envelope.payload.commentBody).digest('hex'),
        },
      },
      select: { id: true, occurredAt: true, status: true, payload: true },
    });
    ...
    const row = await prisma.automationEvent.findUnique({
      where: { automation_events_scope_idem: { tenantScope, idempotencyKey } },
      select: { id: true, occurredAt: true, status: true, payload: true },
    });
    if (!row) throw relayError('RELAY_DURABLE_CLAIM_FAILED', 503);
    return { claimed: false, row, marker };
  }
}

async function confirmCommentClaim(prisma, id, detail) {
...
async function findDurableComment(envelope, token, fetchFn, configOverrides, claimedAt) {
  const since = claimedAt?.toISOString?.() || new Date(0).toISOString();
...
async function performIssueComment(envelope, { fetchFn, configOverrides, prisma }) {
  ...
  const recovered = await findDurableComment(envelope, token, fetchFn, configOverrides, claim.row.occurredAt);
  if (recovered) {
    await confirmCommentClaim(prisma, claim.row.id, {
      ...(claim.row.payload && typeof claim.row.payload === 'object' ? claim.row.payload : {}),
      status: 'confirmed',
      comment_id: recovered.commentId,
      comment_url: recovered.commentUrl,
    });
    return { evidence: { comment: recovered }, idempotencyState: 'replay' };
  }
  if (!claim.claimed) {
    // A persisted claim means another process owns the only allowed external
    // write. Do not take over: a late original request could otherwise race.
    throw relayError('RELAY_IDEMPOTENCY_PENDING_MANUAL_RECOVERY', 409);
  }
  ...
  try {
    ...
    // The durable claim remains. Never retry an ambiguous external write.
    const durable = await findDurableComment(envelope, token, fetchFn, configOverrides, claim.row.occurredAt);
    if (durable) {
      await confirmCommentClaim(prisma, claim.row.id, { ...(claim.row.payload || {}), status: 'confirmed', comment_id: durable.commentId, comment_url: durable.commentUrl });
      return { evidence: { comment: durable }, idempotencyState: 'replay' };
    }
    throw relayError('RELAY_AMBIGUOUS_WRITE_UNRESOLVED', 503);
  }
  if (!created?.ok) throw relayError('GITHUB_COMMENT_WRITE_FAILED', 503);
  const createdPayload = await created.json();
  const commentId = Number(createdPayload?.id);
  if (!Number.isInteger(commentId) || commentId < 1) throw relayError('GITHUB_COMMENT_WRITE_INVALID', 503);
...
export function parseAgentRelayWorkEnvelope(input, { nowMs = Date.now() } = {}) {
  ...
  if (operationPolicy.write) {
    ...
    const commentBody = typeof input.payload?.comment_body === 'string' ? input.payload.comment_body.trim() : '';
    if (
      !commentBody ||
