...
function safeTicketId(raw) {
  // Match CMP ticket id normalization rules: keep alnum + . _ -
  return String(raw || '')
    .trim()
...
export default async function factoryGithubPrCreateHandler(req, res) {
  ...
  const ticketId = safeTicketId(body.ticket_id || body.ticketId || '');
  const base = safeBaseRef(body.base || body.base_ref || cfg('CMP_SANDBOX_BASE_REF', '') || 'main') || 'main';
  const providedHead = safeBranchName(body.head || body.head_ref || body.branch || '');
  const slug = safeSlug(body.slug || body.pr_slug || body.change_slug || body.title || '');
  const head = providedHead || (ticketId && slug ? `factory/${ticketId}/${slug}` : '');
  const title = String(body.title || '').trim() || `Factory PR: ${head || ticketId || 'change'}`;
  ...
  if (!head) {
    return res.status(400).json({
      ...
      hint: 'Provide head (e.g. fix/my-change) or provide ticket_id + slug to auto-generate factory/<ticketId>/<slug>.',
    });
  }
  ...
  try {
    ...
    emitTelemetry({
      ...
      cmp: { ticket_id: String(ticketId || 'n/a'), action: 'factory/github/pr-create' },
      payload: { repo: `${owner}/${repo}`, base, head, pr_number: pr.number || null },
    });
