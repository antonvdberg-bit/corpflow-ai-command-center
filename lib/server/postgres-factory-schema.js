...
} from './postgres-ensure-schema-connection.js';
...
export default async function postgresFactorySchemaHandler(req, res) {
  ...
  const pooled = String(cfg('POSTGRES_URL', '')).trim();
  const pgUrl = nonPool || pooled;
  if (!pgUrl) {
    return json(res, 503, {
  ...
  try {
    ...
    const errRaw = e && typeof e === 'object' ? /** @type {Record<string, unknown>} */ (e) : {};
    const prismaCode = typeof errRaw.code === 'string' ? errRaw.code : undefined;
    const label =
    ...
    return json(res, 500, {
      ...
      failed_statement_label: label || undefined,
