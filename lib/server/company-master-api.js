 ...
 *   POST   company-master/artifacts/remove-pending
 *   POST   company-master/artifacts/withdraw
 *   POST   company-master/artifacts/archive
 *   GET    company-master/artifacts/meta?id=
 *   GET    company-master/artifacts/download?id=
 *   GET    company-master/resolve?company_id=&alias=
 *   GET    company-master/jurisdictions
...
import { CM_JURISDICTIONS } from './company-master-constants.js';
import {
  approveArtifact,
  archiveArtifact,
...
export async function companyMasterHandler(req, res, pathSeg) {
  ...
  try {
    if (seg === 'company-master/jurisdictions' && req.method === 'GET') {
      return json(res, 200, { ok: true, jurisdictions: CM_JURISDICTIONS });
    }
    ...
    if (seg === 'company-master/artifacts/remove-pending' && req.method === 'POST') {
      const parsed = parseBody(req);
      if (!parsed.ok) return json(res, 400, { error: parsed.error });
      const result = await removePendingArtifact(
        parsed.body.artifact_id || parsed.body.id,
        { prisma },
        scope,
      );
      if (!result.ok) return json(res, result.status || 400, result);
    ...
    if (seg === 'company-master/artifacts/withdraw' && req.method === 'POST') {
      const parsed = parseBody(req);
      if (!parsed.ok) return json(res, 400, { error: parsed.error });
      const result = await withdrawArtifact(
        parsed.body.artifact_id || parsed.body.id,
        { prisma },
        scope,
        scope.username,
    ...
    if (seg === 'company-master/artifacts/archive' && req.method === 'POST') {
      const parsed = parseBody(req);
      if (!parsed.ok) return json(res, 400, { error: parsed.error });
