 ...
 * Bare "production" / delivery-reality wording must NOT route to Anton —
 * only explicit client_production (or other hard gates).
 *
...
export function detectAntonProtectedGateFromText(blob) {
  ...
  const forbidsClientProduction =
    /no production deploy|without.*production|not.*client.?production|no client.?production|corpflow.?test|treat.*as test|classification\/documentation\/test-workflow|future client production|until a separate production transition/.test(
      text,
    );

  const clientProductionGate =
    !forbidsClientProduction &&
    (/client[_ ]production|approval:production\b|deploy to client.?owned|client-owned production|protected gate:?\s*production\b|requires? (anton )?client.?production|must deploy to client.?production/.test(
      text,
    ) ||
      (/requires? (anton )?production (approval|gate|deploy)/.test(text) &&
        !/corpflow|test environment|preview/.test(text)));

  if (clientProductionGate) return 'production';

  for (const gate of PROTECTED_GATES_ANTON_ONLY) {
    if (gate === 'merge' || gate === 'production') continue;
    if (new RegExp(`\\b${gate}\\b`).test(text)) {
      // Prohibition lists are not activation requests.
      if (
        /protected gates[\s\S]{0,500}without anton approval/i.test(text) &&
        new RegExp(`no .*${gate}|without.*${gate}`, 'i').test(text)
      ) {
        continue;
      }
      return gate;
