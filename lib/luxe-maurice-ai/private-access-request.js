...
/**
 * Prisma where clause for LuxeMaurice private access request leads.
 * @param {string} tenantId
 */
export function buildPrivateAccessLeadListWhere(tenantId) {
  return {
    tenantId,
    intent: LUXE_MAURICE_AI_ACCESS_REQUEST_INTENT,
  };
...
/**
 * @param {{ intent?: string | null, qualificationJson?: unknown }} row
 */
export function isPrivateAccessLeadRow(row) {
  if (!row || safeStr(row.intent) !== LUXE_MAURICE_AI_ACCESS_REQUEST_INTENT) return false;
  const qj = row.qualificationJson && typeof row.qualificationJson === 'object' ? row.qualificationJson : {};
  const ar = qj.access_request && typeof qj.access_request === 'object' ? qj.access_request : {};
  const source = safeStr(ar.source);
  if (source && source !== LUXE_MAURICE_AI_ACCESS_REQUEST_SOURCE) return false;
  return true;
}

function truncateNotesSummary(notes) {
  const text = safeStr(notes);
  if (!text) return null;
  return text.length > 160 ? `${text.slice(0, 157)}…` : text;
}

function deriveAdvisorNextAction(qualificationJson) {
  const qj = qualificationJson && typeof qualificationJson === 'object' ? qualificationJson : {};
  const ar = qj.access_request && typeof qj.access_request === 'object' ? qj.access_request : {};
  const ow = qj.lux_operator_workflow && typeof qj.lux_operator_workflow === 'object' ? qj.lux_operator_workflow : {};
  const note = safeStr(ow.next_action_note);
  if (note) return note;
  if (safeStr(ar.status) === LUXE_MAURICE_AI_ACCESS_REQUEST_STATUS) {
    return 'Pending advisor review';
  }
  return 'Received for advisor review';
}

/**
 * Map a persisted `leads` row to Advisor Pipeline card shape (safe fields only).
 * @param {{
 *   id: string,
 *   name: string,
 *   email: string,
 ...
 * }} row
 */
export function mapLeadRowToAdvisorPipelineItem(row) {
  if (!isPrivateAccessLeadRow(row)) return null;
  ...
  return {
    id: row.id,
    reference_id: formatPrivateAccessReferenceId(row.id),
    created_at: createdAt,
    status: safeStr(ar.status) || safeStr(row.status) || LUXE_MAURICE_AI_ACCESS_REQUEST_STATUS,
    buyer: {
      full_name: row.name,
      email: row.email,
      phone: row.phone || null,
    },
    access_category: ar.access_category || null,
    access_intent: ar.access_intent || null,
    profile: {
      budget_min: ar.budget_min ?? null,
      budget_max: ar.budget_max ?? null,
      currency_code: ar.currency_code || 'USD',
      desired_location: ar.desired_location || null,
    },
    notes_summary: truncateNotesSummary(ar.notes),
    opportunity: row.listing
      ? { slug: row.listing, title: null }
      : ar.property_slug
        ? { slug: ar.property_slug, title: null }
        : null,
    next_action: deriveAdvisorNextAction(qj),
    persisted: true,
  };
}
