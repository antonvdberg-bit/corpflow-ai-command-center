...
/**
 * Bump when favicon / app-icon bytes change so browser tabs do not keep a stale
 * transparent or wrong-generation icon. Query string only — paths stay stable.
 */
export const CORPFLOW_BRAND_ASSET_VERSION = 'white-alpha-v5';

export const CORPFLOW_BRAND_ASSET_PATHS = Object.freeze({
  /** Anton-approved raster; opaque white exterior (no transparent holes). Not an SVG recreation. */
  approvedSource: '/brand/corpflowai/corpflowai-favicon-approved-source.png',
...
function withBrandAssetVersion(assetPath) {
  const v = CORPFLOW_BRAND_ASSET_VERSION;
  if (!v) return assetPath;
  return `${assetPath}${assetPath.includes('?') ? '&' : '?'}v=${encodeURIComponent(v)}`;
}
...
export function listCorpFlowBrandHeadTags() {
  ...
  return [
    { rel: 'icon', href: withBrandAssetVersion(p.faviconIco), sizes: 'any' },
    { rel: 'icon', type: 'image/png', sizes: '16x16', href: withBrandAssetVersion(p.favicon16) },
    { rel: 'icon', type: 'image/png', sizes: '32x32', href: withBrandAssetVersion(p.favicon32) },
    {
      rel: 'apple-touch-icon',
      href: withBrandAssetVersion(p.appleTouchIcon),
      sizes: '180x180',
    },
    { rel: 'manifest', href: withBrandAssetVersion(p.manifest) },
