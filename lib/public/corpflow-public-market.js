...
export const CORPflow_PUBLIC_NAV = [
  { href: ENQUIRY_RECOVERY_PATH, label: 'Lead Rescue' },
...
export const CORPflow_HOMEPAGE_HERO = {
  ...
  subhead:
    `${ENQUIRY_RECOVERY_LOSS_LINE} Lead Rescue — ${ENQUIRY_RECOVERY_PRICE_LINE}. ${ENQUIRY_RECOVERY_SCARCITY_LINE}`,
  primaryCta: { label: ENQUIRY_RECOVERY_PRIMARY_CTA_LABEL, href: ENQUIRY_RECOVERY_DIAGNOSIS_HREF },
};

/** Delivery method for the market gateway (#699). */
export const CORPflow_DELIVERY_STEPS = [
  { step: '1', title: 'Understand', body: 'Clarify the business problem, channels, owners, and what “better” looks like in practical terms.' },
  { step: '2', title: 'Design', body: 'Shape a bounded workflow using existing processes and suitable free/open or already-approved tools where possible.' },
  { step: '3', title: 'Build', body: 'Implement the smallest useful path — intake, routing, follow-up, review — with human-operated controls.' },
  { step: '4', title: 'Verify', body: 'Test on controlled surfaces before anything client-facing goes live. Evidence before claims.' },
  { step: '5', title: 'Review', body: 'Walk through what was delivered, what changed for operators, and what still needs a decision.' },
  { step: '6', title: 'Improve', body: 'Tighten the workflow from real use — without forcing an unnecessary platform replacement.' },
];
...
export const CORPflow_PROOF_ITEMS = [
  {
    ...
    capability: 'Tenant-scoped presentation, enquiry routing, and operator-controlled publishing for a luxury-access style experience — demonstrated on CorpFlowAI test hosts, not as a public client case study.',
    demonstratedBy: 'Internal / test tenant capability (Lux-class surface)',
    link: { href: ENQUIRY_RECOVERY_PATH, label: 'Related product path: Lead Rescue' },
  },
  {
    title: 'Operator desk for enquiry and status control',
    capability: 'Qualified intake, status tracking, and operator review for lead and service workflows — including controlled desk surfaces used in CorpFlowAI test delivery (for example CIPC Desk-style operational UI patterns).',
  ...
  },
  {
    ...
    capability: 'A fictional before/after demo showing how a weak brochure page becomes a clearer enquiry path connected to follow-up — no private client data.',
...
export const CORPflow_PROOF_EXAMPLE = {
  ...
  problem: 'Businesses need credible client or enquiry surfaces without exposing operator systems or private lead data on the public web.',
  delivered: CORPflow_PROOF_ITEMS[0].capability,
  approach: 'Managed delivery on controlled test surfaces; public pages stay clear; operator workflows stay behind appropriate access.',
...
export const CORPflow_TRUST_POINTS = [
  { title: 'Controlled approvals', body: 'Production-facing changes move only after review. Operators keep an accountable path for what goes live.' },
  { title: 'Test before launch', body: 'Work is verified on controlled CorpFlowAI test surfaces before client-facing release where that path applies.' },
  { title: 'Data boundaries', body: 'Tenant and operator data stay scoped. Public marketing pages do not expose private lead or client records.' },
  { title: 'No unnecessary platform replacement', body: 'We improve the weak links in your current operating path. Full CRM or stack migrations are out of scope unless quoted separately.' },
];

export const CORPflow_BUYER_FIT = {
  label: 'Who this is for',
  title: 'Owner-led Mauritius service businesses, generally around 5–30 staff',
  body: 'A fit when one converted enquiry is materially valuable, enquiries already arrive (website, WhatsApp, phone, Facebook, email, or staff), follow-up is not always consistent, and the owner or commercial manager can take part. Professional / technical services, real estate and high-value property services, and boutique hospitality or experience businesses with direct enquiries are the current beachhead. If we cannot identify a commercially meaningful recovery problem, we should not work together.',
};
...
export function offerDiscoveryMailto(slug) {
  return buildDiscoveryCallMailto(RAPID_DELIVERY_OFFERS[slug]);
}

export const OFFER_FAQ_BY_SLUG = {
  'ai-lead-rescue': [
    { q: 'Is this a CRM replacement?', a: 'No. Lead Rescue identifies quiet enquiries and makes follow-up visible. Full CRM migration is out of scope unless quoted separately.' },
    ...
    { q: 'How many clients are you taking?', a: 'A maximum of three during this delivery cycle. If we cannot identify a commercially meaningful recovery problem, we should not work together.' },
  ],
...
export const OFFER_NOT_INCLUDED_BY_SLUG = {
  'ai-lead-rescue': ['Full CRM migration', 'Paid advertising', 'Guaranteed revenue or lead volume', 'Unbounded multi-channel rebuild'],
  'premium-landing-page-rescue': ['Full multi-page website rebuild', 'Copywriting for entire brand without client inputs', 'SEO ranking, traffic, or revenue guarantees', 'Payment checkout on the landing page', 'Lead Rescue follow-up work (quoted separately if needed)'],
  'customer-reputation-recovery': ['Review removal or astroturfing'],
};
