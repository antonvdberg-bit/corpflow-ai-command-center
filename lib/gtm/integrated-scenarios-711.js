...
export function runScenarioALeadRescue() {
  ...
  return {
    ...
    expected:
      'market→maturation→FA→onboarding→acceptance_ready; no send; messaging unauthorized',
    actual: ok
      ? 'all ledger steps passed; acceptance_ready; external_sends_executed=[]'
...
export function runScenarioBWebsiteRescue() {
  ...
  const liveValidation =
    evidence.live_validation && typeof evidence.live_validation === 'object'
      ? evidence.live_validation
      : null;
  const handoverChannels = Array.isArray(handover?.channels)
    ? handover.channels.map((c) => String(c))
    : [];
  ...
  ledger.push({
    step: 'handover_and_maintenance_boundary_evidence',
    ok:
      walkDelivery.acceptance?.ok === true &&
      Boolean(handover?.handover_sent_at) &&
      Boolean(handover?.support_boundary_summary) &&
      Boolean(handover?.what_was_built) &&
      handoverSyntheticOnly &&
      Boolean(maintenance_boundary?.in_scope) &&
      Boolean(maintenance_boundary?.out_of_scope) &&
      Boolean(maintenance_boundary?.escalation_contact) &&
      deployApproval?.simulation_only === true &&
      dnsCutover?.simulation_only === true &&
      liveValidation?.pass_fail === 'pass',
    detail: {
      handover,
  ...
  return {
    ...
    expected:
      'locked-offer market→maturation→FA→onboarding→acceptance_ready; handover evidence; DNS/deploy simulated only; no send',
    actual: ok
      ? 'all ledger steps passed; acceptance_ready; handover evidence present; no real DNS/deploy; external_sends_executed=[]'
      : 'one or more ledger steps failed',
    verdict: ok ? 'PASS' : 'FAIL',
  };
}
