...
### 7.4 Step 2 — Uptime Kuma install evidence (operator-executed 2026-06-16; K1–K4 PASS, K5 PASS-by-construction, Monitor 8 / n8n DEFERRED)

Anton ran `docs/runbooks/UPTIME_KUMA_ON_EXEC01_INSTALL_RUNBOOK_V1.md` end-to-end at L3 on `corpflow-exec-01-u69678` on 2026-06-16. Cursor at L1 captured the operator's evidence summary (no secrets, no tokens, no chat ids) into the runbook § 12 evidence shape below, then opened this docs-only closure PR. Per the user's wording, the verdict for Step 2 is **COMPLETE-WITH-N8N-DEFERRED**: Kuma is installed, the seven CorpFlow public floor URLs are probed every 60 s, and the alert path is operationally independent of n8n; the eighth monitor (n8n origin) is **explicitly deferred** because the operator was not 100% sure of the correct URL and chose to defer rather than configure a guess (which is exactly the right call per runbook § 12.2 — *"Do NOT paste the actual n8n URL into this artifact; just confirm the URL family"*).
...
| 8 | n8n-health                     | DEFERRED      | Operator not 100% sure of the correct n8n origin URL; deferred rather than guessed (correct per runbook § 8.2.8 + § 12.2). Will be configured in a future small follow-up once Anton confirms the URL family. Does NOT block Step 2 closure — the seven CorpFlow public floor URLs (the floor of `MONITORING_ARCHITECTURE.md` § 5) are the substantive third-location coverage. |

(URL family for monitor 8 not pasted into this artifact — operator records the actual URL only inside Kuma when configured.)
...
| K3    | All 8 monitors Up within 60–90 s                         | PASS-WITH-DEFERRAL | 7 / 7 configured CorpFlow public floor URLs Up (monitors 1–7 above). Monitor 8 (n8n) explicitly deferred (operator chose deferral over guessing the URL — correct call). The K3 floor is met for the substantive coverage; the deferred monitor is logged as an open follow-up, not a failure. |
...
- Overall: **PASS** (K1=PASS / K2=PASS / K3=PASS-WITH-DEFERRAL / K4=PASS / K5=PASS-BY-CONSTRUCTION)
- Closure PR opened by Cursor (L1) per runbook § 13: this artifact's § 7.4 (this section), `MONITORING_ARCHITECTURE.md` Monitor #13 → ✅ active, `CORPFLOW_SHARED_TODO.md` Step 2 → COMPLETE-WITH-N8N-DEFERRED, `JE-2026-06-16-2` appended, chat_history bullet added.
...
- **Monitor 8 (n8n origin) deferred** — not a runbook deviation; runbook § 8.2.8 explicitly says the URL is operator-confirmed at install time and § 12.2 explicitly says do not paste it into the artifact. Anton chose deferral over guessing the URL family — this is the conservative, runbook-compliant call. A small future follow-up (single-monitor add inside Kuma's UI; no PR needed) flips Monitor 8 from DEFERRED to Up once Anton confirms which n8n URL is the canonical health endpoint for the in-flight n8n host.
...
## 8. Verdict (current — updated 2026-06-16 after Kuma install)

```text
Delivery Reality Audit (Step 2 — Uptime Kuma install on corpflow-exec-01-u69678):
- Local fix exists:                       YES (live container on the box; ~/uptime-kuma-data/ persistent)
- Merged to main:                         pending Anton's merge of the closure PR (this PR)
- Production deployment ID:               n/a — does not deploy to Vercel
- Commit deployed:                        n/a — runs on `corpflow-exec-01-u69678` (L3), not Vercel
- Live URLs tested:                       7 of 8 monitors Up (§ 7.4 monitors 1–7); off-box public unreachability of host:3001 confirmed (curl exit 28); SSH-tunnel UI reachable
- Expected vs actual result:              K1=PASS / K2=PASS / K3=PASS-WITH-DEFERRAL (monitor 8 / n8n explicitly deferred; foundational floor met) / K4=PASS / K5=PASS-BY-CONSTRUCTION (n8n not in alert path)
- Client-facing flow usable:              YES — all 7 monitored CorpFlow public floor URLs return 200 to Kuma's probes; alerts route to operator's Telegram chat via Kuma's own bot independently of n8n
- Final verdict:                          COMPLETE-WITH-N8N-DEFERRED
```

**Step 2 status (2026-06-16):** Uptime Kuma is installed at L3 on `corpflow-exec-01-u69678`, loopback-bound to `127.0.0.1:3001`, probing the seven CorpFlow public floor URLs every 60 s with Telegram-direct alerting via Kuma's own BotFather bot (separate from in-repo `TELEGRAM_BOT_TOKEN`). The eighth monitor (n8n origin) is explicitly deferred — Anton chose deferral over guessing the URL family. **Blind spot # 7 of `MONITORING_ARCHITECTURE.md` § 6 (no third-location uptime monitoring) is closed for the seven floor URLs.** The deferred n8n monitor will be added inside Kuma's UI in a future small follow-up once Anton confirms the canonical n8n health URL — does not require a new PR.

**Step 1 status (unchanged by this PR):** Still **PARTIAL**. Step 1 will move to COMPLETE when § 7.1 + § 7.2 + § 7.3 are filled in by the operator (or § 7.3 explicitly opted out of as "n8n admin not accessed in this round"). The closure PR for Step 1 is a separate future PR; it is not bundled into this Step 2 closure.

**Step 3 status (unchanged by this PR):** Still HELD. Step 3 (restic) was gated on Step 2 = COMPLETE per `SELF_HOSTED_OPS_STACK_V1.md` § 4. With Step 2 = COMPLETE-WITH-N8N-DEFERRED, the gate is **substantively** satisfied (Kuma is the operational signal that restic discipline backs up); Step 3 authoring is now **eligible** for the next packet round but **not** initiated by this PR — the user explicitly directed *"Do not proceed to restic yet."*
