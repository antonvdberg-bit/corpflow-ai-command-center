...
If you are not signed in, the Advisor Pipeline will not show persisted request details.
...
*Figure 5 — Signed-out visitors see a sign-in prompt (no persisted PII)*
...
The Advisor Pipeline is **read-only** for workflow management today.

| Available in Advisor Pipeline | Available in `/change` (operator console) |
|------------------------------|---------------------------------------------|
| View persisted requests | `concierge-leads-list` — full lead list |
...
| | `concierge-lead-operator-patch` — save workflow changes |

**Planned future enhancement:** inline status changes and advisor assignment directly in the v2 CRM. That is not live yet.
...
Use the fictional **LuxeMaurice Training User** (`training@example.invalid`) when recording advisor training videos. Do not display unrelated real client requests in shared materials.
